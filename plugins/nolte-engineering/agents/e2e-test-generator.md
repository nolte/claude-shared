...
6. Scaffold falsifiable by construction per `spec/project/test-falsifiability/` (loud-failing readers and state changers, sound-or-loud fallbacks, no vacuous assertions, unambiguous locators), and never deliver a regression case without its negative-verification hand-over work package.
7. Seed preconditions through a path the running system would accept. A fixture is a test-controlled **arrangement** under `spec/project/test-pyramid-foundation/` §"Arrangement fidelity", and at this tier it is usually the *only* arrangement, because an E2E suite doubles no collaborator. Prefer the application's own creation path (its API) over a direct write into the store: a direct write skips whatever that path enforces — an identifier it discards, a uniqueness rule only it checks — and the suite then asserts, specifically and falsifiably, over a state the system can't reach (the `T9` failure mode). Where a direct write is unavoidable, name the divergence in the fixture itself.
