...
Build every double at least as strict as the collaborator it replaces along every dimension the test relies on: it rejects there what the real collaborator rejects (its constraints, validation, uniqueness) and doesn't preserve what the real collaborator discards. That bound is what keeps this rule compatible with minimal doubles: tighten the refusals the test actually depends on, and never reimplement inside a double the entire validation surface of what it replaces. The recurring offender at this tier is a hand-written double of a persistence or repository boundary — the real one generates or discards keys, enforces uniqueness, and rejects nulls, while the double silently does none of it, so every test in the file passes for a state the database would never hold (the `T9` failure mode of `spec/project/test-falsifiability/`). Where a divergence can't be closed, name it in the double itself, so the next reader knows what the tests don't cover.
...
4. Use the minimal disciplined doubles, and never one more permissive than what it replaces along any dimension the test relies on — a double rejects there what the real collaborator rejects (constraints, validation, uniqueness), doesn't preserve what it discards, and names in itself any divergence that can't be closed; never over-mock, never mock value objects or types you don't own, and prefer state verification, reserving mocks for when the interaction is the contract.
