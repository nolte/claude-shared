...
description: "Read-only scanner dispatched by the `kpi-derive` skill: mines two surfaces for candidate KPI signals — the source tree (domain events, aggregatable entities, state transitions, funnels, error surfaces) and the requirement/goal documents (project/requirements/<slug>.md acceptance criteria and non-functional targets, project/goals.md and project/mission.md business outcomes) — and returns a structured candidate inventory, each candidate keyed to the goal or requirement it ladders back to where discernible, with file:line attribution. Detection only: it surfaces candidates and never selects the key KPIs, defines them, applies the SMART gate, or writes anything — selection, definition, and the write stay with the skill. It never crosses the measurement boundary into instrumentation. Don't use to select or define KPIs or write the artifact (that's `kpi-derive`), to elicit requirements (`requirements-elicit`), or to scan Dockerfiles or dependencies (other scanners)."
distribution: plugin
