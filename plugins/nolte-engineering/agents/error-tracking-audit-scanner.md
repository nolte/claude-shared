...
- **Default PII off** — judge the *effect* per PII category, never one literal. Two spellings are evidence: the legacy boolean (`send_default_pii` / `sendDefaultPii`) and the structured block (`data_collection` / `dataCollection`). Resolve the category set and each category's default from the pinned SDK's own source when it's installed in the audited working copy — JS: `node_modules/@sentry/core/build/types/types/datacollection.d.ts` and `build/esm/utils/data-collection/resolveDataCollectionOptions.js`; Python: `sentry_sdk/data_collection.py` and `sentry_sdk/consts.py` in the installed site-packages. Otherwise fall back to the table below. Record which source you used.
  ...
  |---|---|---|
  | Sentry JS 10 without a `dataCollection` block (`@sentry/core@10.75.3`) | legacy `sendDefaultPii` | legacy flag decides |
  ...
  | `sentry-sdk` Python without a block | legacy `send_default_pii`; unset is treated as `False` | legacy flag decides |
  ...
  Overall state: `PASS: legacy flag off` only when the honoured legacy flag is a literal off and no honoured block is present; `PASS: every category OFF` only when an honoured block is present and every PII category is `OFF`; `NOT ALL OFF` when an honoured block is present and any category is otherwise; `EXPLICIT TRUE` when the honoured legacy flag is a literal `true` and no honoured block is present; `UNSET` when no honoured spelling carries a literal value: none is present, the honoured legacy flag is non-literal (every category `UNSET→UNKNOWN`), or honouring is undetermined. When the SDK honours the block and a legacy flag is also set, the block decides; report the legacy value as ignored rather than dropping it. A legacy flag on Sentry JS ≥ 11 is likewise reported as ignored, and the overall state follows the structured block or, without one, is `UNSET`.

  On `PASS: legacy flag off`, record what the off flag still leaves collected as an informational note on the output line; it never changes the state. JS 10 (`defaultPiiToCollectionOptions`): `graphQL` (both sub-keys), `stackFrameVariables`, and deny-list `cookies`, `httpHeaders`, `urlQueryParams`. Python (`_map_from_send_default_pii`): `http_bodies` (all types), deny-list `cookies`, `http_headers.request`, `url_query_params`, and `stack_frame_variables` unless `include_local_variables=False`.
