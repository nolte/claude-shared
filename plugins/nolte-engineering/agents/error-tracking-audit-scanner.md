...
- **Default PII off** — judge the *effect* per PII category, never one literal. Two spellings are evidence: the legacy boolean (`send_default_pii` / `sendDefaultPii`) and the structured block (`data_collection` / `dataCollection`). Resolve the category set and each category's default from the pinned SDK's own source when it's installed in the audited working copy — JS: `node_modules/@sentry/core/build/types/types/datacollection.d.ts` and `build/esm/utils/data-collection/resolveDataCollectionOptions.js`, plus `defaultPiiToCollectionOptions.js` for the legacy-flag mapping; Python: `sentry_sdk/data_collection.py` (legacy mapping `_map_from_send_default_pii`) and `sentry_sdk/consts.py` in the installed site-packages. Otherwise fall back to the table below. Record which source you used.

  | SDK (measured 2026-09-25) | Evidence spelling honoured | Omitted category resolves to |
  |---|---|---|
  | Sentry JS 10 without a `dataCollection` block (`@sentry/core@10.75.3`) | legacy `sendDefaultPii` | `false` maps to: `userInfo`, `httpBodies`, `genAI`, `databaseQueryData` OFF; `cookies`, `httpHeaders` (both), `urlQueryParams` RESTRICTED (deny `forwarded`, `-ip`, `remote-`, `via`, `-user`); `graphQL` (both sub-keys), `stackFrameVariables` ON (the SDK redacts literal values in the GraphQL `document`, so the `graphQL` ON rests on `variables`) |
  | Sentry JS 10 with a `dataCollection` block (versions whose `@sentry/core` ships the option, such as `10.75.3`) | the block; a set `sendDefaultPii` is ignored | ON, as on JS ≥ 11 but with no `queues` category; `queryParams` is a deprecated alias of `urlQueryParams` |
  ...
  | `sentry-sdk` Python with a block: `data_collection={…}` from `2.70.0` (which still reads `_experiments` as a fallback); on `2.67.0`–`2.69.2` only `_experiments={"data_collection": …}` | the block; takes precedence over `send_default_pii` | collected for every category: `user_info` `True`; `cookies`, `url_query_params`, `http_headers.request` denylist; `http_bodies` all types; `graphql` and `gen_ai` sub-keys, `database_query_data`, `queues`, `stack_frame_variables` `True` |
  | `sentry-sdk` Python without a block (`2.67.0`–`2.70.0`; mapping identical in `2.67.0`, `2.69.2`, `2.70.0`) | legacy `send_default_pii`; unset is treated as `False` | `False` maps to: `user_info`, `graphql`, `gen_ai`, `database_query_data`, `queues` OFF; `cookies`, `http_headers.request`, `url_query_params` RESTRICTED (same five deny terms); `http_bodies` ON (all types); `stack_frame_variables` = `include_local_variables` from the init call (omitted: default `True`, so `UNSET→ON`) |
  | `sentry-sdk` Python below `2.67.0` (measured `2.64.0`, `2.65.0`, `2.66.1`) | legacy `send_default_pii` only; `2.64.0` ships no `data_collection` module, and on `2.65.0`–`2.66.1` the resolver reads `_experiments["data_collection"]` but no integration consumes it | `UNSET→UNKNOWN` for every category: the integrations gate on `send_default_pii` directly, not through a per-category mapping |

  On a JS 10 version whose installed `@sentry/core` lacks `resolveDataCollectionOptions`, a `dataCollection` block is ignored; report it as such. Trust the runtime `.js` defaults over `.d.ts` `@default` comments when they disagree (`@sentry/core@10.75.3` types say `userInfo` defaults to `false`; the runtime `DEFAULTS` say `true`), and name the conflict in the category source. `frameContextLines` / `frame_context_lines` is a source-context line count, not a PII category; exclude it.

  - **Sub-fields**: a category is `OFF` only when every sub-field resolves off, and otherwise takes its least-off sub-field's state (name the sub-field). JS `httpHeaders` given one value applies it to both `request` and `response`, while `{request: false}` alone leaves `response` ON; `graphQL` (`document`, `variables`) and `genAI` (`inputs`, `outputs`) have two sub-keys each; Python `http_headers` has `request` only.
  - **Off spelling per SDK**: JS takes `false`, or `[]` for `httpBodies`. Python takes `{"mode": "off"}` for the key-value categories (`cookies`, `url_query_params`, `http_headers.request`, and `stack_frame_variables` given as a dict), `False` only for the boolean ones (`user_info`, `database_query_data`, `queues`, `stack_frame_variables`, the `graphql`/`gen_ai` sub-keys), and `[]` for `http_bodies`. A Python key-value category given `False` is `ON`, never `OFF`: the SDK reads a falsy value as `{}` and defaults the mode to `denylist`; append the note "SDK treats `False` as denylist".
  - **Empty deny list**: `{deny: []}` (JS) or `{"mode": "denylist"}` with no `terms` (Python) equals the default, because the built-in sensitive-key filter applies either way; report it `ON`, not `RESTRICTED`.
  - **Non-literal values**: a block value or Python `include_local_variables` that isn't a literal in the source tree (an environment variable, a configuration lookup) resolves every category it could decide to `UNSET→UNKNOWN` (for `include_local_variables`: `stack_frame_variables`). A non-literal legacy flag on an SDK with a known mapping is evaluated both ways: resolve the mapping for `true` and for `false`; a category resolving identically keeps that state (no `UNSET→` prefix: the flag is set, its value just doesn't matter there), and only the categories that differ become `UNSET→UNKNOWN`. Measured: Sentry JS `10.75.3` keeps `graphQL` and `stackFrameVariables` `ON`; `sentry-sdk` keeps `http_bodies` `ON`, and `stack_frame_variables` follows `include_local_variables` alone. Without a known mapping every category is `UNSET→UNKNOWN`.

  Per-category states: `OFF` (every sub-field off in the SDK's own spelling), `RESTRICTED` (a non-empty allow or deny list: `{allow: […]}` / `{deny: […]}`, `"allowlist"` / `"denylist"` with `terms`; `RESTRICTED (via default)` when the list is the SDK's default for an absent flag), `ON` (explicit `true`, the full target list, an empty deny list, or a Python key-value category given `False`), `UNSET→ON` (omitted, default established as collected by the source or table), `UNSET→UNKNOWN` (omitted with no established default, or decided by a non-literal value).

  Decide on the spelling the pinned SDK **honours** (the table's second column), never on which spellings are present. A block the SDK ignores (JS 10 whose `@sentry/core` lacks `resolveDataCollectionOptions`, or `sentry-sdk` below `2.67.0`) is reported with the note `structured block ignored: SDK predates dataCollection` and counts as absent, so the legacy flag decides. A top-level `data_collection=` on `sentry-sdk` below `2.70.0` makes `init()` raise `TypeError("Unknown option 'data_collection'")`; note `structured block rejected: init raises`, count the block as absent, and report the tool-contract SDK line `MISSING` with the same reason, since the SDK never initialises. When whether the SDK honours a present block can't be established (installed source absent and the JS 10 version can't be placed against the table), note `block honoured: undetermined` and report `UNSET` with every category `UNSET→UNKNOWN`.

  **Legacy flag, judged by effect**: a literal off legacy flag (`sendDefaultPii: false` / `send_default_pii=False`) that the SDK honours is resolved into per-category states through the SDK's own mapping (installed source above, otherwise the table row), exactly like a block; it counts only for the categories it switches off. **An absent legacy flag has the same effect**: both SDKs treat it as the off value (JS `sendDefaultPii === true` fails, Python maps `None` to `False`), so with neither flag nor honoured block, resolve through the same mapping and prefix `UNSET→` where the default decides: collected categories `UNSET→ON`, deny-listed ones `RESTRICTED (via default)`, switched-off ones `OFF`. On JS ≥ 11 without a block every category is `UNSET→ON`. When neither source places the version (a JS 10 release without the mapping, an unknown SDK), its categories are `UNSET→UNKNOWN`. The SDKs' built-in sensitive-key masking (auth, token, secret, session, password, …) applies regardless of flag or block and never makes a category `OFF`.

  Overall state, one rule for both spellings: `PASS: every category OFF` only when the honoured spelling (block, or literal off legacy flag) resolves every PII category `OFF`; `NOT ALL OFF: <categories>` when it resolves any category otherwise; `EXPLICIT TRUE` when the honoured legacy flag is a literal `true` and no honoured block is present. With no spelling present, the default mapping above decides the same way (`evidence: none`; on every measured SDK that is `NOT ALL OFF`). A non-literal honoured legacy flag resolves through the both-ways rule above: `NOT ALL OFF` when any category is `ON`, `RESTRICTED`, or `UNSET→ON`, otherwise `UNSET`. `UNSET`, with every category `UNSET→UNKNOWN`, remains when no mapping places the SDK or honouring is undetermined. When the SDK honours the block and a legacy flag is also set, the block decides; report the legacy value as ignored rather than dropping it. A legacy flag on Sentry JS ≥ 11 is likewise reported as ignored, and the overall state follows the structured block or, without one, the all-`UNSET→ON` default.
...
- PII: default-PII off — <PASS: every category OFF | NOT ALL OFF: <categories> | EXPLICIT TRUE | UNSET>; evidence: <legacy flag | structured block | none> (legacy flag ignored: <value>, <structured block precedence | removed in JS ≥ 11>) (<structured block ignored: SDK predates dataCollection | structured block rejected: init raises | block honoured: undetermined>); categories: <name=OFF|RESTRICTED|RESTRICTED (via default)|ON|UNSET→ON|UNSET→UNKNOWN, …>; category source: <SDK package@version> via <installed source: <path> | body table> (<.d.ts/runtime default conflict | none>) [static] [<file:line> evidence, <file:line> category source]
