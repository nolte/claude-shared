...
- PII: default-PII off — <PASS: legacy flag off (still collected, informational: <categories>) | PASS: every category OFF | NOT ALL OFF: <categories> | EXPLICIT TRUE | UNSET>; evidence: <legacy flag | structured block | none> (legacy flag ignored: <value>, <structured block precedence | removed in JS ≥ 11>); categories: <name=OFF|RESTRICTED|ON|UNSET→ON|UNSET→UNKNOWN, …>; category source: <SDK package@version> via <installed source: <path> | body table> (<.d.ts/runtime default conflict | none>) [static] [<file:line> evidence, <file:line> category source]
