...
- **Default PII off** — judge the *effect* per PII category, never one literal. Two spellings are evidence: the legacy boolean (`send_default_pii` / `sendDefaultPii`) and the structured block (`data_collection` / `dataCollection`). Resolve the category set and each category's default from the pinned SDK's own source when it's installed in the audited working copy — JS: `node_modules/@sentry/core/build/types/types/datacollection.d.ts` and `build/esm/utils/data-collection/resolveDataCollectionOptions.js`, plus `defaultPiiToCollectionOptions.js` for the legacy-flag mapping; Python: `sentry_sdk/data_collection.py` (legacy mapping `_map_from_send_default_pii`) and `sentry_sdk/consts.py` in the installed site-packages. Otherwise fall back to the table below. Record which source you used.
  ...
  |---|---|---|
  | Sentry JS 10 without a `dataCollection` block (`@sentry/core@10.75.3`) | legacy `sendDefaultPii` | `false` maps to: `userInfo`, `httpBodies`, `genAI`, `databaseQueryData` OFF; `cookies`, `httpHeaders` (both), `urlQueryParams` RESTRICTED (deny `forwarded`, `-ip`, `remote-`, `via`, `-user`); `graphQL` (both sub-keys), `stackFrameVariables` ON |
  ...
  - **Non-literal values**: a legacy flag or block value that isn't a literal in the source tree (an environment variable, a configuration lookup) resolves every category it could decide to `UNSET→UNKNOWN`.

  Per-category states: `OFF` (every sub-field off in the SDK's own spelling), `RESTRICTED` (a non-empty allow or deny list: `{allow: […]}` / `{deny: […]}`, `"allowlist"` / `"denylist"` with `terms`), `ON` (explicit `true`, the full target list, an empty deny list, or a Python key-value category given `False`), `UNSET→ON` (omitted, default established as collected by the source or table), `UNSET→UNKNOWN` (omitted with no established default, or decided by a non-literal value).
  ...
  **Legacy flag, judged by effect**: a literal off legacy flag (`sendDefaultPii: false` / `send_default_pii=False`) that the SDK honours is resolved into per-category states through the SDK's own mapping (installed source above, otherwise the table row), exactly like a block; it counts only for the categories it switches off. When neither source places the version, its categories are `UNSET→UNKNOWN`. The SDKs' built-in sensitive-key masking (auth, token, secret, session, password, …) applies regardless of flag or block and never makes a category `OFF`.

  Overall state, one rule for both spellings: `PASS: every category OFF` only when the honoured spelling (block, or literal off legacy flag) resolves every PII category `OFF`; `NOT ALL OFF: <categories>` when it resolves any category otherwise; `EXPLICIT TRUE` when the honoured legacy flag is a literal `true` and no honoured block is present; `UNSET` when no honoured spelling carries a literal value: none is present, the honoured legacy flag is non-literal (every category `UNSET→UNKNOWN`), or honouring is undetermined. When the SDK honours the block and a legacy flag is also set, the block decides; report the legacy value as ignored rather than dropping it. A legacy flag on Sentry JS ≥ 11 is likewise reported as ignored, and the overall state follows the structured block or, without one, is `UNSET`.
...
- PII: default-PII off — <PASS: every category OFF | NOT ALL OFF: <categories> | EXPLICIT TRUE | UNSET>; evidence: <legacy flag | structured block | none> (legacy flag ignored: <value>, <structured block precedence | removed in JS ≥ 11>) (<structured block ignored: SDK predates dataCollection | block honoured: undetermined>); categories: <name=OFF|RESTRICTED|ON|UNSET→ON|UNSET→UNKNOWN, …>; category source: <SDK package@version> via <installed source: <path> | body table> (<.d.ts/runtime default conflict | none>) [static] [<file:line> evidence, <file:line> category source]
