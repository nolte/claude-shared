...
description: "Read-only scanner dispatched by the `observability-audit` skill: detects static presence and wiring of the four mandatory observability pillars (metrics, structured logs with trace context, W3C Trace Context propagation, health/SLO/burn-rate alert) across backend, frontend floor, and third-party floor, plus the high-cardinality and PII-redaction guardrails, per spec/project/monitoring-observability/. Marks values/continuity/firing as runtime-verify. Returns a per-service findings inventory with file:line; severity, report, and plan handover stay with the skill. Don't use for the report or plan (`observability-audit`), Kubernetes probe wiring (`deployment-bestpractices-reviewer`), or the PII-class verdict (`gdpr-data-protection-reviewer`)."
distribution: plugin
...
- dependency/version probes that confirm an OTel SDK/exporter or instrumentation package is actually installed and wired, not merely mentioned — e.g. `pip show opentelemetry-sdk`, `npm ls @opentelemetry/api`. These read local metadata and write nothing. Manifests and lockfiles themselves are read with `Read`, never `cat`, per §Tool access. **Do not run `go list -m all`**: it resolves the whole module graph, writing into the module cache and fetching over the network when the cache is cold — read `go.mod` and `go.sum` instead.
