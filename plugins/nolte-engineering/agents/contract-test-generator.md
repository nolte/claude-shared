...
Under the **replay-verified** flavours — consumer-driven and provider-driven — the contract mock earns its fidelity from provider verification, that replay against the real provider being this tier's whole mechanism, so never hand-write a mock response the provider is not verified against. Under the **bi-directional** flavour no provider code executes at all — the mock is checked against a published API specification only — so it earns none of that mechanical guarantee and is built to the portfolio fidelity rule like any other double: at least as strict as the real provider along every dimension the test relies on, rejecting there what the provider rejects (its constraints, validation, uniqueness) and not preserving what it discards, per the requirement `spec/project/test-tier-contract/` carries for that flavour. That bound is what keeps the rule workable: tighten the refusals the test actually depends on, and never re-derive inside a mock the entire validation surface of the provider it stands in for. A mock that answers a request the real provider would reject, or preserves a field it discards, verifies the consumer against a provider that can't exist (the `T9` failure mode of `spec/project/test-falsifiability/`). Where a divergence can't be closed, name it in the mock itself, so the next reader knows what the verification doesn't cover.

**Provider states are arrangements too, and no flavour protects them.** The exclusion above covers the consumer-side *mock* only. Build a provider state through the provider's own write path rather than by inserting into its store, so it inherits the constraints that path enforces. A state that arranges data the provider would itself refuse produces a green verification for a provider that can't reach it, and replay never catches this under any flavour: the setup runs before the first recorded interaction, so it sits outside everything the replay grades. This is the same `T9` failure mode, reached by the setup instead of by the mock. Where a direct write is unavoidable, name the divergence in the state itself.
...
4. Wire a broker and a can-i-deploy compatibility gate; a contract that is not verified against the current provider is contract drift and is forbidden; under the bi-directional flavour, where no provider code executes, the consumer-side mock earns no mechanical fidelity either and is built at least as strict as the real provider along every dimension the test relies on, naming in itself any divergence that can't be closed.
