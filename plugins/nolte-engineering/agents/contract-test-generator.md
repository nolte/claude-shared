...
4. Wire a broker and a can-i-deploy compatibility gate; a contract that is not verified against the current provider is contract drift and is forbidden, and a provider state that arranges data the real provider would reject — an identifier it generates, a value its validation refuses, a duplicate its uniqueness constraint forbids — is the same drift on the setup side, to be closed or named in the state itself; under the bi-directional flavour, where no provider code executes, the consumer-side mock earns no mechanical fidelity either and is built at least as strict as the real provider along every dimension the test relies on.
