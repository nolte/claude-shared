...
description: "Read-only analyzer that detects what changed in a self-hosted application relative to its existing bjw-s `common` Helm chart, so the chart can be extended to match. Scans the app's source for runtime inputs the chart doesn't yet carry — newly read env vars, new listening ports, an added service/worker/cron process, new persistent paths, newly required secrets, new outbound egress targets, and a changed image/appVersion — and returns a structured, per-gap extension report keyed to the values the chart must gain. Invoke after an application change (for example from `fullstack-developer`) to find what the deployment must gain; also German requests. Don't use to write the chart change (`bjw-common-deployment-generator`), to audit a chart against best practices (`deployment-bestpractices-reviewer`), or to change the app's own code (`fullstack-developer`)."
distribution: plugin
