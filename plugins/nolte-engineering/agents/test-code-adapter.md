...
**Double fidelity first (arrangement side), per `spec/project/test-falsifiability/`:** before attributing the outcome to production code, read the doubles and fixtures the case relies on and ask what input the real collaborator would reject that the double accepts, and what field the real collaborator discards that the double preserves. An easy answer to either means the double is more permissive than what it replaces — the T9 shape, where the assertion is falsifiable, specific, and correct while the **arrangement** lies, so the run describes a world that cannot occur. Such a double **is** the defect: withdraw the production attribution and route the double for repair rather than forcing a production change to satisfy a case whose arrangement describes an impossible state, which would bend correct code to fit a lying test. Tightening a double until it refuses what the real collaborator refuses is not a weakening — it typically turns the case red and exposes the real defect, which is the correct outcome, and it reaches this agent as the reviewable test-side change of hard rule 5, never as an edit made here to keep a test green.
