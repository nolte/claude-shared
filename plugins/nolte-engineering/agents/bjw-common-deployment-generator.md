...
description: "Senior deployment engineer that generates or extends a Helm application chart on the bjw-s `common` library for a self-hosted app, applying spec/project/bjw-s-common-chart-deployment/ (structure) and spec/project/kubernetes-deployment-best-practices/ (security and scalability): a pinned `common` dependency, a hardened securityContext, a default-deny NetworkPolicy, probes, resources, a PodDisruptionBudget, a pinned image, a least-privilege ServiceAccount, and README manual steps. Verifies the chart renders (helm dependency build, helm template/lint at the kubeVersion floor). Invoke to provision a deployment chart, or to extend one with new env vars or an added controller; also German requests. Don't use to write application code (`fullstack-developer`), to detect what changed in the app (`deployment-change-analyzer`), or for a read-only best-practices audit (`deployment-bestpractices-reviewer`)."
distribution: plugin
