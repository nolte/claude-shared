...
Falsifiability is graded per `spec/project/test-falsifiability/`, citing its T-categories: a post-condition swallowed by a bare `except` in a test or helper (T1 — the test-code instance of D1's swallowed-error no-go), an assertion satisfied by the reader's empty default or tautological over its domain, or consisting solely of a negative (T2), a reader returning `""`/`[]`/`None` for "could not look" (T3), a test running no check at all — no assertion in the body, or one the passing path never reaches (T8, the criteria behind D6's assertion-free wording). Back the verdict with the spec's three questions: what input would make this fail; what would a stub returning empty values not satisfy; would deletion of the feature be noticed. These findings attach to D6 (or D1 for the helper shape), carry their T-category in the problem statement, and follow the spec's severity floor — confirmed Critical, suspected at least Warning.
