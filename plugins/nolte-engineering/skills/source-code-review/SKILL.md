...
3. **Verify the report contract.** Confirm the severity vocabulary (Critical / Warning / Suggestion / Info from `spec/claude/review-plan/` §Severity scale), the per-finding file:line + dimension ID + `production|test` marker + confirmed/suspected flag, and that the work packages have **disjoint file sets** with routing targets and explicit dependencies. For a frontend dispatch also confirm: exactly one dimension ID per finding (no D and F on the same finding), an implementation proposal on every Critical and Warning finding, the four recorded baselines, that no finding above Info rests on a rendered impression, and that the slice's test files were reviewed; a report covering component files but no test files is incomplete. For every dispatch confirm the D1 severity floor as well: a confirmed swallowed or missing error path on a fallible operation is Critical and a suspected one at least Warning (F2 in the frontend overlay). A contract violation goes back to the reviewer for one repair pass, not into the artifact.
