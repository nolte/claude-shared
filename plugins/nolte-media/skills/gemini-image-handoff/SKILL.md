...
dont_use_when:
  - situation: "You can use the Gemini API (billing enabled) and want the file written automatically with a sidecar"
    alternative: image-generate
  - situation: "You want a free, fully-automated generation with no UI step"
    alternative: image-generate
  - situation: "You want to author a brand-conformant prompt document"
...
- **Always** state the **SynthID watermark** caveat: every Gemini UI output carries an invisible SynthID watermark. For a watermark-free commercial or blog asset, steer the operator to `image-generate --provider cloudflare` (FLUX.1-schnell or FLUX.2 Klein 4B via `--model`, both Apache-2.0, no watermark) instead.
