...
dont_use_when:
  ...
  - situation: "You want to remove a fake-transparency background or vectorise the result"
...
- **Three byte caps, each a module constant in the bundled script.** A `--ref-image` above `MAX_REF_IMAGE_BYTES` (20 MiB) and a `--prompt-file` or `--from-prompt-doc` above `MAX_PROMPT_FILE_BYTES` (1 MiB) each exit `2` before any network call; a provider response body above `MAX_RESPONSE_BYTES` (64 MiB) exits `1`, is discarded, and writes no file. All three sit far above any conforming input, so hitting one usually means something is wrong — but a job that legitimately needs a larger bound raises the constant in `scripts/image_generate.py`, one line, rather than working around it. A prompt above its cap is refused, never truncated: a shortened prompt would generate an image for text the operator never wrote.
- **A redirect from a provider endpoint is refused, never followed.** All three endpoints are one documented URL each, and `urllib`'s stock redirect handler replays a request's headers at the new location without comparing hosts, so following one would hand `CLOUDFLARE_API_TOKEN`, `POLLINATIONS_API_TOKEN`, or `GEMINI_API_KEY` to whatever host the `Location` names. The script installs an opener that refuses instead, on every status and on both the POST and the GET path, and exits `1` with a message naming the addressed endpoint, the status, and the target host. That message is deliberately distinct from `Provider API returned HTTP <n>`: a refusal means no request reached the second host, not that the provider answered. If a provider ever genuinely relocates an endpoint, the fix is the URL in `scripts/image_generate.py`, not a way around the refusal.
- **Every operator-named file is checked on the open descriptor, not on the path.** `--ref-image`, `--prompt-file` and `--from-prompt-doc` each open with `O_NONBLOCK` and then `fstat`, so a directory, a device or a FIFO exits `2` with a message naming the flag and the path rather than hanging. Checking the path first and opening it afterwards would both hang on a FIFO and leave a window in which the path is swapped. A prompt source that is not valid UTF-8 exits `2` as well, where it used to surface a decode traceback.
