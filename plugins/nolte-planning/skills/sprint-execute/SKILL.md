...
3. **Enforce the at-most-one-active-sprint invariant.** Determine the sprint's current `status`:
   ...
   - `planned` and no other sprint is `active` → invoke operation 1 first, then continue.
...
- **The `## Features` body list and the `features` frontmatter array are kept in lockstep by operation 4.** Mutating only one surface is a partial-write failure mode; `sprint-execute` refuses partial mutations and the operator has to update both atomically.
- **Acceptance criteria and test hooks are gated separately** for the `in_progress → done` transition. Operation 3 confirms both: every acceptance-criterion checkbox is checked AND every test hook reports `passing` or `skipped`. A test hook still in `pending` blocks the transition even when the criteria are visually checked off.
...
- **Never** write a feature status transition without first reading the feature's current on-disk `status` and verifying it is the legal predecessor (`ready → in_progress`, `in_progress → done`). A jump from any other state (for example `draft → done`) is refused, whatever the conversation claims the state to be; an operator-decided deviation requires a dated deviation record in the feature's `## Consistency notes` before the write (F-17/F-18 precedent, #498).
- **Never** write `last_commit` on a sprint as a side effect of anything other than a feature in that sprint reaching `done`. The field's only canonical writer is operation 3.
