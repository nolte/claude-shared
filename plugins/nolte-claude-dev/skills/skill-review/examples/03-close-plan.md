...
2. Delete the plan file at `.audits/skill-review/roadmap-init.md`, then compose the deletion commit message exactly as `chore(skill-review): close roadmap-init—2C/3W/1S/0I` in the subject (em-dash separator, single-letter severity codes, no abbreviations expanded), with the body listing every deferred-issue URL the plan referenced plus a trailing `Reviewed at repo-revision: <sha-from-frontmatter>` line.
