...
1. Run the four-file precondition check under `spec/claude/`, resolve `<canonical>` from `spec/.spec-config.yml` (`en`), confirm `.audits/` is tracked, default the target to `plugins/nolte-planning/skills/roadmap-init/`, and check that `.audits/skill-review/roadmap-init.md` does not yet exist.
