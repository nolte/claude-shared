...
- `plugins/nolte-planning/skills/roadmap-init/SKILL.md` — the skill the user wants reviewed; only inspected far enough to confirm the request is a review (not a scaffold or revise).
...
2. Refuse to start the review inside this skill: do **not** read the spec files in depth, do **not** open `.audits/skill-review/`, do **not** draft a plan, and do **not** edit `plugins/nolte-planning/skills/roadmap-init/SKILL.md`. Any of these would silently overlap with `skill-review` and produce a non-persistent, non-spec-cited shadow review.
