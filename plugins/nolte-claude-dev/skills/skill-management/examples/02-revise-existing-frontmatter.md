...
"The `description` of `plugins/nolte-engineering/skills/quality-gate/SKILL.md` is over 1024 characters and reads like prose. Trim it under the limit and give it a clean shape so routing stays reliable."

## Input files

- `plugins/nolte-engineering/skills/quality-gate/SKILL.md` — the existing skill whose frontmatter needs revision; the `description` field currently exceeds 1024 characters and lacks an explicit list of user-trigger phrasings.
