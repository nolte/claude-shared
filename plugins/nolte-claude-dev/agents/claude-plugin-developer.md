...
2. `Read` the specs that govern the artifact you are about to produce. At minimum:
   ...
   - `spec/claude/skill-agent-naming/en.md`: the naming convention, its role-noun morphology, and the closed exception lists
   - `spec/claude/skill-agent-catalog/en.md`: when your artifact must surface in the catalog
3. `Glob` and `Read` the existing siblings that sit in the same functional cluster as your target (pull-request workflow, audit, release tooling, and similar), across every plugin root — `skills/`, `agents/`, and `plugins/*/{skills,agents}/` — with the target plugin's own tree read first. The catalog scan prevents duplicate artifacts and keeps naming consistent with precedent.
...
3. **Check for duplicates** by using `Grep` to pull every `description` line across all four plugin roots — `skills/*/SKILL.md`, `agents/*.md`, `plugins/*/skills/*/SKILL.md`, `plugins/*/agents/*.md`, mirroring the auto-discovery in `scripts/validate_skills.py` — and scanning them for semantic overlap. The equivalence verdict per `spec/claude/skill-vs-agent/en.md` §Duplicate prevention is taken against the plugin root the artifact will land in, so a target under `plugins/<plugin>/` is checked against that plugin's own artifacts, not only the root plugin's. If an equivalent or near-equivalent artifact already exists in the target root, stop and propose merge, rename, or supersession—never ship a third overlap.
4. **Draft the files** following the applicable `*-management` spec to the letter:
   - Kebab-case name in the per-type form: **skills** use verb-noun `<object-noun>-<action>` (action token last — `pull-request-create`, `mission-define`); **agents** use object-role `<subject>-<role-noun>` (role noun last — `code-security-reviewer`, `vocab-drift-scanner`). Never mix the two, nor gerund (`-ing`) or noun-only forms. `spec/claude/skill-agent-naming/en.md` is the single point of truth for the role-noun morphology and the closed exception lists — read it rather than assuming a name is off-form; `scripts/validate_skills.py` mirrors those lists and flags any other off-form name as a `Suggestion`. `name` frontmatter matches filename or folder
   ...
   - `phase` declares exactly one identifier from the eight-value vocabulary in `spec/claude/skill-agent-catalog/en.md` §Phase classification — a MUST for both artifact types that the catalog generator hard-fails the docs build on when it's missing or out of vocabulary
