...
Die Planungskadenz selbst wird als eigenes Plugin `nolte-planning` ausgeliefert, das zusätzlich zu `nolte-shared` installiert wird: die Mission-, Roadmap-, Feature- und Sprint-Skills unten werden als `/nolte-planning:<name>` aufgerufen. Die unterstützenden Skills, die diese Seite ebenfalls referenziert — `audience-identify` vorgelagert, `release-notes-curate` und `release-publish-trigger` nachgelagert —, bleiben in `nolte-shared` und behalten das Präfix `/nolte-shared:<name>`. Adoption ist freiwillig — ein Repository ohne `project/`-Verzeichnis installiert das Plugin gar nicht erst und trägt damit auch kein Gewicht davon.
