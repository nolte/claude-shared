---
title: Guides
audience: [maintainer, external-contributor]
content_mode: meta
track: developer-docs
last_updated: 2026-05-28
---

# Guides

Aufgaben-orientierte How-tos und erklärende Hintergrund-Seiten rund um `claude-shared`, von der Mitarbeit am Repository bis zum Verständnis von Skills, Agents und dem Liefer-Lebenszyklus.

## Am Repository arbeiten

- [Entwicklung](development.md) — Einstieg für Mitwirkende
- [Projektstruktur](project-structure.md) — wo liegt was und warum
- [Beitragen](contributing.md) — Workflow, Konventionen, Commits

## Konzepte verstehen

- [Skills](skills-concept.md) — was ein Skill ist und wie er aufgebaut ist
- [Skill-Management](skill-management.md) — der Skill, der Skills anlegt und validiert
- [Spec](spec.md) — der Skill, der mehrsprachige Spezifikationen pflegt
- [Agents](agents-concept.md) — was ein Agent ist und wie er aufgebaut ist

## Lebenszyklus

- [Entwicklungszyklus](development-lifecycle.md) — wo jeder Skill und Agent im Liefer-Lebenszyklus greift
