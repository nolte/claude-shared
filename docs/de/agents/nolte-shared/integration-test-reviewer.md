---
title: integration-test-reviewer
audience: [maintainer]
...
> Prüft bestehende Integrationstests gegen die Integration-Stufen-Spec, liefert ein checklistenbasiertes Konformitätsurteil und wendet nur minimale, gezielte Korrekturen an.

_Reviews existing integration tests against spec/project/test-tier-integration/, returns a checklist-based conformance verdict, and applies only minimal, surgical fixes rather than regenerating. Checks the narrow form (exactly one real external collaborator, the rest doubled), seam-only assertions, real-but-ephemeral dependencies, per-test data isolation, readiness-condition waits, and anti-patterns (broad integration, an in-memory fake that drifts from production, a shared mutable test environment, re-testing unit-tier business logic, fixed sleeps, flaky shared state or container races, hitting a real third-party production API). Use when the user asks to review, audit, or repair existing integration tests. Don't use to scaffold new integration tests (use integration-test-generator), to review unit/component/contract/E2E tests (use the matching tier reviewer), to audit test-tier completeness (use test-pyramid-check), or to run the lint/typecheck/test gate (use quality-gate)._

- **Plugin:** `nolte-shared`
- **Phase:** 5 Review (`review`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `review`
- **Quelle:** [agents/integration-test-reviewer.md](https://github.com/nolte/claude-shared/blob/main/agents/integration-test-reviewer.md)
...
- **you want to scaffold new integration tests for a seam** → [`integration-test-generator`](integration-test-generator.md)
...
You **do not**:
- Scaffold new integration tests or regenerate large parts of a file (that is [`integration-test-generator`](integration-test-generator.md)).
...
2. Apply only minimal, intent-preserving fixes; never regenerate a file wholesale — hand that to [`integration-test-generator`](integration-test-generator.md).
