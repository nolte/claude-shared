...
_Performs a read-only, cross-file deep review of one named target — a file, a route module, a feature, or a stack-wide concern — against spec/frontend/webview-ui-optimization/<canonical_language>.md across all five domains (Performance / Rendering, Security / Sandboxing, Accessibility (WCAG 2.2 AA), Internationalisation, UX / Native-feel). Produces a single severity-sorted findings report that names every rule the target violates, every cross-file coupling that complicates the fix, and every authoritative source from the per-domain research notes under `.audits/webview-ui-expert/`. Read-only: never edits, scaffolds, or runs destructive commands. Invoke when the user — or the `webview-ui-optimize` skill via its `expert-review` operation — needs a deeper read than a single-rule audit row: the auth flow, the dashboard chart, the i18n bootstrap, the CSP-plus-Vite-plus-Emotion pipeline, a complete `prefers-reduced-motion` review, the form-UX contract for a complex form, or the RTL pipeline end-to-end. Also handles equivalent German-language requests (\"Frontend-Review für …\", \"Deep-Review der Dashboard-Charts\"). Don't use for single-rule audits (that's `webview-ui-optimize audit`), for applying fixes (that's `webview-ui-optimize patch`), for dependency-CVE auditing (that's `dependency-audit`), or for prose / Vale review (that's `prose-vale-curator`)._
