...
> Prüft bestehende Contract-Tests gegen die Contract-Stufen-Spec, liefert ein checklistenbasiertes Konformitätsurteil und wendet nur minimale, gezielte Korrekturen an.

_Reviews existing contract tests against spec/project/test-tier-contract/, returns a checklist-based conformance verdict, and applies only minimal, surgical fixes rather than regenerating. Checks the without-both-sides-live model (consumer against a mock, provider by replay), compatibility-only assertions (request/response shape, field presence and types, status codes, protocol), the broker and can-i-deploy gate, and anti-patterns (asserting business logic, treating it as a full integration test, contract drift with no broker or can-i-deploy, over-specifying beyond what the consumer uses, unreconciled versions). Use when the user asks to review, audit, or repair existing contract tests. Don't use to scaffold new contract tests (use contract-test-generator), to review unit/component/integration/E2E tests (use the matching tier reviewer), to audit test-tier completeness (use test-pyramid-check), or to run the lint/typecheck/test gate (use quality-gate)._

- **Plugin:** `nolte-shared`
- **Phase:** 5 Review (`review`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `review`
- **Quelle:** [agents/contract-test-reviewer.md](https://github.com/nolte/claude-shared/blob/main/agents/contract-test-reviewer.md)
...
- **you want to scaffold new contract tests for a boundary** → [`contract-test-generator`](contract-test-generator.md)
...
You **do not**:
- Scaffold new contract tests or regenerate large parts of a file (that is [`contract-test-generator`](contract-test-generator.md)).
...
2. Apply only minimal, intent-preserving fixes; never regenerate a file wholesale — hand that to [`contract-test-generator`](contract-test-generator.md).
