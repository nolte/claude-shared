...
- **Counter-dimension (orchestration, which favours a skill):** the cycle that drives determine → execute → analyse → adapt is a skill ([`test-cycle-orchestrate`](../../skills/nolte-shared/test-cycle-orchestrate.md)); this agent is the adapt step it dispatches, not the loop itself.
...
- Run the full gate (that is [`quality-gate`](../../skills/nolte-shared/quality-gate.md)), or write/repair the test (that is the matching tier generator/reviewer).
