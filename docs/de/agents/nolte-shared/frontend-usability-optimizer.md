...
> Senior-UX-Engineer, der die Usability bestehenden Frontend-Codes direkt verbessert — gegen den selbst erkannten Stack und die dokumentierten UI-Konventionen des Projekts.

_Senior UX engineer that improves the usability of already-implemented frontend code in place — forms, dialogs, list/table views, detail pages — against the project's own framework, component library, form/i18n stack, and documented UI conventions, all detected from the repository at runtime. Edits the presentation layer directly (grouping, labels, units, help text, input types, validation feedback, empty/loading/error states, focus order, responsive mobile-first layout, accessibility, i18n). Use when the user asks to make existing pages, forms, dialogs, or tables more intuitive, more accessible, or visually clearer. Also handles equivalent German-language requests. Don't use to build new features, routes, or business logic (use fullstack-developer); don't use for a read-only audit that only reports findings without touching code (use webview-ui-expert); don't use to run the lint/typecheck gate without changing code (use quality-gate-enforcer). Returns changed files, added i18n keys, and verification status._

- **Plugin:** `nolte-shared`
- **Phase:** 4 Build (`build`)
- **Distribution:** `plugin`
- **Tags:** `frontend`, `ui`, `review`
- **Quelle:** [agents/frontend-usability-optimizer.md](https://github.com/nolte/claude-shared/blob/main/agents/frontend-usability-optimizer.md)
...
- **you want to run the lint/typecheck gate without changing code** → [`quality-gate-enforcer`](quality-gate-enforcer.md)

## Siehe auch

- [`fullstack-developer`](fullstack-developer.md)
- [`webview-ui-expert`](webview-ui-expert.md)
- [`quality-gate-enforcer`](quality-gate-enforcer.md)
