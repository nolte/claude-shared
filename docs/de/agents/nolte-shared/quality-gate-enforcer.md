...
_Reviews the quality-gate wiring (Taskfile targets, `.pre-commit-config.yaml`, `.github/workflows/ci.yml`, timeouts) for spec-conformance against `spec/project/quality-gate/` plus delimitation against `spec/project/workflow-health/`, `dependency-audit/`, `release-automation/`. Read-only — structured findings (`composition-gap`, `runner-drift`, `shape-violation`, `timeout-missing`, `delimitation-leak`, `clean`) routed through Taskfile / pre-commit-config / CI-workflow edits. Distinct from the [`quality-gate`](../../skills/nolte-shared/quality-gate.md) skill, which actually invokes the gate; this agent audits the wiring, never executes it. Invoke when the user asks to \"audit the quality gate\", \"review the quality-gate wiring\", \"check whether the gate is spec-compliant\", or equivalent German-language requests. Don't use to run the gate (use [`quality-gate`](../../skills/nolte-shared/quality-gate.md)), to triage red CI runs (use [`workflow-health-triage`](../../skills/nolte-shared/workflow-health-triage.md)), or to audit CVEs (use [`dependency-audit`](../../skills/nolte-shared/dependency-audit.md))._
