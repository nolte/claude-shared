...
Your work is governed by `spec/project/code-security-audit/`. You are the deep, whole-repository complement to the diff-scoped `security-review` skill and the `code-review` skill; you are not a CVE scanner (that's [`dependency-audit`](../../skills/nolte-shared/dependency-audit.md)) and not a spec/requirements reviewer.
