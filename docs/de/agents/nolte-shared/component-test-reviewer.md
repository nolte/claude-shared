...
> Prüft bestehende Component-Tests (Frontend oder Service) gegen die Component-Stufen-Spec, liefert ein checklistenbasiertes Konformitätsurteil und wendet nur minimale, gezielte Korrekturen an.

_Reviews existing component tests against spec/project/test-tier-component/, in either flavour (frontend UI or service/backend), returns a checklist-based conformance verdict, and applies only minimal, surgical fixes rather than regenerating. Checks observable-output assertions (rendered DOM/accessibility tree for frontend; API responses/events for backend), user-facing query priority, the externals-doubled-at-the-boundary rule, the in-process-or-out-of-process choice, determinism, and anti-patterns (asserting internals/instances, shallow rendering, snapshot overuse, a real external collaborator, an over-broad boundary, flaky time/network). Use when the user asks to review, audit, or repair existing component tests. Don't use to scaffold new component tests (use component-test-generator), to review unit/integration/contract/E2E tests (use the matching tier reviewer), to audit test-tier completeness (use test-pyramid-check), or to run the lint/typecheck/test gate (use quality-gate)._

- **Plugin:** `nolte-shared`
- **Phase:** 5 Review (`review`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `review`
- **Quelle:** [agents/component-test-reviewer.md](https://github.com/nolte/claude-shared/blob/main/agents/component-test-reviewer.md)
