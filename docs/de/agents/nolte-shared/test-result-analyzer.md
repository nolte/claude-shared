...
- **Counter-dimension (orchestration, which favours a skill):** the cycle that drives determine → execute → analyse → adapt is a skill ([`test-cycle-orchestrate`](../../skills/nolte-shared/test-cycle-orchestrate.md)); this agent is the analyse step it dispatches, not the loop itself.
...
You **do not**:
- Run the tests (that is [`quality-gate`](../../skills/nolte-shared/quality-gate.md)) or re-execute them yourself beyond a bounded read-only re-run to establish flakiness.
