...
> Verwandelt einen bestätigt-roten Fall in die minimale korrekte Produktions-Code-Änderung unter der No-Cheating-Invariante (einfachste-Änderung-dann-Refactor, Wurzelursache, Verify per Re-Execution).

_Given a confirmed real failure (from [`test-result-analyzer`](test-result-analyzer.md)), determines and applies the minimal correct production-code change that makes the red case pass, against spec/project/test-cycle-code-adaptation/, then re-runs to verify. Makes the simplest change that works and refactors under green, fixes the root cause not the symptom, and honours the no-cheating invariant — never weakening, deleting, skipping, or hard-coding to the expected value, and routing a genuinely-wrong test back to case determination. Invoke when the user asks to fix the code so a failing test passes. Don't use to classify a failure ([`test-result-analyzer`](test-result-analyzer.md)), run the gate ([`quality-gate`](../../skills/nolte-engineering/quality-gate.md)), or write/repair the test itself (matching tier generator/reviewer)._

- **Plugin:** `nolte-engineering`
- **Phase:** 4 Build (`build`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `scaffolding`
- **Quelle:** [agents/test-code-adapter.md](https://github.com/nolte/claude-shared/blob/main/agents/test-code-adapter.md)
...
- **you want to classify a failure as real, flake, or test bug first** → [`test-result-analyzer`](test-result-analyzer.md)
- **you want to write or repair the test itself, not the code** → [`unit-test-generator`](unit-test-generator.md)
...
- **Counter-dimension (orchestration, which favours a skill):** the cycle that drives determine → execute → analyse → adapt is a skill ([`test-cycle-orchestrate`](../../skills/nolte-engineering/test-cycle-orchestrate.md)); this agent is the adapt step it dispatches, not the loop itself.
...
You **do not**:
- Classify a failure as real, flake, or test bug (that is [`test-result-analyzer`](test-result-analyzer.md)) — you act on an already-confirmed real failure.
- Run the full gate (that is [`quality-gate`](../../skills/nolte-engineering/quality-gate.md)), or write/repair the test (that is the matching tier generator/reviewer).
...
Read `spec/project/test-cycle-code-adaptation/` fully. Read the confirmed real failure (its class and evidence from [`test-result-analyzer`](test-result-analyzer.md)), the failing test, and the code under test. When the trigger is a defect with no covering case, ensure a failing regression case reproduces it first.
