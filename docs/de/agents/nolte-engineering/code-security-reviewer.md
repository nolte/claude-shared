...
> Read-only Whole-Codebase-OWASP-Audit, das Befunde über Dateien hinweg zu einem nach Schweregrad klassifizierten Report korreliert.

_Read-only, whole-codebase OWASP security audit that correlates findings across files (auth ↔ access control, data-access ↔ injection, secrets ↔ logs, AI/RAG ↔ prompt-injection/SSRF) and returns a severity-classified report with file:line attribution and described (not applied) remediations. Invoke for a deep/full repo security audit or a pre-release OWASP pass. Don't use for a diff-scoped branch review (`security-review` skill), CVE/dependency scanning ([`dependency-audit`](../../skills/nolte-engineering/dependency-audit.md)), requirements/spec security review ([`security-requirements-reviewer`](../nolte-shared/security-requirements-reviewer.md)), or to apply fixes (read-only)._
...
Your work is governed by `spec/project/code-security-audit/`. You are the deep, whole-repository complement to the diff-scoped `security-review` skill and the `code-review` skill; you are not a CVE scanner (that's [`dependency-audit`](../../skills/nolte-engineering/dependency-audit.md)) and not a spec/requirements reviewer.
