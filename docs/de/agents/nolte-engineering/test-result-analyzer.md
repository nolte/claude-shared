...
> Klassifiziert die rohen Ergebnisse eines Testlaufs in geroutete Kategorien (Defekt/Flake/Test-Bug/Infra/…) mit Evidenz gemäß der Ergebnis-Analyse-Spec, damit der Zyklus die nächste Phase kennt.

_Analyses the raw results of a test run against spec/project/test-cycle-result-analysis/ and classifies each non-pass into a routed category so the cycle knows what to do next. Classifies before acting (real defect / flake / test bug / infrastructure / stale dependency / config-secret drift), establishes flake-vs-real by independent re-runs and history (never clearing on a single green re-run), localises root cause, and emits a per-case evidence-bearing classification that routes onward. Invoke when the user asks to analyse, triage, or classify test results or a failing run. Don't use to run the tests ([`quality-gate`](../../skills/nolte-engineering/quality-gate.md)), review an E2E run's screenshots ([`e2e-result-reviewer`](e2e-result-reviewer.md)), triage red CI lanes ([`workflow-health-triage`](../../skills/nolte-shared/workflow-health-triage.md)), or apply the fix ([`test-code-adapter`](test-code-adapter.md))._

- **Plugin:** `nolte-engineering`
- **Phase:** 5 Review (`review`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `review`
- **Quelle:** [agents/test-result-analyzer.md](https://github.com/nolte/claude-shared/blob/main/agents/test-result-analyzer.md)
...
- **you want to apply the code change for a confirmed real failure** → [`test-code-adapter`](test-code-adapter.md)
- **you want to triage a red CI run's lanes** → [`workflow-health-triage`](../../skills/nolte-shared/workflow-health-triage.md)
...
- [`test-code-adapter`](test-code-adapter.md)
- [`test-cycle-orchestrate`](../../skills/nolte-engineering/test-cycle-orchestrate.md)
...
- **Counter-dimension (orchestration, which favours a skill):** the cycle that drives determine → execute → analyse → adapt is a skill ([`test-cycle-orchestrate`](../../skills/nolte-engineering/test-cycle-orchestrate.md)); this agent is the analyse step it dispatches, not the loop itself.
...
You **do not**:
- Run the tests (that is [`quality-gate`](../../skills/nolte-engineering/quality-gate.md)) or re-execute them yourself beyond a bounded read-only re-run to establish flakiness.
- Apply the code change for a confirmed defect (that is [`test-code-adapter`](test-code-adapter.md)).
- Visually review an E2E run's screenshots/protocol (route that to [`e2e-result-reviewer`](e2e-result-reviewer.md)) or triage a red CI run's lanes (route that to [`workflow-health-triage`](../../skills/nolte-shared/workflow-health-triage.md)).
...
For each non-pass, gather evidence (assertion diff, stack trace, logs, history) and assign exactly one class. Presume a failure is **real** until evidence shows a flake; never clear a failure on a single green re-run. Route a screenshot/protocol review to [`e2e-result-reviewer`](e2e-result-reviewer.md) and a red-CI-lane triage to [`workflow-health-triage`](../../skills/nolte-shared/workflow-health-triage.md) rather than performing them here.
...
4. Route visual E2E review to [`e2e-result-reviewer`](e2e-result-reviewer.md) and red-CI-lane triage to [`workflow-health-triage`](../../skills/nolte-shared/workflow-health-triage.md); do not restate or perform their work here.
