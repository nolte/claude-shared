...
7. **Attach a recommended specialist** per §Specialist recommendation to every finding:
   - **Match the catalog.** Read every `description` line under `skills/*/SKILL.md` and `agents/*.md`; name the matching slug verbatim (for example [`dependency-audit`](../nolte-engineering/dependency-audit.md), [`workflow-health-triage`](workflow-health-triage.md), [`feature-decompose`](feature-decompose.md), [`pull-request-merge`](pull-request-merge.md), [`vocab-drift-audit`](vocab-drift-audit.md)).
