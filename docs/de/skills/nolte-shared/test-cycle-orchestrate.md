...
> Treibt den iterativen Test-Zyklus (ermitteln → ausführen → analysieren → anpassen → erneut ausführen) für ein Feature, dispatcht jede Phase und schleift bis Grün unter der No-Cheating-Invariante.
...
- **Tags:** `quality-gate`
- **Quelle:** [skills/test-cycle-orchestrate/SKILL.md](https://github.com/nolte/claude-shared/blob/main/skills/test-cycle-orchestrate/SKILL.md)
...
- **you want to scaffold tests for a tier without running the loop** → [`unit-test-generator`](../../agents/nolte-shared/unit-test-generator.md)
...
- **Orchestrator that chains other capabilities:** the cycle dispatches [`test-case-extractor`](../../agents/nolte-shared/test-case-extractor.md) / per-tier generators, [`quality-gate`](quality-gate.md), [`test-result-analyzer`](../../agents/nolte-shared/test-result-analyzer.md), and [`test-code-adapter`](../../agents/nolte-shared/test-code-adapter.md) in a loop; the skill-orchestrates pattern (per `skill-vs-agent`) defaults the orchestrator to skill form.
...
Ensure the cases that should be green exist and currently fail or are absent. Dispatch [`test-case-extractor`](../../agents/nolte-shared/test-case-extractor.md) to derive abstract cases from a requirement, and the per-tier generator ([`unit-test-generator`](../../agents/nolte-shared/unit-test-generator.md), [`component-test-generator`](../../agents/nolte-shared/component-test-generator.md), [`integration-test-generator`](../../agents/nolte-shared/integration-test-generator.md), [`contract-test-generator`](../../agents/nolte-shared/contract-test-generator.md), or [`e2e-test-generator`](../../agents/nolte-shared/e2e-test-generator.md)) to scaffold the runnable test at the tier the foundation's lowest-tier-that-gives-confidence rule selects. When the trigger is a confirmed defect, a **failing regression case is written first**.
...
Dispatch [`test-result-analyzer`](../../agents/nolte-shared/test-result-analyzer.md) to classify each non-pass into a routed category (real defect / flake / test bug / infra / stale dep / config drift) with evidence. Route a flake to quarantine, a test bug back to phase 1 as a reviewable case change, and infra/stale/config to the environment (via [`workflow-health-triage`](workflow-health-triage.md)).

#### Phase 4 — Adapt code

For each confirmed real failure, dispatch [`test-code-adapter`](../../agents/nolte-shared/test-code-adapter.md) to apply the minimal correct change that satisfies the asserted behaviour, fixing the root cause. The fix **re-enters phase 2** (re-execute); never assume green without re-running.
...
5. **Never** restate a phase's internals here; dispatch its capability ([`test-case-extractor`](../../agents/nolte-shared/test-case-extractor.md) / tier generators, [`quality-gate`](quality-gate.md), [`test-result-analyzer`](../../agents/nolte-shared/test-result-analyzer.md), [`test-code-adapter`](../../agents/nolte-shared/test-code-adapter.md)) and orchestrate the loop. When a phase spec disagrees with this skill, the spec wins; propose a skill update rather than diverging.
