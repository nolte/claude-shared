...
- **Phase:** 4 Build (`build`)
- **Tags:** `prose`, `audience`
- **Quelle:** [skills/blog-author/SKILL.md](https://github.com/nolte/claude-shared/blob/main/skills/blog-author/SKILL.md)

---

## Blog Author

Operationalises the consumer-side blog-author contract in the `nolte/blog` repository: the role-and-process layer that produces a bilingual EN-canonical + DE-translated post pair, runs the pre-handoff self-check against the two sibling specs, and hands the pair off to the editorial layer via `lektorat-apply`.
...
- **Orchestration role**: this skill dispatches `lektorat-apply` for the audit step at handoff, and may dispatch `prose-vale-curator` during the transitional regime; the skill-orchestrates pattern (per `spec/claude/skill-vs-agent/`) defaults the orchestrator to skill form.
...
Detect the user's language from their message and respond in it. The Markdown files written into `src/content/posts/en/` are **always English** (the canonical source language per `blog/CLAUDE.md`); the files written into `src/content/posts/de/` are **always German**. The delivery-contract artefacts (self-check manifest, source-to-claim mapping, handoff manifest) are written in English so downstream tooling and `lektorat-apply` can parse them reliably; prose around the artefacts in the operator dialogue is localised.
...
Every run produces, at minimum, the post pair plus three delivery-contract artefacts. The post pair lands at the paths above; the delivery artefacts land alongside, written in English so `lektorat-apply` and any downstream skill can parse them.
...
- **Handoff manifest** — three lines naming (a) the chosen transitional regime or `lektorat-apply` invocation, (b) the `task build` status and the command used, (c) the repository state (branch name, optional commit SHA) the self-check ran against.

The skill writes all three under `project/handovers/<slug>.md` in the consumer repo (a single Markdown file with three top-level headings, one per artefact); the path is part of the merge-commit so `lektorat-apply` and any reviewer can read them together with the post pair.
...
- Never re-implement editorial mechanics in this skill. The five quality dimensions (D1–D5), the three operations (`audit` / `patch` / `revise`), the severity floor, and the language-resolution chain live in `spec/project/lektorat/` and are owned by `lektorat-apply`.
...
- **The lektor transitional regime is still alive in the spec.** `spec/project/blog-author/` §Übergangsregime allows either `prose-vale-curator` over the EN file or a documented author self-review, until the lektor spec lifts `Status: draft`. This skill defaults to the target-state path (dispatching `lektorat-apply`) because the skill exists and is operational; record the chosen path in the handoff manifest so the spec-update trigger is visible.
