...
[`sprint-execute`](sprint-execute.md) Operation C (`in_progress → done`), step 6, dispatches this skill for the just-completed feature after it surfaces the updated sprint state. The dispatch is the reference wiring for the spec's `feature → done` trigger event; the operator then walks Operations 3 to 5 here. A feature done via a direct invocation of this skill (not through [`sprint-execute`](sprint-execute.md)) follows the identical path.
