...
- **You want to draft the post itself rather than decide and derive the briefing** → [`blog-author`](blog-author.md)
- **You want to publish a release rather than trigger a blog post** → [`release-publish-trigger`](release-publish-trigger.md)
...
- **Source consumer** — this repository: hosts features under `project/features/<slug>.md` and drives transitions via [`sprint-execute`](sprint-execute.md). The `in_progress → done` transition is the trigger event.
...
When [`sprint-execute`](sprint-execute.md) automatically dispatches this skill, it passes the just-transitioned feature's `id`/`slug`. When invoked directly, resolve the payload from `project/features/<slug>.md` plus `git log` for the transition SHA.
...
Apply the deterministic rules from `spec/project/blog-author-trigger/` §Briefing derivation. The derived values are starting points the operator refines at [`blog-author`](blog-author.md) intake, never final values:

- **topic-as-thesis** — feature `title` plus the first sentence of `description`, phrased as an assertion.
- **grounded artefact** — `<owner>/<repo>` plus the `done`-transition SHA (satisfies [`blog-author`](blog-author.md) §Briefing inputs minimally).
- **primary audience** — left **unset**; the operator selects it at [`blog-author`](blog-author.md) intake (the trigger has no basis to choose).
...
- [`sprint-review`](sprint-review.md) surfaces unconsumed deferrals at sprint close (the source consumer's wiring; this skill writes the artefact, doesn't drive the surfacing).
...
- **Never** silently `cd` into the blog-consumer working tree. Surface its path and let the operator confirm the working-directory switch (or open a new session in the blog clone) before any [`blog-author`](blog-author.md) dispatch.
- **Never** write files into the blog consumer's working tree without explicit operator confirmation.
- The pre-staged `project/blog-triggers/<feature-slug>.briefing.md` uses the briefing shape [`blog-author`](blog-author.md) §Briefing inputs expects, so the operator can copy or open it in the blog session.

When the source consumer **is** the blog consumer, the handover collapses: dispatch [`blog-author`](blog-author.md) in-place with no working-directory switch.

### Automatic dispatch from sprint-execute

[`sprint-execute`](sprint-execute.md) Operation C (`in_progress → done`), step 5, dispatches this skill for the just-completed feature after it surfaces the updated sprint state. The dispatch is the reference wiring for the spec's `feature → done` trigger event; the operator then walks Operations 3 to 5 here. A feature done via a direct invocation of this skill (not through [`sprint-execute`](sprint-execute.md)) follows the identical path.
...
- **Never** draft the post itself; dispatching [`blog-author`](blog-author.md) is the only authoring path.
