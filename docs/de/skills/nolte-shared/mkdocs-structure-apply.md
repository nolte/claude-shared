...
- **Quelle:** [skills/mkdocs-structure-apply/SKILL.md](https://github.com/nolte/claude-shared/blob/main/skills/mkdocs-structure-apply/SKILL.md)
...
- **Per-page structure**: every page in `docs/<lang>/` starts with a single `# H1` matching the nav label, declares `title` / `audience` / `last_updated` frontmatter, audience values match IDs declared in the project's audience artifact.
...
- **Missing or invalid per-page frontmatter**: propose the minimum addition (`title` derived from the H1, `audience` set to a placeholder ID with a `# TODO: replace with audience artifact ID` comment, `last_updated` set to today's date). When the audience artifact is missing, route the user to `audience-identify` rather than fabricating an audience ID.
