...
- A requirement artefact under `project/requirements/` covers the work being decomposed, with `U_gate >= tau_high`, per `spec/project/requirements-elicitation/` SS Consumer contract. When none exists or it is below threshold, dispatch [`requirements-elicit`](requirements-elicit.md) first, or record an explicit operator override; decomposing against unstated or weakly-understood requirements is the failure this gate prevents.
...
([`sprint-plan`](sprint-plan.md) retargeting `sprint`, or [`sprint-execute`](sprint-execute.md) adding the feature
