...
> Führt ein rohes GitHub-Issue end-to-end: durchdringen, klassifizieren, in spezialisten-gerechte Arbeitspakete zerlegen, routen oder dispatchen und bis zu einem offenen PR verifizieren.
...
- **Phase:** 2 Plan (`plan`)
- **Tags:** `triage`, `audit`
- **Quelle:** [skills/issue-orchestrate/SKILL.md](https://github.com/nolte/claude-shared/blob/main/skills/issue-orchestrate/SKILL.md)
...
- **You want to merge the pull request the orchestration produced** → [`pull-request-merge`](pull-request-merge.md)
- **You want to triage a failing CI workflow run, not an issue** → [`workflow-health-triage`](workflow-health-triage.md)
- **You already have a roadmap item and just want it decomposed into features** → [`feature-decompose`](feature-decompose.md)
- **You want to triage portfolio audit findings rather than a single issue** → [`continuous-improvement-triage`](continuous-improvement-triage.md)
...
- `infra` about CI → hand to [`workflow-health-triage`](workflow-health-triage.md) rather than decompose here.
...
- **Route to the pipeline** when the issue spans more than one goal outcome, needs
  ...
  Hand it to [`feature-decompose`](feature-decompose.md) (existing roadmap item) or [`roadmap-plan`](roadmap-plan.md) (new
...
open the PR via [`pull-request-create`](pull-request-create.md) (the operator confirms title and body per that
...
an open, audit-trailed PR. The merge belongs to [`pull-request-merge`](pull-request-merge.md), which
...
"next action: invoke [`pull-request-merge`](pull-request-merge.md) after CI is green".
...
- **Never** decompose an issue that belongs in the formal pipeline (more than one
  ...
  implementation; route it to [`feature-decompose`](feature-decompose.md) or [`roadmap-plan`](roadmap-plan.md) instead.
...
- **Never** merge the PR ([`pull-request-merge`](pull-request-merge.md) owns that), pass `--admin`, mask a
...
- **An `infra` issue about a red workflow is not decomposed here.** A CI-failure
  issue is [`workflow-health-triage`](workflow-health-triage.md)'s domain; decomposing it into work packages
