...
Topic folders group related specs (e.g. `spec/claude/`, `spec/api/`). **Exactly one level** of topic nesting is allowed; without a topic, a spec lives directly under `spec/<slug>/`.
...
If the file is missing, defaults apply. It is created on the first `create` operation so downstream projects have a visible extension point.
...
4. Translate into every other configured language. Structure stays identical. RFC 2119 keywords stay English and are glossed in-language, e.g. `MUSS [MUST]`, `SOLLTE [SHOULD]`, `KANN [MAY]`.
