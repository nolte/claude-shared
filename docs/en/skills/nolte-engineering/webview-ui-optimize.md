...
> Audits and patches a browser-rendered frontend across Performance, Security, A11y (WCAG 2.2 AA), i18n, and UX.

_Audits a browser-rendered frontend against the canonical-language file under spec/frontend/webview-ui-optimization/ and, with per-item user approval, patches findings across five domains: Performance, Security, Accessibility (WCAG 2.2 AA), Internationalisation, and UX. Three operations: `audit` (written to `.audits/webview-ui-optimize/`), `patch` (one finding at a time), `expert-review` (dispatches [`webview-ui-expert`](../../agents/nolte-engineering/webview-ui-expert.md)). Invoke when the user asks to \"audit the frontend\", \"check the UI against the webview-ui spec\", \"optimise for performance / a11y / i18n / UX / security\", \"fix the CSP\", \"wire up vitest-axe\", or equivalent German-language requests. Don't use for brand-design decisions, audience artefact ([`audience-identify`](../nolte-shared/audience-identify.md)), prose linting ([`prose-vale-curator`](../../agents/nolte-shared/prose-vale-curator.md)), CVE audits ([`dependency-audit`](dependency-audit.md)), or release-pipeline (`release-automation`). Supports resume on re-invocation per `spec/claude/resumable-work/`._
...
- [`webview-ui-expert`](../../agents/nolte-engineering/webview-ui-expert.md)
...
- **Orchestrator pattern.** The skill dispatches [`webview-ui-expert`](../../agents/nolte-engineering/webview-ui-expert.md) for deep cross-file review, dispatches [`dependency-audit`](dependency-audit.md) for the supply-chain MUSTs, and routes to [`quality-gate`](quality-gate.md) for the post-patch lint/type/test sweep. Per `spec/claude/skill-vs-agent/` §Hybrid pattern, the orchestrator is always a skill.
- **Precedent.** Follows the audit + patch shape of [`mkdocs-structure-apply`](../nolte-shared/mkdocs-structure-apply.md), [`readme-structure-apply`](../nolte-shared/readme-structure-apply.md), [`docs-audience-tracks-apply`](../nolte-shared/docs-audience-tracks-apply.md), [`project-structure-apply`](../nolte-shared/project-structure-apply.md); portfolio-wide consistency favours the same artefact type.
...
For a named file, route module, or feature whose findings cross several spec rules (e.g. "the auth flow", "the dashboard chart", "the i18n bootstrap"), dispatch the [`webview-ui-expert`](../../agents/nolte-engineering/webview-ui-expert.md) agent. The agent receives:
...
- `spec/claude/skill-vs-agent/` — the skill-vs-agent decision dimensions that justify this artefact as a skill, with the [`webview-ui-expert`](../../agents/nolte-engineering/webview-ui-expert.md) agent as the deep-review specialist.
