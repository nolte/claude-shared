...
- Severities are the canonical scale from `spec/claude/review-plan/` §"Severity scale"; surface findings for [`spec-drift-audit`](spec-drift-audit.md) to reconcile. Treat any open Critical as a gate failure (the silent fork must not reach `develop`).
