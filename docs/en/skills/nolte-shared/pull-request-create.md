...
> Opens a spec-conformant draft GitHub pull request on the current feature branch.
...
- **Tags:** `pull-request`
- **Source:** [skills/pull-request-create/SKILL.md](https://github.com/nolte/claude-shared/blob/main/skills/pull-request-create/SKILL.md)
...
- **Audit-triggered remediation PRs carry two extra lines in Risk / rollout notes.** When this PR remediates an in-scope finding from a portfolio audit ([`spec-drift-audit`](spec-drift-audit.md), `workflow-health`, [`project-structure-apply`](project-structure-apply.md), [`vocab-drift-audit`](vocab-drift-audit.md), [`portfolio-audit`](portfolio-audit.md), [`portfolio-inflight-triage`](portfolio-inflight-triage.md), [`dependency-audit`](../nolte-engineering/dependency-audit.md), `prose-style`/`markdown-formatting` lint, or a manual review Issue), `spec/project/continuous-improvement/<canonical_language>.md` §"Traceability in remediation artifacts" **MUST**-requires the **Risk / rollout notes** section to additionally record both of the following lines:
