...
4. **Validate** — `skill-management` in validation mode. Mechanical defects (frontmatter mismatch, absolute paths, missing hard-rules) can be auto-fixed on request.
...
- Mention what you tested (e.g. "loaded plugin via `claude --plugin-dir .`, `/nolte-shared:spec` shows up in the `/skills` dialog").
...
- **`description` too vague** → Claude cannot route reliably. At least three concrete phrasings.
