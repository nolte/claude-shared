...
> Scaffolds spec-conformant unit tests (FIRST, AAA, observable-behaviour assertions, disciplined doubles, TC-IDs) for a module, defaulting to a pytest reference profile.

_Scaffolds spec-conformant unit tests for a module or feature against spec/project/test-tier-unit/, defaulting to a pytest reference profile when no other stack is declared. Wires the FIRST properties (fast, isolated, no real input/output), the project's solitary-or-sociable style, Arrange-Act-Assert with one behaviour per test, intention-revealing names, disciplined Meszaros test doubles (no over-mocking), observable-behaviour assertions through the public interface, and TC-ID traceability. Use when the user asks to generate/scaffold unit tests for a module or feature, or turn abstract test cases into runnable unit tests. Don't use to review or repair existing unit tests (use unit-test-reviewer), to scaffold component/integration/contract/E2E tests (use the matching tier generator), to derive abstract test cases from a requirement (use test-case-extractor), or to run the lint/typecheck/test gate (use quality-gate)._

- **Plugin:** `nolte-shared`
- **Phase:** 4 Build (`build`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `scaffolding`
- **Source:** [agents/unit-test-generator.md](https://github.com/nolte/claude-shared/blob/main/agents/unit-test-generator.md)
