...
> Scaffolds spec-conformant contract tests (consumer-driven by default: consumer expectations + provider verification, broker, can-i-deploy), asserting agreement compatibility only.

_Scaffolds spec-conformant contract tests against spec/project/test-tier-contract/, verifying a service-to-service agreement without standing up both sides live. Defaults to the consumer-driven model (the consumer records its expectations as a contract; the provider is verified independently by replaying it), and also scaffolds the provider-driven or bi-directional variant when the project uses it. Wires the consumer mock that emits the contract, provider verification with provider states, a contract broker, and a can-i-deploy compatibility gate; asserts only agreement compatibility (request/response shape, field presence and types, status codes, protocol), never business logic. Use when the user asks to generate/scaffold contract or consumer-driven-contract tests for a service boundary. Don't use to review existing contract tests (use contract-test-reviewer), to scaffold unit/component/integration/E2E tests (use the matching tier generator), or to run the lint/typecheck/test gate (use quality-gate)._

- **Plugin:** `nolte-shared`
- **Phase:** 4 Build (`build`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `scaffolding`
- **Source:** [agents/contract-test-generator.md](https://github.com/nolte/claude-shared/blob/main/agents/contract-test-generator.md)
