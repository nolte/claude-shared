...
4. **Draft the files** following the applicable `*-management` spec to the letter:
   - Kebab-case name in the per-type form: **skills** use verb-noun `<object-noun>-<action>` (action token last — [`pull-request-create`](../../skills/nolte-shared/pull-request-create.md), [`mission-define`](../../skills/nolte-shared/mission-define.md)); **agents** use object-role `<subject>-<role-noun>` (role noun last, usually `-er`/`-or`/`-ist` — [`code-security-reviewer`](../nolte-engineering/code-security-reviewer.md), [`vocab-drift-scanner`](vocab-drift-scanner.md)). Never mix the two, nor gerund (`-ing`) or noun-only forms. Closed exceptions are skills [`spec`](../../skills/nolte-shared/spec.md) / [`yaml-json-schema`](../../skills/nolte-shared/yaml-json-schema.md) / [`quality-gate`](../../skills/nolte-engineering/quality-gate.md) and agents [`png-to-transparent-svg`](../nolte-media/png-to-transparent-svg.md) / [`audience-review`](audience-review.md) (`scripts/validate_skills.py` mirrors both lists and flags any other off-form name as a `Suggestion`). `name` frontmatter matches filename or folder
