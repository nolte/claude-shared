...
> Derives structured, framework-agnostic, traceable test cases from a requirement document, written from the user-observable-behaviour perspective.

_Derive structured, framework-agnostic test cases from a requirement or specification document. Decomposes the requirement into functional requirements, acceptance criteria, state changes, validation rules, and observable error states, then writes user-observable-behaviour test cases (no internal implementation detail) as structured Markdown documents with full traceability and a coverage summary. Works in the source document's language and adapts to the project's user-facing interface (browser, CLI, API). Use when the user asks to derive/extract test cases or E2E/acceptance scenarios from a requirement or spec, to map coverage against a specification, or to produce test-case documentation. Don't use to generate test-automation code (Selenium/Playwright/Cypress), to run tests or fix failures (use quality-gate), or to audit the test-tier/pyramid shape._

- **Plugin:** `nolte-shared`
- **Phase:** 3 Design (`design`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `scaffolding`
- **Source:** [agents/test-case-extractor.md](https://github.com/nolte/claude-shared/blob/main/agents/test-case-extractor.md)
...
- **you want to run the test suite, classify failures, or fix test code** → [`quality-gate`](../../skills/nolte-shared/quality-gate.md)
