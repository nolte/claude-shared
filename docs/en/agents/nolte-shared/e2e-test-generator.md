...
> Scaffolds a spec-conformant E2E suite (page objects, waits, screenshots, markers, protocol) for a feature, defaulting to the Selenium + pytest reference profile.

_Scaffolds a spec-conformant end-to-end test suite for a feature against spec/project/e2e-test-automation/, defaulting to the Selenium + pytest reference profile when no other stack is declared. Wires page-object encapsulation, data-testid-first locators, condition-based waits, screenshot checkpoints, markers, TC-ID traceability, and the machine-generated protocol; scaffolds the directory layout and adapts the shipped templates. Use when the user asks to generate/scaffold E2E or browser tests for a feature, turn test cases into runnable E2E tests, or set up an E2E suite. Don't use to review or repair an existing suite (use e2e-test-reviewer), to review a run's screenshots/logs (use e2e-result-reviewer), to derive abstract test cases from a requirement (use test-case-extractor), or to run the unit/lint/typecheck gate (use quality-gate)._

- **Plugin:** `nolte-shared`
- **Phase:** 4 Build (`build`)
- **Distribution:** `plugin`
- **Tags:** `quality-gate`, `scaffolding`
- **Source:** [agents/e2e-test-generator.md](https://github.com/nolte/claude-shared/blob/main/agents/e2e-test-generator.md)
