...
> Read-only in-flight data collector: open issues, PRs (incl. drafts), branches without PR, unresolved review threads + Discussions across nolte/*.
...
- **Distribution:** `plugin`
- **Tags:** `audit`
- **Source:** [agents/portfolio-inflight-collector.md](https://github.com/nolte/claude-shared/blob/main/agents/portfolio-inflight-collector.md)
...
Read `agents/portfolio-inflight-collector/output-shape.md` for the full report template (every field, every data-source sub-block, and the aggregated-overview totals). The calling [`portfolio-inflight-triage`](../../skills/nolte-shared/portfolio-inflight-triage.md) skill parses that shape, so reproduce it exactly.
