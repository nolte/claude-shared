...
> Drafts or refines audience-tailored documentation (README, release notes, MkDocs pages) against an existing audience artifact.

_Draft or refine an audience-tailored documentation artifact (README, release notes, MkDocs pages, or any doc whose governing spec lives under spec/project/) against an existing audience artifact from [`audience-identify`](../../skills/nolte-shared/audience-identify.md). Invoke when the user asks to write, draft, or refactor a doc for specific audiences; also German requests. Needs an existing audience artifact — dispatch [`audience-identify`](../../skills/nolte-shared/audience-identify.md) first. Don't use for plugin skills/agents ([`claude-plugin-developer`](claude-plugin-developer.md)), spec authoring ([`spec`](../../skills/nolte-shared/spec.md)), or a greenfield README ([`readme-structure-apply`](../../skills/nolte-shared/readme-structure-apply.md), which then dispatches this agent). Supports resume per `spec/claude/resumable-work/`._

- **Plugin:** `nolte-shared`
- **Phase:** 3 Design (`design`)
- **Distribution:** `plugin`
- **Tags:** `audience`, `prose`
- **Source:** [agents/audience-doc-author.md](https://github.com/nolte/claude-shared/blob/main/agents/audience-doc-author.md)
