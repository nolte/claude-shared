...
The suite ships as its own plugin, `nolte-planning`, installed on top of `nolte-shared`; every skill below is called as `/nolte-planning:<name>`. Adoption is voluntary—a repository without a `project/` directory doesn't install the plugin at all, and so carries none of its weight.
