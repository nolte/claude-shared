...
- The repository `nolte/claude-shared`, published via the plugin marketplace as the Claude Code plugin `nolte-shared` plus the three companion plugins `nolte-media`, `nolte-engineering`, and `nolte-claude-dev` under `plugins/`, all versioned in lockstep (the authoritative version is whatever `.claude-plugin/plugin.json` carries; no copy of it is restated here, so this file can't drift against a release).
