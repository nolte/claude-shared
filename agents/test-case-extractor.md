...
description: Derive structured, framework-agnostic test cases from a requirement or specification document. Decomposes the requirement into functional requirements, acceptance criteria, state changes, validation rules, and observable error states, then writes user-observable-behaviour test cases (no internal implementation detail) as structured Markdown documents with full traceability and a coverage summary. Works in the source document's language and adapts to the project's user-facing interface (browser, CLI, API). Use when the user asks to derive/extract test cases or E2E/acceptance scenarios from a requirement or spec, to map coverage against a specification, or to produce test-case documentation. Don't use to generate test-automation code (Selenium/Playwright/Cypress), to run tests or fix failures (use quality-gate), or to audit the test-tier/pyramid shape.
distribution: plugin
