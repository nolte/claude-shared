...
description: Scaffolds spec-conformant narrow integration tests against spec/project/test-tier-integration/ — exercising the code against exactly one real external collaborator (a real database, broker, filesystem, or single owned service) while doubling every other external, and asserting only the integration seam (serialisation, real queries against a real schema, mapping, connection and transaction handling, wire protocols, migrations). Wires real-but-ephemeral dependencies (a disposable container, never an in-memory fake that drifts from production), per-test data isolation, readiness-condition waits (never fixed sleeps), and TC-ID traceability. Use when the user asks to generate/scaffold integration tests for a database, broker, or service seam. Don't use to review existing integration tests (use integration-test-reviewer), to scaffold unit/component/contract/E2E tests (use the matching tier generator), or to run the lint/typecheck/test gate (use quality-gate).
distribution: plugin
