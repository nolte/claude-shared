...
- **Stale restatement:** for every AC that *does* trace to a Requirement, read the two against each other. An AC that's narrower, wider, or carries an exception the Requirement doesn't states a different rule than the one it traces to → `Warning`. This is a semantic comparison of the paired statements, not a verb-strength check, so the no-prose-contradiction rule of Phase 2 doesn't apply here: the pairing is what's compared. It's the residue of an edit that reached the Requirement and not what restates it, so read the exception clauses especially closely — "(X excepted)" on one side and nothing on the other is the typical shape.
