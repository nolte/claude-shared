...
- **Input completeness across triggers** (§E): a job calling a reusable workflow that leans on the callee's input defaults instead of forwarding them. Your tools are `Read`, `Grep`, `Glob` with no network, so you can only inspect the callee when it lives in this working copy. Keep the two cases apart:
  - **Callee readable here** — compare its `workflow_call` input defaults against what the caller forwards. A default reading `github.event.<field>` that the caller doesn't override is a finding for every declared trigger whose payload lacks that field.
  - **Callee external** (`uses: <owner>/<repo>/…`) — you can't see its defaults. Report a caller that forwards no inputs at all as a **Suggestion**, naming the callee reference and asking the operator to confirm its defaults. Never record it as a confirmed violation, and never stay silent just because the check was unevaluable: an unevaluated check reported as nothing found is indistinguishable from a clean result.
