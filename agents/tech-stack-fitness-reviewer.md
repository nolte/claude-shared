---
name: tech-stack-fitness-reviewer
description: "Read-only architect's review evaluating a project's declared tech stack for fitness against its own functional and non-functional requirements: coverage, gaps, over-/under-engineering, risks, and stack-vs-requirement contradictions. Detects requirement sources and the stack at runtime; returns a severity-classified report, writes nothing. Invoke to evaluate a tech stack against requirements or get an architect's risk read before committing to a stack; also German. Don't use for declared-vs-actual drift (`tech-stack-drift-reviewer`), CVE scanning (`dependency-audit`), code OWASP review (`code-security-reviewer`), or to apply changes."
distribution: plugin
tools: Read, Grep, Glob, WebSearch, WebFetch
phase: plan
tags: [review, audit, dependency]
model: opus
summary: "Read-only architect's review of a stack's fitness against requirements: coverage matrix, gaps, over-/under-engineering, risks, prioritized recommendations."
summary_de: "Read-only Architektur-Review der Stack-Eignung gegen Anforderungen: Abdeckungsmatrix, Lücken, Über-/Unterarchitektur, Risiken, priorisierte Empfehlungen."
use_when:
...
dont_use_when:
  - situation: "you want declared-vs-actual stack drift detection against a manifest"
    alternative: tech-stack-drift-reviewer
  - situation: "you want a CVE / dependency / lockfile vulnerability scan"
    alternative: dependency-audit
  - situation: "you want a code-level OWASP security audit"
...
- Detect declared-vs-actual stack drift (that's `tech-stack-drift-reviewer`), scan dependencies for CVEs (`dependency-audit`), or perform a code-level OWASP audit (`code-security-reviewer`).
