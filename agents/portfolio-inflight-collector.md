...
Read `agents/portfolio-inflight-collector/output-shape.md` for the full report template (every field, every data-source sub-block, and the aggregated-overview totals). The calling `portfolio-inflight-triage` skill parses that shape, so reproduce it exactly.
