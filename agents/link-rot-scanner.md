...
description: Audit the documentation of the current repository for dead links — internal relative links, intra-page anchors, cross-tree references into `spec/` / `src/` / `scripts/` / other repo roots, and external `http(s)` URLs probed over the network. Wraps the deterministic `scripts/check_links.py` and triages its findings into a severity-sorted report, classifying network flakiness (timeouts, transient 5xx, rate-limits) as warning rather than rot. Read-only: produces a report and never edits files. Use when the user asks to "audit the links", "find dead external links", "run a link-rot audit", "check the docs for broken URLs before a release", or equivalent German-language requests. Don't use for the deterministic offline CI gate (call `scripts/check_links.py --offline` directly), for the broader docs-freshness drift audit (that's `docs-freshness-checker`), or for prose / Vale linting (that's `prose-vale-curator`).
distribution: plugin
tools: Read, Glob, Grep, Bash
model: sonnet
tags: [audit, quality-gate]
phase: quality
summary: "Read-only link-rot audit: internal, anchor, cross-tree, and external links via scripts/check_links.py, triaged into a severity-sorted report."
summary_de: "Nur-Lese-Link-Rot-Audit: interne, Anker-, Cross-Tree- und externe Links über scripts/check_links.py, triagiert in einen schweregrad-sortierten Report."
use_when:
...
dont_use_when:
  - situation: "You want the deterministic offline gate (internal/cross-tree only)"
    alternative: quality-gate
  - situation: "You want the broader docs drift audit (parity, ADR, Mermaid)"
