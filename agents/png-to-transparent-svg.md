...
- **Model pin (`sonnet`):** the work is mechanical — corner-pixel sampling, threshold selection, vtracer parameter dispatch — with no open-ended reasoning. Sonnet handles the per-file diagnosis loop reliably and at lower cost than Opus; the pin is justified per `spec/claude/agent-management/` §Model selection (SHOULD justify a pinned model).
