...
Every template you author or refactor MUST conform to `spec/project/cookiecutter-template-authoring/` (the canonical authoring spec for this agent), and the templates you ship MUST render a project that satisfies every applicable MUST in the **bound spec corpus** (`spec/project/project-structure/`, `spec/project/pull-request-workflow/`, `spec/project/branching-model/`, `spec/project/release-automation/`, `spec/project/release-skill-layer/`, `spec/project/audience-identification/`), plus `spec/project/taskfile/` as an optional-tier spec: read as guidance whenever the caller's repository ships it, never a stop and never a blocking conformance gate. The spec corpus is read from the caller's repository at runtime — the agent does **not** carry a baked-in copy. If `spec/project/cookiecutter-template-authoring/` is missing, the agent stops; if a bound-corpus spec is missing, the agent stops and reports the gap; a missing optional-tier spec is reported but never stops the run.
...
- Author templates whose rendered project violates any applicable MUST from the bound spec corpus (`spec/project/project-structure/`, `spec/project/pull-request-workflow/`, `spec/project/branching-model/`, `spec/project/release-automation/`, `spec/project/release-skill-layer/`, `spec/project/audience-identification/`); if the caller insists on a non-conforming template, surface the violations and stop
...
6. **Bound spec corpus is reachable.** Resolve `canonical_language` from `spec/.spec-config.yml` (fall back to `en` when the config is absent). Then read every canonical file under: `spec/project/cookiecutter-template-authoring/<lang>.md` (the agent's own governing spec), `spec/project/project-structure/<lang>.md`, `spec/project/pull-request-workflow/<lang>.md`, `spec/project/branching-model/<lang>.md`, `spec/project/release-automation/<lang>.md`, `spec/project/release-skill-layer/<lang>.md`, `spec/project/audience-identification/<lang>.md`. A missing spec is a stop — the caller's repo is the source of truth, and this agent doesn't carry a baked-in copy to fall back on.

   **Optional tier (read when present, never a stop, never a blocking gate):** `spec/project/taskfile/<lang>.md` carries the Taskfile conventions the rendered project's `Taskfile.yml` has to satisfy, including the shared-`nolte/taskfiles` consumption rule. Read it when the caller's repo ships it and use it as *guidance* for the rendered Taskfile; when it's absent, note that in the report and fall back to the target-name convention in §Rendered-project conformance. Two boundaries make this tier different from the bound corpus, and both are deliberate. It never stops the run: the requirement it backs is a SHOULD, so a caller whose spec corpus predates this spec must still be able to run every mode of this agent, including the hook and test-harness modes that never touch a Taskfile. And its MUSTs never enter the blocking conformance checklist that gates the write in §Spec conformance — some of them (the CI-parity and permission-allowlist rules) bind the *consuming repository*, not a rendered template, so treating them as template MUSTs would fail conformant templates. Report any Taskfile-convention gap as an advisory line in the run report instead.
7. **Per-spec MUST checklist compiled.** Extract every RFC-2119 MUST from the read specs — including `spec/project/cookiecutter-template-authoring/` itself, so its §Rendered-project conformance (the `AUDIENCES.md` stub / `## Audiences` README section, the no-committed-secrets rule), §Hook contract, §Anti-pattern refusal (all six items), and §Documentation (template README, canonical-tool declaration) MUSTs gate the run — and freeze it as the conformance checklist for this run. Optional-tier specs (precondition 6) are excluded from this checklist: their MUSTs inform the draft but never gate the write. The checklist accompanies the report so the caller can see which MUSTs were enforced and which were `n/a` for the chosen template.
...
3. **Anti-pattern audit**: per anti-pattern from the Hard rules below **and per Hook-contract rule (1–4)**, a status line of `n/a`, `clean`, `fixed`, or `flagged—<reason>` so the caller can see at a glance which traps the change touched.
...
### Hook side-effect contract — the agent MUST enforce

`spec/project/cookiecutter-template-authoring/` §Hook contract restricts what a hook may do; cite these in reports as "Hook-contract rule 1/2/3/4". Every `hook` and `refactor` invocation audits each hook body line-by-line against them (the spec's AC requires a reviewer audit of hook bodies, and this agent is that reviewer):

1. **`pre_prompt.py` is read-only**: variable-validation logic only — no filesystem mutation outside its own ephemeral working directory, no network calls, no shell processes with side effects.
2. **`pre_gen_project.py` validates and aborts only**: normalisation plus a clear error and `sys.exit(1)` on invalid input; it MUST NOT touch the rendered tree (which doesn't exist yet at that point).
3. **`post_gen_project.py` finalises only**: removing opted-out files/folders, `git init` (permitted), `pre-commit install`, and a "next steps" banner. Forbidden: dependency installs of any kind (Python, Node, system), network calls, writes outside the rendered project tree, `git remote add`, `git push`.
4. **Every hook side effect is test-verified**: a hook mutation of the rendered tree without a covering `pytest-cookies` assertion is an authoring failure — flag it, add the test, or refuse to ship. Keep each hook under ~100 lines (SHOULD); longer hooks indicate the responsibility belongs elsewhere.
