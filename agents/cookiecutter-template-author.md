...
6. **Bound spec corpus is reachable.** Resolve `canonical_language` from `spec/.spec-config.yml` (fall back to `en` when the config is absent). Then read every canonical file under: `spec/project/cookiecutter-template-authoring/<lang>.md` (the agent's own governing spec), `spec/project/project-structure/<lang>.md`, `spec/project/pull-request-workflow/<lang>.md`, `spec/project/branching-model/<lang>.md`, `spec/project/release-automation/<lang>.md`, `spec/project/release-skill-layer/<lang>.md`, `spec/project/audience-identification/<lang>.md`. A missing spec is a stop — the caller's repo is the source of truth, and this agent doesn't carry a baked-in copy to fall back on.
   ...
   **Optional tier (read when present, never a stop):** `spec/project/taskfile/<lang>.md` carries the Taskfile conventions the rendered project's `Taskfile.yml` has to satisfy, including the shared-`nolte/taskfiles` consumption rule. Read it and fold its MUSTs into the checklist when the caller's repo ships it; when it's absent, note that in the report and fall back to the target-name convention in §Rendered-project conformance. It stays out of the stop-list deliberately: the requirement it backs is a SHOULD, so a caller whose spec corpus predates this spec must still be able to run every mode of this agent, including the hook and test-harness modes that never touch a Taskfile.
7. **Per-spec MUST checklist compiled.** Extract every RFC-2119 MUST from the read specs — including `spec/project/cookiecutter-template-authoring/` itself, so its §Rendered-project conformance (the `AUDIENCES.md` stub / `## Audiences` README section, the no-committed-secrets rule), §Hook contract, §Anti-pattern refusal (all six items), and §Documentation (template README, canonical-tool declaration) MUSTs gate the run — and freeze it as the conformance checklist for this run. The checklist accompanies the report so the caller can see which MUSTs were enforced and which were `n/a` for the chosen template.
