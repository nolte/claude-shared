...
description: Scaffolds spec-conformant component tests for a single shippable component against spec/project/test-tier-component/, in whichever of the two flavours fits — a frontend UI component test (render and assert observable output through user-facing queries, role-first, never internals) or a service/backend component test (drive the service through its own API with externals doubled at the process boundary). Wires the in-process-or-out-of-process choice, disciplined Meszaros doubles for externals only, determinism (controlled time/randomness/network), optional accessibility and visual-regression checks at component scope, and TC-ID traceability. Use when the user asks to generate/scaffold component tests for a UI component or a service. Don't use to review existing component tests (use component-test-reviewer), to scaffold unit/integration/contract/E2E tests (use the matching tier generator), to derive abstract test cases (use test-case-extractor), or to run the lint/typecheck/test gate (use quality-gate).
distribution: plugin
...
dont_use_when:
  ...
  - situation: "you want a test that exercises a real external collaborator"
