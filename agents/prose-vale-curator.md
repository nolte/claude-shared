...
5a. **Run a Voice-and-tone spot check** against `spec/project/prose-style/` §Voice and tone (the editorial MUSTs that Vale doesn't enforce yet). This is a lightweight Vale-adjacent signal, **not** a full editorial audit: the authoritative six-dimension review — including D4 writing style — is `lektorat-scanner`'s job (dispatched by `lektorat-apply`). Keep these to quick heuristics and defer any deep D4 style analysis to `lektorat-scanner`. Drive each heuristic with `Grep` over the target files (the patterns below are the regexes to run) and `Read` only the surrounding lines a finding needs for its quoted span. Surface heuristic findings only — don't rewrite. Heuristics to apply per file:
