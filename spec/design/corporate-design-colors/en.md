...
- **[deferred—decided, target bound]** The exact `brand-primary` OKLCH triplet (hue, chroma, lightness) is deferred to the first concrete implementation skill or agent (most likely the one governed by `spec/design/graphic-prompt-authoring`) and is one of that skill's declared outputs; this spec defines only the *contract* the picked value must satisfy, which is its full job here. The objectively-correct value can't be derived from any existing artifact, so a human brand owner must choose it. Recommended non-binding starting anchor: a muted indigo (the descriptive example already used twice in §AI image color contract), which gives good gamut headroom for the +60°/180° derivations and reads as a credible technical-portfolio signature. Downstream skills and agents that depend on the picked value can't produce conforming output until the deciding artifact lands; the dependency is explicit, not implicit.
- **[cross-spec gap]** The "portfolio-versioned token bundle" pinning mechanism is named by analogy to `nolte/vale-style`, but no spec defines the publication registry, the released file format, or the consumer-side pin syntax. Two upstream events, in order, unblock this: (1) a brand owner picks `brand-primary` (this spec's first deferred Open Question), which unblocks (2) selection of a bundle-producer repo and the first published DTCG bundle. Author `spec/portfolio/design-token-distribution/` (or a new section in `release-automation`) only once that producer repo exists and one tagged bundle is published. `release-automation` §Non-Goals explicitly excludes external-registry publishing, so this can't fold into the existing release pipeline as-is. Until then, downstream consumers MUST coordinate the publication and pin mechanics with this spec's owners.
- Default: CMYK fallback palette generation stays `SHOULD` while only screen artifacts ship; the conditional regenerate-on-change `MUST` in §Per-artifact application already bounds the drift risk. Revisit when: escalate to `MUST` with a documented ICC conversion-profile pin (for example a named profile such as ISO Coated v2 / FOGRA39) once ≥1 print/PDF deliverable per release cycle ships across ≥2 portfolio repos, or a brand owner reports an observed CMYK color-drift defect on a shipped print artifact.
- The hand-off contract with a future `spec/design/imagery-style/` for non-color imagery axes awaits that spec. On authoring `spec/design/imagery-style/` under `spec/design/`, add a reciprocal hand-off clause here (the color axis stays owned by `corporate-design-colors`; all non-color imagery axes are owned by `imagery-style`) and a matching pointer in `imagery-style`. Upstream marker: `spec/design/` currently holds only `corporate-design-colors` and `graphic-prompt-authoring`, so `imagery-style` doesn't yet exist; until it lands, the AI image color contract here is load-bearing alone.
