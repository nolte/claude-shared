...
- The repo and path that host `brand-vocabulary.md` and `brand-prompt-library.md` aren't pinned yet. Both files MUST be separate artifacts (`brand-vocabulary.md` carries one row per approved descriptive phrase, `brand-prompt-library.md` carries one row per published hero image), and the open question is only where they live. A candidate is `design/brand/` at the bundle-producer repo. The definitive choice will land in a follow-up `spec/design/imagery-style/` or in a per-publication-repo decision recorded in §AI image color contract.
