...
- **MUST** pin the style-version parameter (`--sv`) next to the code and treat the pair, not the code alone, as the brand signature: Midjourney rewrote style-code semantics in June 2025 and defaults new references to a later `--sv` generation, so the same code renders a different look once the default moves. A code recorded without its `--sv` isn't reproducible, which defeats the point of versioning it like a token (see §Sources)
...
- **MUST** publish the canonical token set in W3C Design Tokens Community Group format (DTCG 2025.10 or later), which is a set of modules rather than one document: the `{group.token}` curly-brace alias syntax and the ban on circular references come from the Format Module, while the color-token shape (`$type: "color"` with a `$value` object carrying `colorSpace` and `components`, plus optional `alpha` and an optional `hex` fallback) is normative in the sibling Color Module of the same stable release ([DTCG Format Module 2025.10](https://www.designtokens.org/tr/2025.10/format/), [DTCG Color Module 2025.10](https://www.designtokens.org/tr/2025.10/color/), [W3C—DTCG first stable announcement](https://www.w3.org/community/design-tokens/2025/10/28/design-tokens-specification-reaches-first-stable-version/))
...
- **MUST** bind the token-bundle requirements of §Token architecture, §Ramp structure, §Light/dark mode pairing, §Contrast gates, §Publication and export, §Governance and change control, and the favicon and social-card token sets of §Per-artifact application to the bundle-producer repository, the one that publishes the bundle. Until that repository is selected, a repository that uses the palette meets this spec through `design/brand/brand-vocabulary.md` and `design/brand/brand-prompt-library.md` and records the bundle as a follow-up instead of building a partial one
...
Criteria about the published token bundle (ramps, semantic tokens, contrast and APCA records, DTCG validity, exports, favicon and social-card sets, badge colors, downstream pinning, ramp change requests, and brand-version change logs) are checked in the bundle-producer repository; a repository without a bundle is checked against the brand anchor, the vocabulary, and the prompt library.
...
_All open questions are resolved. The `brand-primary` anchor was decided by the brand owner on 2026-06-06 as a muted indigo `oklch(0.47 0.12 276)` / `#4A529D` (recorded in §Brand harmony axes); the token-bundle-registry, CMYK, and imagery-style hand-off deferrals were settled the same day. The full record is preserved in git history (decision log, 2026-06-06)._

## Sources

Three assertions in this spec depend on external tooling that moves independently of the brand: the OKLCH-as-canonical-color-space rule in §Color space and source of truth, the Midjourney style-reference contract in §AI image color contract, and the DTCG publication format in §Publication and export. They're author-time external assertions triangulated per `spec/claude/research-triangulate/` §Author-time assertions (author-time tier: at least three independent sources, ordered Primary-first). Retrieval date for every source below: 2026-07-24.

- **OKLCH is a standardised, perceptually uniform color space, and Tailwind v4 ships its default palette in it—unlike HSL, whose lightness varies visibly across hues**: W3C, "CSS Color Module Level 4," which defines the Oklab/OkLCh color spaces and the `oklch()` functional notation (Primary), <https://www.w3.org/TR/css-color-4/>; MDN, "`oklch()`" ("The `L` in `oklch()` is the perceived lightness... This is different from the `L` in `hsl()`") (Primary), <https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch>; Tailwind CSS v4.0 release notes ("We've upgraded the entire default color palette from `rgb` to `oklch`") (Primary), <https://tailwindcss.com/blog/tailwindcss-v4>; Evil Martians, "OKLCH in CSS: why we moved from RGB and HSL" (Secondary), <https://evilmartians.com/chronicles/oklch-in-css-why-quit-rgb-hsl>
- **Midjourney's `--sref` is a reusable style signature, but only when its `--sv` style-version generation is pinned alongside it, and hex codes in the prompt body don't reliably enforce exact colors**: Midjourney, "Style Reference," the vendor documentation of the parameter and its style-weight companion `--sw` (Primary), <https://docs.midjourney.com/hc/en-us/articles/32180011136653-Style-Reference>; Heather Cooper, "Midjourney V8 is live on alpha," recording that style references now default to a later `--sv` generation (Secondary), <https://heatherbcooper.substack.com/p/midjourney-v8-is-live-on-alpha-heres>; `Midlibrary`, "A deep dive into Midjourney sref codes," on codes as "a precise, reusable, and easily accessible method for controlling the visual style" and on the June 2025 semantics rewrite (Secondary), <https://midlibrary.io/midguide/deep-dive-into-midjourney-sref-codes>; Geeky Curiosity, "What colors does Midjourney actually understand?" (hex codes aren't parsed as exact colors) (Secondary), <https://geekycuriosity.substack.com/p/midjourney-prompts10-what-colors>
- **DTCG 2025.10 is the current stable release, and the color-token `$value` object is normative in its Color Module rather than the Format Module**: DTCG, "Design Tokens Format Module 2025.10" ("This specification is considered stable"; `{group.token}` aliases; "References MUST NOT be circular") (Primary), <https://www.designtokens.org/tr/2025.10/format/>; DTCG, "Design Tokens Color Module 2025.10," defining the required `colorSpace` and `components` plus the optional `alpha` and `hex` fallback (Primary), <https://www.designtokens.org/tr/2025.10/color/>; W3C Design Tokens Community Group, "Design Tokens specification reaches first stable version" (Primary), <https://www.w3.org/community/design-tokens/2025/10/28/design-tokens-specification-reaches-first-stable-version/>; the community-group repository's version table, which marks 2025.10 as the latest stable and the later editors' drafts as non-implementable (Primary), <https://github.com/design-tokens/community-group>

Verified 2026-07-24, with three corrections folded into the requirements above rather than left as citations. The Midjourney contract gained the `--sv` pin, because a style code recorded without its style-version generation stops reproducing once Midjourney's default moves—it did so in June 2025, and V8.1 has been the default model since 2026-06-11. The DTCG requirement now names both modules and links the stable 2025.10 snapshot; the previously cited `/tr/drafts/format/` URL resolves to a later editors' draft that's explicitly marked as not to be implemented. Tailwind's OKLCH default survived unchanged into v4.3, so only the source list was strengthened there, with the W3C and MDN primaries added beneath the vendor release note.
