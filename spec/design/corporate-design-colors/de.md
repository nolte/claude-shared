...
- Das Repo und der Pfad, die `brand-vocabulary.md` und `brand-prompt-library.md` hosten, sind noch nicht gepinnt. Beide Dateien MÜSSEN getrennte Artefakte sein — `brand-vocabulary.md` führt eine Zeile pro freigegebener deskriptiver Phrase, `brand-prompt-library.md` führt eine Zeile pro veröffentlichtem Hero-Bild —, und die offene Frage ist nur, wo sie liegen. Ein Kandidat ist `design/brand/` im Bundle-Producer-Repo. Die endgültige Wahl landet in einer Folge-Spec `spec/design/imagery-style/` oder in einer pro-Veröffentlichungs-Repo-Entscheidung, die in §KI-Bild-Farb-Kontrakt festgehalten wird.
