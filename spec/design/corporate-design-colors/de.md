...
- **MUSS [MUST]** die Token-Bundle-Anforderungen aus §Token-Architektur, §Ramp-Struktur, §Light-/Dark-Mode-Kopplung, §Kontrast-Gates, §Veröffentlichung und Export sowie §Governance und Change-Control an das Bundle-Producer-Repository binden, also das Repository, das das Bundle veröffentlicht. Solange dieses Repository nicht ausgewählt ist, erfüllt ein Repository, das die Palette verwendet, diese Spec über `design/brand/brand-vocabulary.md` und `design/brand/brand-prompt-library.md` und hält das Bundle als Follow-up fest, statt ein Teil-Bundle zu bauen
