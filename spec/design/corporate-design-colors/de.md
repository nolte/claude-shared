...
- **MUSS [MUST]** sowohl `brand-vocabulary.md` als auch `brand-prompt-library.md` unter `design/brand/` im Bundle-Producer-Repository ablegen, parallel zur `design/prompts/`-Konvention, die `spec/design/graphic-prompt-authoring` für Prompt-Dokumente festlegt; der relative Pfad ist normativ, auch wenn das Bundle-Producer-Repository noch nicht ausgewählt ist (dieses Deferral ist in `.audits/decisions/2026-06-06-settle-open-questions.md` festgehalten)
...
_Alle offenen Fragen sind aufgelöst. Der `brand-primary`-Anker wurde vom Brand-Owner am 2026-06-06 als gedämpftes Indigo `oklch(0.47 0.12 276)` / `#4A529D` entschieden (festgehalten in §Brand harmony axes); die Deferrals zu Token-Bundle-Registry, CMYK und imagery-style-Übergabe wurden am selben Tag entschieden. Siehe `.audits/decisions/2026-06-06-settle-open-questions.md` für den vollständigen Eintrag._
