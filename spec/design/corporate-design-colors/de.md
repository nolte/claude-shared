...
- **MUSS [MUST]** sowohl `brand-vocabulary.md` als auch `brand-prompt-library.md` unter `design/brand/` im Bundle-Producer-Repository (oder, solange keines ausgewählt ist, im Repository, das die Palette pflegt) ablegen, parallel zur `design/prompts/`-Konvention, die `spec/design/graphic-prompt-authoring` für Prompt-Dokumente festlegt; der relative Pfad ist normativ, auch wenn das Bundle-Producer-Repository noch nicht ausgewählt ist (dieses Deferral ist im Entscheidungslog in der Git-Historie festgehalten)
...
- **MUSS [MUST]** die Token-Bundle-Anforderungen aus §Token-Architektur, §Ramp-Struktur, §Light-/Dark-Mode-Kopplung, §Kontrast-Gates, §Veröffentlichung und Export, §Governance und Change-Control sowie die Favicon- und Social-Card-Token-Sets aus §Anwendung pro Artefakt an das Bundle-Producer-Repository binden, also das Repository, das das Bundle veröffentlicht. Solange dieses Repository nicht ausgewählt ist, erfüllt ein Repository, das die Palette verwendet, diese Spec über `design/brand/brand-vocabulary.md` und `design/brand/brand-prompt-library.md` und hält das Bundle als Follow-up fest, statt ein Teil-Bundle zu bauen
...
Kriterien zum veröffentlichten Token-Bundle (Ramps, semantische Tokens, Kontrast- und APCA-Nachweise, DTCG-Validität, Exporte, Favicon- und Social-Card-Sets, Badge-Farben, Downstream-Pinning, Ramp-Change-Requests und Brand-Versions-Changelogs) werden im Bundle-Producer-Repository geprüft; ein Repository ohne Bundle wird gegen den Brand-Anker, das Vokabular und die Prompt-Library geprüft.
