...
Das Marken-Vokabular definiert drei — und nur drei — tragende Farb-Rollen. Operator-Entscheidungen sind auf die unten beschriebenen geometrischen Beziehungen beschränkt.
...
- **MUSS [MUST]** den vollen Prompt-Stack (Modell + Version, `--sref`, Seed, Style-Weight, deskriptive Phrasen, Hex-Reinforcements) für jedes veröffentlichte Hero-Bild in einer versionierten Brand-Prompt-Library protokollieren, damit ein Regenerate-with-Tweaks-Vorgang reproduzierbar bleibt
...
- [ ] `brand-tertiary` ist entweder mit der M3-+60°-/C24/T40-Ableitungsregel deklariert oder explizit mit Begründung weggelassen
...
- [ ] Die Vier-Slot-Vokabular-Tabelle (`brand-primary`, `brand-secondary`, `brand-accent`, `brand-complement`) ist vorhanden, und kein fünfter Farb-Rollen-Begriff taucht im veröffentlichten Bundle oder in dessen Konsumenten-Doku auf
...
- [ ] Eine versionierte Brand-Prompt-Library existiert an einem veröffentlichten Ort (benannt in §KI-Bild-Farb-Kontrakt oder in einer Schwester-Spec) und führt pro veröffentlichtem Hero-Bild einen Eintrag mit Modell+Version, `--sref`, Seed, Style-Weight, deskriptiven Phrasen und Hex-Reinforcements
...
- **[blockiert — bewusste Vertagung]** Der genaue Hue, Chroma und Lightness für `brand-primary` sind noch nicht gewählt. Diese Spec definiert den *Kontrakt*, den der gewählte Wert erfüllen muss; die Wahl selbst geschieht in der ersten konkreten Implementierungs-Skill oder -Agent und ist eines der deklarierten Outputs dieser Skill. Nachgelagerte Skills und Agents, die vom gewählten Wert abhängen, können bis dahin keine konforme Ausgabe erzeugen; die Abhängigkeit ist explizit, nicht implizit.
- **[Cross-Spec-Lücke]** Der globale Injektions-Mechanismus für das Mermaid-Theme-Variablen-Mapping (über `mkdocs.yml`-`extra_javascript`, einen `%%{init: ... }%%`-Block in einem MkDocs-Hook oder einen anderen Pfad) ist weder in dieser Spec noch in `spec/project/mermaid-diagrams/` definiert. Der natürliche Ort für die Verdrahtungs-Regel ist die bestehende `spec/project/mermaid-diagrams/`-§MkDocs-Setup-Sektion. Ein Follow-up wird dort eine MUSS-Regel ergänzen und sie von §Anwendung pro Artefakt aus referenzieren.
- **[Cross-Spec-Lücke]** Der "portfolio-versionierte Token-Bundle"-Pinning-Mechanismus ist hier per Analogie zu `nolte/vale-style` benannt, aber derzeit definiert keine Spec die Publication-Registry, das Datei-Format unter Release oder die Konsumenten-seitige Pin-Syntax. Eine Folge-Spec — am ehesten ein neuer Abschnitt in `spec/project/release-automation/` oder eine neue `spec/portfolio/design-token-distribution/` — wird diese Lücke schließen. Bis dahin MÜSSEN nachgelagerte Konsumenten die Veröffentlichung und das Pinning mit den Eignerinnen und Eignern dieser Spec abstimmen.
- Die in §KI-Bild-Farb-Kontrakt geforderte "versionierte Brand-Prompt-Library" hat noch keinen definierten Ort und kein definiertes Datei-Format. Ein Kandidat ist `design/brand/prompt-library.md` im Bundle-Producer-Repo, eine Zeile pro veröffentlichtem Hero-Bild; die endgültige Wahl landet in einer Folge-Spec `spec/design/imagery-style/` oder in einer pro-Veröffentlichungs-Repo-Entscheidung, die in §KI-Bild-Farb-Kontrakt festgehalten wird.
- Ob die Marke ein kanonisches Mermaid-Theme veröffentlicht oder ein pro-Modus-(light/dark)-Paar, wird entschieden, sobald das erste Mermaid-konsumierende Downstream-Repo das verdrahtet; beide Optionen bleiben unter den aktuellen Regeln legal.
- Die CMYK-Fallback-Palette-Generierung ist derzeit `SHOULD`; falls das Portfolio anfängt, Print-Artefakte in nennenswertem Umfang zu produzieren, muss die Regel mit einem dokumentierten Konversions-Profil-Pin auf `MUST` hochstufen.
- Die Grenze zwischen dieser Spec und einer künftigen `spec/design/imagery-style/` für nicht-farbliche Bild-Achsen (Komposition, Beleuchtung, photographisches-vs-illustratives Register) braucht einen Übergabe-Kontrakt, sobald die Spec landet; bis dahin trägt der KI-Bild-Farb-Kontrakt hier allein.
- Ob die `brand-tertiary`-Ableitung Material 3's algorithmischer +60°/C24/T40 wörtlich folgen soll oder pro OKLCH-Gamut-Realität von Hand getuned wird, wird neu bewertet, sobald `brand-primary` gewählt ist und das Gamut-Clipping bei +60° messbar wird.
