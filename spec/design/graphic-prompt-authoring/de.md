...
- Der tatsächliche Aufruf eines Bildgenerators oder das Schreiben einer Bilddatei (im Besitz von `spec/tools/gemini-image-generation/` und jedem künftigen pro-Generator-Geschwister)
...
- **MUSS [MUST]** eine explizite Negativ-Prompt-/Vermeidungs-Klausel einschließen (zum Beispiel: kein eingebetteter Text, keine Logos anderer Firmen, kein Wasserzeichen), passend zum Zielgenerator, weil generierter Text und Streumarken über aktuelle Diffusionsmodelle hinweg unzuverlässig sind
...
- **MUSS [MUST]** jedes Dokument `<asset-type>_<slug>.md` benennen, wobei `<asset-type>` einem dokumentierten Typ-Vokabular entstammt (zum Beispiel `app-icon`, `logo`, `nav-icon`, `illustration`, `empty-state`, `onboarding`, `hero`, `badge`, `pattern`, `diagram`) und `<slug>` eine kebab-case-Beschreibung ist
...
- [R2] Nachgelagertes Bildgenerierungs-Tool (Prompt rein, Bilddatei raus): `spec/tools/gemini-image-generation/`
...
- Soll das Default-Design-Prompts-Verzeichnis `design/prompts/` sein (hier gewählt) oder unter dem Docs-Baum des Konsumenten-Repos liegen, und soll es automatisch angelegt werden oder vorab existieren müssen?
- Ist für Generatoren ohne `--sref`-Mechanismus (Gemini) ein festes Referenzbild oder ein kanonischer Stil-Absatz das bessere „pro-Modell-Äquivalent", und soll `corporate-design-colors` diese Wahl festschreiben statt sie dieser Spec zu überlassen?
- Soll der Agent eine Zeile an ein Vor-Generierungs-Prompt-Ledger anhängen (verschieden vom Post-Generierungs-`brand-prompt-library.md`), oder genügt das Pro-Asset-Dokument als dauerhafter Record?
- Soll das Asset-Typ-Vokabular hier normativ sein oder an eine künftige `spec/design/imagery-style/` delegiert werden, sobald jene Spec landet?
