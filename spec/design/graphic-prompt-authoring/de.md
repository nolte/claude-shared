...
- Default: Das Asset-Typ-Vokabular bleibt hier provisorisch normativ, weil das Dateinamen-MUST es jetzt braucht; `spec/design/imagery-style/` ist auf nicht-farbliche Bild-Achsen (Komposition, Beleuchtung, fotografisch-vs-illustratives Register) beschränkt, sodass das erwartete Ergebnis ist, dass das Benennungs-Vokabular hier bleibt. Erneut prüfen, wenn: eine Datei unter `spec/design/imagery-style/` erscheint—in diesem Moment explizit bestätigen, ob das Dateinamen-Typ-Vokabular zu `imagery-style` migriert oder hier als reine Benennungs-Taxonomie bleibt.
