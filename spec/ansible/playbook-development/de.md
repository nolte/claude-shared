...
Konsumenten: Dieses Repository konsumiert §Repository-Profile mittelbar. `spec/project/project-structure/` §Source-Layout zitiert ihn für die Inventory-Baum-Profile, und der Agent `project-structure-reviewer` setzt jenes Layout durch — diese Regeln sind hier also tragend, obwohl kein Skill und kein Agent diesen Pfad nennt. Die Autoren-Leitlinien unten haben keinen lokalen Konsumenten und schulden auch keinen: Dieses Repository liefert keine Playbooks aus, seine Leserschaft sind die Bootstrap- und Provisionierungs-Repositories des Portfolios. Eine Inventur, die Skills und Agents nach diesem Pfad durchsucht, findet nichts; das ist eine Grenze der Methode, keine verwaiste Spec.
