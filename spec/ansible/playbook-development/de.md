...
## Offene Fragen
- Das Profil *single-environment-bootstrap* in eine eigene dedizierte Spec (`spec/ansible/edge-device-bootstrap/`) auslagern, sobald ein ZWEITES Portfolio-Repository `profile: single-environment-bootstrap` deklariert UND die beiden Adopter messbar divergieren: jeder trägt eine eigene Inventar-Form, ein eigenes CI-Gating oder eine eigene Rollen-Inlining-Regel, die der aktuelle gemeinsame §Repository-Profile-Punkt ohne adopter-spezifische Verzweigung nicht ausdrücken kann. Konkret: in dem Moment, in dem ein PR an diese Spec dem single-environment-bootstrap-Punkt einen „für Repo A tu X, für Repo B tu Y"-Zweig hinzufügen müsste. Heute ist die Adopter-Zahl null (beide Ansible-Specs bleiben `Status: draft`, documentary-only, per Verweis konsumiert; kein Ansible-Repo lebt in diesem Plugin, und ein grep von `spec/ansible/` zeigt nur `playbook-development` und `role-development`).
- `sops`-Integration in §Secrets-Behandlung von **KANN [MAY]** auf **SOLLTE [SHOULD]** anheben, sobald eine portfolioweite Secret-/Key-Store-Konvention in `spec/` committet ist (eine dedizierte Spec wie `spec/project/secret-store/` oder eine äquivalente SOPS- / HashiCorp-Vault-Baseline) UND mindestens ein Ansible-Repo sie übernimmt UND ein zweites Repo denselben Key-Store wiederverwendet. Heute gilt nichts davon: ein grep von `spec/` nach `sops` / `key-store` / `key store` / `hashicorp` liefert nur diese Spec (`en.md` + `de.md`)—es gibt keine externe Key-Store-Spec, gegen die man anheben könnte.
