...
## Open Questions
- Split the *single-environment-bootstrap* profile into its own dedicated spec (`spec/ansible/edge-device-bootstrap/`) when a SECOND portfolio repository declares `profile: single-environment-bootstrap` AND the two adopters measurably diverge: each carries a distinct inventory shape, distinct CI gating, or distinct role-inlining rule that the current shared §Repository profiles bullet can't express without a per-adopter conditional. Concretely: the moment a PR to this spec would need to add a "for repo A do X, for repo B do Y" branch to the single-environment-bootstrap bullet. Today the adopter count is zero (both Ansible specs stay `Status: draft`, documentary-only, consumed by reference; no Ansible repo lives in this plugin and a grep of `spec/ansible/` shows only `playbook-development` and `role-development`).
- Elevate `sops` integration from **MAY** to **SHOULD** in §Secrets handling when a portfolio-wide secret/key-store convention is committed to `spec/` (a dedicated spec such as `spec/project/secret-store/` or an equivalent SOPS / HashiCorp-Vault baseline) AND at least one Ansible repo adopts it AND a second repo reuses the same key store. Today none of these hold: a grep of `spec/` for `sops` / `key-store` / `key store` / `hashicorp` returns only this spec (`en.md` + `de.md`)—there's no external key-store spec to elevate against.
