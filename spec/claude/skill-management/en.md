...
- **SHOULD** head the standard sections with the established plural form where the section holds a list: `## Preconditions`, `## Inputs`, `## Operations`, `## Examples`, `## Gotchas`, `## Hard rules`. Only `## Operations` carries the MUST above; the rest are a SHOULD so an existing singular heading is a cleanup item rather than a defect.
...
- **MUST** include a **table of contents** at the top of any file under a skill's `references/` or `templates/` directory that's longer than 100 lines, so partial-read previews still surface the file's full scope; an `examples/` file is loaded whole when its trigger fires and needs none ([R2](#references))
...
- **MUST**, whenever a rule in `SKILL.md` changes, carry the change into every supporting file that restates that rule in the same edit—a `references/` digest, a checklist, a template, an example. Two documented mechanics make a stale supporting file actively misleading rather than merely redundant: a reference file is loaded on its own trigger and is then the text in front of the run, while the body that contradicts it may by that point have been compacted or re-attached in truncated form (see the authoring token cap above, the partial-read hazard the table-of-contents rule addresses, and the compaction behaviour documented under §Runtime & lifecycle awareness below). The same applies to counts stated in the body about the supporting set ("distilled from the seven grounding specs") and to any rule the governing spec changed upstream
...
- [ ] Every `references/` or `templates/` file longer than 100 lines opens with a table of contents
- [ ] No supporting file states a rule that `SKILL.md` has since changed, no count the body states about its supporting set disagrees with what's there, and no rule the skill restates from its governing spec lags behind that spec's wording
