...
- **MUST**, whenever a rule in `SKILL.md` changes, carry the change into every supporting file that restates that rule in the same edit—a `references/` digest, a checklist, a template, an example. Progressive disclosure is exactly what makes this dangerous: a run loads the digest *instead of* the body, so a rule corrected in `SKILL.md` and left standing in `references/` isn't a redundant copy but the version the run will actually follow. The same applies to counts stated in the body about the supporting set ("distilled from the seven grounding specs") and to any rule the governing spec changed upstream
