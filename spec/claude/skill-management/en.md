...
- **MUST**, whenever a rule in `SKILL.md` changes, carry the change into every supporting file that restates that rule in the same edit—a `references/` digest, a checklist, a template, an example. Two documented mechanics make a stale supporting file actively misleading rather than merely redundant: a reference file is loaded on its own trigger and is then the text in front of the run, while the body that contradicts it may by that point have been compacted or re-attached in truncated form (see the 5,000-token cap above and the partial-read hazard the table-of-contents rule addresses). The same applies to counts stated in the body about the supporting set ("distilled from the seven grounding specs") and to any rule the governing spec changed upstream
...
- [ ] No supporting file states a rule that `SKILL.md` has since changed, and no count the body states about its supporting set disagrees with what's there
