...
- **SOLLTE [SHOULD]** für den Skill-Namen die **Gerundium-Form** bevorzugen (Verb + `-ing`: `processing-pdfs`, `analyzing-spreadsheets`, `managing-databases`) gemäß Anthropic-Konvention; Verb-Substantiv (`process-pdfs`) und Substantiv-Phrasen (`pdf-processing`) sind akzeptable Alternativen, aber gemischte Formen innerhalb eines Repositories nicht ([R2](#referenzen))
  - **Wahl des `nolte-shared`-Plugins**: Dieses Repository liefert jeden Skill in **Verb-Substantiv-Form** aus, konkret `<object-noun>-<action>` — die führenden Token benennen das Objekt, das **nachgestellte** Token die Aktion, ausgedrückt entweder als finites Verb (`pull-request-create`, `roadmap-init`, `feature-decompose`, `mission-define`) oder als verb-abgeleitetes Aktionsnomen (`dependency-audit`, `gemini-image-handoff`, `skill-management`). Die Aktion steht **am Ende**, spiegelbildlich zur agent-seitigen `<subject>-<role-noun>`-Reihenfolge, bei der das Rollen-Nomen am Ende steht. Die Wahl ist hier festgehalten, damit ein Reviewer die Konvention nicht in jeder Iteration erneut bemängelt. Die bestehende Surface in die Gerundium-Form umzubenennen wäre ein Breaking Change für jeden Downstream-Konsumenten, der `subagent_type:` aufruft, und ist nicht geplant. Neue Skills in diesem Plugin **MÜSSEN [MUST]** der Verb-Substantiv-Konvention folgen; eine eingestreute Gerundium-Namensform würde selbst die "gemischte Formen sind nicht akzeptabel"-Hälfte der Upstream-Regel verletzen. Eine künftige koordinierte Portfolio-Umbenennung (mit Deprecation-Periode) **DARF [MAY]** die Wahl umkehren; bis dahin ist Verb-Substantiv die Regel
    - **Dokumentierte Ausnahmen** von der Verb-Substantiv-Regel: Drei etablierte Skill-Namen enden nicht auf ein Aktions-Token und werden bewusst unverändert gelassen — `spec` (ein nacktes Substantiv) und `yaml-json-schema` (ein Substantiv-Kompositum), beide älter als die Konvention, sowie `quality-gate` (ein Substantiv-Substantiv, dessen nachgestelltes `gate` ein Ding benennt, keine Aktion). Eine Umbenennung wäre ein Breaking Change für jede Konsumenten-Aufrufstelle und, im Fall von `spec`, für die `$ref`-/Querverweis-Maschinerie, die darauf zeigt; die Bruchkosten überwiegen den Gewinn an Benennungs-Kohärenz. Ein Reviewer **DARF NICHT [MUST NOT]** diese drei Namen als Verb-Substantiv-Verletzung bemängeln. Die Ausnahme ist geschlossen: Sie umfasst genau `spec`, `yaml-json-schema` und `quality-gate`, und jeder *neue* Skill **MUSS [MUST]** weiterhin der Verb-Substantiv-Konvention folgen. Die geschlossene Liste ist als `SKILL_NAME_FORM_EXCEPTIONS` in `scripts/validate_skills.py` gespiegelt, das jeden anderen aktionslosen Skill-Namen als `Suggestion` meldet
...
Ein Skill, der mehr als eine **dispatchbare Operation** anbietet (einen vom Aufrufer wählbaren Modus wie `audit` vs. `migrate` vs. `patch`), dokumentiert sie in einem `## Operations`-Block. Dieser Abschnitt legt Benennung und Überschriften-Form dieses Blocks fest, damit Skill-Autoren, Reviewer und die Sweep-Tooling eine einheitliche Terminologie teilen. Er bindet die *dispatchbaren Operationen* eines Skills, nicht die sequentiellen Verfahrensschritte einer einzelnen Operation.

- **MUSS [MUST]** `## Operations` (Plural) als Überschrift des Operations-Blocks verwenden; das Singular `## Operation` ist nicht konform
- **SOLLTE [SHOULD]** eine Operation, die eine Standard-Scaffolding-Lifecycle-Aktion ausführt, mit dem passenden Verb aus dem Lifecycle-Vokabular benennen, damit Artefakte desselben Clusters gleich lesen: `audit` (lesender Check), `scaffold` (Greenfield-Anlage), `patch` (additive Korrektur), `apply` (audit + scaffold + patch in einem Ablauf), `migrate` (Brownfield → konform), `run` (Standard-Verb für einen Skill mit einer einzigen Operation), `update` (bestehendes Artefakt verändern), `close` (Lifecycle beenden)
- **DARF [MAY]** eine Operation mit einem domänenspezifischen Verb benennen, wenn sie kein sauberes Lifecycle-Äquivalent hat oder das Domänen-Verb wesentlich klarer ist (z. B. das `add` / `promote` / `retarget` eines Planungs-Skills, das `transition` / `decline` eines Sprint-Skills, das `revise` / `propose` eines Content-Skills); die Lifecycle-Liste ist der empfohlene Standard, kein geschlossenes Set, das jedes andere Verb verbietet. Bevorzuge dennoch ein Lifecycle-Verb, wenn eines sauber passt (`audit` statt `validate` oder `scan`, `update` statt `revisit`, `migrate` statt `refactor`), damit die Oberfläche vorhersehbar bleibt
- **Verfahrensschritte sind vom Vokabular ausgenommen.** Ein Skill mit einer einzigen dispatchbaren Operation, der sie als geordneten Ablauf dokumentiert (z. B. `### 1. Detect project kind`, `### 2. Run auditors`, `### 3. Render the report`), bezieht seine Schritt-Überschriften aus der Aufgabe, nicht aus einer Verb-Liste; das sind Schritte einer Operation, keine dispatchbaren Operationen. Für sie gilt nur die Überschriften-Form-Regel unten
- **MUSS [MUST]** Teil-Operationen und Verfahrensschritte als `### N. <verb-or-step>` (nummeriert) oder als H3-Überschrift gefolgt von einem Backtick-zitierten Befehls-Verb betiteln; Buchstaben (`A.`/`B.`/`C.`) und `### Step N` sind nicht konform
...
- **MUSS [MUST]** `paths:` als **Auto-Invocation-Gate verstehen, nicht als Kontext-Budget-Hebel**: Wenn gesetzt, invoziert Claude den Skill automatisch nur, solange die Session mit Dateien arbeitet, die auf die Globs matchen; ein expliziter `/<plugin>:<name>`-Aufruf funktioniert immer, unabhängig davon ([R3](#referenzen)). Es passt daher zu einem Skill, dessen automatische Aktivierung an einen Datei-Kontext gebunden sein soll; es passt **nicht** zu einem Skill, den Operatoren *cold* per natürlicher Sprache aufrufen (die meisten `…-apply`- und Audit-Operatoren hier werden cold aufgerufen), weil das Modell den Skill außerhalb eines Pfad-Matches nicht mehr von selbst erreicht. `paths:` ist **kein** verlässlicher Weg, die dauerhaft geladene Routing-Fläche zu verkleinern — die `description` des Skills treibt weiterhin die Auffindbarkeit —, daher für Routing-Budget-Entlastung `disable-model-invocation: true` plus eine knappere `description` nutzen, nicht `paths:`. Die beiden Felder sind orthogonal: `disable-model-invocation` blockiert die Modell-Invocation vollständig, die Kombination mit `paths:` ist daher redundant
- **DARF NICHT [MUST NOT]** `disable-model-invocation: true` auf einem Skill setzen, den ein anderer Skill **mid-flow als Teil seiner Orchestrierung dispatcht**. Das Flag sperrt die modellgetriebene Invocation, sodass der orchestrierende Skill ihn nicht mehr erreicht und die Kette still ins Leere läuft — etwa `sprint-review`s Opt-in-Dispatch in `release-publish-trigger` oder jeder Skill, den ein anderer Skill in einem „dann dispatch X"-Schritt benennt. Das Flag bleibt **terminalen, user-invozierten** Seiteneffekt-Skills (`/deploy`-artig) vorbehalten, die kein anderer Skill aufruft; vor dem Setzen gegen die Skill-Fläche prüfen (Skill-Namen über benachbarte `SKILL.md`-Bodies grep'en) ([R3](#referenzen))
...
- [ ] Falls `paths:` deklariert ist, soll die automatische Aktivierung des Skills datei-kontext-gebunden sein (kein cold per natürlicher Sprache aufgerufener Operator), und `paths:` wird nicht als Routing-Budget-Hebel verwendet
- [ ] Kein Skill setzt `disable-model-invocation: true`, während ein anderer Skill ihn mid-flow dispatcht (geprüft durch Grep des Skill-Namens über benachbarte `SKILL.md`-Bodies)
