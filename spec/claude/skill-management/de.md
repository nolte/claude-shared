...
- **Namensform**: Die vollständige Namenskonvention — Upstream-Gerundium-Basis, die `<object-noun>-<action>`-Wahl dieses Plugins, die geschlossene Ausnahmeliste (`spec`, `yaml-json-schema`, `quality-gate`), die Eine-Form-pro-Plugin-Regel und die Rename-Policy — gehört `spec/claude/skill-agent-naming/`. Diese Spec wiederholt bewusst nichts davon; bei Diskrepanz gewinnt die Naming-Spec, und `check_name_form` im Validator spiegelt deren geschlossene Listen.
...
- **MUSS [MUST]**, wann immer sich eine Regel in `SKILL.md` ändert, die Änderung im selben Arbeitsgang in jede Hilfsdatei tragen, die diese Regel wiederholt — ein `references/`-Digest, eine Checkliste, ein Template, ein Beispiel. Zwei dokumentierte Mechaniken machen eine veraltete Hilfsdatei aktiv irreführend statt bloß redundant: Eine Referenzdatei wird über ihren eigenen Trigger geladen und ist dann der Text vor dem Lauf, während der widersprechende Body bis dahin kompaktiert oder verkürzt neu angehängt worden sein kann (siehe den 5.000-Token-Cap oben und die Gefahr partieller Reads, der die Inhaltsverzeichnis-Regel begegnet). Dasselbe gilt für im Body genannte Anzahlen über die Hilfsdateien („destilliert aus den sieben zugrunde liegenden Specs") und für jede Regel, die die maßgebliche Spec stromaufwärts geändert hat
...
- [ ] Keine Hilfsdatei formuliert eine Regel, die `SKILL.md` inzwischen geändert hat, und keine im Body genannte Anzahl über die Hilfsdateien widerspricht dem, was dort liegt
