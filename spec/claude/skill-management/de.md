...
- **MUSS [MUST]**, wann immer sich eine Regel in `SKILL.md` ändert, die Änderung im selben Arbeitsgang in jede Hilfsdatei tragen, die diese Regel wiederholt — ein `references/`-Digest, eine Checkliste, ein Template, ein Beispiel. Gerade Progressive Disclosure macht das gefährlich: Ein Lauf lädt das Digest *statt* des Bodys, eine in `SKILL.md` korrigierte und in `references/` stehengebliebene Regel ist also keine redundante Kopie, sondern die Fassung, der der Lauf tatsächlich folgt. Dasselbe gilt für im Body genannte Anzahlen über die Hilfsdateien („destilliert aus den sieben zugrunde liegenden Specs") und für jede Regel, die die maßgebliche Spec stromaufwärts geändert hat
