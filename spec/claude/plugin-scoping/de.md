...
## Offene Fragen
- Anthropic dokumentiert keine Obergrenze oder thematisches Kohäsions-Kriterium für ein einzelnes Plugin. Sollte `nolte-shared` eine *weiche*, selbst auferlegte Artefakt-Obergrenze als Review-Trigger einführen (die einen Duplikat-Präventions- und Routing-Qualitäts-Durchgang anstößt), oder sich rein auf beobachtete Routing-Qualitäts-Signale ohne angehängte Zahl verlassen?
- Ein offener Claude-Code-Bug (anthropics/claude-code#14882, eingereicht 2025-12-20, unbestätigt) berichtet, dass vollständige `SKILL.md`-Bodies beim Start vorgeladen werden, was dem dokumentierten reinen Metadaten-Vorladen widerspricht ([R4](#referenzen)). Sollte das Portfolio bis zur Behebung Skill-Anzahl-Disziplin defensiv durchsetzen, obwohl das dokumentierte Design keine Anzahl-Strafe verhängt?
- Ist `nolte-shared`s „ein Plugin für geteilte Workflows" die dauerhafte Form, oder wird eine Stabilitäts-Aufteilung (eine stabile gegenüber einer experimentellen Oberfläche) irgendwann einen echten Distributions-Vertrags-Unterschied zutage fördern, der ein zweites Plugin rechtfertigt?
- Sollte der Consumer-Mode-Katalog (mehrere Plugin-Quell-Wurzeln in `skill-agent-catalog`) eine künftige portfolio-weite Plugin-Karte speisen, die den Distributions-Vertrag jedes Plugins festhält, sodass Aufteilungs-Entscheidungen über das gesamte Portfolio hinweg auditierbar sind statt pro PR?
