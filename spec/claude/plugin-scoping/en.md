...
- **MUST** keep artifact naming consistent across the **whole** plugin—one naming convention for every skill and agent it ships (per `skill-management` §Frontmatter validation, this plugin's verb-noun convention); inconsistent naming patterns within a single collection are a documented discoverability anti-pattern ([R2](#references))
...
## Open Questions
- Skill-count discipline (full `SKILL.md` body preload). No spec change until `anthropics/claude-code#14882` (currently OPEN, labeled "has repro" since 2026-05-16, that's confirmed-reproduced but unfixed) reaches one of: (a) CLOSED as "working as intended" / "wontfix"—meaning full-body preload is the real design, at which point context-budget cost becomes a measured count penalty and a defensive discipline becomes evidence-grounded; or (b) CLOSED as fixed in a released Claude Code version—at which point this open question is moot and should be removed. Upstream: <https://github.com/anthropics/claude-code/issues/14882>
- A second plugin for a stability split. Revisit when a concrete consumer (or the portfolio maintainer) needs the stable surface but not the experimental one—that is, an artifact set acquires a genuinely divergent distribution contract: a separate release cadence or version line, a separate marketplace entry, or a consumer-install decision distinct from `nolte-shared`'s single one. Operationally: when any capability currently marked `status: experimental` in `project/portfolio.yml` would ship on its own version line or marketplace entry rather than `nolte-shared`'s single 0.1.x line. At that moment §When to split into a separate plugin applies mechanically and the splitting PR must name the contract difference.
