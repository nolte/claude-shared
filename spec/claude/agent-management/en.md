...
- **MUST** scope `tools` to the minimum set needed for the agent's responsibility (principle of least authority); read-only agents MUST NOT receive write, edit, or execution tools
  ...
  - **Sanctioned command classes beyond the strict read-only set**: three recurring command classes sit outside the side-effect-free framing but are sanctioned for a read-only agent when its `## Read-only Bash justification` section names them explicitly and states the bound: (1) **ephemeral tool runners** (`npx --yes <tool>`, `uvx <tool>`) whose only write is the runner's own tool cache outside the repository—sanctioned for resolving a pinned analysis tool the repository doesn't vendor; (2) **network reads** (`curl` or a tool's own fetch against read-only endpoints) for resolving an upstream identifier or release—never a mutating request; (3) **a bounded re-run of a single suspected-flaky test** when the agent's responsibility is test-result analysis—it executes project code and isn't side-effect-free, so the justification names the single-test bound and forbids suite-wide runs. A class the section doesn't list stays forbidden, and the `agent-review` downgrade applies to these classes exactly as to the git/`gh` set.
