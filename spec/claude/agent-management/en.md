...
- **MUST** set `name` to match the filename without the `.md` suffix
  - **Name form**: the complete agent naming convention—the `<subject>-<role-noun>` object-role choice, the role-noun morphology, the closed exception list (`png-to-transparent-svg`, `audience-review`), the one-form-per-plugin rule, and the rename policy—is owned by `spec/claude/skill-agent-naming/`. This spec deliberately restates none of it; on any discrepancy the naming spec wins, and the validator's `check_name_form` mirrors its closed lists.
...
- **MUST** scope `tools` to the minimum set needed for the agent's responsibility (principle of least authority); read-only agents MUST NOT receive write, edit, or execution tools
  ...
  - **Sanctioned command classes beyond the strict read-only set**: five recurring command classes sit outside the side-effect-free framing but are sanctioned for a read-only agent when its `## Read-only Bash justification` section names them explicitly and states the bound: (1) **ephemeral tool runners** (`npx --yes <tool>`, `uvx <tool>`) whose only write is the runner's own tool cache outside the repository—sanctioned for resolving a pinned analysis tool the repository doesn't vendor; (2) **network reads** (`curl` or a tool's own fetch against read-only endpoints) for resolving an upstream identifier or release—never a mutating request; (3) **a bounded re-run of a single suspected-flaky test** when the agent's responsibility is test-result analysis—it executes project code and isn't side-effect-free, so the justification names the single-test bound and forbids suite-wide runs. (4) **package-manager metadata reads** (`pip show <pkg>`, `npm ls <pkg>`, or the equivalent query for the ecosystem) when the audit turns on whether a dependency is actually installed and pinned rather than merely mentioned in prose—the bound is query-only: never `install`, `update`, `add`, or any command that resolves or writes a lockfile; (5) **a documented single-command export or report-mode run of an analysis tool the repository already vendors**, when the tool has no read-only query form and the run's only write is the report path the agent then reads—the justification names the exact command and the path it writes. Where the agent resolves its toolchain at run time and so can't name a fixed command, it instead names the **command class** (for example a coverage report command, or a lint run in report mode), states that the mode must be report-only, and names what the class excludes; a class stated that way satisfies this requirement. A class the section doesn't list stays forbidden, and the `agent-review` downgrade applies to these classes exactly as to the git/`gh` set.
