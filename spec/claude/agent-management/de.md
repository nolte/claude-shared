...
- **MUSS [MUST]** `name` exakt auf den Dateinamen ohne die Endung `.md` setzen
  - **Wahl des `nolte-shared`-Plugins**: Dieses Repository benennt jeden Agent in **Objekt-Rolle-Form** — `<subject>-<role-noun>`, wobei das nachgestellte Token die Rolle ist, die der Agent gegenüber dem führenden Subjekt einnimmt (`code-security-reviewer`, `feature-consistency-reviewer`, `portfolio-manifest-collector`, `vocab-drift-scanner`, `lektorat-scanner`). Dies ist das Agent-seitige Gegenstück zur Verb-Substantiv-Konvention von skill-management für Skills; die Wahl ist hier festgehalten, damit ein Reviewer sie nicht in jeder Iteration bemängelt. Neue Agents in diesem Plugin **MÜSSEN [MUST]** der Objekt-Rolle-Konvention folgen und das Benennungs-Muster über die gesamte Agent-Surface konsistent halten (eine eingestreute Verb-Substantiv- oder Gerundium-Agent-Namensform wäre selbst das Discovery-Anti-Pattern, vor dem `plugin-scoping` §Namespace- und Benennungs-Kohärenz warnt). Eine künftige koordinierte Portfolio-Umbenennung (mit Deprecation-Periode) **DARF [MAY]** die Wahl umkehren; bis dahin ist Objekt-Rolle die Regel
    - **Rollen-Nomen-Morphologie und dokumentierte Ausnahmen**: Das nachgestellte Rollen-Nomen trägt fast immer `-er`/`-or`/`-ist`-Morphologie (`-reviewer`, `-checker`, `-scanner`, `-collector`, `-curator`, `-enforcer`, `-extractor`, `-generator`, `-author`, `-developer`); ein Akteur-Nomen, das eine Rolle ohne diese Morphologie benennt, ist dennoch konform — `webview-ui-expert` (`expert`) ist der etablierte Fall, geführt in `AGENT_ROLE_NOUNS` in `scripts/validate_skills.py`. Zwei etablierte Agent-Namen passen gar nicht auf `<subject>-<role-noun>` und werden bewusst unverändert gelassen: `png-to-transparent-svg` (eine Transformations-Phrase ohne Rollen-Token) und `audience-review` (dessen nachgestelltes `review` eine Aktion benennt, keinen Akteur — sein Objekt-Rolle-Gegenstück wäre `audience-reviewer`). Eine Umbenennung würde jede `subagent_type:`-Aufrufstelle brechen; die Bruchkosten überwiegen den Kohärenzgewinn. Ein Reviewer **DARF NICHT [MUST NOT]** diese zwei Namen als Objekt-Rolle-Verletzung bemängeln. Die Ausnahme ist geschlossen: Sie umfasst genau `png-to-transparent-svg` und `audience-review`, gespiegelt als `AGENT_NAME_FORM_EXCEPTIONS` in `scripts/validate_skills.py` (das jeden anderen Nicht-Rollen-Nomen-Agent-Namen als `Suggestion` meldet), und jeder *neue* Agent **MUSS [MUST]** weiterhin der Objekt-Rolle-Konvention folgen
