...
Leser: Skill- und Agent-Autoren, die die `.claude/settings.json`-Allowlist eines Repositorys pflegen, die Autoren des `permission-allowlist-maintain`-Skills, die den Pflegeprozess operationalisieren, sowie Reviewer, die prüfen, dass eine committete Allowlist weder verknöchert noch wuchert.
...
- **SOLLTE [SHOULD]** ein wiederkehrendes read-only GitHub-MCP-Read-Tool genauso behandeln wie ein read-only `gh`-Kommando: es ist read-only, tritt in GitHub-berührenden Skills und Agents häufig auf und gehört in die Allowlist, damit es nicht promptet, bewertet gegen dieselben drei Bedingungen oben. Der Referenz-Server ist unter dem Server-Namen `github` registriert, ein Tool wird also `github:<tool>` in der Prosa geschrieben (die `ServerName:tool_name`-Syntax aus `spec/claude/skill-management/`) und `mcp__github__<tool>` als `.claude/settings.json`-Allowlist-Eintrag. Der least-privilege-Read-Set ist: `github:get_me` und `github:list_repository_collaborators` (die Trust-Auflösungs-Reads, die `spec/claude/trusted-author-injection-guard/` verlangt), dazu `github:list_issues`, `github:issue_read`, `github:search_issues`, `github:list_pull_requests`, `github:pull_request_read`, `github:search_pull_requests`, `github:list_branches`, `github:list_commits`, `github:get_file_contents`, `github:list_releases`, `github:get_latest_release`, `github:get_release_by_tag`, `github:list_discussions`, `github:get_discussion_comments` und `github:list_discussion_categories`
- **DARF NICHT [MUST NOT]** ein schreibfähiges GitHub-MCP-Tool (Issue- oder Pull-Request-Erstellung/-Aktualisierung, Merge, Datei-Writes, Review-Abgabe) auf die committete Liste setzen — nur Read-Tools qualifizieren sich, analog zum Ausschluss mutationsfähiger `gh`-Wildcards oben; ein Write-Tool bleibt von der Liste fern, damit es weiterhin promptet
