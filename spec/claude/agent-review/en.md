...
- **MUST** verify that the system prompt opens with the agent's role and boundaries and declares the expected output format either before the working method **or** as an explicit terminal "Report" / output-contract phase closing the working method, per the SHOULD in `agent-management`; the plugin's house template uses the closing-phase form and it's conformant, so only an implicit or scattered output shape is a `Warning`
