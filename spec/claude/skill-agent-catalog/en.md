...
For a consolidated cross-artifact reference of every catalog and routing field this spec owns (`phase`, `summary`, `summary_<lang>`, and the use-case metadata), alongside the skill and agent fields and their provenance, see `spec/claude/skill-agent-frontmatter/`. That reference maps and points back to this spec; it doesn't restate the rules here.

**Readers**: catalog-generator implementors (the `scripts/docs/gen_catalog.py` generator module, in its `on_pre_build` hook, standalone pre-build step, or `mkdocs-gen-files` script form), `skill-agent-catalog-apply` skill authors (consumer-side wiring), task-oriented landing-page authors. Skill and agent authors themselves should read `skill-management` and `agent-management` instead—those specs hold the per-artifact authoring rules and delegate catalog-specific schema details (per-language summary, use-case metadata) back to this spec.
...
- **Plugin extension** (per `spec/project/mkdocs-structure/` §Extension hooks → Plugin extension):
  ...
  - **Baseline interaction**: `mkdocs-static-i18n` with `docs_structure: folder` (a baseline MUST) discards files whose `abs_src_path` isn't under `docs_dir`, so the `mkdocs-gen-files` form silently drops generated pages; this spec's §Generation mechanism therefore recommends a **physical-file pre-build surface** for repositories on that i18n setup—the `on_pre_build` hook (the recommended default, registered via `hooks:`) or a standalone `task docs` pre-build step, both writing physical files under `docs/<lang>/<section>/` that `static-i18n` collects and localizes like hand-authored pages. The extension **MUST NOT** disable any baseline plugin.
...
- **SHOULD** declare the reverse `dont_use_when` entry when the two artifacts form a **peer pair**: both address overlapping trigger phrases and a caller could plausibly reach for either first. Reciprocity is owed for the pair, not for every reference.
- **MUST NOT** require the reverse entry when the referenced artifact is a **hub** *and the two don't form a peer pair by the rule above*: one that many artifacts delimit against because it owns a single well-known responsibility (a gate, an audit, a triage flow). A hub naming every artifact that points at it inflates the one field loaded on every routing decision, so the one-way reference is correct there and isn't a finding. As of 2026-08-22 the inventory carries 148 one-way references of 300, concentrated on `dependency-audit` (14), `quality-gate` (11), and `workflow-health-triage` (10), all hubs.
- **Cross-plugin references**: an `alternative` or `see_also` entry MAY name an artifact shipped by a *different* plugin of the same monorepo—the docs build resolves across every configured source root, so the reference is valid there. Because a consumer may install only a subset of the plugins, prose surfaces (a `description` negative trigger, a body redirect) SHOULD phrase such an edge conditionally ("from the `nolte-engineering` plugin, if installed") or resolve the target from the runtime inventory; the structured `alternative` value itself stays the bare artifact `name` and is never annotated, or resolution breaks
...
- **MUST** keep the published catalog current by one of two mechanisms, chosen by whether the docs-deploy pipeline actually runs the generator on the deploy build:
  - **Deploy-time generation (preferred):** when the deploy pipeline invokes the catalog generator on the deploy build, the repository **MUST NOT** commit generated catalog markdown into `docs/`; the catalog is regenerated on every build and no committed-artifact freshness check is needed. The generator runs on the deploy build through any of its §Generation mechanism invocation surfaces: an `on_pre_build` hook (which fires inside **every** `mkdocs build`, including a bare `mkdocs build` and the `mkdocs gh-deploy` run by `mhausenblas/mkdocs-deploy-gh-pages`, with no Taskfile wiring), a `reusable-mkdocs.yaml` that runs `task docs` when a Taskfile with a `docs` target exists, or `mkdocs build` with the generator wired as a `mkdocs-gen-files` script.
  - **Committed catalog with a freshness gate (fallback):** when the shared deploy reusable **doesn't** run the generator—for example `nolte/gh-plumbing`'s `reusable-mkdocs.yaml@v1.1.19` deploys through `mhausenblas/mkdocs-deploy-gh-pages`, which runs `mkdocs build` only and never invokes `task docs`, and so the repository **MUST** commit the generated catalog tree and **MUST** enforce a CI freshness check that fails when the committed tree has drifted from a fresh regeneration (the generator still runs locally via the `task docs` pre-build dependency and the `docs-catalog-fresh` pre-commit hook). This keeps the published GitHub Pages output complete despite the deploy build skipping the generator. A repository on this fallback **MUST** retire it in favour of deploy-time generation once the shared deploy reusable invokes the generator. The **`on_pre_build` hook form is the recommended way to exit this fallback** even when the deploy reusable runs a bare `mkdocs build` or `mkdocs gh-deploy`: because the hook fires inside that build, the catalog regenerates at deploy time without any Taskfile or CI wiring, so the committed tree and its freshness gate are no longer needed for deploy freshness. The exit precondition is that the hook's generator module and its dependencies are installed in the deploy environment and its configured source roots are checked out, or the build fails loudly (an acceptable hard failure, never silent staleness).
- **MUST** wire catalog navigation through `mkdocs-literate-nav` declared in `mkdocs.yml`
- **MUST** invoke a catalog generator that produces the per-artifact pages, per-section index pages, per-section `SUMMARY.md` files for literate-nav, and the tag index. The generator is **one module with a single rendering core**, exposed through one of up to three **invocation surfaces**; the form choice concerns the invocation surface only and **MUST NOT** fork the rendering logic into per-form implementations (a thin entry point per surface delegating to the one shared core):
  - an **`on_pre_build` hook** registered via `hooks:` in `mkdocs.yml`, writing physical files under `docs/<lang>/<section>/` from inside `mkdocs build`;
  - a **standalone pre-build step** (for example a Taskfile target, or the module's `__main__` entry point, invoked before `mkdocs build`) that writes the same physical files; or
  - a **`mkdocs-gen-files` plugin script** that emits the pages as virtual files.

  A repository using `mkdocs-static-i18n` with `docs_structure: folder` **MUST** choose one of the two physical-file surfaces (the hook or the standalone pre-build step), because `mkdocs-static-i18n` 1.3.x discards files whose `abs_src_path` isn't under `docs_dir` and therefore silently drops every page emitted by `mkdocs-gen-files`. Among those two the **`on_pre_build` hook is the recommended default**: `mkdocs` runs file collection *after* `on_pre_build`, so hook-written physical pages are collected and localized exactly like hand-authored pages, and the hook regenerates inside every build surface (`mkdocs build`, `mkdocs serve`, `mkdocs gh-deploy`) with no Taskfile dependency. The standalone pre-build step stays **equally valid and explicitly supported**, and it's the surface that drives a committed-catalog freshness gate and local `git diff` review
...
- **MUST** regenerate the catalog automatically as part of the normal docs build, with no separate manual step, so local and CI builds produce identical output. This output-identity invariant is realized by any of the §Generation mechanism invocation surfaces: the `on_pre_build` hook satisfies it **inside `mkdocs build` itself**, so it holds for every build entry point (including a bare `mkdocs build` or `mkdocs gh-deploy`) without a Taskfile dependency; the standalone pre-build step satisfies it by declaring the generator as a Taskfile dependency of the `docs` task; the `mkdocs-gen-files` script satisfies it within the plugin build
...
## Acceptance Criteria
- [ ] A docs build (via `task docs`, or a bare `mkdocs build` / `mkdocs gh-deploy` when the generator is wired as an `on_pre_build` hook) produces a docs site whose navigation contains a Skills section with one page per skill across all configured plugin source roots
- [ ] A docs build (via `task docs`, or a bare `mkdocs build` / `mkdocs gh-deploy` when the generator is wired as an `on_pre_build` hook) produces a docs site whose navigation contains an Agents section with one page per agent across all configured plugin source roots
...
- [ ] Removing a skill or agent removes the corresponding catalog page on the next docs build (any invocation surface)
...
- [ ] The catalog generator is exposed through one of its invocation surfaces (an `on_pre_build` hook registered via `hooks:` in `mkdocs.yml`, a standalone pre-build step wired into `task docs`, or a `mkdocs-gen-files` script in `mkdocs.yml`), all delegating to one shared generator core, and a repository on `mkdocs-static-i18n` folder mode uses one of the two physical-file surfaces
...
- [ ] Generated catalog markdown isn't committed under `docs/`; the docs-deploy pipeline regenerates the catalog on every build (via an `on_pre_build` hook that fires inside `mkdocs build`, via `task docs` when a Taskfile `docs` target exists, or via `mkdocs build` with the generator wired as a `mkdocs-gen-files` script)
- [ ] On the committed-catalog freshness-gate fallback (when the deploy pipeline doesn't run the generator), the generated catalog tree is instead committed under `docs/` and a CI freshness check fails when the committed tree has drifted from a fresh regeneration
- [ ] A skill or agent with invalid frontmatter causes the docs build to fail (via `task docs`, or via the `on_pre_build` hook raising on a bare `mkdocs build`) with an error that names the offending file and its plugin source root
