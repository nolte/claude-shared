...
- **Plugin-Extension** (gemäß mkdocs-structure §Extension hooks → Plugin extension):
  ...
  - **Baseline-Interaktion**: `mkdocs-static-i18n` mit `docs_structure: folder` (ein Baseline-MUST) verwirft Dateien, deren `abs_src_path` nicht unter `docs_dir` liegt, sodass die `mkdocs-gen-files`-Form generierte Seiten still verwirft; der §Generierungs-Mechanismus dieser Spec empfiehlt daher die Pre-Build-Form (eine `task docs`-Abhängigkeit, die physische Dateien unter `docs/<lang>/<section>/` schreibt) für Repositories mit diesem i18n-Setup. Die Extension **DARF [MUST] NICHT** ein Baseline-Plugin deaktivieren.
...
- **MUSS [MUST]** den veröffentlichten Katalog über einen von zwei Mechanismen aktuell halten, abhängig davon, ob die Doku-Deploy-Pipeline den Generator beim Deploy-Build tatsächlich ausführt:
  - **Generierung zur Deploy-Zeit (bevorzugt):** Wenn die Deploy-Pipeline den Katalog-Generator beim Deploy-Build aufruft (zum Beispiel ein `reusable-mkdocs.yaml`, das `task docs` ausführt, wenn ein Taskfile mit einem `docs`-Target existiert, oder `mkdocs build` mit dem als `mkdocs-gen-files`-Skript verdrahteten Generator), **DARF [MUST] NICHT** das Repository generiertes Katalog-Markdown nach `docs/` committen — der Katalog wird bei jedem Build neu generiert und es ist kein Frische-Check für eingecheckte Artefakte nötig.
