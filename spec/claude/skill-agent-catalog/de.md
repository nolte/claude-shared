...
- **MUSS [MUST]** alle vier Felder in dieser Spec als optional behandeln; die Autoren-Pflicht (wann Autoren sie **SOLLTEN [SHOULD]** deklarieren) liegt bei `skill-management` und `agent-management`
...
- Sollen `use_when`, `dont_use_when`, `see_also` und `examples` für neu verfasste Artefakte nach erfolgreicher Pilot-Migration von optional auf Pflicht heraufgestuft werden, oder bleiben sie dauerhaft optional, sodass `description` weiterhin doppelte Arbeit leistet?
- Soll der Katalog auch Rück-Referenzen auf ein Artefakt zeigen (also: Artefakt `A` listet `B` in `see_also`—soll die Katalog-Seite von `B` einen Rück-Link auf `A` zeigen)? Nützlich für Navigation, kostet einen zusätzlichen Render-Pass.
