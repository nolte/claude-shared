...
- Ein Briefing liefert daneben **Fakten**, auf die der Spezialist handeln soll, statt sie zu prüfen — einen gemessenen Wert, ein Datum, einen Pfad, eine Version. Jeder solche Fakt **MUST** [MUST] seinen Anker inline tragen (ein Kommando samt Ausgabe oder ein `file:line`), gepinnt nach `spec/claude/claim-provenance/` §B, sofern er aus einer veränderlichen Referenz gelesen wurde. Einen Fakt, der im Briefing ohne Anker steht, **MUST** [MUST] der Spezialist als Hypothese nach dieser Spec behandeln, nicht als Gegebenheit.
- Ein Spezialist, der einen **aus dem Briefing stammenden Fakt in ein dauerhaftes Artefakt schreibt** — Dokumentation, eine Spec, eine Commit-Message, ein Issue, einen veröffentlichten Bericht —, **MUST** [MUST] diesen Fakt zuvor gegen die Arbeitskopie verifizieren und **MUST NOT** [MUST NOT] ihn durch Zitieren des Briefings einlösen. Das Briefing ist ein Artefakt und löst nichts ein, was es nicht selbst belegt hat; `spec/claude/claim-provenance/` §B besitzt diese Regel, hier steht ihr delegierter Fall. Ein unverifizierter Fakt, der vom Briefing in ein dauerhaftes Artefakt wandert, erhält die Autorität des Artefakts und behält die Evidenz des Briefings — der Fehler kostet dann eine Korrektur an jeder Stelle, an der er gelandet ist, statt einer Antwort.
...
- [ ] Fakten im Briefing müssen einen inline-Anker tragen, und ein ungeankerter Fakt ist als Hypothese statt als Gegebenheit definiert.
- [ ] Ein Spezialist muss einen aus dem Briefing stammenden Fakt verifizieren, bevor er ihn in ein dauerhaftes Artefakt schreibt, und darf ihn nicht durch Zitieren des Briefings einlösen.
