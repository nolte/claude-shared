...
## Offene Fragen
- Auf ein Claude-Code-Release achten, das ein eigenständiges Command-/CLI-Artefakt-Format mit eigener On-Disk-Struktur und eigenem Frontmatter ausliefert (also einen Slash-Command, der nicht bloß die Aufruf-Oberfläche eines Skills ist). Konkret messbar als: (a) die Claude-Code-Doku zu Skills/Subagents ([R2](#referenzen) <https://code.claude.com/docs/en/skills>, [R1](#referenzen) <https://code.claude.com/docs/en/sub-agents>) erhält ein separat dokumentiertes Command-Artefakt mit eigenem Dateispeicherort und eigenem Frontmatter-Schema, ODER (b) `.claude-plugin/plugin.json` / das Plugin-Manifest-Schema fügt eine First-Class-`commands`-Deklaration hinzu, die sich von `skills` unterscheidet. In beiden Fällen: einen dritten Zweig zur Entscheidungsregel dieser Spezifikation hinzufügen und eine Schwester-Spezifikation `command-management` neben skill-management/agent-management aufsetzen.
