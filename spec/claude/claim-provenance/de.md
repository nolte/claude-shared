...
- Eine Behauptung, deren Anker eine **veränderliche Referenz** liest — einen Branch-Namen, `HEAD`, einen Default-Branch-Alias, ein bewegliches Tag wie `latest`, eine „aktuelle" Sicht einer API — **MUSS [MUST]** zusätzlich den **unveränderlichen Bezeichner** festhalten, auf den diese Referenz aufgelöst hat (Commit-SHA, Content-Digest, Revisions-Id), und **MUSS [MUST]** gegen diesen Bezeichner belegt werden statt gegen den beweglichen Namen. Eine veränderliche Referenz benennt einen Zeitpunkt, keinen Zustand: Wer den Anker morgen erneut ausführt, beantwortet eine andere Frage als die Autorin gestellt hat — ein ungepinnter Anker ist damit ausgerechnet für die eine Leserin unwiderlegbar, die den Fehler finden würde.
- Eine Behauptung, die **zwei oder mehr Werte aus einem Artefakt zusammensetzt** — zwei Felder eines Manifests, eine Version zusammen mit einem Digest, eine Einstellung zusammen mit ihrer Wirkung — **MUSS [MUST]** belegen, dass diese Werte aus **derselben Revision** jenes Artefakts stammen. Werte, die nie gemeinsam existiert haben, setzen sich zu einer Aussage zusammen, die falsch ist, während jeder Teil für sich zutrifft; wer die Teile einzeln nachliest, bemerkt es nie.
- Eine Behauptung, die eine bereits belegte Behauptung aus einem anderen Artefakt wiederholt, **KANN [MAY]** die Anforderung durch Zitieren jenes Artefakts erfüllen — **aber nur, wenn das zitierte Artefakt die Behauptung selbst als belegt führt und sein Anker auflöst.** Ein Artefakt zu zitieren, das die Behauptung lediglich *aufstellt*, verschiebt die Vermutung, statt sie einzulösen; die Zitatkette liest sich dann wie Evidenz und ruht auf nichts. Ein Dispatch-Brief, ein Issue-Kommentar oder eine frühere Analyse ist ein Artefakt nach §A wie jedes andere: Es löst eine Behauptung nur ein, wenn es seinen eigenen auflösenden Anker trägt.
...
- `spec/project/e2e-failure-diagnosis/` §A besitzt die **Mechanismus-Schwelle** für einen End-to-End-Fehlercluster: die drei zulässigen Beweisformen und den Hinreichend-und-notwendig-Standard, der eine Remediation gated. Sie wird von einem roten Lauf aus erreicht und gated eine Änderung. Diese Spec gated die *Behauptung* und wird von jedem Artefakt aus erreicht. §A wendet diese Spec an; keine wiederholt die andere, und die Beweisformen von §A bleiben dort. Die beiden Schwellen liegen bewusst unterschiedlich hoch, und ein Leser sollte sie nicht verwechseln: **belegt** heißt hier, dass eine widerlegende Beobachtung versucht wurde und nicht widerlegte — genug, um eine Ursache ehrlich aufzuschreiben —, während die Mechanismus-Schwelle von §A Hinreichendheit und Notwendigkeit verlangt, bevor jene Ursache eine Änderung treiben darf.
...
- [ ] Die Wiederholung einer bereits belegten Behauptung kann durch Zitieren des belegenden Artefakts erfüllt werden **nur dann, wenn jenes Artefakt die Behauptung als belegt führt und sein Anker auflöst**, damit eine Zitatkette nicht in einer blossen Behauptung enden kann
- [ ] Ein Anker, der eine veränderliche Referenz liest, muss den unveränderlichen Bezeichner festhalten, auf den sie aufgelöst hat, und gegen diesen belegt werden
- [ ] Eine Behauptung, die mehrere Werte eines Artefakts zusammensetzt, muss belegen, dass sie aus einer Revision stammen, damit nie gemeinsam existierende Werte sich nicht zu einer falschen Aussage fügen
- [ ] Eine Spec, die diese Regel braucht, muss sie referenzieren statt wiederholen, und die bei der Autorenschaft verdrahteten Nachbarn tun genau das
- [ ] Beide Einbindungspflichten sind gegen ein Artefakt prüfbar: ein Agent trägt die Unterscheidung im Befund selbst, und ein Skill wendet §B auf das dauerhafte Artefakt an, das es schreibt
