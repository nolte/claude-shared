...
- A claim whose anchor reads a **mutable reference** — a branch name, `HEAD`, a default-branch alias, a floating tag such as `latest`, a "current" view of an API — **MUST** additionally record the **immutable identifier** that reference resolved to (a commit SHA, a content digest, a revision id), and **MUST** be established against that identifier rather than against the moving name. A mutable ref names a moment, not a state: re-running the anchor tomorrow answers a different question than the author asked, so an unpinned anchor can't be falsified by the one reader who would catch the error.
- A claim that **composes two or more values read from one artefact** — two fields of a manifest, a version together with a digest, a setting together with its effect — **MUST** establish that those values come from the **same revision** of that artefact. Values that never co-existed compose into a statement that is false while every part of it is separately true, and re-reading the parts one at a time never reveals it.
- A claim that repeats an already-established claim from another artefact **MAY** discharge the requirement by citing that artefact — **but only when the cited artefact itself presents the claim as established and its anchor resolves.** Citing an artefact that merely *asserts* the claim moves the guess rather than discharging it, and the citation chain then reads as evidence while resting on nothing. A dispatch brief, an issue comment, or a prior analysis is an artefact under §A like any other: it discharges a claim only if it carries its own resolving anchor.
...
- [ ] Repeating an already-established claim can be discharged by citing the artefact that established it **only when that artefact presents the claim as established and its anchor resolves**, so a citation chain can't terminate in an assertion
- [ ] An anchor that reads a mutable reference is required to record the immutable identifier it resolved to, and to be established against that identifier
- [ ] A claim composing several values from one artefact is required to establish that they come from one revision, so values that never co-existed can't compose into a false statement
