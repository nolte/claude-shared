...
- **MUST** triangulate every factual assertion that refers to something **not verifiable** in the current working copy, including:
  ...
  - product, package, or brand names that are not anchored in the current repo
...
- **MUST NOT** triangulate subjective decisions — they are not factual claims and have no independent sources to compare against
...
- **MUST** include **at least one** Primary OR Secondary source in every triangulation — three Web-aggregator hits do not satisfy the requirement on their own
...
- **MUST**, when the operator is not reachable (autonomous loop, scheduled run, cron-driven dispatch), **abort the write** and persist the conflict as a findings report rather than guess at a resolution
...
- [ ] Repo-internal assertions are not triangulated against external sources; they are verified via `Read` or `Grep` against the working copy
- [ ] Model memory (pre-training knowledge, memory entries without a cited source) does not count toward the independent-source threshold
...
## Open Questions
- How does this methodology handle sources that announce a **future** behaviour (a release note for a planned version that has not shipped yet)? Current default: such sources do not count toward the threshold until the behaviour is observable in a Primary source; should this be made explicit or remain a per-skill judgement?
