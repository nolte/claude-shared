...
- **MUST** delete the plan file when every item is either `- [x]` or carries a `→ deferred: <url>` annotation; the deletion commit message **MUST** be `chore(<review-type>): close <target>—<C>C/<W>W/<S>S/<I>I` (counts of Critical, Warning, Suggestion, Info at creation time), so the git log is the searchable audit trail. The line is the subject of the squash commit, so the pull request that deletes the plan carries it as its title; the type is `chore` because the pull-request type vocabulary has no `review` type. Plans closed before that vocabulary was enforced carry the former `review(<review-type>): close …` subject, which stays searchable
...
- [ ] Every plan deletion in `git log` is accompanied by a commit message matching `chore(<review-type>): close <target>—<counts>`, or the former `review(<review-type>): close` subject for plans closed before the type vocabulary was enforced, so the audit trail is searchable
