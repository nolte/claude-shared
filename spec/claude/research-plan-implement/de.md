...
- Explorations-Rauschen aus dem implementierenden Kontext heraushalten, ohne dass die verdichtete Zusammenfassung zu einer Behauptung wird, die niemand auditieren kann wird.
...
- Eine Capability, die Research oder Plan ausführt, **MUST NOT** [MUST NOT] in einen Pfad schreiben, der vom betroffenen Repository versioniert wird. Das Schreiben des phaseneigenen Artefakts auf eine unversionierte Arbeitsfläche (`.resume/`, `.audits/` oder ein Scratch-Verzeichnis) ist die einzige Ausnahme, und dieses Artefakt **MUST NOT** [MUST NOT] die Änderung selbst sein.
- Research **MUST** [MUST] ein **Findings-Artefakt** erzeugen und Plan **MUST** [MUST] ein **Plan-Artefakt** erzeugen, wann immer die Phase überhaupt läuft. Eine Phase, die kein Artefakt erzeugt, ist nicht gelaufen, sondern erzählt worden.
- Der Übergang von Plan zu Implement ist das **Schreib-Gate**. Er ist der erste Punkt, an dem die Capability versionierten Zustand ändern darf.
...
- Eine Capability **MUST NOT** [MUST NOT] still unter die Stufe fallen, die die Arbeit verlangt. Klassifiziert sie herunter (etwa eine Mehrdatei-Änderung als Stufe 1), **MUST** [MUST] sie die Klassifikation und ihre Begründung im erzeugten Artefakt festhalten, damit die Entscheidung reviewbar statt unsichtbar ist.
...
- Ein read-only Reviewer- oder Scanner-Agent ist eine **Research-Phasen-Capability**. Sein deklariertes Tool-Set **MUST NOT** [MUST NOT] Schreib-Tools enthalten, und seine Ausgabe **MUST** [MUST] die obige Findings-Artefakt-Anker-Regel erfüllen.
...
- Automatisierte Durchsetzung ist noch nicht verdrahtet. `skill-review` und `agent-review` prüfen derzeit Frontmatter und Description-Budgets; ob sie zusätzlich prüfen sollen, dass ein schreibendes Skill seine Stufe und sein Schreib-Gate benennt, ist zurückgestellt. **Revisit-Auslöser:** wenn ein zweites Skill in diesem Monorepo dabei angetroffen wird, das Schreib-Gate ohne Plan-Artefakt zu überschreiten, oder wenn `skills-agents-sweep` irgendeine Body-Content-Prüfung über seine derzeitigen hinaus erhält, was die Grenzkosten einer weiteren Prüfung nahe null bringt.
