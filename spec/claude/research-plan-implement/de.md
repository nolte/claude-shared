...
- Ein read-only Reviewer- oder Scanner-Agent ist eine **Research-Phasen-Capability**. Sein Tool-Set regelt `spec/claude/agent-management/` §Tool-Zugriff, einschließlich der dortigen engen Ausnahmen für read-only `Bash` und Netzwerk-Lesezugriffe; diese Spec ergänzt kein eigenes Tool-Verbot. Seine Ausgabe **MUST** [MUST] die obige Findings-Artefakt-Anker-Regel erfüllen.
