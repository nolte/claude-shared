...
- The phase names of this spec **MUST NOT** be conflated with the `phase:` frontmatter field, and neither **MUST** be expressed through the other. `spec/claude/skill-agent-frontmatter/` defines `phase:` over the enum `vision`, `plan`, `design`, `build`, `review`, `quality`, `close-release`, `cross-cutting`: a **delivery-lifecycle** position, meaning where in the portfolio's flow a capability sits. Research, Plan and Implement are a **workflow-internal** sequence, meaning which part of its own execution a capability is in. The tokens `plan` and `review` occur in both with different meanings, so a skill declaring `phase: plan` says nothing about whether it runs a Plan phase, and a Research-phase reviewer agent needn't carry `phase: review`. A capability states its tier and its write gate in its **body**, never by reusing that frontmatter field.
...
- [ ] The delivery-lifecycle `phase:` frontmatter field and this spec's phase names are stated to be different axes, so a capability can't satisfy one by declaring the other.
...
- **Decision (2026-08-23): enforcement is wired for skills, and stays deferred for agents.** The revisit trigger above hadn't fired (an audit of all 65 skills found none crossing the write gate without a review surface), so this is an operator decision taken ahead of it, recorded here rather than presented as the trigger firing. The mechanical half (citation present, tier stated, write gate named) is checked by `scripts/validate_skills.py`; the judgement half (does the declared tier fit the work, is the gate named at the right point) belongs to `skill-review` per `spec/claude/skill-review/` §"Checks derived from research-plan-implement" below. The check is **adoption-keyed**: an un-adopted skill is counted into one aggregate backlog finding, and adopting the citation promotes that skill's tier and write-gate rules to blocking. This is the baseline-and-ratchet model `spec/project/test-tier-static-analysis/` §"Severity gating and the baseline-and-ratchet model" requires when introducing analysis over an existing corpus. It's why the whole backlog doesn't block at once. The agent half is untouched: `agent-review` asserts nothing from this spec yet. **Revisit trigger for agents:** when the skill-side ratchet has locked on its first adopters, so the check shape is proven before it's mirrored.
