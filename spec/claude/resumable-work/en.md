...
- **MUST** treat `.resume/` as a single repo-root-scoped convention regardless of how many language ecosystems the repository hosts; the directory is keyed by Claude Code artefact name, not by toolchain, so it's never split per language ecosystem (the lockfile analogy doesn't transfer), keeping autodetect and the single `/.resume/` gitignore entry uniform across single-purpose repositories and polyglot monorepos alike
...
- **MAY** apply a deterministic normalisation (case-folding, whitespace trimming/collapsing) when comparing `inputs:` for human-typed identifiers, so that two spellings of the same target resolve to one in-progress run rather than spuriously re-asking answered gates; the normalisation **MUST** itself be documented and deterministic, consistent with §Resume detection's determinism requirement
...
- **MUST**, when exactly one in-progress run matches, prompt the operator with: `Resume run <run_id> from phase <phase> (last checkpoint <last_checkpoint_at>)?` and offer exactly three choices: `resume` (re-hydrate state and continue), `start-new` (begin a fresh run; leave the existing file as `in_progress`), `discard` (delete the existing file, then begin a fresh run); the prompt stays at these three choices with no "diff against existing" preview, because `start-new` is non-destructive (it leaves the existing file intact—only `discard` deletes), so there is no data-loss risk a preview would need to mitigate
...
- Until observed stale-file accumulation justifies it (see the measurable trigger in §Open Questions), the provisional default is no dedicated prune skill: each artefact owns its own cleanup via the status flips and the 30-day stale-surfacing rule above
- **MUST** treat direct `ls .resume/` and `cat .resume/<name>/<run-id>.yml` inspection as the supported baseline for operators; any `task resume:list` / `task resume:prune` helper is optional and **MUST NOT** be required, since the format is deliberately designed to be inspectable without bespoke tooling (per §Goals)
...
## Open Questions
- Default: no dedicated portfolio-wide prune skill; each artefact owns its own cleanup via the status flips, the one-`completed`-cycle retention, and the 30-day stale-surfacing rule in §Completion and cleanup. Revisit when: ≥10 stale `in_progress` files (`last_checkpoint_at` older than 30 days) spanning ≥2 distinct skill or agent names co-exist in one working copy's `.resume/` tree, or an operator reports the per-skill 30-day surfacing being insufficient in practice
