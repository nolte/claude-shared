...
## Open Questions
- Soll `.resume/` ein pro-Sprach-Ökosystem-Belang sein (separat für Python-Tooling vs. Node-Tooling, ähnlich wie Lockfiles) oder strikt eine einzige Top-Level-Konvention? Die aktuelle Spec nimmt Letzteres an, aber ein polyglottes Monorepo könnte von Ersterem profitieren
- Gibt es einen portfolioweiten Housekeeping-Skill, der periodisch `completed`- und veraltete `in_progress`-Dateien bereinigt (analog zu `git worktree prune`), oder ist jeder Skill für sein eigenes Aufräumen verantwortlich? §Abschluss und Aufräumen erlaubt beides, schreibt aber nichts vor
- Soll diese Spec einen CLI-Helper vorschreiben — etwa `task resume:list` und `task resume:prune` —, damit Operator:innen das Verzeichnis inspizieren können, oder reicht direkter `ls .resume/`- und `cat .resume/<name>/<run-id>.yml`-Zugriff? Letzteres ist die aktuelle Annahme
- Wie verzahnt sich das mit Skills, die bereits unter `.audits/` für ihre Findings schreiben (`spec-drift-audit`, `portfolio-audit`, `skill-review`, `agent-review`)? Ist `.audits/` immer der Final-Output und `.resume/` immer der In-Flight-Scratchpad, oder kann ein einzelnes Skill-Artefakt beide Rollen spielen, indem es bei Abschluss von `.resume/` nach `.audits/` verschoben wird?
- Soll das Resume-Prompt eine `start-new`-Option mit „Diff gegen Bestehendes"-Vorschau anbieten, bevor verworfen wird, damit die Person sieht, was verloren ginge? Aktuell hält die Spec das Prompt minimal (drei Wahlmöglichkeiten, keine Vorschau)
- Soll das `inputs:`-Matching exakt sein, oder darf jeder Skill ein „normalisiertes Match" deklarieren (case-insensitiv, whitespace-tolerant) für von Menschen getippte Identifier? Die Spec überlässt den Matching-Algorithmus aktuell jedem Skill, fordert aber Determinismus
