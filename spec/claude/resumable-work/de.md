...
## Open Questions
- Default: kein dedizierter portfolioweiter Prune-Skill; jedes Artefakt besorgt sein eigenes Aufräumen über die Status-Flips, das Behalten eines `completed`-Zyklus und die 30-Tage-Surfacing-Regel in §Abschluss und Aufräumen. Revisit, wenn: ≥10 veraltete `in_progress`-Dateien (`last_checkpoint_at` älter als 30 Tage), die ≥2 verschiedene Skill- oder Agent-Namen umspannen, in der `.resume/`-Struktur einer einzigen Working Copy koexistieren, oder eine Operatorin bzw. ein Operator berichtet, dass das per-Skill-30-Tage-Surfacing in der Praxis nicht ausreicht
