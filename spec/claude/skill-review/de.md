...
### Checks aus `research-plan-implement`

- **MUSS [MUST]** die drei Bindungsprüfungen aus `spec/claude/research-plan-implement/` §„Bindung für Skill- und Agent-Autorenschaft“ gegen ein Skill anwenden, dessen Workflow in versionierte Pfade schreibt: Der Body zitiert jene Spec, benennt die Stufe oder den Stufenbereich, auf den er zielt, und benennt den Punkt, an dem er das Schreib-Gate überschreitet
- **MUSS [MUST]** die Schwere an die **Adoption** binden, gemäß dem Baseline-and-Ratchet-Modell, das `spec/project/test-tier-static-analysis/` §„Severity-Gating und das Baseline-and-Ratchet-Modell“ für die Einführung von Analyse über einem bestehenden Korpus verlangt. Ein Skill, das die Spec zitiert und dann Stufe oder Schreib-Gate weglässt, ist ein `Critical`. Ein Skill, das sie nirgends zitiert, ist **kein** Einzelbefund: Der gesamte Korpus ist älter als die Spec, und 65 Einzelbefunde würden die handlungsrelevanten des Reports ersticken. `scripts/validate_skills.py` meldet jenen Rückstand einmal als aggregiertes `Info`; der Reviewer **DARF NICHT [MUST NOT]** ihn je Skill erneut aufwerfen
- **MUSS [MUST]** die mechanische Hälfte als bereits abgedeckt behandeln und **DARF NICHT [MUST NOT]** sie als Befund wiederholen: Ob Zitat, ein Stufen-Token und die Wendung *write gate* vorhanden sind, prüft `scripts/validate_skills.py`. Was dieses Review hinzufügt, ist die Hälfte, die ein Parser nicht erreicht:
  - ob die **genannte Stufe zur tatsächlichen Arbeit passt**, beurteilt gegen die vier Zeilen der Stufentabelle und ihre Höchste-Treffer-Regel. Ein Skill, das ein Repository-Layout gerüstet und Stufe 1 deklariert, ist ein `Critical`; ebenso eines, das ein dreiphasiges Interview für die Änderung eines einzelnen Frontmatter-Feldes fährt — die Über-Zeremonie, die die Spec als Defekt der Capability statt als Sorgfalt benennt
  - ob das **Schreib-Gate an der richtigen Stelle benannt** ist — beim ersten Schreiben versionierten Zustands, der Teil der Änderung ist, nicht beim ersten Schreiben überhaupt. Ein Skill, das sein Plan-Artefakt-Schreiben als Gate benennt, hat es falsch etikettiert: `research-plan-implement` §„Phasendefinitionen und die Schreibgrenze“ stellt Artefakt-Schreibvorgänge auf die read-only-Seite der Grenze, „artifact versus change, not tracked versus untracked“
  - ob bei deklarierter Stufe 2 oder 3 tatsächlich ein Operator-Zustimmungs-Gate zwischen Plan und Implement sitzt
- **DARF NICHT [MUST NOT]** einen `phase:`-Frontmatter-Wert als Erfüllung eines der obigen Punkte akzeptieren. Jenes Feld ist eine Position im Lieferzyklus und teilt die Tokens `plan` und `review` mit den Phasennamen dieser Disziplin bei anderer Bedeutung; §„Bindung für Skill- und Agent-Autorenschaft“ verbietet, das eine durch das andere auszudrücken
- Begründung: Die Bindungsregeln sind so geschrieben, dass sie allein aus dem Artefakt prüfbar sind — mechanisch ist aber nur ihr Vorhandensein. Stufenpassung und Gate-Platzierung sind genau die Urteile, für die ein Review existiert; sie ungeprüft zu lassen, ließe ein Skill dem Buchstaben der Disziplin genügen, ohne etwas davon zu praktizieren
...
- **MUSS [MUST]** Findings in dieser Reihenfolge erzeugen: externe-Validator-Findings → Frontmatter → Description/Trigger → System-Prompt-Body → Rationale-Abschnitt → referenzierte Assets → Duplikat-Prävention-Check → Best-Practices-Checks → Spec-Anchor-Check → research-plan-implement-Bindungsprüfungen → INFO-Beobachtungen
