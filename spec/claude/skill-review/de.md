...
- Soll das Review eines Skills auch verifizieren, dass die `description`-Trigger des Skills nicht mit einem Runtime-Slash-Command oder einem Claude-Code-Built-in-Command überlappen, und wenn ja gegen welche autoritative Liste? Entsperren, sobald ENTWEDER (a) der Claude-Code-Harness eine abfragbare, versionierte Built-in-/Reserved-Command-Liste bereitstellt, die dieses Repo pinnen und grepen kann (die Slash-Command-Oberfläche in der Claude-Code-Doku beobachten, <https://code.claude.com/docs/en/slash-commands>) — die Reproduzierbarkeits-Latte aus §Ziele („identische Ergebnisse auf demselben Quellbaum") lässt sich gegen eine ungepinnte, sich entwickelnde Liste nicht erreichen; ODER (b) `skill-vs-agent` §Offene Fragen Slash-Commands als erstklassige Artefakt-Klasse auflöst, woraufhin diese Spec einen definierten Command-Namespace erbt, gegen den die Description-Trigger-Überlappung geprüft werden kann. Keines von beidem ist eingetreten: #228 hat 178 Fragen aufgelöst, die `skill-vs-agent`-Dritte-Artefakt-Klasse-Frage aber explizit offen gelassen, und kein Reserved-Command-Katalog existiert in `spec/`, `scripts/` oder `.claude-plugin/`.
