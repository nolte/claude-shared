...
- Ob die konsumierenden UX-/Build-Agents (`frontend-usability-optimizer`, `webview-ui-expert`, `fullstack-developer`) diese Spec aus ihrer eigenen `description`/`use_when` zitieren sollten, oder ob die hier deklarierte Bindung ausreicht. Vorläufiger Default: Die Spec deklariert die Bindung; die Verdrahtung der Frontmatter jedes Agents ist ein separater Folge-Schritt, nicht Teil dieser Spec.
