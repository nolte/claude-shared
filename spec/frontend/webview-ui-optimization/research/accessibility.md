...
- **`palette.contrastThreshold` open question resolved (2026-07-24):** the setting still applies under the MUI v9 `colorSchemes` + `cssVariables` API—each color scheme's palette runs through the same `augmentColor` / `createPalette` derivation at theme-creation time (default threshold stays `3`), and CSS variables change only how resolved tokens are emitted, not how `contrastText` is computed. Two caveats keep it a heuristic, not a hard gate: it affects only custom palette colours (built-in defaults ship fixed `contrastText`), and MUI documents the curve as non-linear and warns it can be counterproductive. Held to the volatile-external ≥ 3-source tier per `spec/claude/research-triangulate/` §Author-time assertions: [1] MUI palette doc `mui.com/material-ui/customization/palette/` (threshold semantics, custom-colours-only clause, non-linear warning); [2] MUI CSS theme variables doc `mui.com/material-ui/customization/css-theme-variables/overview/` (`colorSchemes` emit resolved palette as CSS variables); [3] MUI v9 release `mui.com/blog/introducing-mui-v9/` (the `colorSchemes` API). The spec's §Visual MUST is qualified so the WebAIM / manual contrast pass is the gate, not the threshold value alone.
