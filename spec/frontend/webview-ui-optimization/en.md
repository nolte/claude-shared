...
- **MUST** send `require-trusted-types-for 'script'` and a named `trusted-types` policy that pipes any HTML insertion through DOMPurify. This directive guards script/HTML DOM-XSS sinks (`innerHTML`, `script.src`, `document.write`) only; Emotion's `<style>`-tag injection is not a `'script'` sink and runs cleanly under it (it's governed instead by the `style-src` nonce bound to Emotion's `cache.nonce`, above), so an audit **MUST NOT** flag Emotion or MUI v9 styling as a Trusted-Types-for-`script` violation.
...
- **MUST** configure MUI's `palette.contrastThreshold` to `4.5` so palette-derived `contrastText` choices target WCAG 1.4.3 AA (body text 4.5:1); verify custom palettes via WebAIM contrast checker. The setting still applies under MUI v9 `colorSchemes` + `cssVariables`: each scheme's palette runs through the same `augmentColor` / `createPalette` derivation at theme-creation time, and CSS variables change only how resolved values are emitted, not how `contrastText` is computed. It affects only **custom** palette colours (MUI's built-in defaults ship fixed `contrastText` and are unchanged), and it rides a coarse non-linear curve MUI itself warns can be counterproductive, so the WebAIM / manual contrast pass is the actual gate, not the threshold value alone.
...
- **MUST** keep the research audit trail under `spec/frontend/webview-ui-optimization/research/<domain>.md` in lockstep with this spec; every normative rule above is anchored to at least one entry there, which in turn cites ≥ 2 independent authoritative sources. A **volatile external assertion** (an upstream version pin, a third-party library's API signature / default / runtime behaviour, or an external-tool default) that directs the `webview-ui-optimize` skill or `webview-ui-expert` agent toward a write or an audit `fail` in a consumer repository is held to the stricter **Release/dispatch tier** of three independent sources per `spec/claude/research-triangulate/` §Author-time assertions; the two-source floor remains the baseline for durable standards-level claims (W3C / WCAG / ARIA / MDN), where a second authoritative source already clears the triangulation gate's hallucination-risk bar.
...
- ~~Emotion + Trusted Types: whether the Emotion runtime operates cleanly under an enforced `require-trusted-types-for 'script'` CSP with MUI v9 is unverified; verify against the reference stack before treating audit `fail` rows on the Trusted-Types MUST as actionable.~~ **Settled (2026-07-24):** it operates cleanly. `require-trusted-types-for 'script'` guards script/HTML DOM-XSS sinks only (`innerHTML`, `script.src`, `document.write`), which the W3C Trusted Types work has not yet extended to CSS/style sinks; Emotion (still MUI v9's default engine) injects via `<style>` tags governed by the `style-src` nonce, not by a script sink, so it's outside the directive's scope. The MUST at §Security › CSP is qualified accordingly: it binds HTML-insertion sinks, and an audit **MUST NOT** raise a Trusted-Types-for-`script` `fail` against Emotion/MUI styling. Sources (≥ 3, volatile-external tier per `spec/claude/research-triangulate/` §Author-time assertions): MUI CSP guide (`mui.com/material-ui/guides/content-security-policy/`, Emotion→`style-src-elem` nonce); W3C Trusted Types explainer (`github.com/w3c/trusted-types`, `'script'`-sink scope, CSS not yet covered); `content-security-policy.com/require-trusted-types-for/`; MUI v9 keeps Emotion default (`npmjs.com/package/@mui/material` v9). Recorded in `research/security.md` currency addendum.
...
- ~~Whether `palette.contrastThreshold` behaves identically under the MUI v9 `colorSchemes` + `cssVariables` API (and whether v9 tightens dark-mode contrast defaults) is unverified; re-check before enforcing the 4.5 threshold as a hard audit gate.~~ **Settled (2026-07-24):** it still applies under `colorSchemes` + `cssVariables`, because each color scheme's palette is processed by the same `augmentColor` / `createPalette` derivation at theme-creation time (default `contrastThreshold` remains `3`); CSS variables change only how resolved tokens are emitted, not how `contrastText` is computed. Two caveats make the threshold a heuristic, not a guarantee, so the 4.5 value is **not** a standalone hard audit gate: it affects only **custom** palette colours (built-in defaults carry fixed `contrastText`), and MUI documents the curve as non-linear and warns it can be counterproductive, recommending contrast verification. The MUST at §Visual is qualified to make the WebAIM / manual contrast pass the actual gate. Sources (≥ 3, volatile-external tier per `spec/claude/research-triangulate/` §Author-time assertions): MUI palette doc `mui.com/material-ui/customization/palette/` (threshold semantics, custom-colours-only, non-linear warning); MUI CSS theme variables doc `mui.com/material-ui/customization/css-theme-variables/overview/` (`colorSchemes` emit resolved palette as variables); MUI v9 release `mui.com/blog/introducing-mui-v9/` (confirms the `colorSchemes` API). Recorded in `research/accessibility.md` currency addendum.

## Sources

Every normative rule above is anchored to the per-domain research notes under `spec/frontend/webview-ui-optimization/research/`, each entry of which cites at least two independent authoritative sources. Rules explicitly marked as project conventions are deliberate house rules and exempt from the two-source anchor. Volatile external assertions (upstream version pins, third-party API signatures / defaults / runtime behaviour, external-tool defaults) that direct a consuming skill or agent toward a repo-external write or audit `fail` are held to the ≥ 3-source Release/dispatch tier per `spec/claude/research-triangulate/` §Author-time assertions—the two-OQ resolutions above are the worked examples—while durable standards-level claims keep the two-source floor. Post-publication currency corrections (Recharts 3 `accessibilityLayer` default, Vite 8 `output.codeSplitting`, Zod 4 error API, MUI `Dialog` implementation, Trusted-Types and Popover-API browser support, RFC 9110) are recorded in each research file's dated currency addendum (last: 2026-07-24):
