...
- **MUSS [MUST]** ausschließlich den Google-AI-Studio-Endpunkt der Generative Language API ansprechen: `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent`. Vertex-AI-Endpunkte (`*-aiplatform.googleapis.com`) sind verboten. Das `v1beta`-API-Versions-Segment ist zusätzlich zur Modell-ID versionsfest verdrahtet; eine Google-Promotion zu `v1` (oder einem anderen Versions-Segment) verlangt eine Spec-Revision, bevor die Implementierung sie übernehmen darf. Damit liegen Endpunkt-Form-Änderungen (Request-Schema, Response-Form, Kontingent-Semantik) unter demselben Review-Gate wie Modell-ID-Änderungen. SDK und Implementierungssprache sind unbeschränkt — jedes SDK (Python `google-genai`, Node `@google/generative-ai`) oder blankes `curl` ist zulässig, sofern der Aufruf genau diesen festgelegten Endpunkt und dieses Modell trifft und Vertex AI vermeidet; die Wahl gehört zum operationalisierenden Skill/Agent, nicht in diese Spec.
...
- **MUSS [MUST]** HTTP 429 (Rate-Limit / Kontingent erschöpft) als terminalen Fehler des aktuellen Aufrufs behandeln: klare Meldung an die Betreiberin, die das aktuelle Free-Tier-Rate-Limit enthält (damit die Betreiberin versteht, woran sie hängt), und Exit. Automatisches Retry ist verboten, weil jeder Retry zusätzliches Free-Tier-Kontingent verbrennt.
...
- **MUSS [MUST]** den Zielpfad explizit von der Betreiberin verlangen. Kein stilles Default auf das aktuelle Arbeitsverzeichnis. Dies ist eine entschiedene Anforderung: Ein Komfort-Fallback auf `$XDG_PICTURES_DIR/gemini/` ist hier ausdrücklich außerhalb des Geltungsbereichs und wäre, falls er je hinzukäme, ein separates, optionales **SOLLTE [SHOULD]** mit eigenem Akzeptanzkriterium, das trotzdem niemals still schreibt.
- **MUSS [MUST]** das Überschreiben einer existierenden Zieldatei ohne ausdrückliche Bestätigung im selben Aufruf verweigern. Die Form der Bestätigung (CLI-Flag, interaktiver Prompt etc.) ist implementierungsdefiniert.
- **MUSS [MUST]** vor der ersten erfolgreichen Generierung in einer Umgebung einen einmaligen Datenschutzhinweis anzeigen und eine explizite Bestätigung der Betreiberin verlangen, bevor fortgefahren wird:
  ...
  Die Bestätigung MUSS [MUST] unter `$XDG_STATE_HOME/nolte-shared/gemini-image-generation/ack` persistiert werden (Fallback `$HOME/.local/state/nolte-shared/gemini-image-generation/ack`, wenn `XDG_STATE_HOME` nicht gesetzt ist), damit die Betreiberin auf derselben Maschine über Sessions hinweg nicht erneut gefragt wird. Die Bestätigungs-Datei MUSS [MUST] einen SHA-256-Hex-Digest des exakten der Betreiberin angezeigten Hinweistexts speichern; wenn eine künftige Spec-Revision den Hinweistext ändert (etwa nachdem Google die zugrunde liegenden Terms aktualisiert), passt der gespeicherte Digest nicht mehr zum aktuellen Hinweis und die Betreiberin MUSS [MUST] mit dem aktualisierten Text erneut zur Bestätigung aufgefordert werden. Der Digest ist das Versions-Signal der Einwilligung; ohne ihn würde eine alte Bestätigung stillschweigend die MUSS-Regel gegen einen neueren Hinweis erfüllen, den die Betreiberin nie gelesen hat.
- **MUSS [MUST]** neben jedes erzeugte Bild eine Sidecar-Metadatendatei `<image>.meta.json` schreiben, die mindestens enthält:
  - `prompt` — der wortgetreu übermittelte Prompt; das Sidecar spiegelt den Prompt unverändert, deshalb verantwortet die Betreiberin den Prompt-Inhalt gemäß dem Datenschutzhinweis oben — sensibles Material im Prompt landet wortgetreu im Sidecar
...
- **SOLLTE [SHOULD]** das Bildformat aus der Dateiendung des Zielpfads ableiten (`.png`, `.jpg`, `.webp`) und gegen den von der API zurückgegebenen MIME-Typ validieren; Abweichung ist eine Warnung, kein Abbruch. Warnung statt Abbruch erhält die Antwort (das Bild wird trotzdem auf die Platte geschrieben), weil die Alternative — die Antwort bei Abweichung zu verwerfen — das bereits verbrauchte Free-Tier-Kontingent der Betreiberin für einen behebbaren Anwenderfehler verbrennen würde: Die Betreiberin kann die Datei umbenennen oder mit korrigierter Endung erneut aufrufen, ohne den Pro-Bild-Kontingentbeitrag noch einmal zu zahlen.
...
- [ ] Ein erfolgreicher Aufruf mit `n>1` (sofern die Implementierung den optionalen `n`-Parameter unterstützt) schreibt genau `n` Bilddateien und genau `n` Sidecar-Dateien; das `prompt`-Feld ist über jedes in diesem Aufruf erzeugte Sidecar identisch.
- [ ] Die Bestätigungs-Datei am persistierten Pfad enthält ein SHA-256-Hex-Digest-Feld; das Überschreiben des Digests auf einen nicht passenden Wert und erneutes Aufrufen der Implementierung löst den Datenschutzhinweis genau einmal erneut aus (Beweis, dass der Digest bei jedem Aufruf gelesen wird, statt nur die Existenz der Datei zu prüfen).

## Offene Fragen

- Entschieden: Der explizite Zielpfad ist verpflichtend (siehe Anforderungen). Ein künftiger optionaler `$XDG_PICTURES_DIR/gemini/`-Fallback kann als **SOLLTE [SHOULD]** ergänzt werden, falls in der Praxis Reibung gemeldet wird; er verlangte eine eigene MUSS-Regel / ein eigenes Akzeptanzkriterium und darf niemals still schreiben.
