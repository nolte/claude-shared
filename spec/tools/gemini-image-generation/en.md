...
- The `imagen-3.0-*` and `imagen-4.0-*` model families—paid only, out of scope for this spec.
...
- **MUST** require the operator to specify the target file path explicitly. No silent default to the current working directory. This is a settled requirement: a convenience `$XDG_PICTURES_DIR/gemini/` fallback is explicitly out of scope here and, if ever added, would be a separate opt-in **SHOULD** with its own acceptance criterion that still never writes silently.
...
- [ ] The HTTP 429 error message contains a quantitative rate-limit figure (for example "10 requests per minute" or "100 requests per day").
- [ ] A simulated HTTP 401 response causes an auth-failure message linking to the key-management page; exit code is non-zero.
- [ ] An induced network / DNS / filesystem failure produces a human-readable error message and a non-zero exit code; no silent crash, no raw stack-trace dump as the only operator-visible output.
- [ ] First successful invocation in a clean environment prints the data-protection notice and requires explicit acknowledgement; subsequent invocations—including in a fresh shell session on the same machine—don't re-prompt, and the acknowledgement file exists at the path declared in the MUST.
...
- Resolved: the explicit target path is mandatory (see Requirements). A future opt-in `$XDG_PICTURES_DIR/gemini/` fallback may be added as a **SHOULD** if real-world friction is reported; it would require its own MUST/AC and must never write silently.
