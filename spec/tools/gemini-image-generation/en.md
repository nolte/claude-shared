...
- Should the skill/agent expose an optional default target directory (for example `$XDG_PICTURES_DIR/gemini/`) when the operator omits the path, or is the explicit-path requirement absolute? The current MUST is the safer default; revisit if it proves friction in practice.
- Does the implementation language bind us to a specific SDK (Python `google-genai`, Node `@google/generative-ai`, plain `curl`)? The endpoint is fixed; the SDK choice is an implementation detail that belongs to the implementing skill/agent, not this spec.
