...
- The pinned `gemini-2.5-flash-image` ID carries a published shutdown date of 2026-10-02, with `gemini-3.1-flash-image` named as Google's replacement (see §Sources). The pin therefore has to migrate before that date; which successor the `gemini` provider adopts—and whether the billing requirement, the SynthID caveat, and the `v1beta` endpoint carry over unchanged—is settled when the migration is scheduled, not pre-emptively here.

## Sources

The Gemini billing assertions in §Context and §`gemini` are author-time external assertions triangulated per `spec/claude/research-triangulate/` §Author-time assertions (author-time tier: at least three independent sources, ordered Primary-first). They earn the stricter treatment because an earlier revision of this capability was built on the inverse premise. Retrieval date for every source below: 2026-07-24.

- **No Gemini image-generation model on the Gemini Developer API carries a free tier; `gemini-2.5-flash-image` calls without billing fail against a zero-valued free-tier quota metric**: Google, "Gemini Developer API pricing", whose free-tier row reads "Not available" for every image model including `gemini-2.5-flash-image` and `gemini-3-pro-image`, and which notes separately that Google AI Studio's own UI usage is free of charge (Primary), <https://ai.google.dev/gemini-api/docs/pricing>; Home Assistant core, issue #157289 "Google Generative AI can no longer generate free images", reporting `Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests, limit: 0` (Secondary, independent consumer), <https://github.com/home-assistant/core/issues/157289>; googleapis `js-genai`, issue #1322 "429 error for gemini-2.5-flash-image", carrying the same zero-valued quota metric from the official JavaScript SDK (Secondary), <https://github.com/googleapis/js-genai/issues/1322>
- **The pinned model ID is deprecated: `gemini-2.5-flash-image` shuts down on 2026-10-02, and Google names `gemini-3.1-flash-image` as its replacement**: Google, "Gemini API model deprecations", listing the release and shutdown dates plus the named replacement (Primary), <https://ai.google.dev/gemini-api/docs/deprecations>; Google, "Gemini API models", which lists `gemini-3.1-flash-image`, `gemini-3.1-flash-lite-image`, and `gemini-3-pro-image` as the current stable image models (Primary), <https://ai.google.dev/gemini-api/docs/models>; Curlscape, "Google Gemini API Pricing Guide 2026", independently placing the image models outside the free tier and the 3.x line as current (Tertiary), <https://curlscape.com/blog/google-gemini-api-pricing-guide-2026>

Verified 2026-07-24, with two qualifications the requirements above deliberately keep out of the operator-facing message. First, Google no longer publishes a numeric per-model free-tier request table, so the durable evidence is the pricing page's "Not available" row rather than a quota figure. Second, a `limit: 0` body is not proof that a project lacks billing: in February 2026 paid Tier-1 projects were reported hitting the same zero-valued free-tier quota metric on image models (<https://discuss.ai.google.dev/t/bug-paid-tier-1-account-getting-free-tier-requests-limit-0-on-image-generation-models-gemini-2-5-flash-image-gemini-3-pro-image-preview/123906>), so the §"Provider-agnostic (shared layer)" requirement to surface the upstream `error.message` verbatim is what keeps that case diagnosable.
