...
- **MUST** refuse an HTTP redirect from a provider endpoint rather than follow it. Every endpoint in the registry is one documented URL, and the standard library's default redirect handler copies a request's headers to the new location without comparing hosts, so following a redirect would hand the provider credential to whatever host the response names. The refusal **MUST** name the endpoint host the request addressed, the status, and the redirect target's host, so the operator can tell which call was refused and where it was pointed; it **MUST** read that host from the host field of the URL alone, because a `Location` of `http://real-provider.example@attacker.example/` would otherwise read as the provider's own host. The tool **MUST** keep the refusal distinct from an HTTP error the provider itself returned, so a reader can tell that no request reached the second host.
...
- **MUST** bound every file it reads on the operator's behalf, whichever flag names it. The tool **MUST** confirm the path is a regular file **on the open descriptor** rather than on a second path resolution, and **MUST** stop the read at a declared byte cap. Both halves are load-bearing: opening a FIFO without `O_NONBLOCK` blocks forever, and a path checked before it's opened can be replaced in between. Naming something that's not a regular file, or exceeding a cap, **MUST** be a usage error raised before any network call, naming the path and the cap. The tool **MUST NOT** truncate the input instead, which would generate an image from text the operator never wrote. Each cap **MUST** be a module constant carrying its reason, so raising one is a one-line, reviewable decision.
...
- [ ] A local server answering 301, 302, 303, 307 or 308 with a `Location` on a second host draws no request to that second host, on a POST path and on a GET path alike, and the error names the addressed endpoint host, the status, and the target host; a `Location` that embeds the provider's name before an `@` doesn't put it in the host position; a 3xx carrying no `Location` at all, and one naming a scheme outside `http`, also read as a refusal rather than as a provider error; removing the redirect handler makes exactly those assertions fail.
...
- [ ] For every flag that names a file to read, a directory, a FIFO and a file one byte above the cap each exit with the usage code before any network call, and each message names the path and the cap; every cap is a module constant.
