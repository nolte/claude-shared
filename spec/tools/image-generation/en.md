...
- [ ] A local server answering 301, 302, 303, 307 or 308 with a `Location` on a second host draws no request to that second host, on a POST path and on a GET path alike, and the error names the addressed endpoint host, the status, and the target host; a `Location` that embeds the provider's name before an `@` doesn't put it in the host position; removing the redirect handler makes exactly those assertions fail.
