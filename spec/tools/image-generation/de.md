...
- **MUSS [MUST]** einen HTTP-Redirect eines Provider-Endpunkts verweigern statt ihm zu folgen. Jeder Endpunkt der Registry ist eine dokumentierte URL, und der Default-Redirect-Handler der Standardbibliothek kopiert die Header einer Anfrage an den neuen Ort, ohne die Hosts zu vergleichen; einem Redirect zu folgen würde die Provider-Credential also an genau den Host übergeben, den die Antwort nennt. Die Verweigerung **MUSS [MUST]** den vom Request adressierten Endpunkt-Host, den Status und den Host des Redirect-Ziels nennen, damit die Betreiberin erkennt, welcher Aufruf verweigert wurde und wohin er zeigte; sie **MUSS [MUST]** Userinfo aus diesem Ziel verwerfen, weil ein `Location` der Form `http://real-provider.example@attacker.example/` sonst wie der Host des Providers selbst läse. Das Tool **MUSS [MUST]** die Verweigerung von einem HTTP-Fehler unterscheidbar halten, den der Provider selbst zurückgegeben hat, damit erkennbar bleibt, dass keine Anfrage den zweiten Host erreicht hat.
...
- [ ] Ein lokaler Server, der mit 301, 302, 303, 307 oder 308 und einem `Location` auf einem zweiten Host antwortet, zieht keine Anfrage an diesen zweiten Host nach sich, auf einem POST- wie auf einem GET-Pfad, und der Fehler nennt den adressierten Endpunkt-Host, den Status und den Ziel-Host; ein `Location` mit Userinfo bringt den Provider-Namen nicht in die Host-Position; entfernt man den Redirect-Handler, schlagen genau diese Assertions fehl.
