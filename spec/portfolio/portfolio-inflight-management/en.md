...
- **MUST**, on every audit run, collect the four primary data sources from each in-scope Portfolio-Member repository:
  ...
  - **Consumer-measured-finding flags** on the open issues of the repository that holds the portfolio-wide specs: per issue, whether its body names a Portfolio-Member repository other than that one, and whether it carries that repository's spec-finding label. Both are read off data already collected; neither is a new API call.
...
- **MUST NOT** classify an issue authored by a bot account (author login `renovate[bot]`, `dependabot[bot]`, or `github-actions[bot]`) as a stalled-issue finding (the `issue/stalled-issue-no-triage` class). These are permanent, bot-managed dependency-dashboard issues, living issues by design that no specialist can "resolve," so they carry no triage signal and would otherwise be permanent noise in every report. The exclusion is scoped to the issue class only: the `branch/stalled-branch-no-pr` class stays in scope regardless of who pushed the branch, since a stale branch collapses to a one-line per-branch disposition with no specialist and no dashboard semantics. This codifies the gate-2 decision of the 2026-07-24 in-flight audit (recorded against the standing escalation record and this spec's amendment issue).
...
- **MUST** report, for the repository that holds the portfolio-wide specs, the **count of open consumer-measured findings waiting for intake**, the union of the two flags collected above, and **MUST** flag it when it reaches the threshold `spec/project/continuous-improvement/` §"Portfolio gap closure" states. That spec owns the threshold and the action; this one owns the collection and the report line, so the number appears in exactly one place. The flag names the action that spec names and **MUST NOT** carry it out: scheduling an `issue-batch-orchestrate` run is the operator's call per §"Operator authority"
- **MUST** detect `security_relevance: true` only when a concrete advisory marker is present: a CVE / GHSA indicator surfaced by `dependency-audit` or a Renovate / Dependabot `vulnerabilityAlerts` pull request, a supply-chain advisory referenced by the repository's lockfile, or a secret-scanning leaked-credential alert. A pull-request or issue label, a repository topic, or a branch name that merely contains the word `security` (for example a Renovate group label on a version-pinning group such as `security pinned versions`) **MUST NOT** be treated as a `security_relevance` signal on its own; it counts only when backed by one of the concrete markers above. This mirrors the no-inference, no-fuzzy-matching rule the `release_blocking` and `cross_repo_blocking` detection MUSTs already apply, and matches the §Acceptance Criteria security-trigger criterion.
...
- **SHOULD** carry the consumer-measured-finding count in that `## Summary` table as its own row, with the threshold state beside it, so the intake backlog is visible in the same glance as the severity counts
...
- [ ] The agent `portfolio-inflight-collector` exists at `agents/portfolio-inflight-collector.md`, declares only read-only tools (`Bash` for read-only `gh` calls plus the read-only `mcp__github__*` tools that `spec/claude/mcp-tool-preference/` prefers), and has been reviewed against `spec/claude/agent-review/` at least once with the resulting plan closed.
...
- [ ] The Findings-Report states the count of open consumer-measured findings for the spec-holding repository and whether it reached the threshold, and this spec cites `spec/project/continuous-improvement/` §"Portfolio gap closure" for the intake threshold instead of stating it
