...
Readers: maintainers of `nolte/*` repositories, the Claude Code skill that runs the periodic in-flight audit, downstream consumers of the rendered triage report.
...
- **MUST NOT** include private repositories, archived repositories, or forks of upstream projects in the collection. The Portfolio-Member-set resolution from `portfolio-management` already filters these out; the in-flight audit inherits the same filter.
...
- Should the default staleness thresholds (30 / 7 / 14 days) be configured globally in this spec or per-repository in a new `project/inflight.yml`? Per-repo gives flexibility but spreads the source of truth. Current proposal: global defaults in this spec, per-repo overrides via `project/inflight.yml`.
- Should the audit cover archived repositories' open issues and PRs at all? `portfolio-management` excludes archived repos from active scope, but the in-flight question is sharper: an open PR in an archived repo is almost certainly orphaned, and the report might want to surface it once for closure even though the rest of the audit excludes the repo.
- For repositories with a high open-issue count (for example because a maintainer uses issues as a personal task list), does the audit risk producing unmanageable Findings-Reports? A per-repository finding cap with an `Info`-grade "see remaining N similar findings" footer might be needed to keep the report navigable.
- Should "unresolved review comments on closed PRs" enter scope? They're sometimes legitimate follow-ups that need a fresh issue; sometimes they're noise from a hurried merge. The current scope excludes them—should the spec revise that?
- How does this spec interact with `pull-request-merge` and `pull-request-create`? Those skills already act on open PRs; in-flight findings about open PRs frequently recommend dispatching one of them, but the dispatch surface needs to be wired explicitly so the operator doesn't have to translate "stalled PR" into "invoke pull-request-merge" manually each time.
- Should the audit produce a per-Portfolio-Member sub-report (one file per repo) in addition to the aggregated `.audits/portfolio-inflight/` artefact, so individual maintainers can subscribe to "just their own repo's findings"? Or is the single aggregate sufficient?
- What happens to a finding identifier across audit runs when the underlying issue / PR / branch is closed between runs? The identifier should remain stable for trend-tracking but disappear from the active report—how's "closed since last audit" surfaced? The current proposal is an `Info`-grade "closed since previous audit" entry; alternatives include a separate `## Resolved since previous audit` section in the report.
- Should the GitHub Discussions data source also include user-discussions outside the per-repository surface—for example portfolio-wide announcement or feedback channels? The current scope is per-repository only; widening this would require a new source of truth for "portfolio-wide discussion locations."
