...
- **MUSS [MUST]** jeden Eintrag in `tech_stack.overrides[]` als Override-Record strukturieren, der genau zwei Felder trägt: `name` (verweisend auf den `name` eines existierenden globalen Eintrags) und `rationale` (ein nicht-leerer Fließtextsatz). Die implizite Semantik ist `inherit: false`; das Feld wird zur Lesbarkeit explizit benannt und **MUSS [MUST]** vorhanden und auf `false` gesetzt sein:
  ```yaml
  overrides:
    - name: mkdocs
      inherit: false
...
- **MUSS [MUST]** Tech-Stack-Befunde nach der kanonischen Severity-Skala aus `spec/claude/review-plan/` klassifizieren:
  ...
  - `Warning` — ein Override verweist auf einen globalen Eintrag, den es nicht gibt; ein deklarierter Eintrag ist nicht in Repo-Signalen nachweisbar; ein Konsumer rendert Doku-HTML, ohne den globalen `docs`-Eintrag zu erben und ohne expliziten Override.
  - `Suggestion` — ein globaler Eintrag ist `deprecated`, und mindestens ein Konsumer erbt ihn nach einem geschlossenen Sprint immer noch; ein als `other` klassifizierter Eintrag besteht über zwei aufeinanderfolgende Audits fort.
...
- Sollten `experimental`-Einträge im globalen Stack für Konsumer Opt-in sein (nur bei expliziter Listung geerbt) statt standardmäßig geerbt? Der aktuelle Default — Experimental-Einträge erben — entspricht dem aktuellen Lifecycle-Vokabular von `portfolio-management`; Opt-in würde davon abweichen.
