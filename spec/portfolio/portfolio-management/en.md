...
- Should the tolerance window for confirmed duplicates be one closed sprint per repository (current proposal) or a fixed calendar window (for example 30 days)? The sprint-based tolerance scales with repository activity but is harder to enforce mechanically.
- How's "capability description overlap" detected mechanically by the audit, whether by semantic embedding similarity above a threshold, an LLM-based comparison pass, or a lighter-weight keyword-intersection signal? The choice affects audit cost, false-positive rate, and reproducibility.
- Should `project/portfolio.yml` be generated automatically from `project/mission.md` and `project/roadmap.md` when a repository first adopts the spec, or always hand-authored from scratch? Automatic generation gives a fast start but risks drift from the actual capability set.
- How does this spec interact with `mission` for repositories whose mission is itself "be a shared library / plugin used by other Portfolio-Member repositories" (for example `nolte/claude-shared`, `nolte/vale-style`, `nolte/gh-plumbing`)? The capability list in those repositories tends to enumerate what they offer to consumers; the audit shouldn't mistake this for ownership of consumer-side capabilities.
...
- Should `portfolio-audit` also surface capabilities whose `status` is `experimental` for longer than a defined window (for example four closed sprints) as a stagnation `Suggestion`, similar to how `roadmap` items have a detail-level invariant? The intent would be to prevent indefinite experimental status as a hidden form of drift.
