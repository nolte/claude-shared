...
- **MUSS [MUST]** eine Findings-Report-Datei unter `.audits/portfolio/<YYYY-MM-DD>.md` im `claude-shared`-Repository produzieren, die der `review-plan`-Artefakt-Spezifikation entspricht (die diesen datierten Report in §Lifecycle von ihrer No-Timestamp-Regel ausnimmt), einschließlich der vier Pflicht-Abschnitte (`## Scope`, `## Summary`, `## Findings`, `## Processing log`) und des kanonischen Schweregrad-Vokabulars (`Critical` / `Warning` / `Suggestion` / `Info`) gemäß `review-plan` §Schweregrad-Skala.
