...
- For repos hosted under `nolte/*` whose primary purpose is to *be* a portfolio anchor (`gh-plumbing`, `vale-style`, `taskfiles`), should the discovery flow refuse to write a `tech_stack:` block at all (since their tech-stack is, definitionally, the portfolio's tech-stack), or should they still participate by carrying a manifest that's mostly empty? The answer interacts with how `portfolio-audit` distinguishes anchor repos from consumer repos.
