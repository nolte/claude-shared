...
- **MUST** treat a curated, closed allowlist of well-known JavaScript / TypeScript-ecosystem markers declared under `package.json:dependencies` / `devDependencies` as first-class discovery signals, so a web-application repository's framework, language, and CSS toolkit are captured as signal-backed candidates rather than as free-form `acknowledged-missing-signal:` additions. The allowlist maps each marker to a `kind`: `astro`, `next`, `nuxt`, `@sveltejs/kit`, `@remix-run/react`, `gatsby`, `@angular/core`, `vue`, `react`, `solid-js`, `qwik`, and `tailwindcss` → `kind: framework`; `typescript` → `kind: language`; `vite` → `kind: build`. The matching dependency's version range is copied verbatim into the candidate's `version:`. Extending the allowlist is a coordinated edit of this spec plus the capture skill's signal-source map (mirroring the hand-curation discipline of §Global stack curation in `claude-shared`), never a heuristic "emit every dependency" sweep: the discovery flow stays "ask rather than guess."
...
- [ ] The capture skill's signal-source map (`skills/tech-stack-capture/references/signal-source-map.md`) carries exactly one row per marker in the §Discovery sequence JS / TS curated allowlist, and no allowlist row exists in the map without a corresponding spec entry; a mismatch in either direction is drift.
