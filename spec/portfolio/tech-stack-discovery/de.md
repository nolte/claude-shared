...
- **Onboarding-Beitragender beim Lesen der gerenderten Doku.** Oberfläche: Die Doku-Site unter `docs/<lang>/portfolio/`, die den globalen Stack-Abschnitt und die pro-Repository-Abschnitte mit inherited-/repo-specific-/suppressed-Badges zeigt. Erwartung: Eine einzige Seite beantwortet „worauf baut dieses Repo auf", ohne dass man die Lockfiles und Workflow-Dateien des Repositorys greppen muss. Kritikalität: peripher (angeglichen an `AUDIENCES.md` → `external-contributor`, dort `peripheral`). Mapping zu AUDIENCES.md → contributors / maintainers → `"External contributors via pull request"`.
