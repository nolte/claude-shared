...
- **MUSS [MUST]** von `spec/portfolio/tech-stack/` (kanonisch und jede Übersetzung) per Ein-Satz-Verweis referenziert werden, der diese Spec als Eigentümer der Ermittlungs-Methodik, des Audience-Modells und der Benefits-Prosa nennt; das Wiederholen einer der drei Sachen in der Schema-Spec ist verboten.
...
- Sollte der Discovery-Flow für Repos, deren primärer Zweck es ist, ein Portfolio-Anchor zu sein (`gh-plumbing`, `vale-style`, `taskfiles`), das Schreiben eines `tech_stack:`-Blocks gänzlich verweigern (da ihr Tech-Stack per Definition der Portfolio-Tech-Stack ist), oder sollen sie trotzdem teilnehmen, indem sie ein größtenteils leeres Manifest tragen? Die Antwort interagiert damit, wie `portfolio-audit` Anchor-Repos von Konsumer-Repos unterscheidet.
