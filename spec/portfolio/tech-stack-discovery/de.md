...
- Default: §Benefits bei fünf Bullets belassen und keinen sechsten „Release-Notes-Erzeugung"-Bullet hinzufügen; die MAY-Klausel des §Benefits-Dokumentations-Gates ist der Mechanismus, der ihn später ergänzt. Erneut prüfen, wenn alle drei in einem PR gelten: (a) `spec/project/release-notes-audience-analysis/` verlässt `Status: draft` (erreicht `active`/`stable`); (b) ein konkreter Konsumer im Release-Pfad, nämlich `release-notes-curate` (Skill A in `spec/project/release-skill-layer/`) oder `portfolio-audit`, liest tatsächlich einen `project/portfolio.yml`-`tech_stack:`-Eintrag, um eine Release-Notes-Entscheidung zu treiben; und (c) `project/goals.md` trägt eine Outcome-ID, an die der neue Bullet ankern kann. Solange (c) nicht gilt, blockiert das §Benefits-Dokumentations-Gate **MUSS [MUST]** („jeden Bullet an mindestens eine Outcome-ID ankern") den Bullet unabhängig von (a)/(b).
