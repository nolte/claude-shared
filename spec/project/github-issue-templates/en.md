...
GitHub repositories in the nolte portfolio cover very different project types — Claude Code plugins (skills, agents, specs), Python applications (for example kamerplanter), CLIs, libraries, documentation-only repos. A single generic "bug report" / "feature request" template gives triagers and contributors too little context: a Claude-plugin bug needs the plugin version and the affected skill name; a kamerplanter bug needs the camera model, the firmware version, and the failing planting step. Without project-specific prompts, issues land underspecified and triage cost rises.

The `project-structure` spec leaves this gap explicitly open: community-health files (issue templates, CODEOWNERS) are not yet prescribed. This spec closes the issue-template half of that gap and defines a methodology — not a fixed template — that a downstream skill can run against any repo to generate the right templates for that repo's project type.
...
- Standardise on GitHub Issue Forms (YAML) so triage data is structured, validated, and queryable, not free-form prose.
...
- Stay aligned with the `audience-identification` spec — issue templates are a documentation artefact aimed at identified audiences (reporters, triagers, maintainers).

## Non-Goals

- Pull-request templates. Those are governed by `pull-request-workflow`.
- Authoring concrete templates for every imaginable project type. This spec defines the *method*; templates themselves are scaffolded per repo.
...
- **MUST** keep all template content in English regardless of the repo's documentation language, because the GitHub issue UI does not translate.
...
- `question.yml` — only when GitHub Discussions are not enabled; otherwise route to Discussions via `config.yml`.
...
2. **Resolve audience profile.** If an audience artefact already exists per `audience-identification`, read it; otherwise dispatch the `audience-identify` skill against the repo first. Issue templates are written for the *reporter* audience and *triager* audience; both must be identified.
3. **Derive triage questions.** For each baseline template plus any project-type-specific extras, list the questions a triager will ask within five minutes of receiving the issue. Those questions become required fields. Examples:
...
4. **Encode questions as Issue Forms components.** Use the minimum component complexity that fits:
   ...
   - Long free text → `textarea` (with `render: shell` for logs / tracebacks).
...
5. **Set labels and assignees.** Pre-fill `labels:` from the project's label taxonomy (often a `.github/labels.yml` or Probot `settings.yml`). Pre-fill `assignees:` only when the repo has a stable triage owner.
...
- **MAY** prefill the issue title via the form's top-level `title:` key when the project type has a strong title convention (for example `[bug] <area>: <summary>` or `[feat] `).

#### Bug reports — strict by design

- **MUST** mark every field that triagers need before they can act as `validations: required: true`.
- **SHOULD NOT** include a free-form "additional context" field as the only structured input — at least one structured field per template must capture triage-critical data.
...
#### Feature requests — deliberately permissive

A feature request often arrives as a half-formed thought that needs room to be discussed and refined before it converges on a shape. Tight required-field gates discourage submission and force premature commitment to one solution; the spec biases feature templates toward openness:
...
- write `.github/ISSUE_TEMPLATE/*.yml` and `.github/ISSUE_TEMPLATE/config.yml` together — never leave the directory in a half-configured state.
...
- [ ] `feature_request.yml` does not require any closed-taxonomy field (severity, priority, target release, milestone, owner).
