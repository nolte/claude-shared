...
5. **Labels und Assignees setzen.** `labels:` aus der Label-Taxonomie des Projekts vorbelegen (häufig `.github/labels.yml` oder Probot `settings.yml`). `assignees:` nur dann vorbelegen, wenn das Repo einen stabilen Triage-Owner hat.
   Ein Template, dessen Issues von einer Portfolio-Regel gezählt werden, **MUSS [MUST]** das Label vorfüllen, das diese Regel liest, statt es der einreichenden Person zu überlassen. Ein Label, an das sich jemand erinnern muss, führt genau den menschlichen Schritt wieder ein, den die zählende Regel beseitigen soll.
