...
- PII controls **MUST** be active at the SDK boundary: the SDK's default-PII behaviour stays off (no raw cookies, auth headers, or request bodies with personal data), and a scrubbing hook (the SDK's before-send filter or equivalent) removes or masks identified fields before the event leaves the process. The evidence for default-PII off depends on the SDK version and is judged by its effect, not by one option name: either a legacy boolean flag set explicitly off where the pinned SDK still honours it, or a structured per-category data-collection option with every personal-data category set explicitly off. A structured option that names only some categories counts only for those, because every omitted category takes the SDK's own default, and where both forms are set the structured one decides (for example Sentry JavaScript 11 replaced `sendDefaultPii` with `dataCollection`, and the Python SDK's `data_collection` supersedes `send_default_pii`). This is the error-event instance of the `[locked]` emission-boundary redaction pillar in `spec/project/monitoring-observability/`; what counts as personal data is defined by `spec/project/gdpr-audit-process/` and isn't restated here
