...
- PII controls **MUST** be active at the SDK boundary: the SDK's default-PII behaviour stays off (no raw cookies, auth headers, or request bodies with personal data), and a scrubbing hook (the SDK's before-send filter or equivalent) removes or masks identified fields before the event leaves the process. The evidence for default-PII off depends on the SDK version and is judged by its effect, not by one option name: every personal-data category the SDK collects resolves to off, whether through a legacy boolean flag or a structured per-category data-collection option. A legacy flag set explicitly off counts only for the categories it actually switches off, because current SDKs still collect some categories with it (the Python SDK still sends request bodies), and a structured option that names only some categories counts only for those, because every omitted category takes the SDK's own default. Where both forms are set, the one the pinned SDK honours decides (for example Sentry JavaScript 11 replaced `sendDefaultPii` with `dataCollection`, and the Python SDK's `data_collection` supersedes `send_default_pii`). This is the error-event instance of the `[locked]` emission-boundary redaction pillar in `spec/project/monitoring-observability/`; what counts as personal data is defined by `spec/project/gdpr-audit-process/` and isn't restated here
...
All five original open questions were settled by the operator on 2026-07-25, and the audit capability's shape—the one remainder they left open—was settled on 2026-08-01. Nothing in this spec is open; the decision records stay here for traceability.
...
- **`error-tracking-audit` capability (planned 2026-07-25, shape settled 2026-08-01).** Tracked as `nolte/claude-shared#516` and authored once the kamerplanter dogfooding had produced a real adopter to calibrate against (nolte/kamerplanter#777, closed 2026-07-29). The capability ships as a pair in the `nolte-engineering` plugin, mirroring its `observability-audit` sibling: the read-only `error-tracking-audit-scanner` agent detects, and the `error-tracking-audit` skill owns severity, the report, and the remediation handover. Its check set is the statically decidable slice of the requirements above—(1) a Sentry-protocol-compatible SDK declared in the dependency manifest **and** initialised at process entry; (2) the DSN sourced from environment or deployment configuration rather than hardcoded, degrading to a no-op when absent; (3) `environment` and `release` set from deployment metadata; (4) default-PII off plus a before-send scrubbing hook; (5) a recorded, deliberate sampling decision; (6) findings reported on the severity scale of `spec/claude/review-plan/`. Runtime behaviour—events actually arriving, alert rules firing, triage-service-level adherence, and server-side retention—stays advisory and verify-at-runtime, and never hard-fails an audit, because it isn't statically decidable. End-to-end validation against a live instance waits on nolte/k8s-home-lab#833, which gates that validation alone and never the static checks.
