...
- **Python project** — `pyproject.toml`, `requirements.txt`, `requirements-dev.txt` → `flowchart` of dependencies (direct vs. dev), `classDiagram` for module hierarchies derived from the source tree
...
- **MUST NOT** apply per-node or per-edge inline styling (`style`, `linkStyle`, or `classDef` with hardcoded colors); rely on Material's Mermaid theme bridge so light- and dark-mode rendering stays correct
...
- [ ] `mkdocs.yml` keeps `theme.name: material` and does not add `mkdocs-mermaid2-plugin` (or any other Mermaid-only plugin) to its `plugins:` list
...
- [ ] No Mermaid block in the documentation tree contains a `style`, `linkStyle`, or `classDef` directive with a hardcoded color
