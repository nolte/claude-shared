...
- The agent emits only the human-readable test-case documents; their per-case `requirement_id` frontmatter plus tags and the per-document coverage summary are the machine-parseable traceability surface—the reverse `case → requirement` edge. The downstream reader this contract used to wait for now exists: `spec/project/test-cycle-case-determination/` declares the single project-level requirement → TC-ID traceability matrix, generated from and checked against exactly this frontmatter. The division of labour is therefore settled—the central matrix is the case-determination phase's artefact, and this agent still **MUST NOT** emit a separate traceability index of its own, so no second derivation-side artefact widens the determinism and regeneration surface alongside the declared one.
