...
- Der Agent emittiert nur die menschenlesbaren Testfall-Dokumente; ihr `requirement_id`-Frontmatter pro Fall plus Tags und die Abdeckungs-Zusammenfassung pro Dokument sind die maschinell parsebare Rückverfolgbarkeits-Oberfläche — die Rückwärts-Kante `Fall → Anforderung`. Der nachgelagerte Leser, auf den dieser Vertrag wartete, existiert inzwischen: `spec/project/test-cycle-case-determination/` deklariert die einzelne projektweite Anforderung → TC-ID-Rückverfolgbarkeits-Matrix, die aus genau diesem Frontmatter generiert und dagegen geprüft wird. Die Arbeitsteilung ist damit entschieden — die zentrale Matrix ist das Artefakt der Fall-Ermittlungs-Phase, und dieser Agent **DARF WEITERHIN NICHT [MUST NOT]** einen eigenen separaten Rückverfolgbarkeits-Index emittieren, damit kein zweites ableitungsseitiges Artefakt die Determinismus- und Regenerations-Oberfläche neben dem deklarierten verbreitert.
