...
- **DE-seitiges Lesbarkeitsziel.** **Aufgelöst**: der DE-Body trägt jetzt das sprachübergreifende **LIX**-Ziel (a-4a) gemäß [`spec/project/readability-lix/`](../readability-lix/de.md), was die frühere `a-4`-DE-Ausnahme aufhebt. Offen bleibt die *Kalibrierung*, nicht die *Existenz*: erneut prüfen, wenn ein einzelner Konsument 5 veröffentlichte DE-Posts erreicht, um die LIX-Verteilung über sie zu messen und den DE-Blog-Post-Korridor sowie den Offset Δ = 5 gegen die gemessenen Daten zu rekalibrieren (verfolgt in `readability-lix` §Offene Fragen). Ein ergänzendes DE-Wiener-Sachtextformel-Band **DARF [MAY]** später ergänzt werden, falls sich zeigt, dass LIX ein DE-spezifisches Lesbarkeitsproblem verfehlt. Heute hält das Korpus 2 DE-Posts (`src/content/posts/de/`: `welcome.md`, `claude-shared-baseline.md`).
