...
- **DARF NICHT [MUST NOT]** den Post in einer entschuldigenden Rahmung um das KI-Entworfen-Sein wickeln („Das wurde mit Claude geschrieben, bitte sei nachsichtig …"). Direkte Endleser-Audiences erwarten, dass KI-entworfener Inhalt denselben Standard wie handgeschriebener Inhalt erfüllt. Die Disclosure ist strukturell—heute über die AI-Disclosure-Flag im Frontmatter; Konsumenten, die ein sichtbares Per-Post-AI-Disclosure-Badge versenden (verlinkt auf eine About-Page-Erklärung), machen dieses Badge zum Teil der Disclosure-Struktur. Die MUSS ist heute auf der Frontmatter-Flag in Kraft; die Badge-Hälfte wird per Roadmap-Signal des Konsumenten verifizierbar.
...
- **MUSS [MUST]** die gesamte Liste bei jedem Claude-Modellfamilien-Übergang (z. B. Claude 4.x → 5.x) re-reviewen; das Re-Review wird als Continuous-Improvement-Eintrag oder als dedizierter Commit festgehalten, nicht still übergangen.
...
- [ ] **a-17** Ein Konsument, der zusätzlich zur Frontmatter-Flag ein sichtbares AI-Disclosure-Badge ausliefert, trägt das Badge auf der gerenderten Seite; Konsumenten, die das Badge noch nicht ausliefern, sind von diesem Kriterium ausgenommen, wobei die `aiGenerated: true`-Frontmatter-Flag, abgedeckt durch `a-10`, der heute in Kraft befindliche Teil von §AI-Disclosure-Tonalität ist.
...
- **Quantitatives Lesbarkeits-Gate.** §Lesbarkeits-Schwellenwerte fixiert Flesch–Kincaid 7–10 als Ziel. Ein Lint-Hook, der FK-Grade pro Post misst, ist noch nicht gebaut. Bis er es ist, ist das Gate Reviewer-Urteil; ein zukünftiges Feature sollte `textstat` (oder Äquivalent) in die Build-Pipeline des Konsumenten verdrahten. Die Schwelle selbst kann eine Rekalibrierung erfordern, sobald echte Messungen über die ersten 10 Posts in einem gegebenen Konsumenten vorliegen.
- **DE-seitiges Lesbarkeitsziel.** Flesch–Kincaid ist für Englisch kalibriert. Das deutsche Äquivalent ist die Wiener Sachtextformel oder der Amstad-justierte Flesch. Diese Spec fixiert mangels Daten keine DE-seitige Schwelle; der Reviewer stützt sich für DE-Posts allein auf §Person, Voice und Ton + §Zweisprachige Typografie. Eine Folge-Revision **SOLLTE [SHOULD]** ein DE-spezifisches Lesbarkeitsziel hinzufügen, sobald mindestens 5 DE-Posts in einem gegebenen Konsumenten zur Kalibrierung vorliegen.
- **Pflege der Verbotswörter-Liste.** Die geschlossene Liste unter §Verbotswörter ist forschungsabgeleitet zum Spec-`Status: draft`-Datum. Neue AI-Tell-Tics werden auftauchen, sobald Claude-Versionen und Prompting-Muster sich verschieben. Die Liste **MUSS [MUST]** bei jedem Modellfamilien-Übergang (z. B. Claude 4.x → 5.x) re-reviewt werden; der Mechanismus für dieses Re-Review ist hier nicht definiert und sollte von [`spec/project/continuous-improvement/`](../continuous-improvement/de.md) oder einem dedizierten Triage-Eintrag aufgegriffen werden.
- **Override-Verfahrens-Formalismus.** §Override-Verfahren erlaubt heute die in-Prosa-Begründung eines Verbotsworts. Ein strengerer Mechanismus—Frontmatter-Feld `style_overrides: ["seamless"]` mit Begründung—würde Overrides im Diff reviewfähig statt in Prosa vergraben machen. Aufgeschoben, bis der dritte dokumentierte Override-Fall zeigt, dass der Prosa-Mechanismus nicht skaliert.
- **AI-Disclosure-Badge-Verdrahtung.** §AI-Disclosure-Tonalität MUSS heute die Frontmatter-Flag und lässt die Badge-Hälfte konsumentenseitig konditional. Eine zukünftige Revision sollte einen portablen Badge-Vertrag benennen (Text-Inhalt, Position, Link-Ziel), sodass Konsumenten, die das Badge verdrahten, derselben Konvention folgen. Ausgelöst vom ersten Konsumenten, der das Badge ausliefert.
