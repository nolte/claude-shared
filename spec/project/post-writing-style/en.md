...
- **DE-side readability target.** **Resolved**: the DE body now carries the cross-language **LIX** target (a-4a) per [`spec/project/readability-lix/`](../readability-lix/en.md), which lifts the former `a-4` DE exemption. What remains open is *calibration*, not *existence*: revisit when a single consumer reaches 5 published DE posts to measure the LIX distribution across them and recalibrate the DE blog-post corridor and the offset Δ = 5 against the measured data (tracked in `readability-lix` §Open Questions). A supplementary DE Wiener-Sachtextformel band MAY be added later if LIX proves to miss a DE-specific readability problem. Today the corpus holds 2 DE posts (`src/content/posts/de/`: `welcome.md`, `claude-shared-baseline.md`).
