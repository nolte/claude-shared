...
- **MUST** address the reader in a single, consistent informal register: in EN posts the second-person `you` throughout; in DE posts the informal `du` (and its forms `dich` / `dir` / `dein`) throughout. **MUST NOT** switch to the formal German `Sie`, and **MUST NOT** fall back to the impersonal `man` (EN `one`) in a sentence that addresses the reader—rewrite it to `du` / `you` instead. This rule governs only how the *reader* is addressed; the first-person author voice (`I` / `ich`) from the first bullet of this section is unaffected.
...
- **MUST NOT** produce a **calque**: a DE sentence whose structure mirrors the EN word-for-word, even when every individual word is correct (for example *„Was die Kosten kaufen, ist Eigentum."* for *„What the costs buy is ownership."*). Re-express the thought as a German author would compose it from scratch; the detection-side counterpart is the D6 dimension in [`spec/project/lektorat/`](../lektorat/en.md).
- **MUST** give a borrowed term the **gender and inflection of the host language** („die Bridge", „der Hub", „das Repository"), never the gender carried over from the source language.
...
- [ ] **a-17a** The reader is addressed in one consistent informal register throughout each body — EN second-person `you`, DE informal `du` — with no formal German `Sie` and no impersonal `man` / `one` in any sentence that addresses the reader (§Person, voice, and tone). The first-person author voice (`I` / `ich`) is exempt.
- [ ] **a-17b** The DE body contains no calque (a sentence mirroring the EN word-for-word) and no loanword-gender error; EN idioms are rendered idiomatically, not literally (§Bilingual typography). The detection-side counterpart is the D6 dimension in [`spec/project/lektorat/`](../lektorat/en.md).
...
_All previously deferred open questions were settled on 2026-06-06: each provisional default is now the standing rule. The per-item decisions and rationale are preserved in git history (decision log, 2026-06-06). The former DE-side-readability-target question is additionally resolved on the implementation side by the cross-language LIX target (`a-4a`) per [`spec/project/readability-lix/`](../readability-lix/en.md)._
