...
- **SHOULD** handle an uncommitted `.audits/` diff in a worktree deliberately before `git worktree remove <path>` by committing it onto the worktree's branch, transferring it to another worktree per §Uncommitted changes between worktrees, or explicitly acknowledging it as discarded, so the audit trail doesn't silently disappear with the worktree directory; the `rm -rf` prohibition from §Lifecycle: Retire holds independently
...
- [ ] No `.audits/skills-agents-sweep/*.md` file with `status: open` was introduced by a commit unreachable from `develop` (verifiable with `git log --all -- .audits/skills-agents-sweep/` plus a `git merge-base --is-ancestor` check against `develop`), so the "only one open sweep" constraint from `spec/claude/skills-agents-sweep/` remains observable on the primary checkout even when feature work happens in parallel worktrees
