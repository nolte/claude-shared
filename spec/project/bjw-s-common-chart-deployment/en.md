...
- Each container **MUST** declare `resources` requests (and limits where the workload warrants bounding); an unbounded container with no resource requests **MUST NOT** be emitted
- Each container (or the pod default) **SHOULD** set a hardened `securityContext` (`runAsNonRoot`, a non-zero `runAsUser`/`runAsGroup`, `readOnlyRootFilesystem` where the app tolerates it, and dropped capabilities), preferring a single `defaultPodOptions.securityContext` over per-container repetition
