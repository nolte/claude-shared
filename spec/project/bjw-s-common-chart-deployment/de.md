...
- Jeder Container **MUSS** `resources`-Requests deklarieren (und Limits, wo der Workload eine Begrenzung rechtfertigt); ein ungebundener Container ohne Resource-Requests **DARF NICHT** ausgegeben werden
- Jeder Container (oder der Pod-Standard) **SOLLTE** einen gehärteten `securityContext` setzen — `runAsNonRoot`, eine `runAsUser`/`runAsGroup` ungleich null, `readOnlyRootFilesystem`, wo die App es verträgt, und weggelassene Capabilities — wobei ein einzelnes `defaultPodOptions.securityContext` einer Wiederholung pro Container vorzuziehen ist
