...
## Nicht-Ziele
- Die Prüfung externer Links (alles, was `http://` oder `https://` ist): die Tradeoffs um Rate Limits, Flakiness, Geoblocking und False Positives gehören in ein anderes Werkzeug — jetzt `spec/project/link-validation/` und sein `link-rot-scanner`-Agent
- Die deterministische Erkennung von Interner-Link-Rot und Cross-Tree-Referenz-Rot: diese beiden Kategorien werden von `scripts/check_links.py` aus `spec/project/link-validation/` erkannt; dieses Audit delegiert die Erkennung dorthin und behält die Kategorien in seinem Report für ein einheitliches Drift-Bild (siehe §Abgrenzung)
...
- **Interner-Link-Rot**: ein relativer Markdown-Link, dessen Ziel auf der Disk nicht existiert. Anker werden strikt aufgelöst — die Datei muss existieren; das Anker-Ziel innerhalb der Datei ist eine `SOLLTE`-Prüfung, keine `MUSS`-Prüfung, weil Anker-Erkennung über Themes hinweg fragil ist. Diese `SOLLTE`-Prüfung ist ein aufgeschobener Härtungsschritt und keine dauerhafte Obergrenze: da `spec/project/mkdocs-structure/` nun eine einzige Theme-Engine (`mkdocs-material`) vorschreibt, ist die Theme-übergreifende Variabilität portfolioweit verschwunden und das Slugifizierungs-Ziel ein einziger bekannter Algorithmus, sodass die Prüfung auf `MUSS` anhebbar ist, sobald ein Material-Slugify-basierter Detektor eine niedrige False-Positive-Rate auf realen Docs-Bäumen misst (die Anker-Behandlung in §Schweregrad-Klassifikation bleibt unverändert). **Die Erkennung dieser Kategorie ist an `scripts/check_links.py` aus `spec/project/link-validation/` delegiert (das genau diesen Material-Slugify-Anker-Detektor implementiert); der `docs-freshness-checker`-Agent ruft dessen `--offline`-Schicht auf, statt Link-Rot von Hand neu zu erkennen.**
- **Cross-Tree-Referenz-Rot**: ein Link aus den Docs nach `spec/`, `src/`, `scripts/`, `docker/`, `helm/`, `tests/`, `tools/`, dessen Zielpfad im Arbeitsbaum nicht mehr existiert. **Die Erkennung ist an `scripts/check_links.py` aus `spec/project/link-validation/` delegiert (Cross-Tree-Klasse), wie Interner-Link-Rot oben.**
...
- **i18n-Basis-URL-Drift**: in einem mehrsprachigen Repository (das `mkdocs-static-i18n`-Plugin deklariert mehr als eine Sprache) fehlt in `mkdocs.yml` ein `site_url`, oder `site_url` passt nicht zur veröffentlichten Basis-URL des Repositories, abgeleitet aus `repo_url`/dem `origin`-Remote. Ohne `site_url` baut das Plugin die absoluten Sprachumschalter- und `<link rel="alternate" hreflang>`-Tags vom Site-Root `/` aus, sodass ein GitHub-Pages-Projekt-Subpfad-Deployment falsche Alternativsprachen-Links ausliefert (`https://<owner>.github.io/de/` statt `https://<owner>.github.io/<repo>/de/`). Die normative Anforderung steht in `spec/project/mkdocs-structure/` §i18n und Parität; dieses Audit meldet die Drift read-only mit Severity warning, und `mkdocs-structure-apply` besitzt den Fix. Ein vorhandenes `site_url` ohne abschließenden Schrägstrich wird nicht geflaggt (MkDocs normalisiert es).
...
Zusätzliche Kategorien **DÜRFEN** von einem Repository hinzugefügt werden, wenn seine Dokumentation sie braucht (zum Beispiel ein API-Referenz-vs-Code-Check in einem Repository, das eine OpenAPI-Spec ausliefert; `spec/project/api-documentation/` verankert seine Drift-Regel in dieser Kategorie), aber die portfolioweiten Kategorien oben sind die Untergrenze.
...
- **MUSS** die deterministische Erkennung von `Interner-Link-Rot` und `Cross-Tree-Referenz-Rot` an `spec/project/link-validation/` und sein `scripts/check_links.py` delegieren: diese Spec ist der einzige Eigentümer der Link-Auflösung (offline für intern/cross-tree, online für extern). Dieses Audit behält die beiden Kategorien in seiner Report-Form für ein einheitliches Drift-Bild, aber der `docs-freshness-checker`-Agent bezieht die Befunde durch Ausführen der `--offline`-Schicht des Checkers, nicht durch Neu-Implementierung der Link-Auflösung. Externer-Link-Rot wird end-to-end von `spec/project/link-validation/` (dem `link-rot-scanner`-Agent) besessen, was dieses Audit nicht dupliziert
