...
## Non-Goals
- Checking external links (anything `http://` or `https://`): the tradeoffs around rate limits, flakiness, geoblocking, and false positives belong in a different tool—now `spec/project/link-validation/` and its `link-rot-scanner` agent
- Deterministically detecting internal-link rot and cross-tree reference rot: those two categories are detected by `spec/project/link-validation/`'s `scripts/check_links.py`; this audit delegates the detection there and keeps the categories in its report for a unified drift picture (see §Delimitation)
...
- **Internal-link rot**: a relative markdown link whose target doesn't exist on disk. Anchors are resolved strictly—the file must exist; the anchor target inside the file is a `MUST` check, aligned with `spec/project/link-validation/` §Severity. Now that `spec/project/mkdocs-structure/` mandates a single theme engine (`mkdocs-material`), the cross-theme variability is gone portfolio-wide and the slugification target is one known algorithm, and `link-validation`'s `scripts/check_links.py` implements exactly this Material-slugify anchor detector—the previously deferred promotion condition is met, so the anchor check is a `MUST` here, no longer the earlier deferred `SHOULD` (§Severity's anchor handling is unchanged). **Detection of this category is delegated to `spec/project/link-validation/`'s `scripts/check_links.py` (which implements exactly this Material-slugify anchor detector); the `docs-freshness-checker` agent invokes its `--offline` slice rather than re-detecting link rot by hand.**
- **Cross-tree reference rot**: a link from the docs into `spec/`, `src/`, `scripts/`, `docker/`, `helm/`, `tests/`, `tools/` whose target path no longer exists in the working tree. **Detection is delegated to `spec/project/link-validation/`'s `scripts/check_links.py` (cross-tree class), like internal-link rot above.**
...
- **i18n base-URL drift**: in a multilingual repository (the `mkdocs-static-i18n` plugin declares more than one language), `mkdocs.yml` lacks a `site_url`, or `site_url` doesn't match the repository's published base URL derived from `repo_url`/the `origin` remote. Without `site_url` the plugin builds the absolute language-switcher and `<link rel="alternate" hreflang>` tags off the site root `/`, so a GitHub-Pages project-subpath deployment ships wrong alternate-language links (`https://<owner>.github.io/de/` instead of `https://<owner>.github.io/<repo>/de/`). The normative requirement lives in `spec/project/mkdocs-structure/` §i18n and parity; this audit reports the drift read-only at warning severity, and `mkdocs-structure-apply` owns the fix. A present-but-slashless `site_url` isn't flagged (MkDocs normalises it).
- **Content-staleness delta**: in a multilingual repository, counterpart files whose last-commit timestamps diverge beyond a threshold (default 30 days) or whose sizes diverge beyond 2×; these are spot-checked on the N most recently modified files per tree (default N=5) rather than checked exhaustively. The N=5 cap is a deliberate cheap floor; it's revisitable when a large docs tree (more than 50 files) demonstrates a missed divergence older than 90 days.
...
- **MUST** delegate deterministic detection of `Internal-link rot` and `Cross-tree reference rot` to `spec/project/link-validation/` and its `scripts/check_links.py`: that spec is the single owner of link resolution (offline for internal/cross-tree, online for external). This audit keeps the two categories in its report shape for a unified drift picture, but the `docs-freshness-checker` agent obtains the findings by running the checker's `--offline` slice, not by re-implementing link resolution. External-link rot is owned end-to-end by `spec/project/link-validation/` (the `link-rot-scanner` agent), which this audit doesn't duplicate
