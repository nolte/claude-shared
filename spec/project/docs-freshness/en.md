...
- **Internal-link rot**: a relative markdown link whose target doesn't exist on disk. Anchors are resolved strictly—the file must exist; the anchor target inside the file is a `SHOULD` check, not a `MUST`, because anchor detection is fragile across themes. This `SHOULD` is a deferred hardening step rather than a permanent ceiling: now that `spec/project/mkdocs-structure/` mandates a single theme engine (`mkdocs-material`), the cross-theme variability is gone portfolio-wide and the slugification target is one known algorithm, so the check is promotable to `MUST` once a Material-slugify-based detector measures a low false-positive rate on real docs trees (§Severity's anchor handling is unchanged). **Detection of this category is delegated to `spec/project/link-validation/`'s `scripts/check_links.py` (which implements exactly this Material-slugify anchor detector); the `docs-freshness-checker` agent invokes its `--offline` slice rather than re-detecting link rot by hand.**
