...
- Die in §Geltungsbereich und §Dimension 3 ergänzte Sprachdatei-Regel ist **teilweise** erfüllt. Ein erster Bereinigungslauf hat die exakt belegbare Klasse geschlossen: 43 Referenzen in 30 übersetzten Dateien, die einen kanonischen Sektionsnamen für ein Ziel zitierten, dessen übersetzte Überschrift überprüfbar war — über `review-plan`, `research-triangulate`, `dependency-audit`, `lektorat` und `mkdocs-structure`. Offen bleibt bewusst der Rest, der Einzelfall-Beurteilung braucht. Vier aufeinanderfolgende Regex-Durchgänge über den Korpus lieferten stark abweichende Kandidatenzahlen, weil jeder einen Mangel behob und den nächsten offenlegte: Folgeprosa, die in den Sektionsnamen gezogen wurde, Buchstaben-Überschriften, ein `§`, das sich in einer Zeile mit mehreren Referenzen an den falschen Pfad band, sowie spec-interne Referenzen, die an den nächstgelegenen Pfad gebunden wurden. Ein derart instabiler Detektor darf keine Umschreibung autorisieren; die verbleibenden Klassen brauchen daher einen menschlichen Durchgang: bewusste Verkürzungen, die korrekt lesen (`spec-drift-audit` zitiert als §"Delimitation" für `Delimitation from other specs and skills`), Kompositformen in übersetzter Prosa sowie Referenzen, deren Zielüberschrift keine saubere Entsprechung hat. Ein unterwegs gefundener Fall löst in **keiner** Sprache auf: `spec-drift-audit` zitiert `review-plan` §"Required sections", eine Überschrift, die in keiner Sprache jener Spec existiert — ein schlichtes `Critical` unter der bestehenden Regel, das demselben Durchgang überlassen bleibt.
