...
- **MUSS** verifizieren, dass ein Akzeptanzkriterium, das auf eine Anforderung zurückführt, keine *andere* Regel aussagt als diese Anforderung — ein AK, das ihr widerspricht, sie erweitert oder eine Ausnahme trägt, die die Anforderung nicht kennt, ist `Warning`, der Befund der **veralteten Wiederholung**. Ein **engeres** AK ist ausdrücklich kein Befund: Ein AK ist konstruktionsgemäß eine konkrete, testbare Instanz einer allgemeinen Regel, Verengung ist also genau seine Aufgabe, und die Abdeckungsgrenze darüber lautet bewusst ein testbares AK je Anforderung, nicht je MUSS. Ein AK, das auf ein **Ziel** zurückführt, liegt außerhalb dieser Prüfung, denn ein Ziel trägt keine RFC-2119-Regel, gegen die verglichen werden könnte. Dieser Befund unterscheidet sich vom verwaisten Fall darüber: Das AK führt sehr wohl auf eine Anforderung zurück, es sagt nur etwas aus, was die Anforderung nicht sagt. Genau das entsteht, wenn jemand eine Anforderung ändert, ohne durchzugehen, was sie wiederholt, und es zählt, weil ein Konsument, der Akzeptanzkriterien als Schranke behandelt, die aufgegebene Fassung umsetzt. Anders als die Widerspruchsprüfungen aus §Dimension 1 braucht er auf der AK-Seite kein RFC-2119-Verb, denn das Verb sitzt auf der Anforderungsseite des verglichenen Paars
...
- **MUSS** den exakt entscheidbaren Teil dieser Prüfung mechanisch durchsetzen, wo immer ein Repository einen deterministischen Resolver mitbringt, sodass er dessen Gate scheitern lässt, statt auf das nächste Audit zu warten. Dieser Teil umfasst drei Fälle: eine Referenz, die in einer anderen Sprachdatei ihres Ziels auflöst, aber nicht in ihrer eigenen, einen Skill- oder Agent-Pfad in Backticks, der nach dem Umzug des Artefakts unter `plugins/<plugin>/` noch auf die Repository-Wurzel zeigt, und eine Akzeptanzkriterien-Überschrift, die nicht die Vertragsform ihrer Sprache nutzt. Der Resolver dieses Repositorys ist `scripts/check_section_refs.py`, der als pre-commit-Hook und damit in der `lint`-Lane läuft. Eine Referenz, die in **keiner** Sprache auflöst, bleibt `Critical` dieses Audits, und der Resolver meldet sie nur, weil eine korrekt lesende Verkürzung ohne Anführungszeichen und eine Geister-Referenz für einen Resolver gleich aussehen
...
- **MUSS** Reife-Befunde gemäß den in §Dimension 1, 2 und 3 dokumentierten Mustern auf die Skala abbilden:
  ...
  - **Warning**: MUST-vs.-SHOULD-Widerspruch; nicht identifizierbare Audience; abgeleitete Audience, deren Bedürfnisse nicht adressiert werden; Ziel ohne passende Anforderung; Anforderung ohne testbares Akzeptanzkriterium; verwaistes Akzeptanzkriterium; veraltete Wiederholung (ein Akzeptanzkriterium, das etwas anderes aussagt als die Anforderung, auf die es zurückführt)
...
- [ ] In einem Repository mit deterministischem Resolver lassen eine Sektions-Referenz, die nur in einer anderen Sprache auflöst, ein Skill- oder Agent-Pfad, der nach dem Umzug unter `plugins/` noch auf die Wurzel zeigt, und eine Akzeptanzkriterien-Überschrift außerhalb der Vertragsform jeweils das Gate scheitern; in diesem Repository endet `scripts/check_section_refs.py` auf `develop` mit Exit-Code null
- [ ] Kein Akzeptanzkriterium, das auf eine Anforderung zurückführt, widerspricht ihr, erweitert sie oder trägt eine Ausnahme, die sie nicht kennt; wo doch, hat das Audit es als `Warning` wegen veralteter Wiederholung festgehalten, statt die Spec durchzuwinken
...
- Die in §Geltungsbereich und §Dimension 3 ergänzte Sprachdatei-Regel wird seit 2026-09-13 für ihren entscheidbaren Teil durchgesetzt. Zwei Bereinigungsläufe haben die Klassen geschlossen, die ein Resolver exakt entscheidet: 43 Referenzen in 30 übersetzten Dateien im ersten, 113 Referenzen in 30 übersetzten Dateien im zweiten, zusammen mit 22 umgezogenen Artefakt-Pfaden und 29 Akzeptanzkriterien-Überschriften außerhalb der Vertragsform. Der zweite Lauf hat auch den Resolver eingeführt, sodass diese Klassen nicht unbemerkt zurückkehren. Die früheren Regex-Durchgänge scheiterten, weil sie rieten, wo ein Sektionsname endet; der Resolver liest stattdessen die Überschriften des Ziels und akzeptiert eine Referenz, wenn eine Überschrift ein Präfix des zitierten Texts ist oder der zitierte Text in Anführungszeichen ein Präfix einer Überschrift. Offen bleibt der Rest, der in keiner Sprache auflöst, 389 Referenzen am 2026-09-13, aufgelistet von `scripts/check_section_refs.py --report`. Er mischt echte Geister-Referenzen mit korrekt lesenden Verkürzungen ohne Anführungszeichen, Kompositformen in übersetzter Prosa und Zitaten externer Styleguides, und nur ein menschlicher Durchgang kann sie auseinanderhalten.
