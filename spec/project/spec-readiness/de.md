...
- **MUSS** als `Critical` flaggen: ein Akzeptanzkriterium, das eine Regel aussagt, die seine eigene Anforderung nicht aussagt — enger, weiter oder mit einer Ausnahme, die die Anforderung nicht kennt. Das ist die Klasse der **veralteten Wiederholung**, und sie entsteht genau dann, wenn jemand eine Regel ändert, ohne durchzugehen, was sie wiederholt: Die beiden Aussagen unterscheiden sich nicht in der Stärke, eine von ihnen ist schlicht veraltet, und wer Akzeptanzkriterien als Schranke behandelt, setzt die aufgegebene Fassung um. Sie ist statisch erkennbar, ohne Diff, indem jedes Kriterium gegen die Anforderung gelesen wird, die es zusammenfasst
- **MUSS** als `Warning` flaggen: eine genannte Anzahl, die dem widerspricht, was das Dokument aufzählt — „sieben Dimensionen" über acht Überschriften, „die drei zugrunde liegenden Specs" über einer Liste von fünf, „diese vier Entscheidungen" über sechs Punkten. Eine Anzahl ist eine Wiederholung wie jede andere, sie veraltet genauso, und wer ihr vertraut, hört bei der Zahl auf zu zählen
- **SOLLTE** beide Prüfungen auf die Artefakte ausdehnen, die die Regeln einer Spec wiederholen, statt nur auf die Spec selbst — das Arbeitsdigest eines Skills, eine Checkliste, ein Template —, soweit der Audit-Scope sie einschließt; eine in der Spec korrigierte und im Digest stehengebliebene Regel ist derselbe Defekt, nur eine Ebene weiter außen
