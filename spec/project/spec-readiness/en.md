...
- **MUST** verify that every Acceptance Criterion tracing to a Requirement doesn't state a *different* rule than that Requirement—an AC that contradicts it, widens it, or carries an exception the Requirement doesn't is a Warning, the **stale-restatement** finding. A **narrower** AC is explicitly not a finding: an AC is by construction a concrete, testable instance of a general rule, so narrowing is what an AC does, and the coverage bar above is deliberately one testable AC per Requirement rather than per MUST. An AC tracing to a **Goal** is outside this check, because a Goal carries no RFC-2119 rule to compare against. This finding differs from the orphan case above: the AC does trace to a Requirement, it simply says something the Requirement doesn't. It's what an author produces by editing a Requirement without walking what restates it, and it matters because a consumer that treats Acceptance Criteria as the gate implements the abandoned version. Unlike §Dimension 1's contradiction checks it needs no RFC-2119 verb on the AC side, because the verb sits on the Requirement side of the compared pair
...
- **MUST** enforce the exactly decidable part of this check mechanically wherever a repository ships a deterministic resolver, so it fails that repository's gate instead of waiting for the next audit. That part covers three cases: a reference that resolves in another language file of its target but not in its own, a skill or agent path in backticks that still points at the repository root after the artifact moved under `plugins/<plugin>/`, and an acceptance-criteria heading that doesn't use its language's contract form. This repository's resolver is `scripts/check_section_refs.py`, run as a pre-commit hook and so in the `lint` lane. A reference that resolves in **no** language stays this audit's Critical, and the resolver only reports it, because a correctly reading unquoted shortening and a ghost reference look the same to a resolver
...
- **MUST** map readiness findings to the scale per the patterns documented in §Dimension 1, 2, and 3 above:
  ...
  - **Warning**: MUST vs SHOULD contradiction; unidentifiable audience; derived audience whose needs aren't addressed; Goal without a matching Requirement; Requirement without a testable Acceptance Criterion; orphan Acceptance Criterion; stale restatement (an Acceptance Criterion stating a different rule than the Requirement it traces to)
...
- [ ] In a repository that ships a deterministic resolver, a section reference resolving only in another language, a skill or agent path left at the root after a move under `plugins/`, and a non-contract acceptance-criteria heading each fail the gate; in this repository `scripts/check_section_refs.py` exits zero on `develop`
- [ ] No Acceptance Criterion tracing to a Requirement contradicts it, widens it, or carries an exception it doesn't; where one does, the audit recorded it as a stale-restatement Warning instead of passing the spec
...
- The per-language cross-reference rule added to §Scope and §Dimension 3 is enforced for its decidable part since 2026-09-13. Two remediation passes closed the classes a resolver decides exactly: 43 references in 30 translated files in the first, and 113 references in 30 translated files in the second, together with 22 moved artifact paths and 29 acceptance-criteria headings off the contract form. The second pass also landed the resolver, so those classes can't return unnoticed. The earlier regex passes failed because they guessed where a section name ends; the resolver reads the target's headings instead and accepts a reference when a heading is a prefix of the cited text, or the quoted text a prefix of a heading. What stays open is the residue that resolves in no language, 389 references on 2026-09-13, listed by `scripts/check_section_refs.py --report`. It mixes genuine ghost references with unquoted shortenings that read correctly, compound forms in translated prose, and citations of external style guides, and only a human pass can tell them apart.
