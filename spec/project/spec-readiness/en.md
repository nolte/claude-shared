...
- **MUST** verify that every Acceptance Criterion says what the Requirement it traces to says—an AC that's narrower, wider, or carries an exception the Requirement doesn't is a Warning, the **stale-restatement** finding. It differs from the orphan case above: the AC does trace to a Requirement, it simply states a different rule than the one it traces to. This is what an author produces by editing a Requirement without walking what restates it, and it matters because a consumer that treats Acceptance Criteria as the gate implements the abandoned version. Unlike §Dimension 1's contradiction checks it needs no RFC-2119 verb on the AC side, because what's compared is the pairing, not the verb strength
...
- [ ] No Acceptance Criterion in an audited spec states a rule its own Requirement doesn't; where one does, the audit recorded it as a stale-restatement Warning instead of passing the spec
