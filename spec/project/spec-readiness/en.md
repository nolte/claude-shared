...
- The per-language cross-reference rule added to §Scope and §Dimension 3 is **partially** met. A first remediation pass closed the exactly-provable class: 43 references in 30 translated files that cited a canonical section name for a target whose translated heading was verifiable, across `review-plan`, `research-triangulate`, `dependency-audit`, `lektorat`, and `mkdocs-structure`. What remains is the judgement-call residue, and it stays open deliberately. Four successive regex passes over the corpus produced wildly different candidate counts because each fixed one flaw and exposed another: trailing prose swallowed into the section name, letter-form headings, a `§` binding to the wrong path on a multi-reference line, and intra-spec references bound to the nearest preceding path. A detector that unstable can't authorise a rewrite, so the remaining classes need a human pass: deliberate shortenings that read correctly (`spec-drift-audit` cited as §"Delimitation" for `Delimitation from other specs and skills`), compound forms in translated prose, and references whose target heading has no clean counterpart. One case found on the way is a reference that resolves in **neither** language: `spec-drift-audit` cites `review-plan` §"Required sections," a heading that exists in no language of that spec. That's a plain Critical under the pre-existing rule, left for the same pass.
