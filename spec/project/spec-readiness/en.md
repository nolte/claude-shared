...
- **MUST** flag as Critical any acceptance criterion that states a rule its own requirement doesn't—narrower, wider, or carrying an exception the requirement omits. This is the **stale-restatement** class, and it's what an author produces by editing a rule without walking what restates it: the two statements don't disagree on strength, one of them is simply out of date, and a consumer that treats acceptance criteria as the gate will implement the abandoned version. It's detectable statically, without a diff, by reading every criterion against the requirement it rolls up
- **MUST** flag as Warning any stated count that disagrees with what the document enumerates—"seven dimensions" above eight headings, "the three grounding specs" above a list of five, "these four decisions" above six bullets. A count is a restatement like any other, it goes stale the same way, and a reader who trusts it stops counting
- **SHOULD** extend both checks to the artifacts that restate a spec's rules rather than to the spec alone—a skill's working digest, a checklist, a template—wherever the audit's scope includes them; a rule corrected in the spec and left standing in the digest is the same defect one hop further out
