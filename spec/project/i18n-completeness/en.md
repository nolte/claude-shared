...
- **MUST** emit a single severity-sorted report ordered **critical** (used-but-undefined; missing in a locale), then **warning** (structural mismatch), then **info** (orphans, identical values, empty values, placeholder drift), led by a summary metrics table (per-locale key counts, missing, orphan, empty, identical, dynamic-skipped)
