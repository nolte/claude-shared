...
- Default: Die „identisch über Locales"-Heuristik nur als Info-Tier-Befund behalten, ohne Ausnahme-Allowlist, da das niedrigste Info-Tier die Rauschkosten bereits begrenzt und nie den Kritisch- oder Warnungs-Zähler aufbläht. Erneut prüfen, wenn: ein echter Audit-Lauf auf einer lokalisierten App berichtet, dass legitim-by-design identische Werte (Eigennamen, Markennamen, Einheiten) mehr als die Hälfte der Identische-Werte-Info-Befunde ausmachen, oder ein Operator nach einem solchen Lauf explizit Unterdrückung anfordert. Ein solcher Lauf existiert noch nicht — es gibt keine `project/i18n-audit.yml`-Config und kein i18n-Audit-Artefakt irgendwo im Repo.
- Default: Die Platzhalter-Parität bei Simple-Platzhalter-Granularität (`{{var}}` / `{var}` / `%s`) belassen und ICU-MessageFormat-Plural- und Select-Bodies als opake Strings behandeln, statt sie zu parsen. Erneut prüfen, wenn: ein Projekt, das ICU MessageFormat verwendet (FormatJS / react-intl ist eine in §Eingaben gelistete Ziel-Bibliothek), das Audit laufen lässt und die Simple-Granularitäts-Parität ein nachweislich irreführendes Ergebnis bei einer `{count, plural, …}`- oder `{…, select, …}`-Form erzeugt — ein echter False-Positive- oder False-Negative-Paritäts-Befund, der auf einen ICU-Body zurückführbar ist. Upstream-Signal zum Beobachten: Die portfolio-eigene Plural-Konvention ist i18next-JSON-v4-Suffix-Grammatik (`_one` / `_other`) gemäß `spec/frontend/webview-ui-optimization/` §i18next plurals, nicht ICU, sodass der Trigger spezifisch ein Nicht-Portfolio-Konsument ist, der ICU adoptiert.
