...
- **Trigger aus `ready → in_progress` (Start-der-Arbeit-Post).** Manche Posts machen mehr Sinn, wenn die Arbeit beginnt (ein „hier ist das Problem, das ich angehe"-Post), als wenn die Arbeit endet. Ob diese Spec ein zweites Trigger-Ereignis für den Start-der-Arbeit-Fall wachsen lässt, bleibt offen. Erneut prüfen, wenn: eine explizite Operator-Anfrage nach einem Start-der-Arbeit-Post eintrifft ODER ein Roadmap-Item oder Feature im Quell-Konsumenten ein `ready → in_progress`-Trigger-Ereignis verlangt. Stand 2026-05-29 existiert keines von beidem. Weder `project/roadmap.md` noch `project/features/` enthalten einen Start-der-Arbeit- oder `ready → in_progress`-Post-Eintrag, und kein `project/blog-triggers/`-Artefakt verweist auf einen. Bis dahin sind Start-der-Arbeit-Posts operator-initiiert wie jeder andere.
- **Sprint-Zusammenfassungs-Trigger.** Ein Sprint-Level-Zusammenfassungs-Post (ein Post pro Sprint, der alle Features dieses Sprints abdeckt) ist eine andere Form als ein pro-Feature-Post. Ob diese Spec ein `sprint → review`-Trigger-Ereignis für Sprint-Zusammenfassungen wachsen lässt—verdrahtet an [`sprint-review`](../../../skills/sprint-review/SKILL.md) —, bleibt offen; die abgeleitete-Vorschlag-Heuristik des §Operator-Entscheidungs-Vertrags antizipiert den Komponieren-mit-Sprint-Zusammenfassung-Fall bereits. Default: das aktuelle Einzel-Ereignis-Design (`feature → done` only) beibehalten; noch keinen Sprint-Zusammenfassungs-Trigger hinzufügen. Erneut prüfen, wenn: der pro-Feature-Trigger in mindestens einem vollen Sprint tatsächlich gefeuert und eine echte Wahl-1/2/3-Entscheidung erzeugt hat, belegt durch ein Artefakt unter `project/blog-triggers/`, UND dieser Sprint `status: closed` erreicht hat. Stand 2026-05-29 ist das Gate im Kern unerfüllt—obwohl Sprint 0004 nach dem Landen der Trigger-Verdrahtung schloss, enthält `project/blog-triggers/` nur `README.md` (null Zurückstellungs- oder Briefing-Artefakte), sodass der pro-Feature-Trigger nie wirklich gelaufen ist.
## Referenzen
