...
- **Verdrahtungsmechanismus.** Die Spec ist absichtlich verdrahtungsagnostisch. Die vier Kandidaten-Mechanismen sind: (a) ein Claude-Code-`settings.json`-`Stop`-Hook, der feuert, nachdem `sprint-execute` den `done`-Status des Features geschrieben hat; (b) eine Folgedispatch innerhalb von `sprint-execute` selbst (die bestehende Skill wächst um einen „blog-author triggern?"-Prompt nach dem Markieren eines Features als `done`); (c) eine separate dedizierte Trigger-Skill (`/nolte-shared:blog-author-trigger`), die der Operator nach `sprint-execute` aufruft; (d) eine GitHub Action, die `project/features/*.md`-Commits beobachtet und ein Erinnerungs-Issue postet. Jede hat Trade-offs (a ist am automatischsten aber starr; b koppelt `sprint-execute` an Blog-Belange; c ist am explizitesten aber fügt einen Schritt hinzu; d überschreitet in den Out-of-Session-Bereich). Gelöst durch die erste Implementierung; die Wahl kann vom Automatisierungs-Appetit des Quell-Konsumenten abhängen. **Gelöst (Referenz-Implementierung):** `nolte/claude-shared` kombiniert die Mechanismen **c + b** — eine dedizierte `blog-author-trigger`-Skill (c), die `sprint-execute` aus seinem `in_progress → done`-Schritt automatisch dispatched (b), wodurch eine explizite wiederverwendbare Einheit mit In-Session-Automatik gepaart wird, während die dreifache Operator-Wahl erhalten bleibt. Die Spec bleibt verdrahtungsagnostisch; andere Konsumenten dürfen weiterhin einen anderen Mechanismus wählen.
- **Mehr-Feature-Batch-Trigger.** Ein Sprint mit fünf in rascher Folge done-en Features könnte fünf Trigger in einer Claude-Code-Session feuern. Die Spec beschreibt einen pro-Feature-Trigger; ob die Implementierung sie batched (eine Operator-Entscheidung deckt „was ist mit diesen fünf?" ab) oder sequenziell zur Oberfläche bringt, ist offen. Wahrscheinlich läuft die Implementierung sie sequenziell mit einer „alle verbleibenden überspringen → ins Backlog zurückstellen"-Abkürzung durch.
...
- **Zurückstellungs-Artefakt-Lebenszyklus bei Feature-Abbruch.** Wenn ein Feature `done → cancelled` übergeht (selten, aber erlaubt), wird das Zurückstellungs-Artefakt für dieses Feature veraltet. Ob der Trigger es automatisch beim Abbruchs-Ereignis als `status: cancelled` markiert, ist offen; heute legt die Spec die Last auf die `sprint-review`-Oberfläche des Operators.
- **Cross-Repository-Schreibautorität.** §Cross-Repository-Übergabe verweigert stille Schreibvorgänge in den Working-Tree des Blog-Konsumenten; ob eine zukünftige Iteration einen Opt-in für vollautomatisches Cross-Repo-Posting trägt (geeignet für ein Single-Operator-Portfolio, in dem Kontextwechsel Reibung sind), ist offen. Heute ist die sicherheitsfirste Haltung bedingungslos.
