...
Das `nolte/gh-plumbing`-Portfolio liefert wiederverwendbare Workflows für Release-Management und Dokumentations-Auslieferung. `spec/project/branching-model/` §Erforderliche GitHub-Workflows ist die kanonische Quelle der verpflichtenden Liste (aktuell vier Workflows). Diese Spec hebt zusätzlich die Dokumentations- und Packaging-Begleiter hervor, damit ein Projektstruktur-Audit sie auch dann erkennt, wenn die branching-model-Spec isoliert betrachtet wird, listet die verpflichtende Menge selbst aber nicht — Änderungen daran erfolgen in `branching-model`, nie hier.

- **MUSS [MUST]** jeden Release-Management-Workflow enthalten, den `spec/project/branching-model/` §Erforderliche GitHub-Workflows vorschreibt, jeweils verkabelt mit dem entsprechenden wiederverwendbaren Workflow unter `nolte/gh-plumbing/.github/workflows/`. Der aktuelle Satz ist `release-drafter.yml`, `release-publish.yml`, `release-cd-refresh-master.yml` und `automerge.yaml`; die autoritative Aufzählung lebt in `branching-model`
...
- Eine `.github/stale.yml` ist nicht gefordert: `nolte/gh-plumbing` hat die `commons-stale.yml`, die sie erweitert hat, am 2021-12-26 gelöscht (`11deb9e`), sodass die Datei nur auf eine fehlende Quelle zeigen kann. Die automatische Behandlung inaktiver Issues und Pull Requests ist eine Entscheidung je Repository außerhalb dieser Spec (#626)
...
- **MUSS [MUST]**, wenn `project/` vorhanden ist, die Dateien wie folgt organisieren: `project/roadmap.md` (die Queue), `project/goals.md` (Vision plus Outcomes), `project/sprints/<NNNN>-<slug>.md` (eine Datei pro Sprint), `project/features/<slug>.md` (eine Datei pro Feature) und — sofern Out-of-Band-Releases auftreten — `project/release-artifacts/out-of-band/<NNNN>-<slug>.md` plus ein regeneriertes `project/release-artifacts/out-of-band/INDEX.md`; die jeweilige Datei-Form wird von den Specs `roadmap`, `sprint`, `feature` und `release-artifact` geregelt. Derselbe Baum enthält außerdem die Dateien, die Geschwister-Specs dort ablegen, jeweils von dieser Spec geregelt: `project/mission.md` (`mission`), `project/portfolio.yml` (`portfolio-management`), `project/requirements/<slug>.md` (`requirements-elicitation`) und `project/blog-triggers/` (`blog-author-trigger`)
...
- **MUSS [MUST]** den primären Quellcode unter einem der folgenden konventionellen Layouts ablegen:
  ...
  - `playbooks/` und ein Inventar-Baum (`inventories/<env>/` je Umgebung im Default-Profil *multi-environment-fleet* oder `inventory/` für das Profil *single-environment-bootstrap* gemäß `spec/ansible/playbook-development/` §Repository-Profile) sowie optional `group_vars/` und `host_vars/`, zusammen mit `ansible.cfg` und `requirements.yml` im Repository-Wurzelverzeichnis für ein Ansible-Bootstrap- bzw. Provisioning-Repository, dessen primäres Lieferobjekt Konfigurations- und Automatisierungs-Code ist und das keinen Runtime-Quellcode enthält. Ein Top-Level-`roles/` ist hier **profilabhängig, nicht bedingungslos**: das Default-Profil *multi-environment-fleet* **DARF NICHT [MUST NOT]** eines tragen (Rollen kommen über `requirements.yml`), und nur das Profil *single-environment-bootstrap* **KANN [MAY]** gerätespezifische Inline-Rollen unter `roles/<name>/` halten. Die Ansible-Standardkonventionen lassen sich nicht in eine `src/`-Hülle umpacken, ohne die Default-Rollen- und Inventory-Suche von `ansible-playbook` zu brechen
  - die Galaxy-konformen Rollen-Verzeichnisse (`defaults/`, `vars/`, `tasks/`, `handlers/`, `templates/`, `files/`, `meta/`) im Repository-Wurzelverzeichnis für ein eigenständiges Ansible-Rollen-Repository gemäß `spec/ansible/role-development/`, optional als Collection unter `<namespace>/<collection>/roles/<role>/` verpackt, wenn mehrere verwandte Rollen zusammen ausgeliefert werden; wie bei der Playbook-Variante lässt sich das Layout eines Rollen-Repos nicht in eine `src/`-Hülle umpacken, ohne Ansibles Rollen-Suche zu brechen
- **MUSS NICHT [MUST NOT]** primäre Quellcode-Dateien lose im Repository-Wurzelverzeichnis halten; dort dürfen nur Tooling-Konfigurationen, Metadaten und kleine Skripte liegen; die zwei Ansible-Varianten oben (Playbook-/Provisioning-Repository und eigenständiges Rollen-Repository) sind eine bewusste Ausnahme von dieser Regel
...
- [ ] `.github/workflows/` enthält jeden Workflow, den `spec/project/branching-model/` §Erforderliche GitHub-Workflows auflistet (aktuell `release-drafter.yml`, `release-publish.yml`, `release-cd-refresh-master.yml`, `automerge.yaml`), jeweils verkabelt mit dem passenden wiederverwendbaren `nolte/gh-plumbing`-Workflow
- [ ] Wenn `mkdocs.yml` vorhanden ist **und** `.github/workflows/release-cd-deliver-docs.yml` ausgeliefert ist, triggert der Workflow auf `release: [published]` (der Workflow selbst ist SHOULD gemäß Anforderungen §Release- und Dokumentations-Workflows; dieses AK prüft den Trigger im Vorhandenseinsfall, nicht das Vorhandensein selbst)
...
- [ ] `.github/boring-cyborg.yml` ist vorhanden und erweitert `nolte/gh-plumbing:.github/commons-boring-cyborg.yml`
...
- [ ] Primärer Quellcode liegt unter `src/`, `src/<component>/`, `custom_components/<name>/`, `.claude-plugin/` + `skills/<name>/`, **oder** das Repository ist ein Ansible-Bootstrap-/Provisioning-Repository mit `playbooks/` und einem Inventar-Baum (`inventory/` oder `inventories/<env>/`) im Wurzelverzeichnis (Top-Level-`roles/` nur im Profil `single-environment-bootstrap`), **oder** ein eigenständiges Ansible-Rollen-Repository mit den Galaxy-Rollen-Verzeichnissen im Wurzelverzeichnis; nicht lose im Wurzelverzeichnis
