...
- **MUST** include a `renovate.json5` (the canonical default—JSON5 is preferred because the portfolio presets rely on inline comments) or `renovate.json` (the fallback, permitted only where JSON5 tooling is unavailable) that `extends` the portfolio-wide preset `github>nolte/gh-plumbing//renovate-configs/common#<tag>`, pinned to a release tag (for example `#v1.1.12`), so Renovate configuration stays aligned across the portfolio; per-repository overrides **SHOULD** stay narrow (typically package-grouping or automerge rules)
- **MUST** have the Renovate GitHub App (<https://github.com/apps/renovate>) installed on the repository so that the `renovate.json5` configuration above actually drives dependency updates; without the app, the config file is inert and no PRs or Dependency-Dashboard issues are produced. The same Probot-style installation pattern applies as for `settings` / `boring-cyborg`, and the install action is human-only
- **MUST** include a `.pre-commit-config.yaml` pinning linters and formatters relevant to the stack
- **MUST** include a `LICENSE` file at the root when the repository is public or intended for redistribution; **SHOULD** otherwise
...
Versioning policy and the Draft→Published release mechanics are governed by `spec/project/release-automation/` and `spec/project/release-artifact/`; this spec references them and never restates them.
...
- For Claude Code plugin / prompt-only repositories (the `.claude-plugin/` + `skills/` layout) with no runtime source, `tests/` is **SHOULD** rather than **MUST**; the validation surface for that project type is the frontmatter/contract check (for example `scripts/validate_skills.py`) wired as the `task test` target
...
- **MUST** place primary source code under one of these conventional layouts:
  ...
  - `playbooks/` and an inventory tree (`inventories/<env>/` per environment in the default *multi-environment-fleet* profile, or `inventory/` for the *single-environment-bootstrap* profile per `spec/ansible/playbook-development/` §Repository profiles) plus optional `group_vars/` and `host_vars/`, alongside `ansible.cfg` and `requirements.yml` at the repository root for an Ansible bootstrap or provisioning repository, where configuration and automation code is the primary deliverable and no runtime source exists. A top-level `roles/` here is **profile-dependent, not unconditional**: the default *multi-environment-fleet* profile **MUST NOT** carry one (roles arrive via `requirements.yml`), and only the *single-environment-bootstrap* profile **MAY** keep device-specific inline roles under `roles/<name>/`. Ansible's standard conventions can't be wrapped under a `src/` shell without breaking `ansible-playbook`'s default role and inventory discovery
  - the Galaxy-conformant role directories (`defaults/`, `vars/`, `tasks/`, `handlers/`, `templates/`, `files/`, `meta/`) at the repository root for a standalone Ansible role repository per `spec/ansible/role-development/`, optionally packaged as a collection under `<namespace>/<collection>/roles/<role>/` when shipping several related roles together; like the playbook variant, a role repo's layout can't be wrapped under a `src/` shell without breaking Ansible's role discovery
- **MUST NOT** keep primary source files loose at the repository root; only tooling configs, metadata, and small scripts may live there; the two Ansible variants above (playbook/provisioning repository and standalone role repository) are a deliberate exception to this rule
...
- [ ] `tests/` exists and contains at least one test, **or** the repository is a Claude Code plugin / prompt-only repository (`.claude-plugin/` + `skills/`) whose `task test` target runs a frontmatter/contract validation (for example `scripts/validate_skills.py`) in lieu of runtime tests
- [ ] Primary source lives under `src/`, `src/<component>/`, `custom_components/<name>/`, `.claude-plugin/` + `skills/<name>/`, **or** the repository is an Ansible bootstrap / provisioning repository with `playbooks/` and an inventory tree (`inventory/` or `inventories/<env>/`) at the root (top-level `roles/` only in the `single-environment-bootstrap` profile), **or** a standalone Ansible role repository with the Galaxy role directories at the root; not loose at the root
...
- [ ] If `project/` is present, planning artefacts live under the layout `project/roadmap.md`, `project/goals.md`, `project/sprints/<NNNN>-<slug>.md`, `project/features/<slug>.md`, or `project/release-artifacts/out-of-band/<NNNN>-<slug>.md` (with `project/release-artifacts/out-of-band/INDEX.md` when at least one out-of-band entry exists); the sibling-spec files `project/mission.md`, `project/portfolio.yml`, `project/requirements/<slug>.md`, and `project/blog-triggers/` are part of the layout, and any other nested or alternative location fails validation
- [ ] CI status badges for the primary workflows appear near the top of `README.md`
- [ ] If the repository is public, a `LICENSE` file exists at the root

## Open Questions
- Should the spec additionally prescribe community-health files `CODEOWNERS`, `SECURITY.md`, and `SUPPORT.md` for `.github/`? Issue templates (`github-issue-templates`) and the pull-request template (`pull-request-workflow`) are now covered by dedicated specs; the remaining community-health files `CODEOWNERS` / `SECURITY.md` / `SUPPORT.md` are owned here and stay open.
