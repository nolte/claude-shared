...
- **MUST** distinguish **legitimate generalisation** of the implementation from **special-casing the test's inputs**: returning the right answer for the general class is a fix; returning a constant matched to the test input (beyond a transient Fake-It step on the way to generalising) is gaming the test. The distinction is **checkable**, not a matter of taste. The governing rule: *a change generalises when it changes a rule; it special-cases when it adds a branch on the case's data.* Three signals decide it, in order:
  1. **Literal overlap.** A value appearing both in the case's input or expected-value set and in the production change is the single strongest indicator of special-casing. A change carrying such a literal **MUST** be treated as special-casing unless signal 3 clears it.
  2. **Predicate shape.** A new conditional whose predicate is an equality or identity check against a case-specific value (`if user_id == 42`, `if tenant == "acme-test"`) is special-casing; a predicate expressed in the domain's own terms (a range, a type, a documented state) is a rule.
  3. **Second-example probe.** Add one further example from the *same* equivalence class with different values. Passing with no further edit at the same site means the change generalised; forcing another edit there means the first edit was a special case. This probe is the tie-break for signals 1 and 2 when they disagree or when the literal is plausibly domain-owned, not a blanket Triangulation mandate.
- **MAY** special-case where the **domain itself** is discontinuous (a documented exception, a regulated boundary, a legacy-compatibility carve-out); such a change **MUST** carry a written rationale naming the domain rule that creates the discontinuity, and **MUST** be accompanied by a case exercising the general branch, so the carve-out is visible in review rather than inferred from a bare literal.
- **MUST** take the **reviewable case-change path** (route to phase 1) when the case itself was demonstrably wrong, with a recorded rationale, rather than silently changing an assertion to match the code. A **legitimate expectation change** is separated from forbidden test-weakening by two checkable properties, its **source** and its **order**: it's sourced *upstream* (a changed requirement, spec, or contract exists and the recorded rationale cites it), never sourced by the failure itself, since "the case is red" isn't a justification; and it lands as **its own change, before the production edit**, so the history reads expectation-change-then-code-change rather than an assertion quietly moved to meet the code. Phase 4 therefore **MUST NOT** edit a case's expectation itself: it hands the case back to phase 1 naming the upstream delta, and resumes once the corrected case is red for the right reason.
...
- [ ] The no-cheating invariant is concrete: no weakening/deleting/skipping/hard-coding-to-the-expected-value; satisfy behaviour not one assertion; the legitimate-generalisation-vs-special-casing distinction is drawn as a checkable three-signal heuristic (literal overlap, predicate shape, second-example probe) with a rationale-bearing carve-out for a genuinely discontinuous domain
- [ ] The reviewable case-change path (test was wrong → phase 1, recorded rationale) is required instead of a code hack, and a legitimate expectation change is bound to an upstream source and lands as its own change before the production edit, phase 4 never editing an expectation itself
...
- ~~What concrete signals best distinguish **legitimate generalisation** from **special-casing the test's inputs**: can the phase name a checkable heuristic, or does it stay a review judgement?~~ **Settled (2026-07-24): a checkable heuristic.** The rule is *changes a rule = generalisation, adds a branch on the case's data = special-casing*, decided by three ordered signals (literal overlap, predicate shape, second-example probe) recorded in §"The no-cheating / test-integrity invariant." Two of the three are mechanically detectable from the diff alone, which is what makes the invariant enforceable rather than aspirational; the probe resolves the residual judgement calls without mandating Triangulation everywhere. A genuinely discontinuous domain keeps a carve-out, but pays for it with a written rationale and a general-branch case, so the exception can't be used as cover.
- ~~When a fix genuinely requires an **existing test's expectation to change** (the old assertion encoded a now-removed behaviour), how does the phase keep that on the reviewable case-change path and out of the forbidden test-weakening category?~~ **Settled (2026-07-24): by source and by order, with phase 4 never touching an expectation.** A legitimate expectation change cites an upstream delta (requirement, spec, or contract) and lands as its own change *before* the production edit; the forbidden kind is justified only by the red result and rides along inside the fix. Both properties are visible in the history, so the distinction survives review without needing intent. Phase 4 hands the case back to phase 1 rather than editing it, keeping the case-change path the only route by construction.
