...
- **MUST** treat the following as the **closed, ordered set** of functional tiers, from foundation to apex. Every per-tier spec, skill, and agent maps to exactly one entry; a behaviour is tested at the lowest applicable entry:
  1. **Static analysis**: lint, type-check, format-check, and static security/complexity rules. Executes without running the program; the pyramid's foundation layer per the Trophy [R9]. Owned in execution by `spec/project/quality-gate/`; its tier contract is `spec/project/test-tier-static-analysis/`.
  2. **Unit**: verifies one unit of behaviour in isolation. **Solitary** unit tests isolate the unit from collaborators with test doubles; **sociable** unit tests exercise real collaborators [R6], [R11]. The definition of "a unit" is **deliberately team-determined** [R6] and a per-project unit spec records it. Its tier contract is `spec/project/test-tier-unit/`.
  3. **Component**: verifies a single deployable/shippable component in isolation from its peers: a rendered frontend component against its own contract, or a backend service exercised through its own interface with its external dependencies replaced by doubles at the process boundary. Its tier contract is `spec/project/test-tier-component/`.
  4. **Integration**: verifies that a unit/component correctly talks to an adjacent real collaborator. **Narrow integration** exercises one real collaborator (for example, a database via an ephemeral container) with the rest doubled; **broad integration** stands up many live services and **MUST** be treated as costly and minimised [R12], [R14]. Its tier contract is `spec/project/test-tier-integration/`.
  5. **Contract**: verifies the agreement at a service boundary without standing up both sides. **Consumer-driven contracts** [R12], [R13] let a consumer's expectations be verified against a provider independently; in microservice architectures contract tests **SHOULD** replace broad integration tests across service boundaries [R13], [R14]. Its tier contract is `spec/project/test-tier-contract/`.
...
- **MUST NOT** let a **test-controlled arrangement** be **more permissive than what production would enforce** along any dimension the test relies on, unless the arrangement can't be made faithful along that dimension **and** the divergence is declared under the rule below—both conditions, never the declaration alone. An *arrangement* is whatever a test sets up to stand in for what production would do: a test double, a seeding fixture, a factory, a provider state, a pre-loaded row. It rejects what production rejects (constraints, validation, uniqueness) and doesn't preserve what production discards.
- **MUST** treat a permissive arrangement as a defect of the **setup**, not of the assertion. A correct, specific, falsifiable assertion over a state that can't occur certifies a world that doesn't exist, so the test passes and grading the assertion harder finds nothing—the failure mode catalogued as `T9` by `spec/project/test-falsifiability/` [R20], which owns its detection route while this rule owns the requirement.
- **MUST** name the divergence **in the arrangement itself** where it can't be made faithful, so the next reader knows what the test doesn't cover. A declared divergence is a bounded trade-off a reviewer can weigh; a silent one is the defect, and this asymmetry is what makes the rule above checkable rather than aspirational. A divergence that's merely declared while remaining trivially closable stays the defect: the declaration records a cost, it doesn't buy one.
- **MUST** bind at every tier on its own terms, whether or not a tier spec elaborates it. A per-tier spec **MAY** apply it to that tier's own arrangements—the seams it doubles, the state it seeds, the constraints most often dropped there—and **MUST NOT** restate the rule or its exemption when it does. Where no tier spec elaborates it, this rule governs that tier's arrangements directly rather than lapsing.
...
- **MUST** treat a double as an arrangement under §"Arrangement fidelity" whose reference point is **the collaborator it replaces**: it's no more permissive than that collaborator along any dimension the test relies on. The rules above bound *how much* a test doubles; that one bounds whether the double can **refuse**. This section fixes the reference point for a double and **MUST NOT** restate the rule or its exemption; a per-tier application belongs under the invariant shape's *Isolation level & permitted test doubles* field.
...
- [ ] Arrangement fidelity is required portfolio-wide (no test-controlled arrangement is more permissive than what production would enforce along a relied-on dimension), the exemption requires both that the arrangement can't be made faithful there and that the divergence is declared in the arrangement, and the rule binds at every tier directly, with per-tier elaboration optional and forbidden from restating it
- [ ] The rule's subject is the arrangement rather than the double: a seeding fixture, a factory and a provider state are covered by the same requirement, and §"Test-double taxonomy" fixes only the reference point for a double (the collaborator it replaces) rather than carrying a second copy of the rule
...
- **Settled 2026-09-13: one spec per tier.** The corpus answered the granularity question in practice: `test-tier-static-analysis`, `test-tier-unit`, `test-tier-component`, `test-tier-integration`, and `test-tier-contract` each own one tier and `e2e-test-automation` owns the sixth, as §Functional tier taxonomy now names.
