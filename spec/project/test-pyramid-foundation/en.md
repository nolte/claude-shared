...
- **MUST** bind at every tier on its own terms, whether or not a tier spec elaborates it. A per-tier spec **MAY** apply it to that tier's doubling practice—the seams it doubles, and the constraints most often dropped there—under the invariant shape's *Isolation level & permitted test doubles* field, and **MUST NOT** restate the rule or its exemption when it does. Where no tier spec elaborates it, as at the E2E tier, this rule governs that tier's doubles directly rather than lapsing.
...
- [ ] Double fidelity is required portfolio-wide (a double never more permissive than what it replaces along a relied-on dimension), the exemption requires both that the double can't be made faithful there and that the divergence is declared, and the rule binds at every tier directly, with per-tier elaboration optional and forbidden from restating it
