...
- **DARF NICHT [MUST NOT]** zulassen, dass ein **testgesteuertes Arrangement permissiver ist als das, was die Produktion durchsetzen würde**, entlang irgendeiner Dimension, auf die sich der Test stützt, sofern das Arrangement nicht entlang dieser Dimension treu gemacht werden kann **und** die Abweichung nach der Regel unten deklariert ist — beide Bedingungen, nie die Deklaration allein. Ein *Arrangement* ist alles, was ein Test aufbaut, um an die Stelle dessen zu treten, was die Produktion täte: ein Test-Double, eine Seeding-Fixture, eine Factory, ein Provider-State, eine vorab eingespielte Zeile. Es lehnt ab, was die Produktion ablehnt (Constraints, Validierung, Eindeutigkeit), und bewahrt nicht, was die Produktion verwirft.
- **MUSS [MUST]** ein permissives Arrangement als Defekt des **Aufbaus** behandeln, nicht der Assertion. Eine korrekte, spezifische, falsifizierbare Assertion über einem Zustand, der nicht eintreten kann, bestätigt eine Welt, die es nicht gibt; der Test besteht, und die Assertion härter zu prüfen findet nichts — das ist der Fehlermodus, den `spec/project/test-falsifiability/` [R20] als `T9` katalogisiert, wobei jene Spec die Detektionsroute verantwortet und diese Regel die Anforderung.
- **MUSS [MUST]** die Abweichung **im Arrangement selbst** benennen, wo es nicht treu gemacht werden kann, damit die nächste lesende Person weiß, was der Test nicht abdeckt. Eine deklarierte Abweichung ist ein begrenzter Trade-off, den ein Reviewer abwägen kann; eine verschwiegene ist der Defekt, und genau diese Asymmetrie macht die Regel oben prüfbar statt bloß wünschenswert. Eine Abweichung, die bloß deklariert ist, während sie trivial schließbar bleibt, bleibt der Defekt: Die Deklaration hält Kosten fest, sie kauft sie nicht frei.
- **MUSS [MUST]** auf jeder Stufe aus eigenem Recht binden, ob eine Stufen-Spec sie ausführt oder nicht. Eine Stufen-Spec **DARF [MAY]** sie auf die eigenen Arrangements ihrer Stufe anwenden — die Nähte, die sie doubelt, den Zustand, den sie seedet, die dort am häufigsten weggelassenen Constraints — und **DARF dabei NICHT [MUST NOT]** die Regel oder ihre Ausnahme wiederholen. Wo keine Stufen-Spec sie ausführt, regiert diese Regel die Arrangements jener Stufe unmittelbar, statt zu entfallen.
...
- **MUSS [MUST]** ein Double als Arrangement nach §„Arrangement-Treue“ behandeln, dessen Bezugspunkt **der Kollaborateur ist, den es ersetzt**: Es ist entlang keiner Dimension, auf die sich der Test stützt, permissiver als dieser Kollaborateur. Die Regeln oben begrenzen, *wie viel* ein Test doubelt; jene begrenzt, ob das Double **ablehnen** kann. Dieser Abschnitt legt den Bezugspunkt für ein Double fest und **DARF NICHT [MUST NOT]** die Regel oder ihre Ausnahme wiederholen; eine Anwendung je Stufe gehört in das Feld *Isolationsgrad & erlaubte Test-Doubles* der invarianten Stufen-Form.
...
- [ ] Arrangement-Treue ist portfolioweit gefordert (kein testgesteuertes Arrangement permissiver als das, was die Produktion durchsetzen würde, entlang einer herangezogenen Dimension), die Ausnahme verlangt beides — dass das Arrangement dort nicht treu gemacht werden kann und dass die Abweichung im Arrangement deklariert ist —, und die Regel bindet auf jeder Stufe unmittelbar, wobei die Ausführung je Stufe optional ist und die Regel nicht wiederholen darf
- [ ] Das Subjekt der Regel ist das Arrangement statt des Doubles: Eine Seeding-Fixture, eine Factory und ein Provider-State fallen unter dieselbe Anforderung, und §„Test-Double-Taxonomie“ legt allein den Bezugspunkt für ein Double fest (den Kollaborateur, den es ersetzt), statt eine zweite Kopie der Regel zu tragen
...
- **Entschieden am 2026-09-13: eine Spec pro Stufe.** Der Korpus hat die Granularitätsfrage in der Praxis beantwortet: `test-tier-static-analysis`, `test-tier-unit`, `test-tier-component`, `test-tier-integration` und `test-tier-contract` besitzen je eine Stufe, und `e2e-test-automation` besitzt die sechste, wie §Funktionale Stufen-Taxonomie sie jetzt benennt.
