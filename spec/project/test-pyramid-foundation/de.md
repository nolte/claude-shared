...
- **DARF NICHT [MUST NOT]** zulassen, dass ein Double **permissiver ist als der Kollaborateur, den es ersetzt**, entlang irgendeiner Dimension, auf die sich der Test stützt, sofern das Double nicht entlang dieser Dimension treu gemacht werden kann **und** die Abweichung nach der Regel unten deklariert ist — beide Bedingungen, nie die Deklaration allein: Es lehnt ab, was der echte Kollaborateur ablehnt (Constraints, Validierung, Eindeutigkeit), und bewahrt nicht, was der echte Kollaborateur verwirft. Die Regeln oben begrenzen, *wie viel* ein Test doubelt; diese begrenzt, ob das Double **ablehnen** kann. Ein Double, das akzeptiert, was die Produktion ablehnt, lässt eine korrekte, spezifische, falsifizierbare Assertion einen Zustand bestätigen, der nicht eintreten kann — der Test besteht also für eine Welt, die es nicht gibt; das ist der Fehlermodus, den `spec/project/test-falsifiability/` [R20] als `T9` katalogisiert, wobei jene Spec die Detektionsroute verantwortet und diese Regel die Anforderung.
- **MUSS [MUST]** die Abweichung **im Double selbst** benennen, wo ein Double nicht treu gemacht werden kann, damit die nächste lesende Person weiß, was der Test nicht abdeckt. Ein untreues Double mit deklarierter Abweichung ist ein begrenzter Trade-off, den ein Reviewer abwägen kann; ein untreues Double mit verschwiegener Abweichung ist der Defekt, und genau diese Asymmetrie macht die Regel oben prüfbar statt bloß wünschenswert.
- **MUSS [MUST]** auf jeder Stufe aus eigenem Recht binden, ob eine Stufen-Spec sie ausführt oder nicht. Eine Stufen-Spec **DARF [MAY]** sie auf die Doubling-Praxis ihrer Stufe anwenden — die Nähte, die sie doubelt, und die dort am häufigsten weggelassenen Constraints — gemäß dem Feld *Isolationsgrad & erlaubte Test-Doubles* der invarianten Stufen-Form, und **DARF dabei NICHT [MUST NOT]** die Regel oder ihre Ausnahme wiederholen. Wo keine Stufen-Spec sie ausführt, wie auf der E2E-Stufe, regiert diese Regel die Doubles jener Stufe unmittelbar, statt zu entfallen.
...
- [ ] Double-Treue ist portfolioweit gefordert (ein Double nie permissiver als das, was es ersetzt, entlang einer herangezogenen Dimension), die Ausnahme verlangt beides — dass das Double dort nicht treu gemacht werden kann und dass die Abweichung deklariert ist —, und die Regel bindet auf jeder Stufe unmittelbar, wobei die Ausführung je Stufe optional ist und die Regel nicht wiederholen darf
