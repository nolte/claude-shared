...
- **MUSS [MUST]** jede einzelne `file` in den Findings in **genau eine** Routing-Klasse klassifizieren, bevor ein Author dispatcht wird. Die Klassen und ihre Auflösung sind:
  ...
  | `rejected` | jeder Pfad, den `lektorat` §Geltungsbereich und Anwendbarkeit ausschließt (unter `spec/`, `skills/**/SKILL.md`, `skills/**/templates/**`, `skills/**/examples/**`, `agents/*.md`, Quellcode, generierte Konfiguration, Binaries) | keiner – **hart abweisen** |
...
- **MUSS [MUST]** für jede geroutete Datei ein **Briefing** komponieren, das der dispatchte Author erhält und das alles davon enthält:
  ...
  2. die **aufgelöste Audience-Menge** der Datei, gewonnen über die **gleiche Prioritätskette** wie `lektorat` §Audience-Bindung (Frontmatter `audience:` → Artefakttyp-Defaults → ganze Audience-Menge); dieser Prozess **DARF NICHT [MUST NOT]** Audiences nach einer anderen Regel auflösen
...
- **MUSS [MUST]** das **Audience-Artefakt** von seinem kanonischen Ort lesen, genau wie `lektorat` §Audience-Bindung vorschreibt, und **MUSS [MUST]** die **Per-Datei**-Remediation (nicht den ganzen Lauf) mit derselben Operator-Meldung anhalten, die auf die `audience-identify`-Skill zeigt, wenn das Artefakt fehlt; der Prozess **DARF NICHT [MUST NOT]** Audiences erfinden und **DARF NICHT [MUST NOT]** einen Author ohne aufgelöste Audience-Menge dispatchen
...
- [ ] Die Audience-Menge wird über die Prioritätskette `lektorat` §Audience-Bindung aufgelöst (Frontmatter → Artefakttyp-Default → ganze Menge), identisch zu einem eigenständigen `lektorat audit`
