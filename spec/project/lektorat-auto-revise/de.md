...
- Findings, Severities (`critical` / `warning` / `suggestion`) oder die fünf Qualitätsdimensionen (D1–D5) definieren, neu klassifizieren oder neu gewichten – alle im Besitz von [`spec/project/lektorat/`](../lektorat/de.md); `Lektorat Auto-Revise` konsumiert den Findings-Report und **DARF NICHT [MUST NOT]** irgendein Feld davon neu definieren
...
- **MUSS [MUST]** für jede geroutete Datei ein **Briefing** komponieren, das der dispatchte Author erhält und das alles davon enthält:
  ...
  4. die **Ziel-Dimensionen** – die einzelnen D1–D5-Werte in den Findings der Datei – sodass der Author weiß, welche Lektorats-Achsen die Revision bewegen muss
...
- [ ] Jeder dispatchte Author erhält ein Briefing, das die Findings der Datei, die aufgelöste Audience-Menge, die gebundene(n) Schreibstil-Spec(s) und die Ziel-Dimensionen D1–D5 enthält
