...
- The documentation route runs **fully autonomously**; the blog route runs **assisted**, preserving the one interactive briefing touchpoint that `blog-author`'s skill contract requires, so the spec is honest about the difference between a dispatchable agent and an interactive skill
...
- **MUST** stop the whole run with an operator-facing message when a file matches **none** of the three classes (an in-scope artefact type this spec doesn't yet route); silently skipping it would hide unremediated findings
- **MUST NOT** make the routing table widenable to any class that `lektorat` forbids; a repository **MAY** add a new routing class only by amending this spec, exactly as `lektorat` gates its scope
...
- **MUST**, for each routed file, compose a **briefing** that the dispatched author receives, containing all of:
  ...
  3. the **bound writing-style spec(s)** for the route (see §Mandatory style and audience binding)
...
- **MUST** bind the **writing-style spec** per routing class and **MUST NOT** dispatch an author without it:
  - `documentation` → [`spec/project/prose-style/`](../prose-style/en.md) (plus the doc-type spec the `audience-doc-author` contract resolves, e.g. `readme-structure`, `release-notes-audience-analysis`)
...
- **MUST** treat a missing **either** input—resolved audience set **or** bound writing-style spec—as a per-file **stop condition**, never a soft default; "no audience" and "no style spec" are not states in which an author may run

### Autonomy and human touchpoints
...
- **MUST** flag a **regression** (post-revision count higher than pre-revision count) to the operator and **MUST NOT** auto-accept a regressed rewrite as converged, even if it cleared the severity floor
...
- **MUST** make any **escalated residual** and any **regressed** file impossible to overlook in `summary.md` (listed first, before converged files); silently truncating unremediated findings is forbidden
...
- **`lektorat-auto-revise` skill**—the orchestrator and only new component. It consumes the audit report, classifies routing, composes briefings, dispatches the existing `audience-doc-author` agent (documentation route) and the existing `blog-author` skill (blog route), re-runs the `lektorat audit` for the convergence gate, owns the assisted-blog operator touchpoint, and writes the `.audits/lektorat-auto-revise/` trail. It is a **skill**, not an agent, because the blog route's interactive briefing touchpoint and the persistent on-disk audit trail are both load-bearing—an agent's fire-and-forget contract would lose the dialogue and the orchestration role
- The skill **MUST NOT** introduce a new scanner; the convergence re-audit reuses the existing `lektorat` audit path (`lektorat-apply` / `lektorat-scanner`), so there is exactly one editorial detection implementation in the portfolio
...
- [ ] A file named in an `inventory_findings` entry of the input report is **not** routed to any author, and the infrastructure condition is surfaced to the operator
...
- [ ] Each dispatched author receives a briefing containing the file's findings, the resolved audience set, the bound writing-style spec(s), and the target D1–D5 dimensions
...
- [ ] The blog route surfaces the findings-derived briefing to `blog-author`, preserves its interactive briefing touchpoint, does not fabricate the briefing inputs `blog-author` requires, and records the route as `assisted`
- [ ] An author revision that drops a code block, list item, table row, checklist entry, link target, frontmatter key, citation, or HTML comment, or that introduces a new command/path/product-name/URL absent from the original, is treated as a failed pass and is not accepted as converged
...
- [ ] A file whose post-revision finding count exceeds its pre-revision count is flagged as a regression to the operator and is not auto-accepted as converged
- [ ] A file that doesn't converge within the bounded number of author passes (default 2) has its residual findings escalated to the operator, and the loop does not continue past the bound
...
- [ ] `summary.md` lists escalated and regressed files before converged files so unremediated findings can't be overlooked
