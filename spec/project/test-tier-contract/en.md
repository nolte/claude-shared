...
- Expressing the portfolio-wide fidelity rule of `spec/project/test-pyramid-foundation/` §"Arrangement fidelity" for this tier's **consumer-side mock**: the requirement below that a consumer's doubles return the **same results the real provider would** is strictly stronger than "no more permissive than the collaborator it replaces," and provider verification enforces it mechanically rather than by review, so restating the weaker floor here would blur which guarantee applies. This exclusion is bounded twice over. It holds for the **replay-verified** flavours (consumer-driven and provider-driven) only; the bi-directional flavour, where no provider code executes, is governed by its own requirement below. And it covers the **mock** only: a **provider state** is an arrangement this exclusion doesn't reach, because replay verifies the interactions the contract records and never the setup that precedes them, so nothing mechanical grades it. That residual exposure is governed by its own requirement under §"Isolation and determinism" rather than left to this entry. [R18]
...
- **MUST** grade the **consumer-side mock** against the portfolio fidelity rule of `spec/project/test-pyramid-foundation/` §"Arrangement fidelity" whenever the project runs the **bi-directional** flavour described above: that flavour verifies the consumer's mock against a published specification with no execution of provider code [R15], so the mock earns none of the mechanical guarantee that replay-based verification supplies, and review is the only check left. Under the replay-verified flavours this grading is unnecessary and the Non-Goals entry above says why.
...
- **MUST NOT** let a **provider state** arrange data the real provider would itself reject, along any dimension the verification relies on. This is this tier's application of `spec/project/test-pyramid-foundation/` §"Arrangement fidelity"; that spec owns the rule and its exemption, and this requirement **MUST NOT** restate either. What's specific here is **why no mechanism catches it**: provider verification replays the interactions the contract records and checks the responses against them, and the state setup runs *before* the first recorded interaction, so it's outside everything the replay grades. A provider state that inserts a row the provider's own write path would refuse therefore produces a green verification for a provider that can't reach that state, and the contract is certified against a provider that doesn't exist. Unlike the consumer-side mock, this exposure survives under **every** flavour, including the replay-verified ones the Non-Goals entry excludes.
- **SHOULD** arrange a provider state through the provider's own write path rather than by writing to its store directly, since that's the cheapest way to inherit the constraints the rule above requires instead of re-implementing them in the setup.
...
- [ ] A provider state is required not to arrange data the real provider would reject, the requirement names why replay verification structurally can't catch it (the setup precedes the first recorded interaction), and it holds under every flavour rather than only the bi-directional one
- [ ] The Non-Goals exclusion of the portfolio fidelity rule is scoped to the consumer-side mock under replay-verified flavours, and states that a provider state isn't covered by it
