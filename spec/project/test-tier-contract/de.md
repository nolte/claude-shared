...
- Die portfolioweite Treue-Regel aus `spec/project/test-pyramid-foundation/` §„Arrangement-Treue“ für den **consumer-seitigen Mock** dieser Stufe auszuprägen: Die Anforderung unten, dass die Doubles eines Consumers **dieselben Ergebnisse liefern wie der echte Provider**, ist strikt stärker als „nicht permissiver als der Kollaborateur, den es ersetzt", und die Provider-Verifikation erzwingt sie maschinell statt per Review, sodass eine Wiederholung der schwächeren Untergrenze hier nur verwischen würde, welche Garantie gilt. Das gilt allein für die **replay-verifizierten** Ausprägungen (consumer-driven und provider-driven); die bi-direktionale Ausprägung, unter der kein Provider-Code läuft, wird von ihrer eigenen Anforderung unten geregelt statt von diesem Ausschluss Diese Ausnahme ist doppelt begrenzt: Sie gilt allein für die **replay-verifizierten** Ausprägungen, und sie deckt allein den **Mock** ab. Ein **Provider-State** ist ein Arrangement, das diese Ausnahme nicht erreicht, denn das Replay verifiziert die Interaktionen, die der Contract aufzeichnet, und nie den Aufbau, der ihnen vorausgeht — mechanisch bewertet ihn also nichts. Jene Restexposition regelt eine eigene Anforderung unter §„Isolation und Determinismus“ statt dieses Eintrags. [R18]
...
- **DARF NICHT [MUST NOT]** zulassen, dass ein **Provider-State** Daten arrangiert, die der echte Provider selbst ablehnen würde, entlang keiner Dimension, auf die sich die Verifikation stützt. Dies ist die Ausprägung von `spec/project/test-pyramid-foundation/` §„Arrangement-Treue“ für diese Stufe; jene Spec verantwortet die Regel und ihre Ausnahme, und diese Anforderung **DARF NICHT [MUST NOT]** das eine oder das andere wiederholen. Spezifisch ist hier, **warum kein Mechanismus das fängt**: Die Provider-Verifikation spielt die vom Contract aufgezeichneten Interaktionen ab und prüft die Antworten dagegen, während der State-Aufbau *vor* der ersten aufgezeichneten Interaktion läuft und damit außerhalb von allem liegt, was das Replay bewertet. Ein Provider-State, der eine Zeile einfügt, die der Schreibpfad des Providers verweigern würde, erzeugt daher eine grüne Verifikation für einen Provider, der diesen Zustand nicht erreichen kann; der Contract ist dann gegen einen Provider zertifiziert, den es nicht gibt. Anders als beim consumer-seitigen Mock überlebt diese Exposition unter **jeder** Ausprägung, auch den replay-verifizierten, die der Non-Goals-Eintrag ausnimmt.
- **SOLLTE [SHOULD]** einen Provider-State über den eigenen Schreibpfad des Providers herstellen statt durch direktes Schreiben in dessen Speicher, denn das ist der billigste Weg, die von der Regel oben geforderten Constraints zu erben, statt sie im Aufbau nachzubauen.
...
- [ ] Ein Provider-State darf keine Daten arrangieren, die der echte Provider ablehnen würde; die Anforderung benennt, warum die Replay-Verifikation das strukturell nicht fangen kann (der Aufbau geht der ersten aufgezeichneten Interaktion voraus), und sie gilt unter jeder Ausprägung statt nur der bi-direktionalen
- [ ] Der Non-Goals-Ausschluss der portfolioweiten Treue-Regel ist auf den consumer-seitigen Mock unter replay-verifizierten Ausprägungen begrenzt und hält fest, dass ein Provider-State nicht davon abgedeckt ist
