...
- **MUST NOT** bypass `main` branch protection; the workflow's job is to publish a release, which then triggers `release-cd-refresh-master.yml`, and updating `main` is that workflow's responsibility rather than this one's. **MUST NOT**, however, read this as a claim that the refresh workflow is permission-complete under `GITHUB_TOKEN`—it isn't whenever the refresh diff touches `.github/workflows/`. `spec/project/workflow-health/` §Known platform constraints owns that constraint and its App-token remediation
