...
- **DARF NICHT [MUST NOT]** den Branch-Schutz von `main` umgehen; Aufgabe des Workflows ist es, ein Release zu publizieren, was dann `release-cd-refresh-master.yml` triggert — das Aktualisieren von `main` verantwortet jener Workflow, nicht dieser. **DARF NICHT [MUST NOT]** dies allerdings als Behauptung lesen, der Refresh-Workflow sei unter `GITHUB_TOKEN` berechtigungsvollständig — das ist er nicht, sobald der Refresh-Diff `.github/workflows/` berührt. Diese Einschränkung samt App-Token-Remediation gehört `spec/project/workflow-health/` §Bekannte Plattform-Einschränkungen
