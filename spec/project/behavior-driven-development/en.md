...
- The derivation **MUST** follow this ordered workflow:
  ...
  4. **MUST** tag each resulting scenario with its source **TC-ID** (for example `@TC-042`) so the requirement-to-scenario link is machine-checkable and bidirectional; the tag is a **hard review gate**, so a scenario without a resolvable TC-ID tag **MUST fail review** rather than merely be flagged. An exploratory scenario that precedes a formal test case first mints its TC-ID in case determination (`spec/project/test-cycle-case-determination/`), so it isn't an exception to the gate—it's traceable by the time it lands.
...
- [ ] The test-case-document to executable-scenario derivation is an ordered MUST/SHOULD workflow: one scenario per TC behavior, precondition/action/result to Given-When-Then, and a TC-ID scenario tag that is a hard review gate (an unresolvable tag fails review) for machine-checkable traceability
...
- ~~Should the TC-ID scenario tag be a hard gate at review time (a scenario without a resolvable TC-ID fails), or a strong SHOULD, given some exploratory scenarios precede a formal test case?~~ **Settled (2026-07-24): hard gate.** A scenario without a resolvable TC-ID tag fails review (step 4 above). This is the only consistent reading of the spec's own normative derivation workflow: every scenario is *derived from* a TC-ID by construction, so an untraceable scenario means its source case wasn't understood—exactly the failure the workflow exists to prevent. Exploratory scenarios are not an exception: they mint their TC-ID in case determination before they land, so the gate never blocks legitimate exploration, it only blocks orphan scenarios.
