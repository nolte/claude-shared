...
- [ ] The test-case-document to executable-scenario derivation is an ordered MUST/SHOULD workflow: one scenario per TC behavior, precondition/action/result to Given-When-Then, and a TC-ID scenario tag that's a hard review gate (an unresolvable tag fails review) for machine-checkable traceability
