...
- **KANN [MAY]** auf `tutorial`- und `how-to`-Seiten einen `prerequisites`-Frontmatter-Schlüssel deklarieren, der das angenommene Wissen oder die vorausgesetzten Seiten auflistet, die ein Leser vor dieser braucht; die Audit-Oberfläche nutzt ihn, um zu verifizieren, dass Prerequisites eine nicht-zyklische Kette bilden (WtD: „Order content cumulatively, covering prerequisites first"). Dieser Schlüssel bleibt `KANN [MAY]`, bis `spec/project/docs-freshness/` eine Prerequisite-Ketten-Drift-Kategorie ausliefert (Zyklus- plus Erreichbarkeits-Erkennung); ihn vorher zu `SOLLTE [SHOULD]` zu verhärten würde eine nicht-erzwingbare Verpflichtung schaffen und die Repo-Konvention brechen, dass jedes Frontmatter-`MUSS [MUST]`/`SOLLTE [SHOULD]` durch eine docs-freshness-Drift-Kategorie gedeckt ist
...
- **Plugin-Erweiterung**: eine projekt-typ-spezifische Spec **KANN [MAY]** zusätzliche MkDocs-Plugins fordern.
  ...
  - Die Erweiterungs-Spec **MUSS [MUST]** erklären, wie das Plugin mit den Basis-Plugins interagiert, insbesondere mit `mkdocs-static-i18n` (das laut §Generierungs-Mechanismus von `spec/claude/skill-agent-catalog/` bekannte Wechselwirkungen mit `mkdocs-gen-files` hat)
  ...
  - Beispiele: `spec/claude/skill-agent-catalog/` verlangt immer `mkdocs-literate-nav`, und `mkdocs-gen-files` nur, wenn die `mkdocs-gen-files`-Invocation-Surface gewählt wird (die `on_pre_build`-Hook- und die eigenständige Pre-Build-Schritt-Surface brauchen über literate-nav hinaus kein Zusatz-Plugin); eine künftige `cookiecutter-template-docs` könnte `mkdocs-include-markdown` verlangen
...
- **MUSS [MUST]** `site_url` in `mkdocs.yml` auf die veröffentlichte Basis-URL der Site setzen. Für die GitHub-Pages-Konvention des Portfolios ist das der Projekt-Site-Subpfad `https://<owner>.github.io/<repo>/` (zum Beispiel `https://nolte.github.io/claude-shared/`); ein Organisations-/User-Pages-Repository mit dem Namen `<owner>.github.io` veröffentlicht unter dem Root `https://<owner>.github.io/`; ein Repository hinter einer eigenen Domain (eine `CNAME`-Datei im Repo-Root oder unter `docs/`) setzt `site_url` auf diese Domain. `mkdocs-static-i18n` leitet die **absoluten** Sprachumschalter-Links und die `<link rel="alternate" hreflang>`-Tags aus `site_url` ab; fehlt `site_url`, fällt das Plugin auf den Site-Root `/` zurück, sodass auf einem Projekt-Subpfad-Deployment Umschalter und `hreflang`-Tags auf den falschen Pfad zeigen (das generierte `https://nolte.github.io/de/` statt des korrekten `https://nolte.github.io/taskfiles/de/`). Dies ist die häufigste stille i18n-Fehlkonfiguration über die Portfolio-Repos hinweg
- **MUSS [MUST]** `site_url` führen, sobald das Plugin `mkdocs-static-i18n` mehr als eine Sprache deklariert; bei einem mehrsprachigen Umschalter ist der Schlüssel tragend statt bloß empfohlen, und ein fehlender Wert liefert Lesern eine kaputte Sprachumschalter-Navigation aus
- **SOLLTE [SHOULD]** `site_url` mit einem abschließenden Schrägstrich (`https://<owner>.github.io/<repo>/`) als kanonische Form abschließen; MkDocs normalisiert einen fehlenden abschließenden Schrägstrich für die Umschalter- und `hreflang`-Ausgabe, der fehlende Slash ist also eine Stil-Kleinigkeit statt eines Build-Bruchs — der Fehlerfall ist das vollständige Fehlen von `site_url`, nicht der fehlende Slash
...
- [ ] Die `mkdocs.yml` jedes Repositories deklariert `site_url` mit der veröffentlichten Basis-URL (per Konvention der GitHub-Pages-Projekt-Subpfad `https://<owner>.github.io/<repo>/`, der Org-/User-Pages-Root oder eine eigene Domain), sodass `mkdocs-static-i18n` korrekte absolute Sprachumschalter- und `hreflang`-Links baut
- [ ] Sobald `mkdocs-static-i18n` mehr als eine Sprache deklariert, ist `site_url` vorhanden (verifiziert durch `mkdocs-structure-apply audit` und als fehlendes/falsches `site_url`-Finding gemeldet; optional durch `docs-freshness` als i18n-Basis-URL-Drift-Warnung aufgezeigt)
...
_Das `prerequisites`-Deferral (von KANN zu SOLLTE) wurde am 2026-06-06 entschieden (Entscheidungslog in der Git-Historie erhalten)._
