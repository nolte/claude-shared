...
- **MUSS [MUST]** Teil-Inhalte einer Quelldatei über Start-/End-Marker einbinden, damit eine spätere Bearbeitung der Quelle die Grenzen des eingebundenen Ausschnitts nicht stillschweigend verschiebt; die kanonische Form ist `{% include-markdown "<relativer-pfad>" start="<start-marker>" end="<end-marker>" %}`, und die Marker verbleiben in der Quelle als Kommentare, angepasst an die Kommentar-Syntax der Datei (`<!-- … -->` für HTML und Markdown, `# …` für YAML und Python, `// …` für JSON-mit-Kommentaren, …). Ein nicht-existenter Marker ist ein Build-Zeit-Fehler, was gewünschtes Verhalten ist, weil es Drift früh aufdeckt
...
- [ ] Kein Paragraph länger als ~3 Zeilen erscheint wortwörtlich auf zwei oder mehr Seiten; jede solche Wiederholung ist durch einen Include aus der kanonischen Quelle via `{% include-markdown … %}` ersetzt
- [ ] Jede `{% include-markdown … start="…" end="…" %}`-Direktive löst zur `mkdocs build --strict`-Zeit erfolgreich auf (ein nicht-existenter Marker brechen den Build)
