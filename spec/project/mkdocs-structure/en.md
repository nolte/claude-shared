...
- **MUST** include partial content from a source file via start/end markers so a future edit to the source doesn't silently shift the included excerpt's bounds; the canonical form is `<opening-tag> include-markdown "<relative-path>" start="<start-marker>" end="<end-marker>" <closing-tag>`, where the `opening_tag` / `closing_tag` pair is pinned per-repository in `mkdocs.yml` (the plugin defaults are `{%` / `%}`; a repository **MAY** pin a different pair, for example `{!` / `!}`, when the default tokens collide with literal occurrences of the directive inside rendered prose, as this repository does because rendered skill-body prose quotes the directive verbatim). The markers stay in the source as comments adapted to the file's comment syntax (`<!-- … -->` for HTML and Markdown, `# …` for YAML and Python, `// …` for JSON-with-comments, …). A non-existent marker is a build-time error, which is the desired behaviour because it catches drift early
...
- **Plugin extension**: a project-type-specific spec **MAY** require additional MkDocs plugins.
  ...
  - The extension spec **MUST** explain how the plugin interacts with the baseline plugins, especially `mkdocs-static-i18n` (which has known interactions with `mkdocs-gen-files` per `spec/claude/skill-agent-catalog/` §Generation mechanism)
  ...
  - Examples: `spec/claude/skill-agent-catalog/` requires `mkdocs-literate-nav` always, and `mkdocs-gen-files` only when the `mkdocs-gen-files` invocation surface is chosen (the `on_pre_build` hook and standalone pre-build step surfaces need neither extra plugin beyond literate-nav); a future `cookiecutter-template-docs` may require `mkdocs-include-markdown`
...
- **MUST** set `site_url` in `mkdocs.yml` to the site's published base URL. For the portfolio's GitHub-Pages convention this is the project-site subpath `https://<owner>.github.io/<repo>/` (for example `https://nolte.github.io/claude-shared/`); an organisation/user-pages repository named `<owner>.github.io` publishes at the root `https://<owner>.github.io/`; a repository served under a custom domain (a `CNAME` file under the repo root or `docs/`) sets `site_url` to that domain. `mkdocs-static-i18n` derives the **absolute** alternate-language switcher links and the `<link rel="alternate" hreflang>` tags from `site_url`; when `site_url` is absent the plugin falls back to the site root `/`, so on a project-subpath deployment the switcher and `hreflang` tags point at the wrong path (the generated `https://nolte.github.io/de/` instead of the correct `https://nolte.github.io/taskfiles/de/`). This is the most common silent i18n misconfiguration across portfolio repos
- **MUST** carry `site_url` whenever the `mkdocs-static-i18n` plugin declares more than one language; with a multi-language switcher the key is load-bearing rather than merely recommended, and a missing value ships broken alternate-language navigation to readers
- **SHOULD** terminate `site_url` with a trailing slash (`https://<owner>.github.io/<repo>/`) as the canonical form; MkDocs normalises a missing trailing slash for the switcher and `hreflang` output, so the absent slash is a style nit rather than a build break—the absence of `site_url` entirely is the failure mode, not the absent slash
...
- [ ] Every repository's `mkdocs.yml` declares `site_url` set to its published base URL (the GitHub-Pages project subpath `https://<owner>.github.io/<repo>/` by convention, the org/user-pages root, or a custom domain), so `mkdocs-static-i18n` builds correct absolute language-switcher and `hreflang` links
- [ ] Whenever `mkdocs-static-i18n` declares more than one language, `site_url` is present (verified by `mkdocs-structure-apply audit` and reported as a missing/incorrect-`site_url` finding; optionally surfaced by `docs-freshness` as an i18n base-URL drift warning)
