...
- **MUST** anchor a probe on its declaration's **content** where the declaration lives in the audited repository, not on the commit it was read at. The squash merge the portfolio mandates gives the change a new commit and drops the branch commit from the default branch's history, while the declaration's content survives it unchanged, so a commit anchor recorded in the same pull request as the declaration change can't be resolved after the merge. A later edit to the declaration then reads as a change, and moving the anchor counts as a re-derivation only when the anchored content changed and the new anchor matches the declaration as the probe's recording commit holds it. An inherited spec stays anchored on its pinned reference; a commit anchor recorded before this rule stays valid until the probe is next re-derived.
- **SHOULD** anchor a probe on the **sections** it cites rather than the whole file where the declaration is a Markdown document and every cited section resolves unambiguously, by a heading number or by the first cell of a table row. A requirement document is edited far more often than any one of its sections: over seventeen edits of one requirement document in a consumer, five probes anchored on the whole file would have gone stale 85 times, and anchored on their sections 11 times. A probe then goes stale when a cited section changes or can no longer be found, and never follows a renumbered or renamed section. A locator that matches more than one place isn't a section anchor, and its probe keeps the file anchor.
- **MUST** offer a **re-confirmation** for a stale probe whose declaration changed without affecting what the probe checks: the operator sees the declaration's change since the anchor, approves each probe separately, and the new anchor is recorded together with a mark that distinguishes the re-confirmation from a re-derivation in the report. A re-confirmation that changes anything of the probe beyond its anchor and its approval is surfaced as a weakening, and moving any anchor stays subject to the same proof as a re-derivation. Probes already anchored on a whole Markdown file are migrated to section anchors once, each approved separately.
...
- [ ] A probe re-derived in the same pull request as its declaration change stays clean after that pull request is squash-merged, a later edit to the declaration makes it stale, and moving its anchor without a change to the anchored content is surfaced as a weakening.
- [ ] An edit outside every section a Markdown-anchored probe cites leaves it clean, an edit inside one of them or a section that can no longer be found makes it stale, and a stale probe can be re-confirmed after the operator has seen the change, with the re-confirmation distinguishable from a re-derivation in the report.
