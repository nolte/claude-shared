...
- **G7—Der Guard behauptet nichts, was der Fix nicht repariert hat.** Ein von einem Fix hinterlassener Guard **DARF NICHT** ein Mitglied der Klasse, das der Fix nicht repariert hat, als gewolltes Verhalten behaupten. Wo ein Fix bewusst partiell ist, **MUSS** der Guard die unreparierten Mitglieder benennen und an ihnen fehlschlagen, sie explizit überspringen oder die Nummer des Folge-Issues an der Assertion tragen. G1 fragt, ob ein Artefakt existiert; G7 fragt, was es sagt. Ein Test, der die unreparierte Hälfte festschreibt, ist schlimmer als kein Test, weil er die Unvollständigkeit als gewollt niederschreibt: Ein späterer Leser, der fragt „ist das abgedeckt?", findet einen grünen, spezifischen, falsifizierbaren Test, der genau das kaputte Verhalten behauptet, und das nächste Vorkommen wird von dem Guard, der dafür existiert, nicht gefunden — der Guard ist der Grund, warum es nicht gefunden wird. Beobachtet: Ein Fix reparierte eine einseitige Kantenlöschung zwei Zeilen über einer bidirektionalen Kante, die auf dieselbe einseitige Weise gelöscht wurde; sein neuer Test schrieb die ausgehende Verankerung für die weiterhin kaputte Kante fest, und sein Integrationscheck fragte hängende Kanten nur für den reparierten Kantentyp ab; der eigene Drei-Regel-Sweep des Autors meldete null verbleibende Verstöße, weil seine Regeln um die reparierte Form herum geschrieben waren
...
- [ ] Kein von einem Fix hinterlassener Guard behauptet ein unrepariertes Mitglied seiner Klasse als gewolltes Verhalten; wo der Fix partiell ist, benennt der Guard die unreparierten Mitglieder und schlägt fehl, überspringt explizit oder trägt das Folge-Issue an der Assertion. Von Hand auf den kamerplanter-PR `#1573` (Issues `#1535`, `#1533`) angewandt, identifizieren die Regeln die ausgehend verankerte Adjazenz-Assertion als den Verstoß; ein Regelwerk, das diesen Pull Request als konform bewertet, ist falsch
...
- G7: PR `#1573` (schließt `#1535` und `#1533`), dessen neuer Test die ausgehende Verankerung der bidirektionalen Kante festschrieb, die der Fix einseitig ließ, und dessen Sweep null Verstöße meldete, weil seine Regeln die reparierte Form matchten; `nolte/claude-shared#640` ist der Arbeitsauftrag
