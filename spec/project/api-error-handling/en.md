...
- Make the check cheap to re-run repeatedly within a sprint, because it is side-effect-free
...
- Defining a project's error contract: the check measures against whatever contract the project declares, it does not impose a canonical schema on projects that have none
- Runtime fault injection or live API probing; the check is static (reads source, does not call the running API)
...
- **MUST** accept a target that is either an explicit handler/router file or directory path, or a requirement/feature identifier the project uses to group endpoints (for example `REQ-013`); when given an identifier, resolve it to the owning handler files via the project's own layout rather than a hard-coded path
...
- **MUST** run a **leakage scan** for internal detail reaching the response body: raw exception/stringified-driver messages, stack traces or tracebacks, rendered database queries, internal host/path/config values, and secrets. Each hit is a security-relevant finding (**critical**), attributed to file:line, and flagged as a pointer into `spec/project/code-security-audit/`
...
- **MUST** attribute each finding to a source location (file and line) so it is actionable
