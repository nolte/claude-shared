...
- Der normative Inhalt der Spec **MUSS** gegen die Anker in §Referenzen gelesen werden: der **OpenSSF Best Practices Badge** [R11] als Präzedenz für eine gestufte Projekt-Qualitätsleiter (seine eigene Metal-Serie läuft passing/silver/gold, deren Form die Bronze/Silber/Gold-Rubrik dieser Spec spiegelt, nicht deren Kriterien); **ISO/IEC 25010** [R8] als das Code-Qualitätsmodell hinter Achse B; **McCabes zyklomatische Komplexität** [R9] als das Wartbarkeits-Signal, auf das Achse B verweist; das portfolio-eigene **Testpyramiden-Fundament** [R2] (und Fowlers Coverage-als-Leitfaden-Vorbehalt [R10]) als Basis von Achse C; und **CMMI** [R12], nur benannt, um Produkt- von Prozess-Reifegrad abzugrenzen
- Jede externe Framework-Zuschreibung ([R8]–[R12]) ist eine Autorenzeitpunkt-externe-Aussage, die gemäß `spec/claude/research-triangulate/` ihre eigene triangulierte Quellenliste in §Referenzen trägt (jeweils mindestens drei unabhängige Quellen, abgerufen 2026-07-24), und **SOLLTE** gegen aktuelle Quellen nachvalidiert werden, sobald diese Spec über `draft` hinaus befördert wird oder ihre Quellen ihre empfohlene Lebensdauer überschreiten; die internen Spec-Referenzen ([R1]–[R7]) sind tragende Querverweise, keine externen Behauptungen
...
Die externen Framework-Anker [R8]–[R12] sind Author-Time-externe Aussagen, trianguliert gemäß `spec/claude/research-triangulate/` §"Author-time assertions" (Author-Time-Stufe: mindestens drei unabhängige Quellen, Primary-first geordnet). Abrufdatum für jede externe Quelle unten: 2026-07-24.

- [R8] ISO/IEC 25010 — Systems and software Quality Requirements and Evaluation (SQuaRE), Produktqualitätsmodell. Die aktuelle zweite Ausgabe, **ISO/IEC 25010:2023**, definiert **neun** Charakteristiken statt der acht der zurückgezogenen Ausgabe :2011: Safety kam hinzu, aus Usability wurde Interaction Capability, aus Portability wurde Flexibility, und die Quality-in-use verlagerte sich nach ISO/IEC 25019:2023. Wartbarkeit, die Charakteristik, auf die sich Achse B stützt, behält ihre fünf Unter-Charakteristiken (Modularität, Wiederverwendbarkeit, Analysierbarkeit, Änderbarkeit, Testbarkeit).
  - ISO/IEC-25010-Portal mit dem Charakteristik-Baum der Ausgabe :2023 inklusive der Wartbarkeits-Unter-Charakteristiken (Secondary): <https://iso25000.com/index.php/en/iso-25000-standards/iso-25010>
  - arc42-Qualitätsmodell, „ISO/IEC 25010", mit Annotation der umbenannten Charakteristiken (Secondary): <https://quality.arc42.org/standards/iso-25010>
  - arc42, „Update on ISO 25010, version 2023", das die Revision auf November 2023 datiert und die Unter-Charakteristik-Änderungen auflistet (Secondary): <https://quality.arc42.org/articles/iso-25010-update-2023>
...
- [R9] Thomas J. McCabe, *A Complexity Measure*, IEEE Transactions on Software Engineering, Band SE-2, Nr. 4, S. 308–320, Dezember 1976 (`doi:10.1109/TSE.1976.233837`); das Paper selbst liegt hinter einer Verlags-Paywall, weshalb die zitierbare Spur die darauf aufbauende Normungs-Methodik plus Tool-Hersteller-Dokumentation der Metrik ist.
  - NIST Special Publication 500-235, *Structured Testing: A Testing Methodology Using the Cyclomatic Complexity Metric* (Watson/McCabe), die Normungs-Behandlung des Maßes von 1976 (Primary): <https://www.nist.gov/publications/structured-testing-testing-methodology-using-cyclomatic-complexity-metric>
  - Microsoft Learn, „Code metrics — cyclomatic complexity", das die Metrik als Testbarkeits- und Wartbarkeits-Signal mit praktischer Obergrenze dokumentiert (Secondary): <https://learn.microsoft.com/en-us/visualstudio/code-quality/code-metrics-cyclomatic-complexity>
  - Überblick zur zyklomatischen Komplexität mit dem vollständigen bibliografischen Nachweis des Papers von 1976 (Tertiary): <https://en.wikipedia.org/wiki/Cyclomatic_complexity>
- [R10] Martin Fowler, *TestCoverage* (Coverage als Leitfaden, nicht als Zielwert): <https://martinfowler.com/bliki/TestCoverage.html> (Primary)
  - Laura Inozemtseva und Reid Holmes, *Coverage Is Not Strongly Correlated with Test Suite Effectiveness*, ICSE 2014 („should not be used as a quality target") (Primary, peer-reviewed): <https://www.cs.ubc.ca/~rtholmes/papers/icse_2014_inozemtseva.pdf>
  - Google Testing Blog, „Code Coverage Goal: 80% and No Less!", die Parabel gegen starre Coverage-Vorgaben (Primary, unabhängige Praktiker-Organisation): <https://testing.googleblog.com/2010/07/code-coverage-goal-80-and-no-less.html>
- [R11] OpenSSF Best Practices Badge Program. Die gestufte Metal-Serie läuft **passing / silver / gold** und ist kumulativ (Gold setzt Silber voraus); seit der Übernahme durch die OpenSSF trägt das Programm zusätzlich eine separate Baseline-Serie (Stufen 1–3), sodass „die Badge-Stufen" keine einzelne Leiter mehr sind.
  - OpenSSF-Best-Practices-Badge-Kriterien, die beide Serien führen (Primary): <https://www.bestpractices.dev/en/criteria>
  - OpenSSF-Projektseite des Best Practices Badge, die Metal- von Baseline-Serie unterscheidet (Primary): <https://openssf.org/projects/best-practices-badge/>
  - Repository `ossf/best-practices-badge`, das die Umbenennung vom CII Best Practices Badge festhält (Primary): <https://github.com/ossf/best-practices-badge>
- [R12] CMMI (Capability Maturity Model Integration) — Prozess-Capability-Reifegrad-Stufen, nur benannt, um Produkt- von Prozess-Reifegrad abzugrenzen. Die Reifegrad-Stufen 1–5 (Initial, Managed, Defined, Quantitatively Managed, Optimizing) sind gestaffelt und organisationsweit, wobei V2.0/V3.0 eine Stufe 0 „Incomplete" ergänzen; die Capability-Stufen sind die kontinuierliche Darstellung je Practice Area. Das Modell wird von der ISACA betreut (die das CMMI Institute 2016 übernommen hat), und CMMI V3.0 ist seit dem 2024-01-01 die einzige Appraisal-Grundlage. CMMI **beurteilt** (appraisal), es zertifiziert nicht.
  - Carnegie Mellon SEI, „Transforming software quality assessment", der Ursprungs-Nachweis der CMM-/CMMI-Linie (Primary): <https://www.sei.cmu.edu/history-of-innovation/transforming-software-quality-assessment/>
  - CIO, „What is CMMI? A model for optimizing development processes", mit der ISACA-Übernahme und dem V3.0-Release 2023 (Secondary): <https://www.cio.com/article/274530/cmmi-explained.html>
  - Core Business Solutions, „CMMI V3.0 update explained", das V3.0 auf April 2023 und den Appraisal-Umstieg auf den 2024-01-01 datiert (Secondary): <https://www.thecoresolution.com/cmmi-v3-update-explained>
  - Überblick zur Capability Maturity Model Integration mit der gestaffelten und der kontinuierlichen Darstellung (Tertiary): <https://en.wikipedia.org/wiki/Capability_Maturity_Model_Integration>

Verifiziert 2026-07-24. Zwei Anker haben sich seit dem Entwurf dieser Spec bewegt, und die Einträge oben sind korrigiert statt bloß belegt: ISO/IEC 25010 trägt in der Ausgabe :2023 neun Charakteristiken (die frühere Formulierung „acht Charakteristiken" beschrieb das zurückgezogene Modell :2011), und der OpenSSF-Badge ist keine einzelne passing/silver/gold-Leiter mehr. Eine Quellen-Grenze ist bewusst festgehalten: Jede primäre CMMI-Oberfläche (die Modellseiten der ISACA, `cmmiinstitute.com` und die zurückgezogenen SEI-Berichte zu CMMI for Development) war am Abrufdatum für automatisierte Abrufe nicht erreichbar, sodass [R12] auf einem primären Ursprungs-Nachweis plus unabhängiger sekundärer Berichterstattung ruht.
