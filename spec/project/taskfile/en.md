...
Established 2026-08-17 by direct observation against Task **3.52.0**, against the collection's MkDocs module before and after the `default` form was applied. Before: an override passed as include-level `vars:`, as the including Taskfile's top-level `vars:`, as a command-line variable assignment, as an environment variable, and through the including Taskfile's `env:` block each left the command resolving to the module default `source ~/.venvs/docs/bin/activate`. After: the same include-level override resolved to `source .venv/bin/activate`, while an invocation passing no override still resolved to `source ~/.venvs/docs/bin/activate`. Passing only the base variable `PYTHON_VENVS_BASEDIR` left the derived value at the module default in both states, which is the leaf-only limitation recorded above. The shipped `templates/Taskfile.yml` was then run end to end against a scratch MkDocs project: `task install` followed by `task docs` produced `site/index.html` through the included `mkdocs:build`, using the project-local `.venv`.
