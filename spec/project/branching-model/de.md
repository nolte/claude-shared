...
- **MUSS [MUST]** Feature-Branches mit einem der Präfixe `feat/`, `fix/`, `chore/`, `docs/` oder `exp/` benennen und in ihrem Pull Request auf `develop` zielen; diese Präfixe sind identisch mit den in PR-Titeln verwendeten Conventional-Commits-Types, sodass Branch-Name und Commit-Type ohne Übersetzung zueinander passen; ein nach `spec/project/pull-request-workflow/` §PR-Rahmenbedingungen freigegebener Dependency-Bot legt Branches nach seinem eigenen Namensschema an und fällt nicht unter diese Regel
...
- [ ] Feature-Branches im Repository verwenden einen der Präfixe `feat/`, `fix/`, `chore/`, `docs/`, `exp/`, abgesehen von Branches, die ein freigegebener Dependency-Bot öffnet
