...
- **MUSS [MUST]** jeden Mechanik-Befund als False Positive behandeln — aufgelöst über den Skip-and-Record-Pfad in §Operationen, nie per Edit —, dessen Auflösung die **kanonische Bezeichnung eines externen Standards oder einer Spezifikation** umbenennen würde (auch innerhalb einer Überschrift, deren Slug gemäß §Refactor-Sicherheit tragend ist) oder ein **etabliertes technisches Idiom** durch eine Publikums-Substitution ersetzen würde, die im Fachregister fehlgreift; kanonische Bezeichnungen und Fach-Idiome stechen allgemeine Stilregeln

#### D4 — Schreibstil

- **MUSS [MUST]** Schreibstil für englischen Text gegen den **anwendbaren** Ausschnitt von `prose-style` §Stimme und Ton bewerten; die dort gelisteten Regeln sind autoritativ und `Lektorat` definiert sie nicht neu
...
- **DARF [MAY]** pro Repository durch eine deklarierte **Haus-Register-Konvention** überschrieben werden: Ein Repository, dessen DE-Dokumentation portfolioweit die informelle `du`-Anrede übernimmt (passend zum üblichen deutschen OSS-Developer-Doc-Register), deklariert diesen Override einmalig, und `Lektorat` behandelt dann eine **durchgängig**-`du`-DE-Seite als konform statt als D4-`Sie`-Default-Verstoß — die `du`/`Sie`-**Inkonsistenz**-Regel (gemischte Anrede auf einer Seite, gemäß dem nächsten Punkt) gilt weiterhin. Dieses Repository (`claude-shared`) übernimmt den `du`-Override; festgehalten aus dem Voll-Audit vom 2026-07-24 (`.audits/lektorat/2026-07-24-1513/`, Befund `lekt-44b1502b7a`)
...
- **MUSS [MUST]** diese Kleinschreibungs-Namen (`critical` / `warning` / `suggestion`, mit bewusst weggelassenem `info`) als die maschinengelesene Editorial-Serialisierung behandeln, sanktioniert durch den Editorial-Sub-Skala-Carve-out in `spec/claude/review-plan/` §Schweregrad-Skala; jeder bildet eins-zu-eins auf die Title-Case-Stufen jener Spec ab
...
- **MUSS [MUST]** **englisch-only-Mechanik** (Vale, prose-style §Stimme und Ton, FRE/FKGL) auf englisch-aufgelöste Dateien anwenden und **deutsch-only-Mechanik** (DE-Rechtschreib/Grammatik-Pipeline, WSTF/LIX, deutsche Ton-Heuristiken) auf deutsch-aufgelöste Dateien
- **MUSS [MUST]** YAML-Frontmatter, umzäunte Code-Blöcke, Inline-Code-Spans, HTML-Kommentare, Emoji- und Icon-Shortcodes (`:name:`-Tokens, die als Symbole gerendert werden) und Markdown-Link- / Bild-Ziele aus der Prosa entfernen, **bevor** der Text an eine satzweise Grammatik-Pipeline geht (Vale auf Englisch, die DE-Pipeline auf Deutsch); die entfernten Tokens **MÜSSEN [MUST]** byte-restlos entfernt werden (keine Ersetzung durch Whitespace), damit die Pipeline Strip-Artefakte nicht als Typografie-Befunde interpretieren kann. Die Zeilennummerierung des gestrippten Textes **MUSS [MUST]** auf die Quelldatei zurückführbar bleiben (Leerzeilen stehen für entfernte strukturelle Elemente), damit Befund-Positionen operator-auditierbar bleiben
- **MUSS [MUST]** jede Markdown-Struktureinheit — eine Überschrift, ein Listen-Item, eine Tabellenzelle, eine Blockquote-Zeile — als **eigenständiges Segment** für jede satzbezogene Regel behandeln (Satzanfangs-Großschreibung, terminale Interpunktion, Satzlänge, Fragezeichen-Platzierung): Struktureinheiten werden nie zu synthetischen Sätzen konkateniert, sodass eine Groß-/Kleinschreib- oder Interpunktionsregel nicht über eine Strukturgrenze hinweg feuern kann. Token-bezogene Regeln (Rechtschreibung, Geschützte-Begriffe-Abgleich) gelten weiterhin innerhalb jeder Einheit, auch in Tabellenzellen
- **MUSS [MUST]** Mechanik-Befunde, deren gematchte Spanne direkt an eine Strip-Stelle grenzt, als Pipeline-Artefakte unterdrücken: das Residuum eines entfernten Elements ist keine verfasste Prosa (die typische Klasse ist ein zielsprachliches Kompositum, das an einen entfernten Inline-Code-Span angedockt war, das Stripping als hängendes Bindestrich-Fragment wie `-Ordner` überlebt und dann Bindestrich-Kompositum-Regeln auslöst). Ein solcher Befund belastet die Pipeline, nicht den Text, und **DARF NICHT [MUST NOT]** dem Operator angezeigt werden
