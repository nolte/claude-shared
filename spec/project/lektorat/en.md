...
- Should `Lektorat` extend its scope to **API reference text generated from source** (typedoc, sphinx, godoc output)? Revisit when a `nolte/*` portfolio repository ships a generated API-reference site (typedoc/sphinx/godoc output rendered into `docs/`) whose page frontmatter `audience:` resolves to at least one non-developer audience ID (track other than `developer-docs`) per the audience artefact. The upstream signal to watch is the first `library-api-docs`-style `mkdocs-structure` extension spec (named as a future extension in [`spec/project/mkdocs-structure/`](../mkdocs-structure/en.md) §Discovery and cross-referencing) being authored—that's when a non-developer reference surface is about to land. Until such a repo exists, there's no usage data to decide against the standing default of **no** (generated reference text is already excluded by §Scope and applicability and the §Non-Goals Markdown-prose boundary).
- Should the `lektorat-scanner` agent be **dispatchable in parallel per file** or **batched** (one agent run for the whole repository)? Default: dispatch batched (one scanner run for the whole in-scope set per audit), the lower-overhead choice that aggregates `language_summary` and applies the §Coordination Vale-rule-ID de-duplication in a single pass; per-file (optionally parallel) dispatch stays legal and produces identical JSON. Revisit when the first real `audit` against a representative bilingual repo (at least 20 in-scope Markdown files across `docs/en` + `docs/de`) records a wall-clock and token-cost comparison of batched versus per-file dispatch that shows batched p95 latency above an operator-acceptable bound (suggested: above 5 minutes per audit) or token cost per audit above the per-file-parallel alternative by a measurable margin. One editorial `audit` run exists (`.audits/lektorat/2026-05-30-0727`), but it records no batched-versus-per-file dispatch comparison, so the revisit condition is unmet and the batched default stands.
