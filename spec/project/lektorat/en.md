...
- Skills and agents that perform editorial work distinguish three operations — `audit` (read-only), `patch` (per-finding edit with explicit approval), `revise` (full-document overhaul with diff review) — and the operation chosen is visible in their output
- Editorial findings are reproducibly classified by **severity** (`critical` / `warning` / `suggestion`) so a downstream gate (CI, sprint-review, release-publish) can decide what blocks and what is advisory
...
- DE-text and EN-text are checked with **language-appropriate mechanics**; the DE pipeline does not depend on Vale, which `prose-style` scopes to English only
...
- Defining or maintaining the Vale configuration, the Vale vocabulary, or the prose rule mechanics — that is owned by [`spec/project/prose-style/`](../prose-style/en.md) and the upstream package at [`nolte/vale-style`](https://github.com/nolte/vale-style)
- First authorship of new pages — that is owned by the `audience-doc-author` agent and its triggering skills (`readme-structure-apply`, `audience-doc-author` itself); `Lektorat` operates on prose that already exists
- Translating prose from one language into another — that is owned by the `spec` skill (for `spec/` content), by `docs-multilingual-authoring` (for atomic per-page DE/EN parity), and by `audience-doc-author` (for new pages); `Lektorat` reviews each language independently
- Synchronising parity between DE and EN versions of the same artefact — that is owned by `docs-freshness` (audit-time detection) and `docs-multilingual-authoring` (authoring-time prevention)
...
- Vale-rule authoring (active-voice detector, gendered-pronoun detector, etc.) — `prose-style` already lists this as a deferred decision and `Lektorat` does not pre-empt it
- Slack messages, wiki pages, blog posts, or other non-Markdown human-facing prose surfaces — `Lektorat` covers GitHub Issue and pull-request bodies as a deliberate scope extension (they are user-visible Markdown that flows into search engines and project history). Note that `prose-style` §Pull-request descriptions and release notes already mandates Vale coverage on EN PR bodies and EN release-note bodies; `Lektorat` does **not** introduce coverage there but extends it with the D1/D2/D5 dimensions and the DE pipeline. Findings from `prose-style`'s Vale CI gate are deduplicated by Vale rule ID per §Coordination with neighbouring specs. Other prose surfaces remain out of scope until separately specified
...
The five dimensions below are the **authoritative** list. Each finding produced by a `Lektorat` operation **MUST** name exactly one dimension. Severities map to a dimension-specific rubric defined under §Severity classification.
...
- **MUST** declare per-`content_mode` target corridors so a `tutorial` page is not held to the same density as a `reference` page; the default corridors are:
...
- **MUST NOT** rewrite a passage on readability grounds alone in `patch` mode without a metric value or a named heuristic citation in the finding; an opinion is not a finding
...
- **MUST** detect and flag the following patterns as comprehensibility findings:
  - **Jargon load** — a domain term appearing without prior definition, where “domain term” is anything not in the project's audience-appropriate base vocabulary (per §Audience binding)
  - **Unexplained abbreviations** — an abbreviation (`SRE`, `RTO`, `CSP`) appearing without expansion on first use in the page
  - **Hidden prerequisites** — an instruction or claim that depends on an earlier file, an environment state, or a tool that is not mentioned on the current page
...
- **MUST** prefer the **add-context** patch (one short sentence, one inline expansion) over the **delete-jargon** patch when a domain term is genuinely load-bearing; comprehensibility findings are not licence to strip technical precision
- **MUST NOT** flag a term that is glossed elsewhere in the page (definition list, prior section, inline parenthetical) as a comprehensibility finding
...
- **MUST** apply **language-specific** spelling and grammar checks:
  ...
  - **German text** — apply a `Lektorat`-owned DE pipeline (portfolio default: LanguageTool HTTP API; see §Language handling) because `prose-style` explicitly scopes Vale to English only and a DE alternative is not available portfolio-wide
...
- **MUST** evaluate writing style against the **applicable** subset of `prose-style` §Voice and tone for English text; the rules listed there are authoritative and `Lektorat` does not redefine them
...
- **MUST** flag audience-fit findings under the following patterns:
  ...
  - **Missing audience-required content** — an artefact whose declared audience expects a specific section (per `docs-audience-tracks` content blocks) that is absent or empty
  - **Wrong-audience content** — a section that targets an audience the page does not declare (typical: a contributor-oriented appendix on an end-user quickstart)
- **MUST** classify register mismatch and missing audience-required content as `critical` for any artefact whose declared audience includes a non-operator audience (end-users, customers, evaluators); a documentation gap that a paying consumer notices is not a `suggestion`
...
- **MUST** classify every finding into one of exactly three severities:
  ...
  - `warning` — fails a named metric corridor, fails a `prose-style` MUST that did not flip into `critical`, or breaks internal consistency
...
- **MUST** keep severity classification **dimension-aware**: a D3 misspelling in a published release note is `critical`, the same misspelling in a draft Markdown comment is `warning`, the same misspelling inside a code identifier is **not a finding** (out of scope per §Scope and applicability)
...
- **MUST** offer a "skip" and a "skip-and-record" path so the operator can defer or permanently dismiss a finding, with the dismissal recorded so future `audit` runs do not re-surface it
- **SHOULD** present findings to the operator in severity order so `critical` items are handled first

#### Operation C — `revise`

- **MUST** rewrite the **full artefact** in one pass, addressing every `critical` and `warning` finding the prior `audit` produced; `suggestion` findings are optional and **SHOULD** be addressed when adopting them does not extend the rewrite scope
...
- **MUST NOT** introduce new factual content (new commands, new file paths, new product names, new URLs) that was not present in the original; if the prose required an addition, the operation **MUST** surface a `suggestion` to the operator and stop
...
- **MUST** resolve each artefact's **applicable audiences** as follows (in priority order):
  ...
  3. The **whole audience set** declared in the artefact — for any in-scope artefact that the prior two rules do not match
...
- **MUST** select the rule set **per file** based on the file's language, resolved in this priority:
  ...
  4. As a last resort, the operator chooses interactively; `Lektorat` **MUST NOT** auto-detect language from text content for scope decisions
...
- **MUST NOT** reorder, merge, or split list items in any operation; lexical edits inside a bullet are allowed, structural edits across bullets are not
...
- **MUST** surface every infrastructure-level scan condition in the `inventory_findings` array, **never** in `findings`. The `findings` array carries only editorial findings classified under the closed severity set (`critical` / `warning` / `suggestion`) of §Severity classification; `inventory_findings` carries pre-evaluation conditions that prevented part of the scan from completing. The `kind` field is a closed enumeration with exactly these five values:
  ...
  - `language-pipeline-missing` — German files are in scope but no DE pipeline config was passed (or the configured endpoint/binary is not callable); D3 for the affected file is skipped. `file` names the affected file; emit one entry per affected file.
  - `language-ambiguous` — the language-resolution priority chain (see §Language handling) cannot resolve the file; the operator decides interactively. `file` names the affected file.
...
- **MUST** label the diff with the operation name, the finding ID(s) it addresses, and the affected file's repo-relative path

### Skill and agent distribution (recommendation)

The spec does **not** mandate the exact implementation shape, but **SHOULD** be implemented as the following split, mirroring the portfolio's hybrid pattern (e.g. `dependency-audit` skill + `dependency-audit-scanner` agent, `vocab-drift-audit` skill + `vocab-drift-scanner` agent):
...
- [ ] A `patch` invocation offers a "skip" and a "skip-and-record" path; a recorded dismissal does not re-surface in subsequent `audit` runs
...
- [ ] A `revise` or `patch` invocation preserves the order and count of every list item, table row, and checklist entry; lexical edits within a single item are allowed, structural cross-item edits are not
...
- [ ] Every Markdown link's `[text](target)` is byte-identical across every operation that does not explicitly produce a finding against that link
...
- Should `Lektorat` produce **diff annotations on pull requests** (for example as PR-line comments) in addition to the `.audits/lektorat/` audit trail? Default for now: **no**, the audit trail is the contract; PR-line comments are a downstream rendering decision that does not belong in this spec.
...
- Bamberger, R., Vanecek, E. (1984). *Lesen — Verstehen — Lernen — Schreiben.* — Wiener Sachtextformel (WSTF) variant definitions.
- Björnsson, C. H. (1968). *Läsbarhet.* — LIX (Läsbarhetsindex) definition; portable to German per readability research consensus.
