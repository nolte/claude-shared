...
- Slack messages, wiki pages, or other non-Markdown human-facing prose surfaces: `Lektorat` covers GitHub Issue and pull-request bodies as a deliberate scope extension (they're user-visible Markdown that flows into search engines and project history). Note that `prose-style` §Pull-request titles and release notes mandates Vale on EN pull-request titles, verifies EN release notes advisory, and deliberately doesn't gate pull-request bodies with Vale; `Lektorat` therefore stays the prose review surface for bodies and adds the D1/D2/D5 dimensions and the DE pipeline. Findings from `prose-style`'s Vale CI gate are deduplicated by Vale rule ID per §Coordination with neighbouring specs. Other prose surfaces remain out of scope until separately specified. **Blog posts** (Markdown under a consumer's content tree, for example `src/content/posts/`) are out of scope **by default**, but a consumer that adopts `spec/project/blog-author/` MAY opt them into scope via the opt-in consumer-surface extension in §Scope and applicability
...
- **MAY** additionally treat Markdown blog posts under a consumer-declared content path (for example `src/content/posts/`) as in scope **when the consumer adopts `spec/project/blog-author/`** (whose §Handover routes target-state route runs `lektorat-apply` over the post pair); this opt-in consumer-surface extension is the one exception to the widen-scope prohibition below, and a consumer that doesn't adopt `blog-author` keeps blog posts out of scope
...
- **MUST** allow a repository to narrow the in-scope set via a `Lektorat`-local configuration (path globs, artefact-type allow-list) but **MUST NOT** allow a repository to widen the scope to include any artefact type that the spec explicitly forbids above, **except** via the opt-in consumer-surface extension declared above (blog posts under `blog-author` adoption)
...
- **MUST** detect and flag the following patterns as comprehensibility findings:
  ...
  - **Implicit assumptions**: a sentence that presumes the reader's role, tooling, or background without saying so. The marker-word list lives in [`spec/project/lektorat/markers-en.yml`](markers-en.yml) (English) and [`spec/project/lektorat/markers-de.yml`](markers-de.yml) (German) as a versioned, maintainable list rather than an inline enumeration; per-entry `severity_floor` (`suggestion` or `warning`) governs the highest severity the marker can reach under the §D2 escalation table
...
- **MUST** treat as a false positive—resolved through the skip-and-record path in §Operations, never by edit—any mechanics finding whose resolution would rename the **canonical designation of an external standard or specification** (including inside a heading, where the slug is load-bearing per §Refactor safety) or replace an **established technical idiom** with a general-audience substitution that misfires in the domain register; canonical designations and domain idioms outrank general-audience style rules
...
- **MAY** be overridden per repository by a declared **house register convention**: a repository whose DE documentation adopts the informal `du`-address portfolio-wide (matching common German OSS developer-doc register) declares that override once, and `Lektorat` then treats a **consistently**-`du` DE page as conformant rather than a D4 `Sie`-default violation—the `du`/`Sie` **inconsistency** rule (mixed address on one page, per the next bullet) still applies. This repository (`claude-shared`) adopts the `du` override; recorded from the 2026-07-24 full audit (`.audits/lektorat/2026-07-24-1513/`, finding `lekt-44b1502b7a`)
...
- **MUST** treat these lowercase names (`critical` / `warning` / `suggestion`, with `info` deliberately omitted) as the machine-read editorial serialisation sanctioned by the editorial sub-scale carve-out in `spec/claude/review-plan/` §Severity scale; each maps one-to-one onto that spec's Title-Case levels
...
- **MUST** strip YAML frontmatter, fenced code blocks, inline-code spans, HTML comments, emoji- and icon-shortcode tokens (`:name:`, rendered as symbols), and Markdown link / image targets from prose before passing the text to a sentence-level grammar pipeline (Vale on English, the DE pipeline on German); the stripped tokens **MUST** be removed byte-restlessly (no whitespace substitution) so the pipeline can't interpret strip artefacts as typography findings. The line numbering of the stripped text **MUST** map back to the source file (empty lines stand in for removed structural elements) so finding positions remain operator-auditable
- **MUST** treat each Markdown structural unit—a heading, a list item, a table cell, a block-quote line—as an **independent segment** for every sentence-scoped rule (sentence-start capitalisation, terminal punctuation, sentence length, question-mark placement): structural units are never concatenated into synthetic sentences, so a case or punctuation rule can't fire across a structural boundary. Token-scoped rules (spelling, protected-terms matching) still apply inside every unit, including table cells
- **MUST** suppress, as pipeline artefacts, mechanics findings whose matched span directly abuts a strip site: the residue of a removed element isn't authored prose (the typical class is a host-language compound docked onto a removed inline-code span, which survives stripping as a dangling hyphen fragment such as `-Ordner` and then triggers hyphen-compound rules). Such a finding indicts the pipeline, not the text, and **MUST NOT** be surfaced to the operator
...
- **MUST** surface every infrastructure-level scan condition in the `inventory_findings` array, **never** in `findings`. The `findings` array carries only editorial findings classified under the closed severity set (`critical` / `warning` / `suggestion`) of §Severity classification; `inventory_findings` carries pre-evaluation conditions that prevented part of the scan from completing. The `kind` field is a closed enumeration with exactly these five values:
  ...
  - `content-mode-missing`: the file is a `docs/<lang>/`-tree page that has no `content_mode` in the caller-supplied map; D1 for that file is skipped (`meta`-exemption depends on a known mode). `file` names the affected file. Top-level repository Markdown (`README.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CHANGELOG.md`, `ONBOARDING.md`) **doesn't** emit this inventory finding—it defaults to `content_mode: meta` per §D1, regardless of whether a frontmatter key is present.
