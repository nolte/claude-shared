...
- **MUST** fail the check, when `## Linked issues` references an issue carrying the audit label, if **Risk / rollout notes** carries no `Originating source:` or no `Dispatched specialist:` line with a value, or if the `Dispatched specialist:` value records a named specialist as not dispatched, which is neither form `spec/project/continuous-improvement/` allows. The lint tells the two forms apart structurally: a value that opens with `no matching specialist existed` is the no-match form and isn't scanned further, and any other value names the specialist that produced the fix and fails when it records a dispatch as not having happened. Contradictory prose after an opening no-match note stays with the coverage review. The linter hands only the issue numbers it parsed from `## Linked issues` to the step that reads their labels through the API; the body never reaches a shell command. The same body **MUST** pass when no linked issue carries the label
...
- [ ] The body check fails a PR whose `## Linked issues` references an audit-labelled issue when **Risk / rollout notes** lacks `Originating source:` or `Dispatched specialist:`, or records a named specialist as not dispatched, and passes the same body when no linked issue carries the label
