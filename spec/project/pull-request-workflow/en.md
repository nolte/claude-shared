...
- **MUST** fail the check, when `## Linked issues` references an issue carrying the audit label, if **Risk / rollout notes** carries no `Originating source:` or no `Dispatched specialist:` line with a value, or if the `Dispatched specialist:` value records a named specialist as not dispatched, which is neither form `spec/project/continuous-improvement/` allows. The linter hands only the issue numbers it parsed from `## Linked issues` to the step that reads their labels through the API; the body never reaches a shell command. The same body **MUST** pass when no linked issue carries the label
