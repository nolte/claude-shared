...
**Enforcement-Status (offene Frage—siehe §Offene Fragen, „Frontmatter-Schema-Impact").** Kriterien `a-1` und `a-2` verweisen auf Frontmatter-Felder (`primaryAudience`, `secondaryAudiences`), die das Static-Site-Schema des Konsumenten möglicherweise noch nicht deklariert. Bis die Schema-Lücke des Konsumenten geschlossen ist, gelten `a-1` und `a-2` für Posts, die nach der Übernahme dieser Spec durch den Konsumenten verfasst oder aktualisiert werden, und bleiben autoreseitige Konventionen, die der Build nicht erzwingen wird. Legacy-Posts vor diesem Übergang sind ausgenommen; Reviewer und Lint-Skills **MÜSSEN [MUST]** die Abwesenheit dieser Felder auf einem Legacy-Post als außerhalb des Spec-Scopes liegend behandeln statt als fehlgeschlagenes Kriterium. Ein Reviewer oder eine Lint-Skill, die auf einen Post ohne `primaryAudience` trifft, **SOLLTE [SHOULD]** ihn als `primaryAudience: A` (technischer Peer-Leser) behandeln, die häufigste Form des Korpus; dieser Default ist es, den die Schema-Migration des Konsumenten erbt.
...
- **Frontmatter-Schema-Impact.** Default: wenn ein Post (oder sein Konsumenten-Schema) keine `primaryAudience` trägt, behandeln Reviewer und Lint-Skills ihn als `primaryAudience: A` (technischer Peer-Leser)—als SHOULD im Enforcement-Status-Block der §Akzeptanzkriterien kodiert. Erneut prüfen, wenn: der Referenzkonsument `nolte/blog` die Schema-Änderung in `src/content.config.ts` ausliefert, die `primaryAudience` und `secondaryAudiences` deklariert (ein Feature-Item in `nolte/blog`), und den Enforcement-Status-Vorbehalt aus seinem `CLAUDE.md` entfernt; an dem Punkt kippen `a-1` / `a-2` von autoreseitiger Konvention auf Build-erzwungen, und dieser Enforcement-Status-Block sowie dieser Punkt werden gelöscht.
- **Korpus-Verteilungs-Gate.** Erneut prüfen, wenn: der Referenzkonsument `nolte/blog` 20 veröffentlichte Posts erreicht UND Traffic-/Referrer-Analytik verfügbar hat; die Re-Prüfung im nächsten Sprint-Review vergleicht die tatsächliche A/B/C-Leserschaft gegen das modellierte `50/20/30` (konkret: den Technischer-Peer-Leser-Anteil `A` senken, wenn der LinkedIn-Portfolio-Reviewer-Traffic `B` materiell höher als modelliert ist). Das Upstream-Signal lebt in der Analytik des Konsumenten-Repositorys, nicht in diesem Portfolio-Plugin.
- **Korrekturkanal-Formalismus.** Erneut prüfen, wenn: das `AUDIENCES.md` von `nolte/blog` (die Benannte-Drittparteien-Audience, Referenzidentifikator `L`) einen dedizierten, benannten Korrektur-/Kontaktkanal hinzufügt. Bestätigt noch nicht vorhanden—die offenen Fragen der [`audience-identification`](../audience-identification/de.md)-Spec nennen keinen Korrekturkanal, und ihre einzige offene Frage betrifft zukünftige Threat-Modeling-/SLA-Specs, sodass die konsumentenseitige Deklaration, die dies entblocken würde, noch nicht existiert.
- **Schwelle für Einwilligung bei nicht-privaten aber persönlichen Charakterisierungen.** §Behandlung benannter Drittparteien fordert explizite Einwilligung nur für Privat-Kommunikationszitate; öffentliche Aussagen werden über die öffentliche Form plus einen Link behandelt. Erneut prüfen, wenn: ein echter strittiger Fall mit einer benannten Drittpartei auftritt (ein `L`-betroffener Post, der einen Fairness-/Einwilligungs-Disput auslöst), ODER genug `L`-betroffene Posts ausgeliefert werden, um ein Muster zu etablieren, dass die Schwelle der öffentlichen Form unzureichend ist. Das spiegelt das reaktive Verschärfungs-Muster von [`blog-author`](../blog-author/de.md)—ein konkreter strittiger Fall löst eine spätere Spec-Verschärfung aus, keine prospektive.
- **Diátaxis-Frontmatter-Signal.** §Diátaxis-Positionierung hält die Haltung implizit im Lede, und ein dediziertes Frontmatter-Feld ist absichtlich nicht erforderlich. Erneut prüfen, wenn: eine nachgelagerte Lint- oder Audit-Skill eingeführt wird, die die Diátaxis-Position programmatisch (maschinenlesbar) lesen muss, statt sie aus dem Lede zu erschließen. Bis ein solcher Konsument existiert, gilt die Implizit-im-Lede-Form; die briefingseitige YAML-Schema-Frage von [`blog-author`](../blog-author/de.md) ist ebenfalls noch offen, sodass auch noch kein maschinenlesbarer Konsument für die Position existiert.
