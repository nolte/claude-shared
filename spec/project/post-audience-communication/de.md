...
- **Frontmatter-Schema-Impact.** Diese Spec setzt zwei Frontmatter-Felder voraus, `primaryAudience` und `secondaryAudiences`. Das Static-Site-Engine-Schema des Konsumenten (das Astro-Zod-Schema des Referenzkonsumenten in `src/content.config.ts`) deklariert sie möglicherweise noch nicht. Das Hinzufügen ist eine nachgelagerte Aufgabe im Konsumenten: ein Feature-Item, das das Schema aktualisiert und entscheidet, ob Legacy-Posts ohne die Felder akzeptiert werden (vermutlich ja, mit dem Technischer-Peer-Leser-Identifikator als Default). Bis das in einem gegebenen Konsumenten gelöst ist, sind die MUSSE der Spec unter §Primär-Audience-Deklaration autoreseitige Konventionen, die der Build nicht erzwingen wird.
- **Per-Post-Audience-Badge-UI.** Das Audience-Signal ist heute intern (Frontmatter); ob es auf der gerenderten Seite zu zeigen ist (z. B. ein kleines „für Entwickler" / „für Portfolio-Leser"-Tag nahe des Post-Headers) ist eine UX-Entscheidung, auf ein zukünftiges Feature verschoben. Sichtbare Audience-Badges könnten defensiv gelesen werden („Ich weiß, dass dieser Post zu technisch für dich ist")—Default ist, es intern zu halten, bis ein echter Leser-Verwirrungs-Fall etwas anderes zeigt.
- **Korpus-Verteilungs-Gate.** Das Referenz-50/20/30 A/B/C-Ziel in §Primär-Audience-Deklaration ist eine Inferenz aus der Audience-Kritikalität des Referenzkonsumenten plus realistischer Hobby-Blog-Frequenz. Es hat noch keine Daten-Stützung. Nach den ersten 20 Posts in einem gegebenen Konsumenten **MUSS [MUST]** die Verteilung gegen tatsächliche Traffic-/Referrer-Daten re-geprüft werden; das Ziel kann nötig sein, den Technischer-Peer-Leser-Anteil zu senken, wenn Portfolio-Reviewer-Traffic von LinkedIn materiell höher als erwartet ist.
- **Korrekturkanal-Formalismus.** §Behandlung benannter Drittparteien lehnt sich an einen impliziten Korrekturpfad (öffentliche Repo-Issues + About-Seiten-E-Mail). Wenn das Audience-Artefakt des Konsumenten einen dedizierten Kanal deklariert, **MUSS [MUST]** diese Spec aktualisiert werden, um den Kanal direkt zu benennen.
- **Schwelle für Einwilligung bei nicht-privaten aber persönlichen Charakterisierungen.** §Behandlung benannter Drittparteien fordert explizite Einwilligung für Privat-Kommunikationszitate. Die Schwelle für nicht-private aber persönliche Charakterisierungen (z. B. der Blog-Post eines Maintainers, den der Autor kritisiert) ist „nutze die öffentliche Form, verlinke auf die öffentliche Aussage". Ob diese Schwelle gehärtet werden muss zu „die Person vor Veröffentlichung benachrichtigen", ist offen und hängt davon ab, wie häufig L-betroffene Posts in der Praxis ausgeliefert werden.
- **Diátaxis-Frontmatter-Signal.** §Diátaxis-Positionierung deklariert, dass die Haltung implizit im Lede ist. Wenn das Post-Linting reift, kann ein Frontmatter-Feld `diataxis: explanation | how-to | blend` nützlich werden. Aufgeschoben, bis mindestens eine nachgelagerte Skill oder ein Audit die Position maschinenlesbar lesen muss.
