...
- **MUST** confirm, per stack, that the chosen tool chain can produce the attribution/NOTICE output §Own-code and in-repo compliance requires: the named defaults cover discovery, SBOM, and classification; for attribution/NOTICE generation the verified cross-stack default is the ORT reporter's NOTICE templates (`PlainTextTemplate`, or its `NOTICE_SUMMARY` variant, both Apache-Freemarker-customizable), and for Go `go-licenses save` bundles each dependency's license text and copyright notice; account for `go-licenses`' documented limits (it warns on non-Go code, which can conceal further license requirements, and can fail to resolve a license URL), so a non-Go or unresolved component is a `review` finding, never a silent pass
