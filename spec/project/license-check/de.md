...
- **SOLLTE [SHOULD]** Patent-Grant- und Patent-Retaliation-Klauseln erfassen, wo die Kategorie sie trägt — Apache-2.0, GPL-3.0, MPL-2.0 (§2.1 Grant, §5.2 Retaliation-Termination), EPL-2.0 (§2(b) Grant ohne Hardware, §7 Retaliation) —, weil eine Patent-Retaliation-Termination den Grant aushebeln kann, auf den sich ein Downstream-Nutzer verlässt
...
- **MUSS [MUST]** je Stack bestätigen, dass die gewählte Tool-Kette die Attribution-/NOTICE-Ausgabe erzeugen kann, die §Eigener-Code- und In-Repo-Compliance verlangt: die benannten Defaults decken Discovery, SBOM und Klassifizierung ab, aber ihre Attribution-Generierungs-Fähigkeit — und die Vendoring-/fehlende-LICENSE-Grenzen von `go-licenses` — ist unverifiziert (siehe Offene Fragen); wo ein Default keine Attribution erzeugen kann, benennt der umsetzende Skill einen dedizierten Attribution-Generator (z. B. den ORT NOTICE-reporter)
...
## Offene Fragen
- Cross-Reference-Neuausrichtung: `spec/portfolio/tech-stack/` §Nicht-Ziele besagt derzeit, Lizenz-Compliance werde von `dependency-audit` regiert, und die §License-audit von `dependency-audit` ist älter als diese Spec. Beide sind als Beschreibungen des Zustands vor `license-check` zu lesen, nicht als vorschreibende Constraints, die den Autoritätsanspruch dieser Spec überschreiben; bei Übernahme ist `license-check` autoritativ. Sobald diese Spec stabilisiert ist, SOLLTEN beide Referenzen gepatcht werden, um `license-check` als Policy-Autorität zu benennen (und der Lizenz-Pass von `dependency-audit` als Umsetzer umgerahmt werden). Keine Sibling-Spec-Änderung, bis diese Spec `draft` verlässt.
- Midjourney und andere Bild-/Asset-Generator-Terms sowie das GitHub-Copilot- / OpenAI-Output-Rechte-Detail über das bereits Festgehaltene hinaus wurden im Recherche-Protokoll nicht vollständig verifiziert; die KI-Provenienz-Anforderungen bleiben auf dem Niveau, das die Evidenz stützt. Die generator-spezifische Terms-Tabelle in einem Follow-up schärfen, bevor ein Repo sich auf ein Per-Generator-Urteil verlässt.
- Der Wortlaut der Patent-Grants von Apache-2.0 §3 und GPL-3.0 §11 wurde in der dedizierten Patent-Runde nicht direkt verifiziert (nur MPL-2.0 und EPL-2.0); den exakten Grant-/Retaliation-Wortlaut bestätigen, bevor die Patent-Klausel-Leitlinie als zitierbar statt kategorial behandelt wird.
- Stack-übergreifende NOTICE-/Attribution-Generierung (ORT NOTICE-reporter, oss-attribution-generator) und die konkreten Grenzen von `go-licenses` (Vendoring, fehlende LICENSE-Dateien, Confidence-Schwelle / False-Positive-Rate) wurden nicht verifiziert; die §Stack-spezifisches-Tooling-Defaults benennen die Tools, aber die Attribution-Generierungs-Mechanik braucht einen verifizierten Pass, bevor sie von `SOLLTE` zu einem benannten Default härtet.
