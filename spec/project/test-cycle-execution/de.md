...
- **DARF [MAY]** ein **gebundenes Flip-Signal-Retry** verwenden (einen fehlschlagenden Test eine feste kleine Anzahl Male laufen lassen, nur um zu *erkennen*, dass er kippt, und ihn als flaky markieren) — Erkennung, kein Reinwaschen.
...
- [ ] Retry-bis-Grün ist verboten; gebundenes Flip-Signal-Retry ist nur Erkennung; bekannte Flakes werden unter Quarantäne gestellt und verfolgt
...
- Wird das gebundene Flip-Signal-Retry besser hier besessen (Ausführung gibt ein `flaky`-Flag aus) oder in Phase 3 (Analyse entscheidet Flake vs. echt) — wo genau lebt die Flake-*Klassifikation* versus die Flake-*Erkennung*?
