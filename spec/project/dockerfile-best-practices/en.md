...
- **hadolint rule-set drift (rules like `DL3005`/`DL3017`/`DL3031` persist and still fire; the rule set changes across releases so a pinned version is required)**: hadolint wiki page for `DL3031` (Primary), `https://github.com/hadolint/hadolint/wiki/DL3031`; hadolint wiki page for `DL3017` (Primary), `https://github.com/hadolint/hadolint/wiki/DL3017`; hadolint issue #1049 confirming `DL3005` still fires (Primary), `https://github.com/hadolint/hadolint/issues/1049`. An earlier draft stated these three rules "were removed"; that's false (the wiki pages are live and the rules still trigger) and was corrected above. *These sources share 1 domain root and count as 1 independent source per `spec/claude/research-triangulate/` §Source classes and independence, below the author-time tier of three; the claim stays unverified until an independent source confirms it.*
