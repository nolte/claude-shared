...
**Leserschaft** dieser Spec sind Implementierer des `lektorat-scanner`-Agents (der LIX berechnet), der `lektorat-apply`- und `lektorat-auto-revise`-Skills (die die iterative Verbesserungsschleife antreiben) und der Autoren-Ebene (`audience-doc-author`-Agent, `blog-author`-Skill), deren Entwürfe an einem LIX-Korridor gemessen werden. Vertrautheit mit [`spec/project/mkdocs-structure/`](../mkdocs-structure/de.md) (dem `content_mode`-Enum), [`spec/project/lektorat/`](../lektorat/de.md) (den D1–D6-Dimensionen, der §Ausgaben-JSON-Form und den §Language-handling-Stripping-Regeln) und [`spec/project/audience-identification/`](../audience-identification/de.md) (dem Audience-Artefakt und den Protected-Terms-Quellen) wird vorausgesetzt; Begriffe aus jenen Specs werden ohne erneute Erklärung verwendet.
...
- **Vorschreiben einer einzigen Implementierung** der Metrik: die Spec beschränkt die Formel, die Korridore, den Pipeline-Vertrag und die Reproduzierbarkeits-Garantien, nicht den Code. Eine konforme stdlib-only Referenz-Implementierung wird mit dem Plugin ausgeliefert (siehe §Reproduzierbarkeit), aber die gewählte Library und der Tokenizer bleiben Implementierungsdetail, unterworfen den Pinning- und Vergleichbarkeits-Anforderungen unten.
...
- **MUSS [MUST]** jede **Markdown-Strukturgrenze** nach dem Strippen der Strukturmarker als Satz-Terminator behandeln: eine Überschrift, ein Listen-Item, eine Tabellenzeile und eine Blockquote-Zeile beenden je einen Satz an ihrem Zeilenende auch ohne terminale Interpunktion, und ein Absatzumbruch beendet stets einen. Verflachter Struktur-Text **DARF NICHT [MUST NOT]** zu einem synthetischen Satz konkatenieren: ohne diese Regel verschmelzen tabellenlastige Referenzseiten ihre Zeilen zu enormen Pseudo-Sätzen und landen weit über ihrem Korridor, während ein Prosa-zu-Liste-Rewrite, der die menschliche Lesbarkeit klar verbessert, den gemessenen Wert *verschlechtert*, weil die kollabierte Liste `B` senkt und `ASL` aufbläht. Eine strukturlastige Seite, die auch mit korrekter Segmentierung über ihrem Korridor bleibt, ist der vorgesehene Anwendungsfall für den Per-Datei-Korridor-Override in [`spec/project/lektorat/`](../lektorat/de.md) §D1, niemals ein Grund, die Zählung zu verzerren.
...
- **SOLLTE [SHOULD]** die **gebündelte Referenz-Implementierung** verwenden, die das Plugin unter `scripts/readability_lix.py` ausliefert (aufgerufen über `${CLAUDE_PLUGIN_ROOT}`, damit sie im installierten Plugin auflöst, nicht im Arbeitsbaum des Konsumenten). Sie ist bewusst stdlib-only — kein `pip install` im Konsumenten-Repo —, wendet die obigen Stripping-, Segmentierungs- und Zähl-Regeln für beide Sprachen identisch an, meldet `pipeline_metadata.readability` mit `library: nolte-readability-lix` und dessen Version und ist durch den Konformitätstest `tests/test_readability_lix.py` gepinnt. Ein Konsument **DARF [MAY]** eine andere Library einsetzen (zum Beispiel `textstat` oder `andreasvc/readability`), sofern sie die kanonische-Formel- und Gleiche-Implementierung-Anforderungen oben erfüllt und ihre eigene `library`/`version` in den Metadaten aufzeichnet.
