...
**Leserschaft** dieser Spec sind Implementierer des `lektorat-scanner`-Agents (der LIX berechnet), der `lektorat-apply`- und `lektorat-auto-revise`-Skills (die die iterative Verbesserungsschleife antreiben) und der Autoren-Ebene (`audience-doc-author`-Agent, `blog-author`-Skill), deren Entwürfe an einem LIX-Korridor gemessen werden. Vertrautheit mit [`spec/project/mkdocs-structure/`](../mkdocs-structure/de.md) (dem `content_mode`-Enum), [`spec/project/lektorat/`](../lektorat/de.md) (den D1–D6-Dimensionen, der §Outputs-JSON-Form und den §Language-handling-Stripping-Regeln) und [`spec/project/audience-identification/`](../audience-identification/de.md) (dem Audience-Artefakt und den Protected-Terms-Quellen) wird vorausgesetzt; Begriffe aus jenen Specs werden ohne erneute Erklärung verwendet.
