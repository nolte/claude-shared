...
- **Per-Konsument-Übernahme der Zielzustand-Übergabe-Route.** §Übergabe-Routen erlaubt entweder die Zielzustand-Route (`lektorat-apply`) oder die Übergangs-Route (`prose-vale-curator` + Reviewer-Urteil). Das Übernahme-Signal eines Konsumenten ist „Übergangs-Klausel aus `CLAUDE.md` entfernen"—der Auslöser, der die Vertragsoberfläche dieser Spec von „zwei Routen" auf „eine Route" umschaltet, ist pro Konsument, nicht portfolio-weit. Ausgelöst, wenn der erste Konsument (wahrscheinlich `nolte/blog`) das Übernahme-Signal ausliefert.
- **Briefing und Liefervertrag als YAML-Schema.** §Briefing-Inputs und §Liefervertrag sind heute Prosa, mit Pflichtfeld- bzw. Pflichtartefakt-Listen. Eine maschinenlesbare Form für beide Seiten—`project/briefings/<slug>.yml` mit Schema-Validierung als **Input**, `project/handovers/<slug>.yml` (oder ein eingebettetes Frontmatter-Unter-Objekt) als **Output**—wäre für die `blog-author`-Skill bequemer als Prosa und würde den Liefervertrag automatisch prüfbar machen statt nur attestierbar. Bis dahin reicht die Prosa-Form auf beiden Seiten. Wenn die maschinenlesbare Form eingeführt wird, sind die drei in §Liefervertrag benannten Artefakte (Selbst-Check-Manifest, Quellen-zu-Behauptungs-Mapping, Übergabe-Manifest) die natürlichen Top-Level-Schemas. Ausgelöst durch die zweite umstrittene Übergabe-Attestierung.
- **Hero-Bild-Korpus-Politik.** Die Verbindlichkeit, der korpusweite Stil, die Generierungs-Pipeline und das Layout-Rendering des Hero-Bild-Felds sind über Konsumenten hinweg offen. Die Spec trägt weiterhin die Alt-Text-Regel und das Lede-Ersatz-DARF-NICHT in §Hero- und OG-Bild; die breitere Politik gehört zu einem konsumentenseitigen Roadmap-Item (der Referenzkonsument verfolgt dies in `project/roadmap.md`).
- **Trigger-Integration.** Diese Spec definiert, was der Autor produziert; sie definiert absichtlich nicht, wann der Autor aufgerufen wird. Die Begleit-Spec [`spec/project/blog-author-trigger/`](../blog-author-trigger/de.md) definiert die Trigger-Bedingungen (`feature → done` und ähnlich). Ihre Schnittstelle sind die §Briefing-Inputs dieser Spec—die Trigger-Spec muss ein Briefing erzeugen, das den Pflichtfeld-Satz hier erfüllt.
