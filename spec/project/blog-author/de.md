...
- **Zielzustand-Route**—die `audit`-Operation aus [`lektorat-apply`](../../../skills/lektorat-apply/SKILL.md) über das EN + DE-Post-Paar ausführen. Jeder Befund der Severity **`critical`** wird vor dem Mergen des Post-Paars gelöst (per `patch`-Operation, per Autor-Edits oder per der eingebauten „skip-and-record"-Ablehnung des Befunds mit dokumentiertem Grund). Der Autor **SOLLTE [SHOULD]** Befunde der Severity **`warning`** adressieren, mit dem Recht auf eine dokumentierte Ablehnung in Einzelfällen. Befunde der Severity **`suggestion`** sind optional.
...
- **Per-Konsument-Übernahme der Zielzustand-Übergabe-Route.** §Übergabe-Routen behält die Zwei-Routen-Oberfläche bei (Zielzustand `lektorat-apply` oder Übergang `prose-vale-curator` + Reviewer-Urteil). Die Umschaltung auf eine Ein-Routen-Oberfläche ist pro Konsument, nicht portfolio-weit, und beide unten stehenden Bedingungen müssen erfüllt sein, bevor sie greift: (1) [`spec/project/lektorat/en.md`](../lektorat/de.md) verlässt `Status: draft` (verifizierbar: `grep '^Status:' spec/project/lektorat/en.md` liest nicht mehr `draft`)—die Übergangs-Route ist von §Übergabe-Routen nur erlaubt, solange Lektorat noch nicht zur Übernahme durch den Konsumenten freigegeben ist; und (2) der Referenzkonsument `nolte/blog` (`~/repos/github/blog/CLAUDE.md`) benennt `lektorat-apply` als seinen Lektor-Einstiegspunkt und trägt keine Übergangs-/prose-vale-curator-Klausel (verifizierbar: `grep -i 'transitional\|prose-vale' ~/repos/github/blog/CLAUDE.md` liefert nichts, während es `lektorat-apply` benennt). Heute gilt keine von beiden: Lektorat ist `Status: draft` und `blog/CLAUDE.md` benennt keine der Routen.
- **Briefing und Liefervertrag als YAML-Schema.** §Briefing-Inputs und §Liefervertrag sind heute Prosa, mit Pflichtfeld- bzw. Pflichtartefakt-Listen. Wieder aufgreifen bei der zweiten umstrittenen Übergabe-Attestierung—das heißt der zweite Fall, in dem die Attestierung eines Übergabe-Manifests (eine `passed`-Zeile, der behauptete Build-Status oder die benannte Route) gegen den tatsächlichen Repository-Stand bestritten wird, also ein festgehaltener Reviewer- oder Lektor-Befund, dass ein §Übergabe-Manifest-Feld die Realität falsch darstellt (das DARF NICHT in §Übergabe-Manifest). Der erste Streitfall liefert eine konkrete Form; der zweite bestätigt, dass sie generalisiert, und rechtfertigt es, die drei Artefakte aus §Liefervertrag (Selbst-Check-Manifest, Quellen-zu-Behauptungs-Mapping, Übergabe-Manifest) zu YAML-Top-Level-Schemas zu erheben. Bis dahin reicht die Prosa-Form auf beiden Seiten.
- **Hero-Bild-Korpus-Politik.** §Hero- und OG-Bild trägt heute die zwei pro-Post-Invarianten (beschreibender Alt-Text, kein Lede-Ersatz); korpusweite Verbindlichkeit, Stil, Generierungs-Pipeline und Layout-Rendering bleiben eine konsumentenseitige Produktentscheidung. Wieder aufgreifen, wenn der Referenzkonsument `nolte/blog` eine Hero-Bild-Politik-Entscheidung in seinem eigenen `~/repos/github/blog/project/roadmap.md` landet (verpflichtend-vs-optional, Korpus-Stil, Generierungs-Pipeline, Layout-Rendering); wenn jenes konsumentenseitige Roadmap-Item schließt, jede portfolio-relevante Invariante, die über `nolte/blog` hinaus generalisiert, zurück in §Hero- und OG-Bild falten. Die Per-Konsument-Korpus-Politik selbst bleibt konsumentenseitig.
