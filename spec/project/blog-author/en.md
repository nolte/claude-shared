...
- **Per-consumer adoption of the target-state handover route.** §Handover routes keeps the two-route surface (target-state `lektorat-apply`, or transitional `prose-vale-curator` + reviewer judgement). The flip to a one-route surface is per consumer, not portfolio-wide, and both conditions below must hold before it lands: (1) [`spec/project/lektorat/en.md`](../lektorat/en.md) leaves `Status: draft` (verifiable: `grep '^Status:' spec/project/lektorat/en.md` no longer reads `draft`)—the transitional route is permitted by §Handover routes only while lektorat hasn't yet been released for the consumer's adoption; and (2) the reference consumer `nolte/blog` (`~/repos/github/blog/CLAUDE.md`) names `lektorat-apply` as its editor entry point and carries no transitional/prose-vale-curator clause (verifiable: `grep -i 'transitional\|prose-vale' ~/repos/github/blog/CLAUDE.md` returns nothing while it does name `lektorat-apply`). Today neither holds: lektorat is `Status: draft` and `blog/CLAUDE.md` names neither route.
- **Briefing and delivery contract as YAML schema.** §Briefing inputs and §Delivery contract are prose today, with mandatory-field and mandatory-artefact lists respectively. Revisit on the second contested handover attestation—that is, the second instance where a handover manifest's attestation (a `passed` line, the claimed build status, or the named route) is disputed against the actual repository state, which means a recorded reviewer or editor finding that a §Handover manifest field misrepresents reality (the MUST NOT in §Handover manifest). The first dispute supplies one concrete shape; the second confirms it generalises and justifies promoting §Delivery contract's three artefacts (self-check manifest, sources-to-claim mapping, handover manifest) to YAML top-level schemas. Until then the prose form is sufficient on both sides.
- **Hero-image corpus policy.** §Hero and OG image carries the two per-post invariants today (descriptive alt text, no lede substitution); corpus-wide mandatoriness, style, generation pipeline, and layout rendering stay a consumer-side product decision. Revisit when the reference consumer `nolte/blog` lands a hero-image policy decision in its own `~/repos/github/blog/project/roadmap.md` (mandatory-vs-optional, corpus style, generation pipeline, layout rendering); when that consumer-side roadmap item closes, fold any portfolio-relevant invariant that generalises beyond `nolte/blog` back into §Hero and OG image. The per-consumer corpus policy itself stays consumer-side.
