...
- Übersetzungen der README selbst — die README ist aus Portfolio-Konsistenzgründen nur auf Englisch; mehrsprachiger Inhalt lebt unter `docs/<lang>/` und wird durch `spec/project/docs-multilingual-authoring/` geregelt
...
1. **`## Purpose`** — **MUSS [MUST]** erscheinen. Erklärt das Problem, das das Repository löst, und wer die beabsichtigten Konsumentinnen sind. Die Formulierung „beabsichtigte Konsumentinnen" stützt sich auf das Audience-Artefakt des Repositories, das gemäß `spec/project/audience-identification/` erstellt wird (die kanonische `AUDIENCES.md` oder die dort erlaubten alternativen Ablageorte); nenne die primären Audiences, statt die vollständige Liste zu wiederholen. Zwei bis sechs Stichpunkte oder ein kurzer Absatz. Keine Feature-Aufzählung.
