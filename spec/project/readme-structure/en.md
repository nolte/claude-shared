...
## Context
Every repository in this portfolio ships a top-level `README.md` — mandated by the `project-structure` spec — but the internal shape of that file has drifted across repositories. Consumers (humans and AI agents) scan READMEs for the same things in the same order: *what is this, why does it exist, how do I use it, how does it fit into the portfolio*. When the section set and ordering differ per repository, downstream tooling (plugin marketplaces, package indexes, AI agents that synthesize summaries) and human readers both pay the cost.
...
- **MUST** include a one- to three-sentence tagline under the badges that states *what this repository is* and *who it is for*, without marketing language
...
5. **`## Status`** — **SHOULD** appear. One short paragraph describing lifecycle state (early stage, stable, maintenance-only, archived). Complements but does not duplicate GitHub's repository metadata.
