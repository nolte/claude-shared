...
1. **`## Purpose`**: **MUST** appear. Explains the problem the repository solves and who the intended consumers are. The "intended consumers" wording draws on the repository's audience artifact produced under `spec/project/audience-identification/` (the canonical `AUDIENCES.md`, or the acceptable alternative locations that spec permits); name the primary audiences rather than restating the full list. Two to six bullet points or a short paragraph. No feature dump.
