...
- **DARF** zusätzlich ein Lizenz-Audit ausführen, wenn das Repository es aktiviert, mit einer Allowlist unter `.license-allowlist.txt`, unter `tool.*` im Manifest oder Äquivalent; fehlende Allowlist bedeutet, dass Lizenz-Befunde als `review` berichtet werden, nicht als Failure
...
- **SOLLTE** die Gesamtzahl aktiver Ignore-Einträge klein halten (Richtwert: weniger als zehn pro Unterordner); eine wachsende Ignore-Liste signalisiert, dass die Dependency-Strategie selbst revidiert werden muss
...
### Audit-Artefakt
- **MUSS** das Ergebnis jedes vollständigen Audits als Commit, Issue oder Datei im Repository persistieren; der Artefaktort **SOLLTE** pro Repository konsistent gewählt werden (zum Beispiel `docs/audits/dependencies-YYYY-Q<n>.md` oder ein GitHub-Issue mit Label `security-audit`)
...
## Offene Fragen
- Soll das Portfolio einen einzigen Artefaktort vorschreiben (zum Beispiel `docs/audits/dependencies.md` mit Quartals-Sektionen), um Cross-Repo-Aggregation zu erleichtern, oder bleibt die Freiheit pro Repository bestehen?
- Gibt es eine portfolioweite Obergrenze für aktive Ignore-Listen-Einträge (Hart-Cap vs. Richtwert), oder genügt das Prinzip „klein, schrumpfend, niemals still"?
- Soll der Lizenz-Audit-Schalter standardmäßig an sein, sobald eine Allowlist existiert, oder bleibt er pro Aufruf opt-in?
