...
- **MUSS [MUST]** das Artefakt gemäß den vier vorgeschriebenen Abschnitten (`## Scope`, `## Summary`, `## Findings`, `## Processing log`, in dieser Reihenfolge, mit genau diesen englischen Überschriften) und der kanonischen vierstufigen Severity-Skala (`Critical`, `Warning`, `Suggestion`, `Info`) strukturieren, die `spec/claude/review-plan` §Severity scale und §Required sections vorschreibt (derselbe Artefakt-Vertrag, den `spec/portfolio/portfolio-management` unter `.audits/portfolio/` persistiert), damit sich Spec-Drift-Befunde identisch zu jedem anderen Audit-Artefakt im Portfolio lesen
