...
- **MUSS [MUST]** die Klasse der Vererbungs-Drift aus `spec/project/portfolio-inherited-spec-layer/` §Drift-Detection einbeziehen: in einem Repository, dessen `spec/.spec-config.yml` einen `inherits:`-Schlüssel trägt, die Befunde der Lokal-vs-vererbt-Drift-Prüfung des `spec`-Skills; im Hub, der keinen trägt, die Validierung der `Portfolio-Scope:`-Header, die `scripts/check_spec_inheritance.py` dort ausführt. Eine nicht deklarierte Abweichung von einer vererbten Spec oder ein fehlerhafter Scope-Header, den die Consumer erben, ist Drift, die keine Implementierungsdatei zeigt
...
- **MUSS [MUST]** das Artefakt gemäß den vier vorgeschriebenen Abschnitten (`## Scope`, `## Summary`, `## Findings`, `## Processing log`, in dieser Reihenfolge, mit genau diesen englischen Überschriften) und der kanonischen vierstufigen Severity-Skala (`Critical`, `Warning`, `Suggestion`, `Info`) strukturieren, die `spec/claude/review-plan` §Schweregrad-Skala und §Plan-Körper-Struktur vorschreibt (derselbe Artefakt-Vertrag, den `spec/portfolio/portfolio-management` unter `.audits/portfolio/` persistiert), damit sich Spec-Drift-Befunde identisch zu jedem anderen Audit-Artefakt im Portfolio lesen
