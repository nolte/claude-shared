...
- **MUSS [MUST]**, wenn `spec/.spec-config.yml` einen `inherits:`-Schlüssel trägt, die Klasse der Lokal-vs-vererbt-Drift aus `spec/project/portfolio-inherited-spec-layer/` §Drift-Detection einbeziehen und ihre Befunde aus der Lokal-vs-vererbt-Drift-Prüfung des `spec`-Skills übernehmen (im Hub-Repository zusätzlich aus `scripts/check_spec_inheritance.py`), weil eine nicht deklarierte Abweichung von einer vererbten Spec Drift ist, die keine Implementierungsdatei zeigt
