...
- **`NetworkPolicy` is inert unless the CNI plugin enforces it (Calico and Cilium enforce it; Flannel does not)**: Kubernetes documentation, "Network Policies" ("Network policies are implemented by the network plugin") (Primary), `https://kubernetes.io/docs/concepts/services-networking/network-policies/`; Calico documentation, "Kubernetes network policy" (Primary), `https://docs.tigera.io/calico/latest/network-policy/get-started/kubernetes-policy/kubernetes-network-policy`; Flannel README, which defers network policy to another project such as Calico (Primary), `https://github.com/flannel-io/flannel`
