...
- **SOLLTE [SHOULD]** im Abschnitt **Risk / rollout notes** des Fix-PRs (neben der Triage-Klassifikation, gemäß `pull-request-workflow`-Spezifikation) festhalten, welcher spezialisierte Agent den Fix erzeugt hat, oder vermerken, dass kein passender spezialisierter Agent existiert und ein Generalist die Arbeit übernommen hat — das macht portfolio-weite Abdeckungslücken sichtbar. **Präzedenz-Hinweis:** `spec/project/continuous-improvement/` hebt dieselbe Aufzeichnung für jedes im Scope liegende Finding — workflow-health-Findings eingeschlossen — auf ein **MUSS [MUST]** an, und ihr Abschnitt §„Verhältnis zu bestehenden Specs" mit der Präzedenz-/Carve-out-Klausel regelt die Auflösung: die stärkere Anforderung gewinnt, sodass ein Workflow-Fix-PR, der die Spezialisten-/Ursprungsquellen-Aufzeichnung auslässt, unter jener Spec nicht konform ist, auch wenn dieser Punkt allein nur ein SOLLTE ist. Triage, Klassifikation und der in dieser Spec definierte Remediation-*Prozess* bleiben Autorität dieser Spec und werden durch jenen Carve-out nicht überschrieben.
