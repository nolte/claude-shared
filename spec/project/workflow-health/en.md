...
## Goals
- A failing workflow run on `develop` or `main` is never silently ignored—every failure is either fixed, explicitly disabled with an owner, or classified against §Triage before remediation and tracked, including the deterministic platform constraints that are neither transient nor fixable in this repository
...
- **MUST**, on the first failure of a previously-green workflow, classify the root cause before any re-run into exactly one of:
  ...
  3. **infra**: upstream provider outage, registry 5xx, rate-limit, network—reproducible only in a narrow time window; **or** a deterministic platform behaviour this repository can't configure away, per §Known platform constraints. The two differ in duration, not in class: neither is a defect in this repository, and neither is fixed by a re-run
...
- **MUST** remediate it through the same credential that lifts the cascade constraint above, minted in the `nolte/gh-plumbing` reusable, and **MUST** widen that credential's scope to cover workflow files: an App installation needs `workflows: write` in addition to `contents: write`, and a PAT on the alternative branch of that pair needs the classic `workflow` scope. `contents: write` alone satisfies neither. Until that upstream credential ships, a repository **MUST** record the gap as open, naming the change it waits on, rather than presenting the refresh path as working.
...
- **MUST**, once a lane breaches the cancellation threshold, tell two causes apart before choosing the remedy, because the same symptom has two independent origins with opposite fixes. Where the runtime exceeds the trigger cadence, the remedy above applies. Where the runtime is far below the cadence and the runs are cancelled because the branch is updated by unrelated merges—`strict: true` with automerge across many open pull requests re-queues every open branch's full workflow fan-out on each merge into the base—the lane has a **churn** problem: the remedy is reducing what each update re-queues (the fan-out, the trigger breadth) or changing the merge model, and again **not** `cancel-in-progress: false`, which the rule above already forbids for reasons that apply identically here. Observed in a consuming project on 2026-09-19: three lanes cancelled in 42–88 % of their concluded pull-request runs, none of them slower than a push (`nolte/claude-shared#644`, `#646`)
- **MUST** observe, for a lane under suspicion, the median wait for a runner slot (`started_at − created_at` of its jobs) measured on jobs with **no `needs:`**, the only jobs whose wait isn't contaminated by dependency waiting, beside the runtime (`completed_at − started_at`); the observation that separates the two causes above is this one, since flake rate and cancellation rate alone can't. The recipe ships with the `cicd-pipeline-design` skill
- **MUST** treat a lane whose wait dominates its runtime as a **capacity** finding for `spec/project/github-actions-best-practices/` §"Runner-slot economy" rather than as a defect in the lane: the lane is starved, not slow, and a fix inside the job changes nothing
...
- **MUST** treat an outage of a third-party check provider as `infra` for triage purposes, not as `defect`
...
- [ ] No push rejected with `refusing to allow a GitHub App to create or update workflow ... without 'workflows' permission` is classified as `secret drift` or `defect` in its fix PR's **Risk / rollout notes**; the recorded class is `infra`
- [ ] Where a repository refreshes a presentation branch from a release ref, one of three holds: the credential performing the push carries workflow-file scope (App installation `workflows: write`, or a PAT with the classic `workflow` scope); the repository records why its refresh diff can never touch `.github/workflows/`; or the repository records the gap as open, naming the upstream `nolte/gh-plumbing` change it waits on
...
- [ ] For a lane above the cancellation threshold, the run history alone tells the reader which cause applies: a runtime longer than the trigger cadence, or a runtime far shorter than it with cancellations caused by base-branch updates; the spec names the observation that separates them
- [ ] The queue-time observation is restricted to jobs with no `needs:`, so the metric measures the wait for a runner and not dependency waiting, and a lane whose wait dominates its runtime is routed to `github-actions-best-practices` §"Runner-slot economy" rather than remediated inside the lane
...
- **A GitHub App identity can't create or update a file under `.github/workflows/` without the `workflows` permission, and the workflow `permissions:` block has no such scope**: GitHub Docs, "Workflow syntax" §`permissions`, whose enumerated scope list carries no `workflows` entry (Primary; the enumeration was read twice on the retrieval date and returned the same sixteen keys both times, but GitHub revises this list, so the absence is evidence rather than proof and the two Secondary sources below carry the positive claim), `https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#permissions`; GitHub community discussion #51520, *"refusing to allow a GitHub App to create or update workflow `.github/workflows/ci.yml` without `workflows` permission"* (Secondary), `https://github.com/orgs/community/discussions/51520`; GitHub community discussion #27072, same rejection with the PAT-`workflow`-scope and App-permission remedies (Secondary), `https://github.com/orgs/community/discussions/27072` *These sources share 1 domain root and count as 1 independent source per `spec/claude/research-triangulate/` §Source classes and independence, below the author-time tier of three; the claim stays unverified until an independent source confirms it.*
