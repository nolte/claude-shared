...
- **MAY**, in a private repository without GitHub Enterprise Cloud, where the platform attestation mechanism isn't available, publish the SLSA v1 provenance as a Cosign attestation of the pushed digest instead (`cosign attest --type slsaprovenance1`); the signature and SBOM requirements above stay unchanged
