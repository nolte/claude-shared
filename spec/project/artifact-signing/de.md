...
- **KANN** in einem privaten Repository ohne GitHub Enterprise Cloud, in dem der Attestierungsmechanismus der Plattform nicht verfügbar ist, die SLSA-v1-Provenance stattdessen als Cosign-Attestation des gepushten Digests veröffentlichen (`cosign attest --type slsaprovenance1`); die Anforderungen an Signatur und SBOM oben bleiben unverändert. Der Keyless-Pfad schreibt Repository-Identität und Workflow-Pfad in das öffentliche Public-Good-Transparenzlog, sodass ein privater Publisher, der beides geheim halten muss, ihn nicht nutzen kann: Der Betrieb einer privaten Sigstore-Instanz liegt außerhalb des Scopes (§Nicht-Ziele)
...
- **MUSS** die Provenance mit dem Plattform-Tooling verifizieren (`gh attestation verify` mit `--signer-workflow`, `--source-ref` und `--deny-self-hosted-runners`), weil die Prüfungen von Source-Ref und Signierer Zertifikats-Extensions auswerten, die der Workflow nicht fälschen kann [R4]; ein Artefakt, das über den Fallback privater Repositories aus §C veröffentlicht wurde, wird stattdessen mit `cosign verify-attestation --type slsaprovenance1` gegen dieselben gepinnten Issuer-, Identitäts- und Source-Ref-Constraints verifiziert
...
- [ ] `cosign verify` und `gh attestation verify` (für den Fallback privater Repositories aus §C `cosign verify-attestation --type slsaprovenance1`) gelingen gegen ein Release-Artefakt allein mit gepinntem Issuer, verankerter Identität, Source-Ref- und Runner-Umgebungs-Constraints — und scheitern, sobald ein einzelner Constraint falsch ist (fasst §F zusammen)
...
- **Entschieden am 2026-09-13: ein privater Publisher nutzt den Cosign-Provenance-Fallback.** GitHub Artifact Attestations für private Repositories erfordern GitHub Enterprise Cloud, das das nutzereigene Konto `nolte` nicht hat. Am 2026-09-13 veröffentlicht keines seiner drei privaten Repositories OCI-Artefakte (kein GHCR-Workflow), sodass §C heute überall so gilt, wie es geschrieben ist, und die Cosign-Provenance-Kann-Regel aus §C einen privaten Publisher abdeckt, falls einer entsteht. Dieser Fallback veröffentlicht die Repository-Identität trotzdem im öffentlichen Public-Good-Transparenzlog; ein Publisher, der sie verbergen muss, hat unter dieser Spec keinen Pfad, weil eine private Sigstore-Instanz ein Nicht-Ziel ist.
