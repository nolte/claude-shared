...
Readers: repo maintainers of hobby-scale nolte projects whose `sprint-review` skill closes sprints by validating an artefact reference, plus the implementors authoring `sprint-review` and the `release-skill-layer` skills against this taxonomy.

The sibling `sprint` spec mandates that every closed sprint points at a concrete deployable artefact via `artifact_ref` and that this artefact materialises the sprint's `value_statement` for the end user. It deliberately does not say what shape that reference takes, what kinds of artefacts are valid for which project type, or how the reference is validated at sprint closure. This spec fills that gap. It defines the **artefact taxonomy** per portfolio project type, the **shape of `artifact_ref`** for each kind, the **validation rules** that `sprint-review` runs at closure, and the **dispatch boundary** to the existing release machinery (`release-automation` for the Draft → Published transition, `release-skill-layer` for operator-side curation and trigger). It does not redefine that machinery; it is the layer where sprint-side planning meets release-side execution, and its job is to keep the two from drifting.
...
- Replacing CI / CD systems. The artefact's existence and addressability is what matters here, not the build pipeline that produced it.
...
- **MUST NOT** count out-of-band artefacts toward sprint cadence; they are discoverable but never satisfy a sprint's `artifact_ref` requirement.
...
- The §Artefact taxonomy "Claude plugin" row currently allows a manual marketplace-catalog edit as an interim fallback while `release-publish.yml` is not yet confirmed to handle marketplace updates end-to-end. Once `nolte-shared`'s `release-publish.yml` is verified against a real plugin sprint, the fallback clause **MUST** be removed and any sprint that subsequently relies on it **MUST** raise a workflow-health finding. Track via the first plugin sprint executed against this spec.
- Should sprint-review automatically populate the `## Review notes` artefact-validation block from the verification command outputs, or stay strictly operator-paste? The current MUST records command outputs in `## Review notes`, which is implementable either way; auto-population is the more operator-friendly default. Revisit after first real adoption against `release-publish.yml`.
- Should out-of-band artefacts retroactively attach to the nearest preceding sprint (so `roadmap_items` traceability extends across emergency releases)? Current model treats them as fully independent with an optional `## Review notes` cross-reference; revisit if traceability becomes a load-bearing concern.
- Should the doc-site deploy artefact carry a verification beyond a HEAD request (a content-hash check against the deploy commit)? Defer; HEAD-200 is a reasonable hobby-scale baseline.
- Should the per-project-type taxonomy table eventually move to `references/<project-type>-bundles.md` next to this spec, mirroring the structure suggested in `release-skill-layer`? Defer until a third project-type-specific extension (beyond HACS) appears.
