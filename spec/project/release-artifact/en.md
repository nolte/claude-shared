...
- Remove the §Artefact taxonomy "Claude plugin" manual marketplace-catalog fallback (the interim clause and its tail sentence) when BOTH conditions hold: (1) the consuming repository's `.github/workflows/release-publish.yml` (or `release-cd-refresh-master.yml`) gains marketplace-catalog-update logic—that is, `grep -niE 'marketplace|plugin\.json|catalog'` returns more than zero hits, versus the zero hits today; AND (2) the first plugin-publish sprint closes with that workflow having performed the marketplace-catalog update end-to-end (today sprints 0001-0004 are dogfood, pipeline, quality-gate, and reviewer work, `project/release-artifacts/` doesn't yet exist, and `plugin.json` is still at `0.1.4`). On both, remove the fallback clause and let the workflow-health-finding rule against fallback reuse take effect.
- Default: keep HEAD-200 as the provisional verification for doc-site deploy URLs—a reasonable hobby-scale baseline, with §Artefact taxonomy already permitting the content-addressed deploy-commit-SHA suffix as the lighter-weight hook a stronger check would build on. Revisit when: a documented false positive lands—a documentation-only or hybrid sprint where the per-kind HEAD request returns 200 but the served content doesn't correspond to the deploy commit (stale CDN snapshot, wrong build promoted), recorded in a sprint's `## Review notes` or a workflow-health finding. The first such concrete incident justifies adding a content-hash check against the deploy commit SHA.
- Should the per-project-type taxonomy table move to `references/<project-type>-bundles.md` next to this spec, mirroring the structure suggested in `release-skill-layer`? Defer until a THIRD project-type-specific extension appears beyond HACS. Measurable count: today the taxonomy has exactly one standout type-specific row (HACS integration) on top of the generic Git-tag and package shapes; the trigger fires when a third such type-specific row is added (for example an OCI-image-specific or another platform-store-specific bundle). Keep this synchronised with `release-skill-layer` (which still carries the matching open question, "Defer until a third project type warrants extraction")—extract both specs' bundle tables together or not at all.
