...
- **SHOULD** persist a standalone coverage review, one not folded into a drift-audit artifact, as the dated record `.audits/continuous-improvement/<YYYY>-Q<n>.md`; like every dated audit record it stays on disk after processing, per `spec/claude/review-plan/` §Relationship to other specs
...
- **Precedence / carve-out (resolving the dispatch-strength tension with `workflow-health`).** `workflow-health` §"Specialized-agent dispatch for remediation" states the dispatch obligation at **MUST** strength but records the specialist in the fix PR at **SHOULD** strength; this spec states both the dispatch obligation (§Specialist dispatch) and the traceability record (§Traceability in remediation artifacts) at **MUST** strength. Where the two specs address the same obligation at different strengths, the **stronger requirement governs**, and the resolution is asymmetric by surface:
  - **Triage, classification, and the workflow-scoped remediation *process*** (the five-category triage, the no-blind-re-run rule, the pinned-tag discipline, the no-admin-override rule, the known-platform-constraint handling) remain **`workflow-health`'s authority**; this spec doesn't restate or override them for workflow findings.
  - **The act of dispatching** a matching specialist for a workflow finding is a **MUST** under both specs; there is no tension to resolve here.
  - **The traceability record** in the fix PR's **Risk / rollout notes** is a **MUST** under this spec and is the stricter rule; for a workflow-health finding the `workflow-health` SHOULD is **read up to this spec's MUST**: a workflow fix PR that omits the specialist/originating-source record is non-conformant under this spec even though `workflow-health` alone would only have it as SHOULD.
  - In all cases this is a **strengthening**, never a weakening: this spec never lowers a `workflow-health` MUST to a SHOULD. A finding whose remediation would require violating `workflow-health` is an Open Question, not a shortcut (see the final bullet of this section).
