...
- **MUST NOT** widen the **external** class to those trees by default: the specification corpus carries reference sections dense with external URLs (two orders of magnitude more than `docs_dir` in this repository), and the deterministic gate is offline by contract. External probing over that corpus stays an explicit, on-request run (§Audit artifact) rather than a gate obligation
