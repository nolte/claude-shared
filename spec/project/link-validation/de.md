...
- **MUSS [MUST]** die hand-gepflegten Markdown-Bäume des Repositorys, die *außerhalb* von `docs_dir` liegen — in diesem Portfolio der Spezifikations-Korpus unter `spec/` —, in die **internen und Cross-Tree**-Link-Prüfungen einschließen, aus demselben Grund wie die vorige Regel: Sie sind hand-geschrieben, verlinken nach `docs/`, `skills/`, `agents/` und untereinander und rotten genauso. Ein Baum, der nur *Linkziel* ist (§Link-Klassifikation, Cross-Tree-Referenz), bleibt eine ungeprüfte *Linkquelle*, solange diese Regel ihn nicht benennt — ein verschobenes oder gelöschtes Ziel in einem solchen Baum fällt dann still durch ein grünes Gate
- **DARF NICHT [MUST NOT]** die **externe** Klasse standardmäßig auf diese Bäume ausweiten: Der Spezifikations-Korpus trägt Referenz-Abschnitte voller externer URLs (in diesem Repository rund 780 verschiedene, ein Mehrfaches des übrigen Geltungsbereichs zusammen), und das deterministische Gate ist per Vertrag offline. Externes Prüfen über diesen Korpus bleibt ein ausdrücklicher Lauf auf Anfrage (§Audit-Artefakt) statt einer Gate-Pflicht
...
- [ ] Der Standard-Geltungsbereich deckt die hand-gepflegten Markdown-Bäume außerhalb von `docs_dir` (in diesem Repository `spec/`) für die internen und Cross-Tree-Klassen ab: Das Löschen einer Datei, auf die eine andere Spec verlinkt, lässt den Offline-Lauf ein `critical` melden, und die externe Klasse wird durch diese Aufnahme nicht ausgeweitet
