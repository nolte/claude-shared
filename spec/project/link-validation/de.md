...
- **DARF NICHT [MUST NOT]** die **externe** Klasse standardmäßig auf diese Bäume ausweiten: Der Spezifikations-Korpus trägt Referenz-Abschnitte voller externer URLs (in diesem Repository zwei Größenordnungen mehr als `docs_dir`), und das deterministische Gate ist per Vertrag offline. Externes Prüfen über diesen Korpus bleibt ein ausdrücklicher Lauf auf Anfrage (§Audit-Artefakt) statt einer Gate-Pflicht
