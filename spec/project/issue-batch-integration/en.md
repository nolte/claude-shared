...
That clusters occur here is **established**: `git log develop` shows `b4dd0f16` (#623), `0316be01` (#624), and `c969bc02` (#625) as three consecutive pull requests, each repairing the same repository-settings surface, each merged separately. Whether grouping would have measurably reduced that cost is **unestablished** — the observation that would settle it is a run of this process against a comparable cluster, and no such run exists yet.
...
- **SHOULD** keep a group small enough that its bundle diff is reviewable in one sitting; a group that outgrows that is split into two groups rather than merged as one unreviewable bundle
...
- **MUST** choose Mode B when any member is foreseeably **droppable**: its acceptance is uncertain, it depends on something outside the group's control, or a review that can reject it is still outstanding — most commonly a member classified `security`. Droppability is the criterion because a Mode B member can be removed from the bundle by dropping one merge, while a Mode A member is entangled in a shared commit strand
- **MUST** otherwise choose Mode A
- **MUST**, when a member turns out to be droppable after Mode A was chosen, treat it as a **structural regression** per `spec/claude/research-plan-implement/` §"Re-planning is explicit": stop, name the assumption that failed, and return to Plan — either extracting that member onto its own branch or removing it from the group
...
- **MUST**, where the bundle's type is `fix`, carry exactly **one** `## Class sweep` section covering the group's defect class as a whole, per `pull-request-workflow` §"Class sweep". A group admitted on thematic coupling of one defect class has one predicate by construction; one sweep per member would restate it `n` times and measure nothing extra
...
- [ ] Every group artifact records its mode with a reason, and no Mode A group carries a member marked droppable
- [ ] Every dropped member is recorded as returned to individual processing and is not closed
...
- Whether a group needs a size backstop in addition to the droppability criterion (§C) is deferred. Droppability names the property Mode B actually buys; a member count would be a proxy whose threshold no run has yet calibrated. **Revisit trigger:** a recorded group whose Mode A strand became unreviewable while no member was droppable.
