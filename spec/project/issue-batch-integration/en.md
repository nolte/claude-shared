...
- **MUST** distinguish a **local adaptation** from a **structural regression** per `spec/claude/research-plan-implement/` §"Re-planning is explicit" once members are being implemented: a member whose work turns out larger or differently shaped than planned, while the group's admission, mode, and ordering still hold, is a local adaptation and is recorded in the artifact without a new approval; anything that invalidates an admission predicate, the mode decision, or the dependency ordering is a structural regression and returns the group to Plan for re-approval
...
- **MUST**, where the group classifies Tier 3, additionally satisfy both Tier 3 obligations of `spec/claude/research-plan-implement/` §"Phase depth scales with blast radius": settle the design question—**where** the change lands—as an explicit operator-facing decision before the plan describes **how** it gets there, and run a verification pass after implementation performed by a context that didn't produce the change
- **MUST**, where the group classifies Tier 3, decompose the plan into independently verifiable slices per `spec/claude/research-plan-implement/` §"The plan is the review surface" so verification happens at slice boundaries rather than only at the finished bundle. The member ordering (§F) is the natural slice boundary: a member whose declared checks are green is a verified slice
- **MUST** persist the group's Research findings and its Plan as a single artifact at `.audits/issue-batch-integration/<group-id>/analysis.md`, carrying: the group id, the question the Research phase was scoped to answer, the one-sentence logical change (§A), the member table (issue, class, admitting predicate, evidence), the cross-member dependency ordering, the shared touch surface, the mode decision with its reason (§C), the completeness matrix below, the risks, the boundary of what this group deliberately leaves out of scope, and the open questions for the operator
...
- **SHOULD** run the group's Research in an isolated context—a dispatched reviewer agent, a subagent, or a separate session—that returns a condensed summary rather than its exploration transcript, per `spec/claude/research-plan-implement/` §"Research is isolated and anchored" so the context that implements the group doesn't pay for the search across every member
- **MUST** keep the artifact readable on its own, without the conversation that produced it, per `spec/claude/research-plan-implement/` §"The plan is the review surface"; the artifact is the hand-off contract a second operator resumes the group from
- **MUST NOT** write any part of the group's change while Research or Plan is running, which is the boundary `spec/claude/research-plan-implement/` §"Phase definitions and the write boundary" draws. The group's **write gate** is the operator approval of this artifact: the artifact itself is a permitted write before the gate, and the first tracked change to a member's files is the first write after it
...
- **MUST** run every check the completeness matrix declares for a member and record the check's **actual output**, not an assertion that it passed; `spec/claude/research-plan-implement/` §"Verification belongs inside the plan" states that obligation, and a recorded pass whose check never ran is worse than no record at all, because it reads as evidence
...
- [ ] Every group artifact states the question its Research phase was scoped to answer and the boundary of what the group leaves out of scope
- [ ] Every group artifact reads on its own without the conversation that produced it, and no member's tracked files were changed before the operator approved the artifact
- [ ] For every Tier 3 group, the artifact records the design decision settled before the plan described how, names the plan's independently verifiable slices, and carries a verification pass performed by a context that didn't produce the change
- [ ] Every member check recorded in a group artifact carries the check's actual output rather than an assertion that it passed
- [ ] Every deviation that arose while members were implemented is recorded either as a local adaptation or as a structural regression that returned the group to Plan for re-approval
