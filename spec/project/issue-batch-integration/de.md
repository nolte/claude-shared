...
- **MUSS [MUST]** die Research-Befunde und den Plan der Gruppe als ein einziges Artefakt unter `.audits/issue-batch-integration/<group-id>/analysis.md` persistieren, das trägt: die Gruppen-Id, die logische Änderung in einem Satz (§A), die Mitgliedstabelle (Issue, Klasse, aufnehmendes Prädikat, Beleg), die mitgliedsübergreifende Abhängigkeitsordnung, die geteilte Berührungsfläche, die Modus-Entscheidung mit Begründung (§C), die untenstehende Vollständigkeitsmatrix, die Risiken und die offenen Fragen an den Operator
