...
- **Er weiß oft nicht, was er will**, bis er einen Kandidaten sieht — das *IKIWISI*-Problem („I'll know it when I see it"). Vieles von dem, was ein Nutzer braucht, ist *stillschweigendes Wissen* (tacit knowledge, Polanyi): real, tragend, aber nie geäußert, weil der Nutzer es nicht bewusst als formulierbaren Fakt besitzt.
...
- **Er wird missverstanden.** Selbst eine wohlgeformte Anfrage kann der Agent fehllesen. Der gefährliche Fall ist der *selbstsichere Fehlgriff*: Der Agent fährt fort, als hätte er verstanden, produziert ein plausibles Artefakt, und die Lücke zeigt sich erst, nachdem teure Arbeit getan ist.
...
Diese Spezifikation definiert dieses Interview-Verfahren und diese KPI. Die KPI ist ein **Confidence-Score je Anforderungsdimension** plus eine **Lücken-Matrix** über eine geschlossene Menge von Dimensionen; zusammen steuern (gaten) sie, ob der Agent eine Rückfrage stellt, welche Frage er stellt und wann das Interview abgeschlossen ist. Das Verfahren ist modell- und domänenunabhängig: Es schreibt die *Form* des Interviews und die *Gestalt* der Metrik vor, nicht eine bestimmte ML-Implementierung oder einen bestimmten Prompt.

Das Modell ist in etablierter Literatur verankert, nicht erfunden: die empirische Typologie von Erfassungs-Interviewfragen und die offen→spezifisch-Sequenzheuristik [R1]; die gemessene Baseline, dass ein LLM-Interviewer ~74 % Anforderungs-Recall erreicht und dass leichtgewichtiges One-Question-at-a-time-Prompting schwere prozedurale Skriptung schlägt [R3]; die Auswahl von Klärungsfragen als Bayessches Versuchsdesign, das den erwarteten Informationsgewinn über den *Lösungsraum* maximiert [R4]; die Trennung von *Spezifikationsunsicherheit* (was der Nutzer will) und *Modellunsicherheit* (was der Agent vorhersagt) sowie EVPI-bewertete, kostenbewusste Frageauswahl [R5], [R12]; der verhaltensbasierte Self-Consistency-Proxy für Mehrdeutigkeit — mehrere Interpretationen sampeln, Divergenz als Mehrdeutigkeitssignal werten [R6]; kontrollierte-natürliche-Sprache-Zielstrukturen (EARS, Rimay), die atomare, eindeutige, vollständige Anforderungen erzwingen [R7], [R9]; und der recall-favorisierte Mehrdeutigkeitserkennungs-Trade-off mit seiner benannten, trigger-wort-gestützten Defekt-Taxonomie [R8], [R10].
...
- **MUST** je anwendbarer Dimension einen **Confidence-Score** `c_d ∈ [0,1]` führen, der ausdrückt, wie gut der Agent diese Dimension zu verstehen glaubt
...
- Die adversariale Verifikationsphase des ursprünglichen Deep-Research-Durchlaufs wurde nicht abgeschlossen (Session-Limit-Enthaltung über alle 25 Claims), daher sind die zitierten Methoden aus Primärquellen berichtet, aber nicht unabhängig trianguliert. Ein nachgelagerter Verifikationsdurchlauf würde die Evidenz-Confidence der Schwellenwert- und Metrik-Claims erhöhen.
- Soll diese Spec einen begleitenden Skill/Agent ausliefern, der das Interview *operationalisiert* (den Dialog führt, die Lücken-Matrix pflegt, das Gating anwendet), analog dazu, wie `audience-identification` mit dem `audience-identify`-Skill gepaart ist? Die Spec definiert derzeit das Verfahren; die ausführende Capability ist ein wahrscheinlicher nächster Schritt.
- Wo genau soll das Output-Artefakt liegen, und in welchem Format (ein strukturiertes `REQUIREMENTS.md` neben der Arbeit, Frontmatter auf einer Feature-Datei oder ein ADR-artiger Eintrag)? Dies sollte sich an dem nachgelagerten Konsumenten (`feature`, `spec-driven-development`) ausrichten, der es aufnimmt.
