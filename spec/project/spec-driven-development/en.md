...
- Automated enforcement of the spec-anchor rule for PR bodies and skill/agent frontmatter isn't yet wired. **Default (in force):** the spec-anchor MUST is operator-enforced at authoring and PR time, and `spec-drift-audit` is the long-tail detector for un-anchored skills and agents (Requirement #4, acceptance criterion on line 59); enforcement tooling stays deferred per Non-Goals. The exemption encodings—Renovate and Dependabot per Requirement #2, `exp/`-branches per the promotion MUST, cosmetic edits per the cosmetic-edit MAY—already define the lint's exact failure surface. **Revisit when** either at least three implementation-path PRs reach `develop` missing a `Refs spec/` line that `spec-drift-audit` had to catch after the fact (the manual rule demonstrably leaks), or the gh-plumbing `reusable-pr-lint` workflow gains any body-content assertion beyond the five section headings. At that point, harden by extending that `reusable-pr-lint` workflow with a `Refs spec/` assertion rather than authoring a standalone `spec-anchor-lint` skill, because the marginal cost of the extension is then near zero.
