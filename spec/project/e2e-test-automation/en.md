...
- A seeding fixture **MUST NOT** establish state the running system would refuse to create, along any dimension a test relies on. This is this tier's application of `spec/project/test-pyramid-foundation/` §"Arrangement fidelity"; that spec owns the rule and its exemption, and this bullet **MUST NOT** restate either. What's specific here is the **shape the exposure takes at this tier**: an E2E suite typically doubles no collaborator at all, so its arrangement is the fixture, and a fixture that writes straight into the store skips whatever the application's own creation path enforces—a caller-supplied identifier that path discards, a uniqueness rule only the application checks. The browser assertions above stay specific and falsifiable while certifying a state the system can't reach, and the run that finally fails does so in another test's setup rather than in the one that seeded it
