...
- Ein Seeding-Fixture **DARF NICHT** Zustand herstellen, dessen Anlage das laufende System verweigern würde, entlang keiner Dimension, auf die sich ein Test stützt. Dies ist die Ausprägung von `spec/project/test-pyramid-foundation/` §„Arrangement-Treue“ für diese Stufe; jene Spec verantwortet die Regel und ihre Ausnahme, und dieser Punkt **DARF** weder das eine noch das andere wiederholen. Spezifisch ist hier die **Form, die das Risiko auf dieser Stufe annimmt**: Eine E2E-Suite doubelt typischerweise überhaupt keinen Kollaborateur, ihr Arrangement ist also das Fixture — und ein Fixture, das direkt in den Speicher schreibt, überspringt, was der Anlage-Pfad der Anwendung durchsetzt: einen vom Aufrufer gelieferten Bezeichner, den jener Pfad verwirft, eine Eindeutigkeitsregel, die nur die Anwendung prüft. Die Browser-Assertions oben bleiben spezifisch und falsifizierbar und bestätigen dabei einen Zustand, den das System nicht erreichen kann; der Lauf, der schließlich scheitert, tut es im Setup eines anderen Tests statt in dem, der geseedet hat
