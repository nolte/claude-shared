...
- **MAY** carry one additional optional field, `cancelled_reason` (enum, optional)—one of `replanned`, `descope`, `obsoleted`, `paused-indefinitely`; **MUST** be null or absent unless `status` is `cancelled`, and when present it complements (never replaces) the mandatory prose rationale in `## Review notes`. This is the only field permitted beyond the nine required fields above.
- **MUST NOT** include effort estimates, velocity metrics, or assignment fields beyond what's declared above; lints flag unknown frontmatter keys—the only key tolerated beyond the nine required fields is the optional `cancelled_reason`.
...
- **MAY** cross-reference an out-of-band release (a mid-sprint hot-fix tracked per the sibling `release-artifact` spec §Out-of-band artefacts) in the active sprint's `## Review notes` as documentation only; such entries are fully independent and never part of closure validation.
...
- **MAY** mark a sprint `cancelled` when reality changes faster than the sprint can adapt, and **MAY** reach `cancelled` from `planned`, `active`, or `review`; cancellation is a first-class outcome, not a failure state, and **MUST** be reflected in `## Review notes` with a one-paragraph rationale before `cancelled` is final. The rationale **MUST** state which lifecycle stage the sprint was in when cancelled and why recovery wasn't feasible at this time. The sprint **MAY** additionally set the optional `cancelled_reason` enum (see §Frontmatter schema) to give consuming skills a machine-parseable cancellation signal; the enum never substitutes for the prose rationale, it complements it.
...
- [ ] Every sprint's frontmatter carries the nine required schema fields (`number`, `status`, `started`, `ended`, `value_statement`, `artifact_ref`, `last_commit`, `roadmap_items`, `features`) in the declared order, plus at most the optional `cancelled_reason` field; lints flag unknown keys (tolerating only `cancelled_reason` beyond the nine) and reject `closed` as a frontmatter field name (the field is named `ended` because it covers both `closed` and `cancelled` terminal states).
...
- Keep the single-active-sprint invariant unchanged; don't introduce a track identifier or relax the invariant until a concrete nolte portfolio repository demonstrably needs two genuinely concurrent value streams (for example a user-facing track and an infrastructure track) that can't be serialised into sequential sprints without blocking user-visible delivery. Observable signal: an operator opens (or requests) two sprint files that both must hold `status: active` simultaneously because the second track's value delivery is blocked behind the first track's multi-week duration. Until such a repo exists, the cost (a frontmatter track-identifier field, redefining "lowest-numbered planned sprint promotes to active," and rewriting the `sprint-execute` hard gate and `sprint-plan`) outweighs the speculative benefit.
- Defer a structured `value_statement` (audience-id + benefit). The audience-identification spec now provides stable per-audience entries with `track` fields that a structured statement could key against, so this is technically feasible; keep free text until a concrete consumer that groups delivered sprint value by audience (for example a portfolio-audit view or a generated per-audience index) is specified. Free text keeps the `sprint-plan` verb-based value-delivery heuristic simple in the meantime.
