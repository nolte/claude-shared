...
- [ ] Every closed sprint's `last_commit` is non-null, and the commit `artifact_ref` resolves to is equal to or reachable from it, matching `spec/project/release-artifact/`; `sprint-execute` is the canonical write authority for this field per §Frontmatter schema (writing it incrementally as features in the sprint reach `done`), and `sprint-review` at `active → review` confirms the field is non-null and consistent with `artifact_ref` rather than producing it.
