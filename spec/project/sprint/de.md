...
- **KANN [MAY]** ein zusätzliches optionales Feld tragen, `cancelled_reason` (Enum, optional) — einer von `replanned`, `descope`, `obsoleted`, `paused-indefinitely`; **MUSS [MUST]** null oder abwesend sein, sofern `status` nicht `cancelled` ist, und wenn vorhanden, ergänzt es (ersetzt niemals) die verpflichtende Begründungs-Prosa in `## Review notes`. Dies ist das einzige Feld, das jenseits der neun oben verpflichtenden Felder erlaubt ist.
- **DARF NICHT [MUST NOT]** Aufwandsschätzungen, Velocity-Metriken oder Zuweisungsfelder jenseits der oben deklarierten enthalten; Lints flaggen unbekannte Frontmatter-Schlüsselwörter — der einzige jenseits der neun verpflichtenden Felder geduldete Schlüssel ist das optionale `cancelled_reason`.
...
- **MUSS [MUST]** ein nicht-leeres `value_statement` aus Endnutzer-Sicht tragen; operator-interne Sprache ist kein Mehrwert-Statement und **MUSS [MUST]** von `sprint-plan` abgelehnt werden, bevor die Sprint-Datei persistiert wird. Die Ablehnungsregel **MUSS [MUST]** mindestens dann greifen, wenn das Statement mit einem internen Verb in der Hauptsprache des Projekts beginnt oder darum aufgebaut ist (`refactor`, `restructure`, `set up`, `configure`, `clean up`, `migrate`, `bump`, `update dependency`, sowie dieselbe Liste auf Deutsch: `refaktorieren`, `umbauen`, `einrichten`, `konfigurieren`, `aufräumen`, `migrieren`, `aktualisieren`, `Abhängigkeit erneuern`); `sprint-plan` **MUSS [MUST]** die Verletzung mit dem zitierten beanstandeten Verb sichtbar machen und **KANN [MAY]** die Regelliste pro Projekt erweitern. Eine Pro-Repo-Erweiterung lebt unter der bestehenden Override-Oberfläche `.github/release-skill-layer.yml` (zum Beispiel ein `sprint_rejection_verbs:`-Schlüssel), derselben Override-Datei, die die Geschwister-Specs `release-artifact` und `release-skill-layer` nutzen; ein Projekt **DARF NICHT [MUST NOT]** für diesen Zweck eine neue Override-Datei (etwa `.github/sprint-rejection-rules.yml`) einführen. Die Prüfung ist eine Heuristik, kein vollständiger Klassifikator — Operatoren, die wirklich eine endnutzer-orientierte Veränderung liefern und deren Formulierung zufällig mit einem dieser Verben startet, **KÖNNEN [MAY]** die Ablehnung mit einer einzeiligen Begründung in der `## Goal`-Section des Sprints überstimmen.
...
- **KANN [MAY]** ein Out-of-Band-Release (ein Mid-Sprint-Hotfix, erfasst laut Geschwister-Spec `release-artifact` §Out-of-Band-Artefakte) in den `## Review notes` des aktiven Sprints rein dokumentarisch querverweisen; solche Einträge sind vollständig unabhängig und nie Teil der Abschluss-Validierung.
...
- [ ] Das Frontmatter jedes Sprints trägt die neun verpflichtenden Schema-Felder (`number`, `status`, `started`, `ended`, `value_statement`, `artifact_ref`, `last_commit`, `roadmap_items`, `features`) in der festgelegten Reihenfolge, plus höchstens das optionale Feld `cancelled_reason`; Lints flaggen unbekannte Schlüsselwörter (dulden jenseits der neun nur `cancelled_reason`) und lehnen `closed` als Frontmatter-Feldname ab (das Feld heißt `ended`, weil es sowohl `closed` als auch `cancelled` als terminale Zustände abdeckt).
...
- Die Single-Active-Sprint-Invariante unverändert lassen; keinen Spur-Identifikator einführen und die Invariante nicht lockern, bis ein konkretes nolte-Portfolio-Repository nachweislich zwei wirklich gleichzeitige Mehrwert-Ströme braucht (zum Beispiel eine Endnutzer-Spur und eine Infrastruktur-Spur), die sich nicht in sequentielle Sprints serialisieren lassen, ohne endnutzer-sichtbare Auslieferung zu blockieren. Beobachtbares Signal: ein Operator legt zwei Sprint-Dateien an (oder fordert sie an), die beide gleichzeitig `status: active` halten müssen, weil die Mehrwert-Auslieferung der zweiten Spur hinter der mehrwöchigen Dauer der ersten Spur blockiert ist. Bis ein solches Repo existiert, überwiegen die Kosten (ein Frontmatter-Spur-Identifikator-Feld, das Neudefinieren von "niedrigst-nummerierter `planned`-Sprint wird `active`" sowie das Umschreiben des `sprint-execute`-Hard-Gates und von `sprint-plan`) den spekulativen Nutzen.
- Ein strukturiertes `value_statement` (Audience-ID + Nutzen) verschieben. Die Geschwister-Spec audience-identification liefert jetzt stabile Pro-Audience-Einträge mit `track`-Feldern, gegen die ein strukturiertes Statement schlüsseln könnte, sodass dies technisch machbar ist; Freitext beibehalten, bis ein konkreter Konsument spezifiziert ist, der ausgelieferten Sprint-Mehrwert nach Audience gruppiert (zum Beispiel eine portfolio-audit-Sicht oder ein generierter Pro-Audience-Index). Freitext hält die verb-basierte Mehrwert-Heuristik von `sprint-plan` in der Zwischenzeit einfach.
