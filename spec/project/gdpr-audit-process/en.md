...
- **MUST**, when the repository **publishes a static site** (a documentation or landing site built by MkDocs, Docusaurus, Astro, Hugo, Jekyll, or similar and deployed to a host such as GitHub Pages, Netlify, Vercel, or Cloudflare Pages), discover that published-site surface from the build config and the CI deploy workflow, including any cookie-consent, privacy-notice, or imprint pages it ships—the published site is a personal-data surface the application-code scan alone never reaches
...
- **MUST** audit the **published-site hosting and transparency layer** (Art. 13, 6(1)(f), 44–49) when the repository publishes a static site: the hosting provider processes visitor connection data—including the **IP address** (an online identifier under Art. 4(1))—in server logs the repository's own code never shows, and a host outside the EU/EEA (for example GitHub Pages, served by a US provider) makes this a third-country transfer; the **presence or absence of a privacy notice** (Art. 13) reachable from the published site is a code-verifiable signal, while the legal sufficiency of that notice and any **imprint obligation** under local law (for example § 5 DDG for a public, non-private offering) are legal-review-required. Delimit this from the cookies/trackers dimension (which audits what the *site loads*) and from the processors dimension (which audits services the *application code* calls)—this dimension audits the *host of the published site itself* and the site's transparency artifacts
...
- [ ] When the repository publishes a static site, the report identifies the deploy host (for example GitHub Pages), flags the host-level visitor-IP logging and any resulting third-country transfer (Art. 13, 44–49), and reports the presence or absence of a privacy notice reachable from the published site; an imprint obligation under local law is surfaced as legal-review-required, never as a verdict
...
- [R7] GDPR Article 5 (principles), Article 13 (information to be provided / privacy notice), Article 25 (data protection by design and by default), Article 30 (records of processing), Article 32 (security of processing), Articles 12–22 (data-subject rights), Articles 44–49 (transfers to third countries)
- [R8] EU GDPR compliance checklist: <https://gdpr.eu/checklist/>
- [R9] German Telecommunications-Digital-Services Data Protection Act (TDDDG, successor to the TTDSG) governing cookies/trackers and telemedia; ePrivacy Directive 2002/58/EC; § 5 German Digital Services Act (DDG, successor to § 5 TMG) governing the imprint obligation for public, non-private offerings
