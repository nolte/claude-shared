...
- Wenn die CI eines Repositorys eine Coverage-Schwelle **bereits** als **erforderlichen Status-Check erzwingt**, **SOLLTE** dieses Repository denselben Coverage-Lauf als Gate-Kategorie bereitstellen (gemäß der DARF-Erweitern-Regel oben), damit das lokale Gate einer beitragenden Person eine Schwellen-Regression aufzeigt, bevor CI es tut — sonst wird Coverage genau die „CI wird zum ersten Ort, an dem Fehler auftauchen"-Kostenstelle, die der §Kontext dieser Spec beseitigen soll. Das macht Coverage **nicht** zu einem Ziel: Die Guide-nicht-Ziel-Haltung von `spec/project/test-pyramid-foundation/` §„Coverage und Suite-Qualitätsmetriken" gilt weiterhin, und dieser Punkt spiegelt nur ein Gate, das das Repository bereits zu erzwingen gewählt hat, damit die „identisch ausführen"-Invariante auch es abdeckt. Wo CI keine Coverage-Schwelle erzwingt, bleibt der Standard (Coverage bleibt aus dem lokalen Gate heraus) unverändert
...
### Erzwungene Lane je Stufe
`spec/project/pull-request-workflow/` §"CI-Gate nach `develop`" besitzt das *Wie* der erforderlichen Status-Checks: deklariert als Code in `.github/settings.yml`, niemals über die GitHub-Oberfläche. Es sagt nicht, *welche* Checks darin stehen müssen. Ein Repository kann jede Regel jenes Abschnitts erfüllen und dabei nichts erzwingen, was das Gate tatsächlich ausführt — und der Deklarations-gegen-Plattform-Audit in `nolte/gh-plumbing` (`reusable-branch-protection-audit.yaml`) bleibt dabei durchgehend grün, weil er die Deklaration gegen das vergleicht, was GitHub erzwingt, und nicht gegen das, was das Repository testet.

Genau diese Lücke schließt dieser Abschnitt. Er ist eine Vollständigkeitsregel über der erforderlichen Menge und steht hier statt in `pull-request-workflow`, weil die Stufen, über die er quantifiziert, die Kategorien des Gates (§Zusammensetzung) und die Teststufen aus `spec/project/test-pyramid-foundation/` sind.

- **MUSS** für jede Stufe, die das Gate ausführt, mindestens einen erforderlichen Status-Check für `develop` haben, der diese Stufe ausführt. Eine Stufe ist eine Gate-Kategorie gemäß §Zusammensetzung; wo die Tests des Repositorys gemäß `test-pyramid-foundation` gestuft sind, zählt jede vom Gate ausgeführte Teststufe eigenständig. Eine Lane je Stufe ist die Messlatte; ein Check je Suite ist nicht nötig
- **MUSS** einen Check, der eine *andere* Stufe ausführt, als keine Abdeckung für diese Stufe werten. Eine erforderliche Frontend-Build-Lane erzwingt keine Backend-Teststufe, und eine Lint-Lane erzwingt keine von beiden. Behauptet wird die Abbildung von Stufe auf Lane, also **MUSS** sie aus der Deklaration lesbar sein: benennt der Kontextname der Lane ihre Stufe nicht, trägt die Deklaration einen Kommentar, der das tut
- **MUSS** eine Ausnahme, wenn eine Stufe keine erzwungene Lane hat, direkt neben der Deklaration der erforderlichen Checks in `.github/settings.yml` festhalten, und diese Ausnahme **MUSS** sowohl einen Grund als auch die Bedingung tragen, unter der sie zurückgenommen wird. Eine Ausnahme ohne Rücknahmebedingung ist ein dauerhaftes Loch mit einer Entschuldigung daneben
- **DARF NICHT** eine Ausnahme für eine Stufe festhalten, die einen Check ausführt, auf den sich das Repository verlässt, um eine bestimmte Defektklasse zu blockieren. Eine Ausnahme sagt „diese Stufe darf zur Merge-Zeit rot sein"; über einem solchen Check gelesen heißt das „diese Defektklasse darf zurückkommen" — das Gegenteil des Grundes, aus dem der Check geschrieben wurde. Ist die Stufe als Ganzes tatsächlich zu instabil zum Erzwingen, wird der Guard in eine eigene Lane getrennt und diese Lane erzwungen, statt dass der Guard in einer ausgenommenen mitfährt
- **MUSS** die Rücknahmebedingung für jemand anderen als ihre Autorin prüfbar machen: ein benanntes Issue, ein benannter Defekt oder eine Bedingung, die eine Außenstehende gegen das Repository auswerten kann. „Sobald die Suite stabil ist" ist eine Stimmung, keine Bedingung
- **DARF NICHT** eine beratende Lane als erzwungene zählen. Ein Job, der die Stufe ausführt, aber außerhalb der erforderlichen Menge berichtet oder unter `continue-on-error` läuft, kann keinen Merge blockieren — und nur um diese Eigenschaft geht es hier
- **SOLLTE** die erforderlichen Kontexte nach den Taskfile-Targets benennen, die sie aufrufen, damit die Abbildung von Stufe auf Lane ohne Kommentar lesbar ist
- Ein Repository, dessen CI eine Stufe erzwingt, die das lokale Gate nicht ausführt, hat die umgekehrte Lücke, und §Invocation-Vertrag verbietet sie bereits: das Gate läuft an beiden Orten identisch

Die stärkere Form dieser Regel ist eine Vollständigkeitsprüfung auf Workflow-Ebene, die die Stufen eines fremden Repositorys erkennt und gegen dessen erforderliche Menge hält. Sie ist bewusst zurückgestellt: das Erkennungsprädikat muss sich erst über den Agent `quality-gate-enforcer` über mehrere Repositories hinweg bewähren, bevor es sich lohnt, es in geteilte CI einzufrieren — und ein Detektor, der eine Stufe falsch rät, lässt ein Repository für eine Stufe durchfallen, die es gar nicht hat.
...
- [ ] Jede Stufe, die das Gate ausführt, hat mindestens einen erforderlichen Status-Check für `develop` in `.github/settings.yml` deklariert — oder eine neben dieser Deklaration festgehaltene Ausnahme mit Grund und Rücknahmebedingung
- [ ] Für jeden deklarierten erforderlichen Kontext ist die von ihm ausgeführte Stufe aus dem Namen oder einem danebenstehenden Kommentar lesbar, sodass die Abbildung von Stufe auf Lane ohne Öffnen des Workflows prüfbar ist
- [ ] Keine Stufe gilt als erzwungen aufgrund einer Lane, die außerhalb der erforderlichen Menge berichtet oder unter `continue-on-error` läuft
- [ ] Keine Ausnahme deckt eine Stufe ab, die einen Check ausführt, auf den sich das Repository zum Blockieren einer Defektklasse verlässt; sitzt ein solcher Check in einer instabilen Stufe, bekommt er stattdessen eine eigene erforderliche Lane
- [ ] Die Rücknahmebedingung jeder Ausnahme benennt ein Issue oder eine Bedingung, die auch andere als ihre Autorin auswerten können
- [ ] Der Skill `plugins/nolte-engineering/skills/quality-gate/` ruft zuerst die Taskfile-Targets des Repositorys auf und fällt nur dann auf native Werkzeug-Erkennung zurück, wenn kein passendes Target existiert
