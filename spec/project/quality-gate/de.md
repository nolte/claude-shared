...
- **MUSS** aus `spec/project/test-falsifiability/` abgeleitete Suite-Qualitätssignale (insbesondere den Mutation-Score) aus den Pflicht-Kategorien heraushalten; sie bleiben per Default advisorisch, und ein Repository **KANN** einen Mutation-Lauf als zusätzliche deklarierte Kategorie gemäß der MAY-extend-Regel in §Komposition exponieren — dieselbe Begründung wie beim Coverage-Thresholding-Ausschluss
