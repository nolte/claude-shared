...
### Composition
- **MUST** include three categories when the repository has relevant code for each: lint, type-check, tests; categories without relevant code (for example type-check in a pure shell repo) are not required
...
- **SHOULD** be invokable from a pre-commit hook when the repository uses pre-commit; repositories that don't use pre-commit rely on the contributor's explicit invocation
...
- [ ] The gate's overall timeout budget per category does not exceed the limits in §Timeouts, or the Taskfile target explicitly documents a longer runtime
