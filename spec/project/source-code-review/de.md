...
- **D11—Guard-Abdeckung für Defektklassen.** Ob die Guards, die das Repository bereits trägt, die Klassen noch halten, für die sie geschrieben wurden, bewertet gegen `spec/project/defect-class-guards/`: ein Guard, dessen Prädikat die Fundstellen statt der Klasse benennt (G3); ein Guard, der seine Subjekte über Dateinamen oder Verzeichnis auswählt, während er eine Eigenschaft der zusammengesetzten Anwendung behauptet (G6); ein Allowlist-Eintrag ohne Begründung oder ohne passendes Subjekt (G4); ein Guard, dessen Name oder Einschränkung keine Issue-Nummer trägt (G5). Diese Dimension prüft **bestehende** Guards, unabhängig davon, ob in der geprüften Änderung ein Defekt behoben wird; ob ein **neuer** Fix einen Guard hinterlässt, ist Aufgabe des Abschlussprozesses gemäß `pull-request-workflow` §"Klassen-Sweep (Conventional-Commits-Typ `fix`)" und nicht die der Reviewerin. Ob die Lane eines Guards erzwungen ist, wird nach `quality-gate` geroutet statt hier beurteilt.

### Sprachprofile

Eine **Oberflächen-Erweiterung** darf diesem Kern Dimensionen hinzufügen, wo ein Review-Gegenstand Belange trägt, die kein Sprachprofil ausdrücken kann. `spec/frontend/source-code-review/` ist die erste: Für browsergerenderten Code legt sie Frontend-Dimensionen (F1–F11) und eine Framework-Profil-Achse über D1–D11. Eine Erweiterung übernimmt die Tooling-first-Regel, das Schweregrad-Vokabular, den Report-Vertrag und den Reviewer-Vertrag dieser Spec unverändert, wiederholt nie eine Kern-Regel und taggt jedes Finding mit genau einer Dimensions-ID.
...
- **MUSS** jedes Finding mit file:line, seiner Dimensions-ID (D1–D11), seinem `production`- oder `test`-Marker und dem Status confirmed oder suspected attribuieren; ein unsicheres Finding wird als suspected gemeldet, nie still verworfen
...
- [R10] Falsifizierbarkeits-Taxonomie und Detektionskriterien hinter D6s Assertionsfrei- und Always-green-Test-Regel und der Testcode-Instanz von D1s Swallowed-Error-No-Go: `spec/project/test-falsifiability/`
