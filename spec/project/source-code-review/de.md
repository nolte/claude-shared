...
- **D1—Korrektheit und Robustheit.** Logikfehler, unbehandelte Randfälle (leer, Grenzwert, Überlauf, Encoding), fehlende Fehlerbehandlung fehlbarer Operationen, Eingabevalidierung an Vertrauensgrenzen, Ressourcen-Leaks, Race Conditions und geteilter veränderlicher Zustand, Off-by-one- und Reihenfolge-Annahmen.
...
- **Performance-Idiome (D7):** `str.join` statt Konkatenation in Schleifen; Set-/Dict-Membership statt Listen-Scans; Generatoren für Streaming statt materialisierter Listen; `functools.lru_cache` nur mit begrenzten, hashbaren Eingaben; Batch-I/O statt Round-Trips pro Element.
