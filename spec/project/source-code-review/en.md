...
- **D11—Defect-class guard coverage.** Whether the guards the repository already carries still hold the classes they were written for, assessed against `spec/project/defect-class-guards/`: a guard whose predicate names the sites the defect was found at rather than the class (G3), a guard selecting its subjects by filename or directory while asserting a property of the assembled application (G6), an allowlist entry without a reason or one that no longer matches anything (G4), and a guard whose name or constraint carries no issue number (G5). This dimension reviews **existing** guards, independently of any defect being fixed in the change under review; whether a **new** fix leaves a guard behind is the closing process's job per `pull-request-workflow` §"Class sweep (Conventional-Commits type `fix`)" instead of the reviewer's. Whether a guard's lane is enforced routes to `quality-gate` rather than being judged here.

### Language profiles

A **surface extension** may add dimensions on top of this core where a review subject carries concerns no language profile can express. `spec/frontend/source-code-review/` is the first: for browser-rendered code it overlays frontend dimensions (F1–F11) and a framework-profile axis onto D1–D11. An extension inherits this spec's tooling-first rule, severity vocabulary, report contract, and reviewer contract unchanged, never restates a core rule, and tags each finding with exactly one dimension ID.
...
- **MUST** attribute every finding with file:line, its dimension ID (D1–D11), its `production` or `test` marker, and whether it's confirmed or suspected; an uncertain finding is reported as suspected, never silently dropped
...
- [ ] Every finding carries file:line, a D1–D11 dimension ID, a `production`/`test` marker, and a confirmed/suspected flag
...
- [R10] Falsifiability taxonomy and detection criteria behind D6's assertion-free and always-green-test rule and the test-code instance of D1's swallowed-error no-go: `spec/project/test-falsifiability/`
