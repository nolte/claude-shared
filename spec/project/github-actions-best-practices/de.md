...
- **MUSS** die Laufzeit-Beobachtung von Runner-Slot-Knappheit (Wartezeit, durch Churn verursachte Abbrüche) an `spec/project/workflow-health/` §„Abbruchraten" verweisen; §L bindet nur die Entwurfsregel, dass ein Job Slot-Kosten hat
...
### L. Runner-Slot-Ökonomie

GitHub-gehostete Runner führen Jobs aus einem Kontingent gleichzeitiger Jobs pro GitHub-Plan aus (Free 20, Pro 40, Team 60, Enterprise 500 Standard-Jobs zum Zeitpunkt des Schreibens [R13][R14][R15]), und aus diesem Kontingent schöpfen alle Repositories des Accounts zugleich [R14][R15]: Ein in einem Repository wartender Job wartet auf einen Slot, den ein Job eines anderen Repositories hält. In diesem Portfolio am 2026-09-19 gemessen, warteten `needs:`-freie Jobs von 16–68 Sekunden in diesem Repository 10–24 Minuten auf einen Runner, während es sonst nichts ausführte, die abhängigkeitsfreien ersten Jobs des konsumierenden Projekts warteten im Median 20–26 Minuten, und zwei Drittel aller Jobs endeten unter einer Minute und trugen dabei 7 % der Arbeit (`nolte/claude-shared#644`). Auf dieser Plattform ist ein Job, nicht eine Sekunde Laufzeit, die Einheit der knappen Ressource, und die Behebung „nebenläufig ausführen" aus `continuous-integration` §B ist nicht kostenlos.

- **MUSS** das Account-weite Kontingent gleichzeitiger Jobs als endliche, von allen Repositories des Accounts geteilte Ressource behandeln und beim Entwurf eines Workflows die *Slot*-Kosten eines Jobs einrechnen, nicht nur seine Laufzeit. Das ist die Plattform-Bindung von `spec/project/continuous-integration/` §B „die Stufe günstiger machen"; gemäß §J wird diese Regel referenziert, nicht wiederholt
- **MUSS**, wenn eine Pipeline als langsam beurteilt wird, die Wartezeit auf einen Runner-Slot getrennt von der Job-Laufzeit messen, weil beide gegensätzliche Behebungen haben: Ein Laufzeitproblem wird im Job behoben, ein Warteproblem durch weniger angeforderte Jobs. Die Beobachtungspflicht zur Laufzeit und die Leseregel liegen in `spec/project/workflow-health/` §„Abbruchraten"; das Messrezept liefert der Skill `cicd-pipeline-design` mit
- **SOLLTE** Arbeit, die deutlich unter einer Minute endet, in einem bestehenden Job halten, statt einen eigenen dafür auszugeben, es sei denn, sie braucht ein anderes Runner-Image, andere `permissions`, Matrix-Parallelität oder muss unter eigenem Namen als Required Status Check berichten — die vier Gründe, die einen eigenen Job wirklich erfordern
- **SOLLTE** einen Gate-Job unter einer Minute aus `needs:` vor den teuren Jobs heraushalten: Unter einem gesättigten Pool kostet jeder Abhängigkeitssprung eine volle Slot-Wartezeit, sodass ein 14-Sekunden-Linter allem dahinter 20 Minuten hinzufügen kann
- **KANN** beratende Checks zu einem Job zusammenlegen, sofern jeder zusammengelegte Schritt weiterhin sein eigenes Urteil berichtet (`if: always()` auf den Folgeschritten, eine Step-Summary je Check), damit die Konsolidierung Durchsatz nicht mit einem Urteil bezahlt, das niemand mehr zuordnen kann
...
- [ ] Kein Workflow gibt einen eigenen Job für Arbeit unter einer Minute aus, ohne dass einer der vier genannten Gründe vorliegt, und kein Gate-Job unter einer Minute sitzt per `needs:` vor den teuren Jobs; beides ist allein aus der Workflow-Datei prüfbar, ohne etwas auszuführen
- [ ] §L referenziert `continuous-integration` §B und `workflow-health` §„Abbruchraten", statt eines von beiden zu wiederholen, und seine Knappheitsaussage trägt eine datierte Quelle in §Referenzen, weil die Concurrency-Zahlen planabhängig sind und altern werden
...
- [R13] *Actions limits* (GitHub Docs, **Primär**, gelesen 2026-09-19): die Tabelle gleichzeitiger Jobs je Plan (Free 20, Pro 40, Team 60, Enterprise 500 Standard-Jobs; ein geteiltes macOS-Teillimit). Die Seite nennt die Zahlen, nicht die Account-weite Geltung in Worten; diese ist in diesem Portfolio durch Beobachtung belegt, wo Jobs eines Repositories hinter Jobs eines anderen warteten, während das erste Repository untätig war (`nolte/claude-shared#644`, am 2026-09-19 in der Folgegruppe zu `nolte/claude-shared#648` erneut gemessen)
- [R14] *Limitations for Concurrent Workers per repo or per organization?* (GitHub-Community-Diskussion #26343, **Sekundär**, akzeptierte Antwort 2020-12-03, gelesen 2026-09-19): Die Gesamtzahl gleichzeitiger Jobs wird je Abrechnungseinheit (Nutzer oder Organisation) gewährt, sodass Repositories eines Free-Plans die 20 Worker teilen, statt je 20 zu erhalten
- [R15] *Concurrency Limit* (WarpBuild-Glossar, **Tertiär**, unabhängiger Runner-Anbieter, gelesen 2026-09-19): Die Obergrenze folgt dem Plan des Accounts, die Zählung ist Account-weit, sodass ein großer Fan-out in einem Repository Workflows in jedem anderen Repository desselben Accounts verzögert, und Jobs über der Obergrenze warten, bis ein Slot frei wird; der Earthly-Artikel *Making the Most of Concurrency in GitHub Actions* (2023-10-17) nennt dieselben Plan-Zahlen
...
- Die zweite Regel von §F leitet die Gruppe aus der Workflow-Identität plus einer Branch- oder Pull-Request-Identität ab. Wo die umkämpfte Ressource repository-global ist, kann keine korrekte Konfiguration das erfüllen: Ein Release-Draft, ein Pages-Deployment-Ziel und ein quartalsweises Tracking-Issue existieren je einmal pro Repository, sodass ein Schlüssel mit einer dieser Identitäten genau die Lane aufteilt, die sie schützt — und zwei Workflows, die um eine Ressource konkurrieren, können eine Lane nur über ein gleiches Literal teilen. Ob §F eine ressourcengescopte Form zulassen sollte, bei der die Gruppe die umkämpfte Ressource benennt und `cancel-in-progress` auf false bleibt, oder ob die Regel bestehen bleibt und solche Gruppen als Abweichungen dagegen festgehalten werden, ist nicht entschieden.
