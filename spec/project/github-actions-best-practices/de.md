...
- **MUSS**, wo der deklarierte Auslöser Eingaben annimmt (`workflow_dispatch`, `workflow_call`), diesen Wert in einer deklarierten Eingabe führen und als `${{ inputs.<name> || github.event.<feld> }}` durchreichen, sodass die Bedienperson liefert, was das Payload nicht liefern kann. Wo der Auslöser überhaupt keine Eingaben annimmt (`merge_group`, `push`, `schedule` und jedes andere reine Payload-Ereignis), **MUSS** der durchgereichte Ausdruck stattdessen aus einem Kontext auflösen, den dieses Ereignis tatsächlich füllt; ein Auslöser, der weder eine Eingabe tragen noch den Wert aus dem eigenen Payload liefern kann, **DARF NICHT** an diesen Reusable-Job verdrahtet werden, statt ihn zur Laufzeit scheitern zu lassen. §K verlangt `merge_group` an jedem Workflow hinter einem Required Check, dieser Fall tritt also praktisch auf und nicht bloß hypothetisch
