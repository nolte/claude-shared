...
- **MUSS** für jede Eingabe eines aufgerufenen Reusable Workflows, deren Vorgabewert aus einem bestimmten Ereignis-Payload abgeleitet ist, diese Eingabe aus dem aufrufenden Job unter jedem Auslöser, den der Aufrufer deklariert, ausdrücklich durchreichen. Ein solcher Vorgabewert — etwa `${{ github.event.release.tag_name }}` — gilt nur für das Ereignis, das dieses Payload trägt; unter jedem anderen Auslöser löst er zur leeren Zeichenkette auf, sodass ein Aufrufer, der sich auf den Vorgabewert verlässt, auf jedem Pfad außer dem des Payloads wirkungslos ist. Die Regel hängt nicht daran, wie viele Auslöser der Aufrufer deklariert: Ein Single-Trigger-`workflow_dispatch`-Aufrufer eines Release-Payload-Vorgabewerts ist auf seinem einzigen Pfad wirkungslos
- **MUSS**, wo der deklarierte Auslöser Eingaben annimmt (`workflow_dispatch`, `workflow_call`), diesen Wert in einer deklarierten Eingabe führen und ausdrücklich durchreichen, sodass die Bedienperson liefert, was das Payload nicht liefern kann. `${{ inputs.<name> || github.event.<feld> }}` ist die idiomatische Form **nur** für einen Wert, der niemals berechtigterweise `false`, `0` oder die leere Zeichenkette sein kann. `||` liefert seinen rechten Operanden gleichermaßen bei `false`, `0` und der leeren Zeichenkette, sodass die Anwendung auf eine Boolean-Eingabe ein ausdrückliches `false` der Bedienperson still verwirft — ein Dispatch mit `dry_run: false` fiele auf das Payload durch und wäre kein Trockenlauf mehr. Kann der Wert berechtigterweise einen dieser drei annehmen, statt `||` auf `github.event_name` verzweigen
- **MUSS**, wo der deklarierte Auslöser keine Eingaben annimmt (`merge_group`, `push`, `schedule` und jedes andere reine Payload-Ereignis), einen Wert durchreichen, der aus einem Kontext auflöst, den dieses Ereignis tatsächlich füllt. Existiert kein solcher Kontext, liegt die Lücke in der Schnittstelle des Reusables und nicht beim Aufrufer: Sie ist gemäß der ersten Regel dieses Abschnitts stromaufwärts zu adressieren. **DARF NICHT** dadurch gelöst werden, dass der Job per `if:` auf `github.event_name` übersprungen wird — das meldet den Required Check auf der Merge Group als `skipped`, also genau den Fehlschlag, den §K verhindern soll
...
- [ ] In jedem Workflow, der einen Reusable Workflow aufruft, wird jede aufgerufene Eingabe mit einem aus einem Ereignis-Payload abgeleiteten Vorgabewert ausdrücklich durchgereicht statt dem Vorgabewert überlassen, und der durchgereichte Ausdruck löst unter jedem vom Aufrufer deklarierten Auslöser auf
- [ ] Kein durchgereichter Ausdruck nutzt den `${{ inputs.x || … }}`-Rückfall für einen Wert, der berechtigterweise `false`, `0` oder die leere Zeichenkette sein kann
