...
- **MUST**, where the declared trigger accepts inputs (`workflow_dispatch`, `workflow_call`), carry that value in a declared input and forward it as `${{ inputs.<name> || github.event.<field> }}`, so the operator supplies what the payload can't. Where the trigger accepts no inputs at all (`merge_group`, `push`, `schedule` and every other payload-only event), the forwarded expression **MUST** resolve from a context that event does populate; a trigger that can neither carry an input nor supply the value from its own payload **MUST NOT** be wired to that reusable job, rather than being left to fail at run time. §K requires `merge_group` on every workflow backing a required check, so this case is reached in practice, not hypothetically
