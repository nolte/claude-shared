...
- **MUST**, for every input of a called reusable workflow whose default is derived from a specific event payload, forward that input explicitly from the calling job under every trigger the caller declares. Such a default—`${{ github.event.release.tag_name }}`, say—is valid only for the event carrying that payload; under any other trigger it resolves to the empty string, so a caller relying on the default is inert on every path except the payload's own. The rule doesn't depend on how many triggers the caller declares: a single-trigger `workflow_dispatch` caller of a release-payload default is inert on its only path
- **MUST**, where the declared trigger accepts inputs (`workflow_dispatch`, `workflow_call`), carry that value in a declared input and forward it explicitly, so the operator supplies what the payload can't. `${{ inputs.<name> || github.event.<field> }}` is the idiomatic form **only** where the left operand can never legitimately be `false`, `0`, or the empty string. `||` returns its right operand for all three, so applying it to a boolean input silently discards an operator's explicit `false`—a `dry_run: false` dispatch would fall through to the payload and stop being a dry run. Where the value can legitimately take one of those, branch on `github.event_name` instead of `||`
- **MUST**, where the declared trigger accepts no inputs (`merge_group`, `push`, `schedule` and every other payload-only event), forward a value resolved from a context that event does populate. Where no such context exists, the gap is in the reusable's interface rather than in the caller: raise it upstream per this section's first rule. **MUST NOT** resolve it by skipping the job with an `if:` on `github.event_name`—that reports the required check as `skipped` on the merge group, which is precisely the failure §K exists to prevent
...
- [ ] In every workflow calling a reusable one, each called input whose default is event-payload-derived is forwarded explicitly rather than left to the default, and the forwarded expression resolves under every trigger the caller declares
- [ ] No forwarded expression uses the `${{ inputs.x || … }}` fallback for a value that can legitimately be `false`, `0`, or the empty string
