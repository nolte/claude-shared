...
GitHub-hosted runners run jobs from one concurrent-job allotment per GitHub plan (Free 20, Pro 40, Team 60, Enterprise 500 standard jobs at the time of writing [R13][R14][R15]), and that allotment is drawn on by every repository of the account at once [R14][R15]: a job queued in one repository waits for a slot a job in another repository holds. Measured in this portfolio on 2026-09-19, `needs:`-free jobs of 16–68 seconds waited 10–24 minutes for a runner in this repository while it ran nothing else, the consuming project's dependency-free first jobs waited a median 20–26 minutes, and two thirds of all jobs finished under a minute while carrying 7 % of the work (`nolte/claude-shared#644`). On this platform a job, not a second of runtime, is the unit of the scarce resource, and `continuous-integration` §B's remedy "run it concurrently" isn't free.
...
- [R14] *Limitations for Concurrent Workers per repo or per organization?* (GitHub Community discussion #26343, **Secondary**, accepted answer 2020-12-03, read 2026-09-19): the concurrent-job total is granted per billing entity (user or organization), so repositories on a Free plan share the 20 workers rather than getting 20 each
- [R15] *Concurrency Limit* (WarpBuild glossary, **Tertiary**, independent runner vendor, read 2026-09-19): the ceiling follows the plan attached to the account, the count is account wide so a large fan-out in one repository delays workflows in every other repository under the same account, and jobs above the ceiling queue until a slot frees; the Earthly article *Making the Most of Concurrency in GitHub Actions* (2023-10-17) states the same per-plan figures
