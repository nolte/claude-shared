...
- [ ] No English-scoped prose carries gendered generic pronouns (`he`, `she`, `his`, `hers`, `he/she`), militaristic or ableist substitutions ratified by the Microsoft Bias-Free Communication table, exclamation marks outside genuine emphasis, or culturally specific idioms; `.vale.ini` runs the Microsoft `Gender`, `GenderBias`, and `BiasFree` rules at `error` level, so every offender those rules know fails the gate
