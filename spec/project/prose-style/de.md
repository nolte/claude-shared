...
### Regeln, die in der Praxis beißen

Die aktive Konfiguration ist maßgeblich (§Texterzeugung), aber einige ihrer Regeln überraschen Schreibende häufig genug, um benannt zu werden. Jede der folgenden ist ein Alert auf `error`-Ebene, keine stilistische Vorliebe. Jedes Beispiel steht in einem Code-Span, weil `IgnoredScopes` `code` abdeckt; als reine Prosa geschrieben würde dieser Abschnitt an genau den Regeln scheitern, die er dokumentiert.

- **MUSS [MUST]** Kontraktionen verwenden, wo der Microsoft-Stil sie erwartet (`it's`, `doesn't`, `isn't`, `can't`, `won't`). Eine Kontraktion zu vermeiden ist selbst ein Alert, keine neutrale Registerwahl
- **MUSS [MUST]** Geviertstriche in englisch-gescopten Dateien ohne umgebende Leerzeichen schreiben (`word—word`, nie `word — word`)
- **MUSS NICHT [MUST NOT]** einen Geviertstrich direkt an einen Inline-Code-Span oder an Fettdruck setzen. `IgnoredScopes` entfernt diese Spans, bevor die Strich-Regel läuft; der Strich wird dann als von Leerzeichen umgeben gelesen und schlägt an, obwohl die Quelle korrekt aussieht. Stattdessen einen Doppelpunkt setzen oder umformulieren
- **MUSS [MUST]** Satzzeichen innerhalb des schließenden Anführungszeichens setzen
- **MUSS [MUST]** die exakte Schreibweise eines akzeptierten Vokabular-Eintrags übernehmen, auch am Satzanfang oder in einem fetten Aufzählungs-Lead-in; ein kleingeschriebener Eintrag bleibt klein, ein Begriff wie `reusability` kann also keinen Satz eröffnen und muss umformuliert werden
- **MUSS NICHT [MUST NOT]** auf einen Doppelpunkt in einer Überschrift ein kleingeschriebenes Wort folgen lassen; den Doppelpunkt streichen oder das Folgewort großschreiben
- **SOLLTE [SHOULD]** einen Listeneintrag, der mit Inline-Code oder Fettdruck beginnt, mit einem Doppelpunkt statt einem Geviertstrich öffnen — das ist die Konvention, der der Spec-Korpus in seinen `## Referenzen`-Abschnitten bereits folgt, und sie umgeht die obige Nachbarschaftsregel vollständig

### Eine Regel unterdrücken

§Texterzeugung verbietet, einen Alert stummzuschalten, dessen eigentliche Lösung upstream liegt. Dieses Verbot bleibt bestehen; dieser Abschnitt schneidet die eine legitime Ausnahme heraus: eine Regel, die mit einem Vertrag kollidiert, den eine andere Spec verbindlich macht. Der Referenzfall ist das RFC-2119-Schlüsselwort `SHOULD NOT`, das `Microsoft.Contractions` umschreiben will, obwohl das Schlüsselwort durch die Anforderungsgrammatik festliegt und nicht kontrahiert werden kann.

- **MUSS [MUST]** eine zulässige Unterdrückung als eng gefasstes Kommentar-Paar ausdrücken, eröffnet mit `<!-- vale <Regel> = NO -->` unmittelbar vor den betroffenen Zeilen und geschlossen mit `<!-- vale <Regel> = YES -->` unmittelbar danach
- **MUSS [MUST]** die Regel in derselben Datei wieder aktivieren; eine bis zum Dateiende offen gelassene Unterdrückung deaktiviert die Regel still für jede spätere Zeile
- **MUSS [MUST]** genau eine Regel pro Paar benennen, damit die Unterdrückung per grep prüfbar bleibt und sich kein zweiter, unabhängiger Alert dahinter verstecken kann
- **MUSS [MUST]** die Unterdrückung auf eine echte Kollision zwischen einer Vale-Regel und einem von einer anderen Spec verbindlich gemachten Vertrag beschränken, und **MUSS NICHT [MUST NOT]** sie für Prosa nutzen, die die schreibende Person schlicht bevorzugt
- **MUSS NICHT [MUST NOT]** eine Unterdrückung nutzen, um einen Rechtschreib-Alert, ein fremdsprachiges Wort oder einen Begriff durchzulassen, der ins gemeinsame Vokabular gehört; dafür gelten weiterhin §Neue Begriffe und Formulierungen sowie §Mehrsprachige Texte
...
- [ ] Keine englisch-gescopte Prosa trägt generische geschlechts-gebundene Pronomen (`he`, `she`, `his`, `hers`, `he/she`), militaristische oder ableistische Ersetzungen, die die Microsoft-Bias-Free-Communication-Tabelle ratifiziert, Ausrufezeichen außerhalb echter Betonung oder kulturell spezifische Idiome; `.vale.ini` lässt das Gate an den Microsoft-Regeln `Gender`, `GenderBias` und `BiasFree` scheitern, während militaristische Begriffe, Ausrufezeichen und Idiome beim redaktionellen Durchgang aus §Stimme und Ton bleiben
- [ ] Das über `.vale.ini` `Packages` synchronisierte Microsoft-Paket trägt die Bias-Free-Ersetzungen (`primary` / `subordinate`, `stop responding`, `perimeter network`, …), und `.vale.ini` hebt `Microsoft.BiasFree` auf `error`, sodass das Gate sie ohne Repository-Vokabular durchsetzt
- [ ] Jede Vale-Unterdrückung im Repository ist ein eng gefasstes Kommentar-Paar, das genau eine Regel benennt und sie in derselben Datei wieder aktiviert; ein grep nach `= NO`-Deaktivierungskommentaren liefert keinen unpaarigen Öffner
- [ ] Keine Vale-Unterdrückung im Repository schaltet einen Rechtschreib-Alert, ein fremdsprachiges Wort oder einen Begriff stumm, der ins gemeinsame Vokabular gehört
