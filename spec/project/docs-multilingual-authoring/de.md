...
- README.md ist explizit aus dem Mehrsprachigkeitsvertrag ausgenommen; sie bleibt nur-Englisch gemäß `spec/project/readme-structure/` §Datei und Sprache
...
- Übersetzung von README.md — die README bleibt nur-Englisch gemäß `spec/project/readme-structure/` §Datei und Sprache, und deren Non-Goals schließen README-Übersetzungen bereits aus
...
- **DARF [MUST] NICHT** `README.md` im Repository-Wurzelverzeichnis übersetzen, umbenennen oder löschen, ebenso wenig eine Datei, die die erzeugende Skill nach Repository-Konvention als nur-Englisch identifiziert (Cross-Ref: `spec/project/readme-structure/` §Datei und Sprache deklariert die README als nur-Englisch)
...
- **MUSS [MUST]** auf `spec/project/mkdocs-structure/` §Site-Layout für den Datei-Baum-Paritätsvertrag und auf `spec/project/docs-freshness/` §Drift-Kategorien für die Audit-Zeit-Erkennung von Verletzungen verweisen; diese Spec **DARF [MUST] NICHT** einen der beiden Verträge wiederholen
- **MUSS [MUST]** auf `spec/project/readme-structure/` §Datei und Sprache als Quelle der README-nur-Englisch-Ausnahme verweisen; diese Spec **DARF [MUST] NICHT** die README-Regel wiederholen
...
- Diese Spec definiert genau einen autorgesetzten Übersetzungsschuld-Marker, `needs-review`. Best-Effort-Generator-Output wird über den Auto-Tag `_translation-pending` aus `spec/claude/skill-agent-catalog/` signalisiert; Kanonik-gegen-Übersetzung-Veraltung wird von `spec/project/docs-freshness/` erkannt. Eine reichere geschlossene Menge landet nur dann in `docs-freshness` §Schweregrad-Klassifikation, wenn ein Audit-Muster zeigt, dass der binäre Marker unzureichend ist (siehe Offene Fragen)
- **MUSS [MUST]** den Marker `<!-- translation-status: needs-review -->` als HTML-Kommentar in der ersten Body-Zeile unmittelbar nach dem Frontmatter-Block platzieren, sodass er in der gerenderten MkDocs-Seite unsichtbar bleibt, aber aus CI heraus greppbar ist (dieselbe Inhalts-Scan-Erkennung, die `spec/project/docs-freshness/` §Drift-Kategorien bereits für veraltete Marker nutzt); der Marker **DARF [MUST] NICHT** als Frontmatter-Schlüssel ausgedrückt werden, was den Frontmatter-Schlüssel-Set-MUSS [MUST] aus §Strukturelle Parität der Übersetzung brechen würde (die kanonische Seite trägt keinen solchen Schlüssel)
...
- **KANN [MAY]** ein Repository, dessen `spec/.spec-config.yml`-`languages:`-Liste genau einen Eintrag enthält, diese Spec als trivial erfüllt behandeln: Jeder Erzeugungsschritt schreibt genau eine Sprachfassung, die zugleich die kanonische ist, und keine Paritäts-MUSSe greifen. Das Repository **MUSS [MUST]** weiterhin das `docs/<lang>/`-Layout gemäß `mkdocs-structure` §Site-Layout verwenden, sodass das Hinzufügen einer zweiten Sprache später eine rein-additive Änderung ist
...
- [ ] `spec/project/mkdocs-structure/` §Site-Layout cross-referenziert diese Spec als Autorenschafts-Pendant (additiver Satz, keine Vertragsänderung)
- [ ] `spec/project/docs-freshness/` §Drift-Kategorien cross-referenziert diese Spec als Autorenschafts-Pendant (additiver Satz, keine Vertragsänderung)
- [ ] `spec/project/readme-structure/` §Nicht-Ziele cross-referenziert diese Spec als die kanonische Deklaration, dass die README-Ausnahme portfolio-weit gilt (additiver Satz, keine Vertragsänderung)
