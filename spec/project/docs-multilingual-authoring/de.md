...
- Wo genau soll der Marker `<!-- translation-status: needs-review -->` leben — als eingezäunter HTML-Kommentar am Anfang der Übersetzungsdatei, als Frontmatter-Schlüssel oder beides? Voreinstellung für jetzt: HTML-Kommentar am Anfang des Übersetzungs-Bodys, damit er in der gerenderten Seite unsichtbar, aus CI heraus aber greppbar ist
- Sollen ADRs unter `docs/<lang>/adr/` mit Status `proposed` von Atomic-Write ausgenommen werden (ein Vorschlag lebt während des Draftens oft in einer Sprache), oder folgen sie demselben Protokoll wie jede andere Seite? Voreinstellung für jetzt: keine Ausnahme — Atomic-Write greift, und der Marker `<!-- translation-status: needs-review -->` ist der Ausweg
- Soll diese Spec auch automatisch erzeugte Katalog-Seiten regeln, die der Generator-Hook aus `spec/claude/skill-agent-catalog/` emittiert (pro-Skill- / pro-Agent-Seiten unter `docs/<lang>/skills/` und `docs/<lang>/agents/`)? Voreinstellung für jetzt: ja, der Generator ist selbst eine doku-erzeugende Fähigkeit und an diese Spec gebunden; die Katalog-Generator-Implementierung liest `spec/.spec-config.yml` und emittiert eine Seite pro Sprache pro Artefakt
- Ist `<!-- translation-status: needs-review -->` der einzige erlaubte Übersetzungsschuld-Marker oder sollte die Spec eine kleine geschlossene Menge aufzählen (`needs-review`, `auto-translated`, `outdated`)? Voreinstellung für jetzt: ein einzelner Marker hält den Vertrag klein; falls eine reichere Taxonomie nötig wird, landet sie in `docs-freshness` als Verfeinerung der Finding-Klassifikation, nicht hier
