...
- **MUSS [MUST]** auf `spec/project/mkdocs-structure/` §Per-language layout für den Datei-Baum-Paritätsvertrag und auf `spec/project/docs-freshness/` §Categories of drift für die Audit-Zeit-Erkennung von Verletzungen verweisen; diese Spec **DARF [MUST] NICHT** einen der beiden Verträge wiederholen
...
- **MUSS [MUST]** den Marker `<!-- translation-status: needs-review -->` als HTML-Kommentar in der ersten Body-Zeile unmittelbar nach dem Frontmatter-Block platzieren, sodass er in der gerenderten MkDocs-Seite unsichtbar bleibt, aber aus CI heraus greppbar ist (dieselbe Inhalts-Scan-Erkennung, die `spec/project/docs-freshness/` §Categories of drift bereits für veraltete Marker nutzt); der Marker **DARF [MUST] NICHT** als Frontmatter-Schlüssel ausgedrückt werden, was den Frontmatter-Schlüssel-Set-MUSS [MUST] aus §Strukturelle Parität der Übersetzung brechen würde (die kanonische Seite trägt keinen solchen Schlüssel)
...
- [ ] `spec/project/docs-freshness/` §Categories of drift cross-referenziert diese Spec als Autorenschafts-Pendant (additiver Satz, keine Vertragsänderung)
