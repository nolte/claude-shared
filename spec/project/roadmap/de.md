...
- **DARF NICHT [MUST NOT]** `target_sprint` als Bereich oder Liste ausdrücken; es ist ein einzelner Integer oder `null`. Ein Roadmap-Item, das konzeptionell mehrere Sprints überspannt, **MUSS [MUST]** gesplittet werden (per `feature-decompose` in Features pro Sprint zerlegt, von denen jedes eine unabhängig verifizierbare Teilmenge liefert) statt als Sprint-Bereich ausgedrückt zu werden. Auf der skalaren Form beruhen sowohl die Detailgrad-Refinement-Regel (die genau einen aktuellen und einen nächsten Sprint auflöst) als auch der Pro-Sprint-Mehrwert-Vertrag der Geschwister-Spec `sprint`.
...
- Default: heute kein `tags`-Feld (verschoben). Das Item-Schema bleibt bei den sieben festgelegten Schlüsseln, und das Filtern großer Warteschlangen wartet. Die gewählte Form ist, wenn sie kommt, ein optionaler Item-Schlüssel `tags: [<string>]` statt einer separaten generierten Index-Seite, weil die Katalog-Generierung bereits in `spec/claude/skill-agent-catalog` lebt. Revisitieren, wenn: irgendeine `project/roadmap.md` im Portfolio 20 oder mehr Items mit `detail` in {`coarse`, `backlog`} erreicht — gemessen daran, dass der `grep -cE '^detail: (coarse|backlog)'`-Zähler eines adoptierenden Repos die 20 überschreitet. Heute hat die einzige reale Roadmap (claude-shared) 8 Items, davon 4 coarse oder backlog, deutlich unter der Schwelle.
- Default: heute kein Abhängigkeitsfeld (verschoben); die flache Reihenfolge von oben nach unten ist die einzige Abhängigkeits-Kodierung. Die gewählte Form ist, wenn sie kommt, ein optionaler Item-Schlüssel `depends_on: [R-<n>]` mit einem Lint, der prüft, dass jede referenzierte ID existiert und der Graph azyklisch bleibt, analog zur bestehenden Outcome-ID-Validierung in §Outcome-Verknüpfung. Revisitieren, wenn: ein reales Roadmap-Item auftaucht, dessen Ausführungsreihenfolge sich wirklich nicht allein über die Listenposition ausdrücken lässt — eine nicht-lineare Blocker-Kette, in der zwei Items ein drittes blockieren, oder eine Abhängigkeit, die der Prioritätsreihenfolge widerspricht. Bis ein solches Item in der `roadmap.md` eines adoptierenden Repos existiert, genügt die Listenreihenfolge.
- Default: Phasen bleiben ID-lose Dokumentation (verschoben); eine Phase trägt keine ID, keinen Status und kein eigenes Frontmatter. Die gewählte Form fügt, wenn sie kommt, einen optionalen stabilen Bezeichner nur den Phasen-Überschriften hinzu, die ein Konsument tatsächlich adressiert, und hält vollständig ID-lose flache Roadmaps gültig. Revisitieren, wenn: mindestens eine konsumierende Spec oder ein Skill eine Phase über eine stabile ID adressieren muss. Heute nachweislich abwesend — `grep -ric phase` liefert 0 sowohl in `spec/project/release-skill-layer/en.md` als auch in `spec/project/release-notes-audience-analysis/en.md`, und `audience-doc-author` arbeitet pro Seite, nicht pro Phase.
