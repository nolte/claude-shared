...
- **MUSS [MUST]** ein Item spätestens dann auf `active` setzen, wenn eines seiner Features laut `feature`-Spec nach `in_progress` geht; der konsumierende Skill (`sprint-execute`, wenn er ein Feature voranbringt) ist der kanonische Durchsetzungspunkt. Für ein Post-MVP-Item (`mvp: false`) ist diese Regel dem Stabilisierungs-Gate der `mission`-Spec untergeordnet: das Item **DARF NICHT [MUST NOT]** auf `active` gesetzt werden (und keines seiner Features darf nach `in_progress` gehen), solange `mvp_status` einer von `defining`, `in_progress` oder `achieved` ist. Das `ready → in_progress`-Gate der `feature`-Spec prüft zuerst `mvp_status`, sodass diese Regel ein Post-MVP-Item nie vor der Stabilisierung auf `active` zwingt.
