...
- **MUST** mark an item `active` no later than the moment one of its features enters `in_progress` per the `feature` spec; the consuming skill (`sprint-execute` when it advances a feature) is the canonical enforcement point. For a post-MVP item (`mvp: false`) this rule is subordinate to the `mission` spec's stabilisation gate: the item **MUST NOT** be marked `active` (and none of its features may enter `in_progress`) while `mvp_status` is any of `defining`, `in_progress`, or `achieved`. The `feature` spec's `ready → in_progress` gate checks `mvp_status` first, so this rule never forces a post-MVP item `active` before stabilisation.
