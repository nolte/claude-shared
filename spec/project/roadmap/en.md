...
- Default: no `tags` field today (deferred). The per-item schema stays the seven declared keys, and large-queue filtering waits. The chosen shape, when it lands, is an optional per-item `tags: [<string>]` key rather than a separate generated index page, because catalog generation already lives in `spec/claude/skill-agent-catalog`. Revisit when: any `project/roadmap.md` in the portfolio reaches 20 or more items with `detail` in {`coarse`, `backlog`}—measured by an adopting repo's `grep -cE '^detail: (coarse|backlog)'` count crossing 20. Today the only real roadmap (`claude-shared`) has 8 items, 4 of them coarse or backlog, well below the threshold.
- Default: no dependency field today (deferred); the flat top-to-bottom list order is the sole dependency encoding. The chosen shape, when it lands, is an optional per-item `depends_on: [R-<n>]` key with a lint that every referenced ID exists and the graph stays acyclic, mirroring the existing outcome-ID validation in §Outcome linkage. Revisit when: a real roadmap item appears whose execution ordering genuinely can't be expressed by list position alone—a non-linear blocker chain where two items both block a third, or a dependency that contradicts priority order. Until such an item exists in an adopting repo's `roadmap.md`, list order suffices.
- Default: phases stay ID-less documentation (deferred); a phase carries no ID, no status, and no separate frontmatter. The chosen shape, when it lands, adds an optional stable identifier only to the phase headings a consumer actually addresses, keeping fully ID-less flat roadmaps valid. Revisit when: at least one consumer spec or skill must address a phase by stable ID. Verified absent today: `grep -ric phase` returns 0 in both `spec/project/release-skill-layer/en.md` and `spec/project/release-notes-audience-analysis/en.md`, and `audience-doc-author` operates per page, not per phase.
