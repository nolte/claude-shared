...
- Should pre-release tags get a distinct project-context bundle than stable releases (for example a "Release-candidate testing checklist" section)? Unblocked when any `nolte/*` repo's `release-drafter` draft (or published release) first carries a SemVer pre-release identifier (a tag matching `/-(rc|beta|alpha)\.\d+$/`) and an operator needs RC-specific section content. Today every portfolio tag is stable SemVer (this repo: `v0.1.1` through `v0.1.4`; `release-automation` §Open-Questions decisions confirm `release-drafter` config in `nolte/gh-plumbing` produces stable tags only). Upstream gate: a `release-drafter` config change in `nolte/gh-plumbing:.github/commons-release-drafter.yml` enabling pre-release tagging.
- How does Skill A behave when `release-drafter` re-runs after Skill A and overwrites the body? Unblocked by one observed curate → `release-drafter`-rerun cycle on a real `develop` draft (for example this repo's next minor): run `release-notes-curate`, then re-fire `nolte/gh-plumbing`'s `reusable-release-drafter.yml` on the same draft, and diff the body—confirm the `<!-- release-skill-layer:project-context-start -->`…`-end -->` block survives byte-for-byte. If `release-drafter` strips trailing body content, file the `workflow-health` tracking GitHub Issue (`workflow-health` §MAY open a tracking GitHub Issue) and amend §Re-run safety and markers with the observed behaviour.
- Which concrete specialised bundle to author next (a HACS-integration bundle, an OCI-image bundle) is data-gated on a real repo of that type needing it. The location is decided: such extensions live in `skills/release-notes-curate/references/project-bundles.md` (see §Skill A), not inline here and not in a sibling spec.
