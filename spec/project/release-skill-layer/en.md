...
- **MUST** validate every gate from `release-automation` §Pre-publish verification locally before dispatch:
  ...
  - every required status check on the head commit of `develop` is `SUCCESS`;
...
- Should Skill A and Skill B share a common "release context loader" helper (project type + audience artefact + open draft) extracted into `references/release-context.md`, or stay independent? The duplication risk is real but small (≈40 lines); re-evaluate after first adoption pair.
- Should pre-release tags (`v0.2.0-rc.1`, `v0.2.0-beta.3`) get a different project-context bundle than stable releases (for example a "Release-candidate testing checklist" section that doesn't appear on stable tags)? Defer until the portfolio cuts its first pre-release.
- How does Skill A behave when `release-drafter` re-runs after Skill A and overwrites the body? The marker pair should survive because `release-drafter` only generates the Conventional-Commits sections above the divider, but a real-world test against `nolte/gh-plumbing`'s `reusable-release-drafter.yml` is needed to confirm the marker block isn't stripped; track as a `workflow-health` open item.
- Should Skill B's `--dry-run` mode also write a "would-publish" comment to the draft body for review, or stay strictly side-effect-free? Side-effect-free is the safer default; revisit if operators ask for the comment.
- Should the project-context bundle table grow per-repo extensions (a HACS-integration-specific bundle, an OCI-image-specific bundle) live inline in this spec, in `references/<project-type>-bundles.md` next to the curate skill, or in a sibling `release-skill-layer-bundles` spec? Defer until a third project type warrants extraction.
- When the audience artefact and the autodetected project type disagree (audience artefact lists "downstream Python integrators" but the repo is detected as a Claude plugin), which takes precedence? Lean toward audience artefact since it's the human-confirmed signal, but record the conflict in the curated body's `## Open questions` subsection.
