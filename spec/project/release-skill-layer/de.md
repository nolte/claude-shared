...
- **MUSS [MUST]** `release-publish.yml` via `gh workflow run release-publish.yml --ref develop -f tag=<tag>` dispatchen; das `tag`-Input ist gemäß `release-automation` §Operativer Kontrakt verpflichtend, unabhängig davon, wie viele Drafts offen sind.
...
- **SOLLTE [SHOULD]** einen `--dry-run`-Modus unterstützen, der jede Precondition-Validierung ausführt und mit `dry_run=true`-Workflow-Input gemäß `release-automation` §Operativer Kontrakt dispatched.
...
- **MUSS [MUST]**, wenn der Workflow-Run nach erfolgreichem Dispatch selbst fehlschlägt, an `release-automation` §Beobachtbarkeit und Audit und `workflow-health` delegieren statt einen zweiten Dispatch aus dem Skill heraus zu versuchen.
...
- [ ] `release-automation` §Non-Goals (oder §Verhältnis zu anderen Specs) verlinkt diese Spec als lokales Skill-Pendant für Body-Curation und Dispatch-Ergonomie.
