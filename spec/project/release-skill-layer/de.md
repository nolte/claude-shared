...
- Sollen Pre-Release-Tags ein eigenes Projekt-Kontext-Bündel erhalten als stabile Releases (zum Beispiel eine "Release-Candidate Testing Checklist"-Section)? Entsperrt, sobald der `release-drafter`-Draft (oder ein veröffentlichtes Release) irgendeines `nolte/*`-Repos erstmals einen SemVer-Pre-Release-Identifier trägt (ein Tag, das auf `/-(rc|beta|alpha)\.\d+$/` matcht) und ein Operator RC-spezifischen Section-Inhalt braucht. Heute ist jedes Portfolio-Tag stabiles SemVer (dieses Repo: `v0.1.1`–`v0.1.4`; `release-automation` §Open-Questions-Entscheidungen bestätigen, dass die `release-drafter`-Konfiguration in `nolte/gh-plumbing` nur stabile Tags produziert). Upstream-Gate: eine `release-drafter`-Konfigurationsänderung in `nolte/gh-plumbing:.github/commons-release-drafter.yml`, die Pre-Release-Tagging aktiviert.
- Wie verhält sich Skill A, wenn `release-drafter` nach Skill A erneut läuft und den Body überschreibt? Entsperrt durch einen beobachteten Zyklus aus Curate → `release-drafter`-Rerun auf einem realen `develop`-Draft (zum Beispiel das nächste Minor dieses Repos): `release-notes-curate` laufen lassen, dann `nolte/gh-plumbing`s `reusable-release-drafter.yml` auf demselben Draft erneut feuern und den Body diffen—bestätigen, dass der `<!-- release-skill-layer:project-context-start -->`…`-end -->`-Block byte-für-byte überlebt. Falls `release-drafter` nachgestellten Body-Inhalt entfernt, das `workflow-health`-Tracking-GitHub-Issue anlegen (`workflow-health` §MAY open a tracking GitHub Issue) und §Re-Run-Sicherheit und Marker um das beobachtete Verhalten ergänzen.
- Welches konkrete spezialisierte Bündel als Nächstes zu schreiben ist (ein HACS-Integrations-Bündel, ein OCI-Image-Bündel), ist daten-gegated auf ein reales Repo dieses Typs, das es braucht. Der Ort ist entschieden: solche Erweiterungen leben in `skills/release-notes-curate/references/project-bundles.md` (siehe §Skill A: Draft-Notes-Kuratierung), nicht inline hier und nicht in einer separaten Spec.
