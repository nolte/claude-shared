...
- `spec/project/test-tier-contract/` [R3] ist das **Geschwister daneben/darüber**: Es verifiziert ein Cross-Service-Agreement, *ohne beide Seiten hochzufahren*. Die Grenze lautet „ein echter Kollaborateur, den du besitzt und kontrollierst" (Integration) vs. „ein Cross-Service-Agreement, gegen einen Contract verifiziert, nicht gegen einen Live-Partner" (Contract).
...
- **MUSS [MUST]** einen Kollaborateur üben, den das Projekt **besitzt oder kontrolliert** (seine eigene Datenbank, seinen Broker oder einen einzelnen eigenen Service); Cross-Service-Agreements mit einem Partner, den das Projekt nicht kontrolliert, gehören zur Contract-Stufe [R3], [R10].
...
- **SOLLTE [SHOULD]** einen Pull Request an den **schnellen schmalen** Integrationstests gaten (ausgeführt gemäß `spec/project/quality-gate/`, als erforderliche Checks gemäß `spec/project/pull-request-workflow/` deklariert) und langsamere oder schwerere Integrationstests in einer dedizierten CI-Stufe oder Nightly ausführen, statt jede Änderung zu blockieren.
...
- [ ] Die Grenze zur Contract-Stufe (ein echter eigener Kollaborateur vs. ein Cross-Service-Agreement ohne beide Seiten live) ist scharf, und das Treffen einer echten Dritt-Produktions-API ist verboten
...
- Wenn ein Seam ein Service ist, den das Projekt besitzt, aber separat deployt, ist seine Verifikation ein Integrationstest (echter eigener Kollaborateur) oder ein Contract-Test (Cross-Service-Agreement) — und sollte die Spec eine Entscheidungsregel über „besitzen und kontrollieren" hinaus geben?
