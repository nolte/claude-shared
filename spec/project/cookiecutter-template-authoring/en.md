...
- **SHOULD** render a Taskfile that consumes portfolio-common automation (MkDocs, pre-commit, and similar cross-repo concerns) from the shared [`nolte/taskfiles`](https://github.com/nolte/taskfiles) collection through an `includes:` block, rather than re-implementing equivalent targets inline, per `spec/project/taskfile/` §Shared Taskfiles. A template that re-implements shared behaviour locally hands every rendered project a private copy that upstream fixes never reach
- **MUST**, when the rendered Taskfile does include files from `nolte/taskfiles`, pin the source location through a single variable (for example `TASK_COLLECTION_BASE`) naming an explicit ref, and render a CI workflow that sets `TASK_X_REMOTE_TASKFILES=1` wherever those includes resolve. Task's remote-Taskfile resolution is experimental and off by default, so a rendered project carrying remote includes without the flag fails on its first CI run
...
- [ ] Every template's rendered `Taskfile.yml` conforms to `spec/project/taskfile/`: portfolio-common automation is included from the shared `nolte/taskfiles` collection through a single pinned base variable, and any rendered project carrying remote includes also renders a CI workflow that sets `TASK_X_REMOTE_TASKFILES`; verifiable by a `pytest-cookies` assertion over the rendered Taskfile and workflow
