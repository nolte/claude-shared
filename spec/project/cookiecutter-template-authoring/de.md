...
- **KANN [MAY]** das gerenderte Projekt zum `audience-identify`-Skill als Post-Generation-Nächster-Schritt verweisen (im Banner gedruckt, nicht automatisch aufgerufen), sodass der Operator dem spec-mandierten Audience-Identifikations-Flow direkt nach der Generierung folgt
...
- **MUSS [MUST]** die Test-Suite in eine GitHub-Actions-Matrix verdrahten, die das Template auf mindestens den in `pyproject.toml` (oder Äquivalent) deklarierten Python-Versionen und mindestens dem OS prüft, das dem Ziel des gerenderten Projekts entspricht — typischerweise `ubuntu-latest`
