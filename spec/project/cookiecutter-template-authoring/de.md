...
- [ ] Das gerenderte `Taskfile.yml` jedes Templates ist konform zu `spec/project/taskfile/`. Ein Template, das keinen Remote-Include rendert, erfüllt dieses Kriterium (die Sammlung deckt heute kein CI-gatendes Target ab, `lint` / `test` / `docs` lokal zu schreiben ist also konform und keine Lücke). Ein Template, das Remote-Includes rendert, pinnt sie zusätzlich über eine einzige Basis-Variable und rendert einen CI-Workflow, der `TASK_X_REMOTE_TASKFILES` setzt und jedes Target mit `task --yes` aufruft; verifizierbar durch eine `pytest-cookies`-Assertion, die davon abhängt, ob das gerenderte Taskfile einen `includes:`-Block trägt
