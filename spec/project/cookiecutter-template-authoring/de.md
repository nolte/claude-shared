...
- **MUSS [MUST]** ein Taskfile rendern, dessen Targets der Portfolio-Konvention entsprechen: `task install`, `task lint`, `task test`, `task docs` (wenn Docs ausgeliefert werden), `task release` (wenn Release-Flow verdrahtet ist); jedes Target ruft die projekt-lokale Toolchain auf statt sich auf global installierte Binaries zu verlassen. Das kanonische Target-Vokabular, das Namespacing-Schema und die Regeln zur Argument-Durchreichung bleiben im Besitz von `spec/project/taskfile/` und werden von dieser Anforderung nicht wiederholt
- **SOLLTE [SHOULD]** ein Taskfile rendern, das portfolio-übliche Automation (MkDocs, pre-commit und ähnliche Cross-Repo-Belange) über einen `includes:`-Block aus der geteilten [`nolte/taskfiles`](https://github.com/nolte/taskfiles)-Sammlung konsumiert, wo diese Sammlung den Bedarf mit einem lauffähigen Target abdeckt, statt äquivalente Targets inline nachzubauen — gemäß `spec/project/taskfile/` §Geteilte Taskfiles. Ein Template, das tatsächlich geteiltes Verhalten lokal nachbaut, gibt jedem gerenderten Projekt eine private Kopie mit, die Upstream-Fixes nie erreichen; umgekehrt liefert ein Template, das an ein nicht existierendes eingebundenes Target delegiert oder an eines, dessen Umgebungsannahmen im gerenderten Projekt nicht zutreffen, ein Taskfile aus, das beim ersten Aufruf fehlschlägt. Prüfe den Include, bevor du an ihn delegierst
- **MUSS [MUST]**, wenn das gerenderte Taskfile tatsächlich Dateien aus `nolte/taskfiles` einbindet, die Quell-Location über eine einzige Variable (zum Beispiel `TASK_COLLECTION_BASE`) mit explizitem Ref pinnen und einen CI-Workflow rendern, der sowohl `TASK_X_REMOTE_TASKFILES=1` setzt als auch jedes Target nicht-interaktiv aufruft (`task --yes <target>`), überall dort, wo diese Includes auflösen. Tasks Remote-Taskfile-Auflösung ist experimentell, standardmäßig aus und zusätzlich hinter einem Trust-Prompt, den ein CI-Runner mangels Terminal nicht beantworten kann — fehlt eines der beiden Teile, schlägt das gerenderte Projekt bereits im ersten CI-Lauf fehl
...
- [ ] Das gerenderte `Taskfile.yml` jedes Templates ist konform zu `spec/project/taskfile/` und aus den dort ausgelieferten `templates/` abgeleitet. Ein Template, das keinen Remote-Include rendert, erfüllt dieses Kriterium weiterhin: Die Sammlung deckt zwar einen CI-gatenden Doku-Build ab, aber nur an Refs, die ihn tragen — `lint` / `test` / `docs` lokal zu schreiben bleibt also konform, wo der Ref, den das Template pinnen würde, kein nutzbares Target bereitstellt. Ein Template, das Remote-Includes rendert, pinnt sie zusätzlich über eine einzige Basis-Variable und rendert einen CI-Workflow, der `TASK_X_REMOTE_TASKFILES` setzt und jedes Target mit `task --yes` aufruft; verifizierbar durch eine `pytest-cookies`-Assertion, die davon abhängt, ob das gerenderte Taskfile einen `includes:`-Block trägt
