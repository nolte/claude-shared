...
- **SOLLTE [SHOULD]** ein Taskfile rendern, das portfolio-übliche Automation (MkDocs, pre-commit und ähnliche Cross-Repo-Belange) über einen `includes:`-Block aus der geteilten [`nolte/taskfiles`](https://github.com/nolte/taskfiles)-Sammlung konsumiert, statt äquivalente Targets inline nachzubauen — gemäß `spec/project/taskfile/` §Geteilte Taskfiles. Ein Template, das geteiltes Verhalten lokal nachbaut, gibt jedem gerenderten Projekt eine private Kopie mit, die Upstream-Fixes nie erreichen
- **MUSS [MUST]**, wenn das gerenderte Taskfile tatsächlich Dateien aus `nolte/taskfiles` einbindet, die Quell-Location über eine einzige Variable (zum Beispiel `TASK_COLLECTION_BASE`) mit explizitem Ref pinnen und einen CI-Workflow rendern, der `TASK_X_REMOTE_TASKFILES=1` überall dort setzt, wo diese Includes auflösen. Tasks Remote-Taskfile-Auflösung ist experimentell und standardmäßig aus, sodass ein gerendertes Projekt mit Remote-Includes ohne dieses Flag bereits im ersten CI-Lauf fehlschlägt
