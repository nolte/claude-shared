...
- [ ] Das gerenderte `Taskfile.yml` jedes Templates ist konform zu `spec/project/taskfile/`: portfolio-übliche Automation wird über eine einzige gepinnte Basis-Variable aus der geteilten `nolte/taskfiles`-Sammlung eingebunden, und jedes gerenderte Projekt mit Remote-Includes rendert zusätzlich einen CI-Workflow, der `TASK_X_REMOTE_TASKFILES` setzt und jedes Target mit `task --yes` aufruft; verifizierbar durch eine `pytest-cookies`-Assertion über das gerenderte Taskfile und den Workflow
