...
- **Authentication and authorization mechanism design**—token issuance, session models, scope taxonomies, and identity providers; this spec only requires that credentials travel in headers over HTTPS, not how they are minted
...
- **MUST** bind for every **newly created** HTTP API and for every **new major version** of an existing API; a major version that has already shipped is **grandfathered**—it is not required to be retrofitted to this standard, and a retrofit MUST NOT be forced onto a live major version
...
- **SHOULD**, within a grandfathered major version, still apply the additive and non-breaking rules of this spec to **new** endpoints where doing so does not contradict the version's established conventions (for example a new endpoint uses resource-oriented plural nouns even if older siblings do not)
...
- **MUST** express an operation that genuinely does not fit resource CRUD as a **custom method** using colon notation on the resource (`POST /orders/{orderId}:cancel`), rather than inventing a verb path segment; custom methods are the documented exception, not the default
- **SHOULD** keep a resource's canonical URL stable across versions and releases; a resource's identity is part of its contract

### Language and casing

- **MUST** write all URL path segments, query-parameter names, and JSON property names in **English**, using **US-ASCII only**—no umlauts or non-ASCII characters, which would otherwise require percent-encoding and become unreadable and error-prone
...
- **MUST** write JSON body property names and query-parameter names in **`camelCase`** (`birthYear`, `includeDetached`)—the portfolio's chosen convention among the valid industry alternatives (the Google/Microsoft ecosystem style); it is applied uniformly and never mixed with `snake_case` within an API
- **MUST** write enumeration values in `UPPER_SNAKE_CASE` (`status: "IN_PROGRESS"`) so they are unambiguous constants distinct from free text
...
- **MUST** keep `GET`, `HEAD`, and `OPTIONS` **safe** (no observable state change) and keep `GET`, `HEAD`, `PUT`, and `DELETE` **idempotent**; `POST` and `PATCH` are not required to be idempotent
...
- **SHOULD**, when hiding the existence of a resource matters for authorization, return `404` rather than `403` so the response does not leak that the resource exists
...
- **SHOULD** use conventional query parameters for collection shaping—`sort` for ordering, `fields` for sparse field selection, a `filter` expression (or discrete field filters) for narrowing—and keep the parameter vocabulary consistent across the whole API
...
- **MUST** include a stable, machine-readable error code as the `camelCase` extension member `code` (an `UPPER_SNAKE_CASE` string enum that is part of the API contract), so clients branch on `code` rather than parsing `detail`; the code MAY additionally be mirrored in a response header
...
- **MUST** reach Richardson Maturity Level 2 (proper resources, methods, and status codes); **full HATEOAS (Level 3) is not required**
...
- [ ] A grandfathered shipped major version is not flagged for retroactive conformance, while a new major version is held to the full standard
...
- Should this spec be promoted from `Portfolio-Scope: local` to `portfolio` so it is inherited portfolio-wide (per `spec/project/portfolio-inherited-spec-layer/`)? It is authored as a portfolio-wide standard, but promotion is an explicit maintainer act and its two sister specs are currently `local`; left `local` until promoted deliberately.
