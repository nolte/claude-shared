...
description: "Audits a browser-rendered frontend (HTML + CSS + the React 19 / Vite 8 / MUI v9 / RTK / react-router v7 / react-hook-form + Zod / react-i18next / notistack / Recharts / nginx stack) against spec/frontend/webview-ui-optimization/<canonical_language>.md and, with per-item user approval, patches findings across five domains: Performance / Rendering, Security / Sandboxing, Accessibility (WCAG 2.2 AA), Internationalisation, and UX / Native-feel. Three operations: `audit` (read-only conformance report, one row per spec rule, written to `.audits/webview-ui-optimize/`), `patch` (additive fix for one finding at a time), `expert-review` (dispatches the `webview-ui-expert` agent for a deep read-only review of a named file or component). Invoke when the user asks to \"audit the frontend\", \"check the UI against the webview-ui spec\", \"optimise this page for performance / a11y / i18n / UX / security\", \"fix the CSP\", \"wire up vitest-axe\", or equivalent German-language requests (\"Frontend gegen die Spec prüfen\", \"WebView optimieren\", \"a11y-Audit\"). Don't use for visual / brand-design decisions (per-product), for the audience artefact (use `audience-identify`), for prose / Vale linting (use `prose-vale-curator`), for dependency CVE audits (use `dependency-audit`), or for release-pipeline shape (use `release-automation`)."
