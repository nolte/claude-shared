...
description: Triage a failing GitHub Actions workflow run on `develop` or `main` per `spec/project/workflow-health/`. Classifies the failure into one of `defect` / `flake` / `infra` / `stale pin` / `secret drift` / `other`, dispatches the most specialised Claude agent that matches the classification (per `workflow-health` §Specialised-agent dispatch), records the classification plus the dispatched agent's name in the eventual fix PR's Risk / rollout notes, and verifies the standard `fix/`-PR flow is honoured. Invoke when the user asks "triage this red workflow", "classify this CI failure", "why did `release-publish.yml` fail and what now", "the docs build is red, route the fix", or equivalent German-language requests ("Workflow-Failure triagen", "rote Pipeline klassifizieren", "Wer fixt diesen CI-Fehler"). Don't use to silence checks via `continue-on-error` shortcuts or by removing required-checks set entries — those are forbidden by the spec; don't use to bypass branch protection (the `enforce_admins` setting on develop has no exception path); don't use to merge a fix PR (that's `pull-request-merge`).
...
- **Never** mask a failure: do not propose `continue-on-error: true` on a required job, do not propose removing a check from the required-checks set in `.github/settings.yml` without a tracking Issue, do not propose repointing a `nolte/gh-plumbing` pin from a tag to a branch.
- **Never** bypass branch protection on the fix PR; `enforce_admins: true` on `develop` has no exception path.
- **Never** dispatch a specialised agent with a classification the spec doesn't list — `defect / flake / infra / stale pin / secret drift / other` is the closed set.
...
- **Renovate-generated bump PRs for `nolte/gh-plumbing` aren't auto-merged in this portfolio.** A `stale pin` remediation that proposes to enable Renovate automerge for `nolte/gh-plumbing` violates `workflow-health` §Upstream drift and the AC against it. The remediation is a human-acknowledged Renovate PR, not an automerge rule.
