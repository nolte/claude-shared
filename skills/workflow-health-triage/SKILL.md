...
1. **Resolve the candidate set.** `Glob` `agents/*.md` (plus `.claude/agents/*.md` (project-distributed; additionally `~/.claude/agents/*.md` for user-level agents) for the project-distributed half), then `Read` the `description:` line of every candidate. Build a (`name`, `description`) table; the table is the runtime inventory.
