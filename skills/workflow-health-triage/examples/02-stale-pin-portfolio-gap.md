...
6. **Branch on the user's answer.**
   - If the user opts to author the agent: dispatch `Agent(subagent_type="nolte-shared:claude-plugin-developer", …)` with the brief *"author a new specialised agent whose responsibility is `nolte/gh-plumbing` reusable-workflow tag-pin remediation, including the `pascalgn/automerge-action` `mergeResult: 'merge_failed'` quirk"*. Wait for the agent's report; then dispatch the newly-authored agent on the actual fix.
