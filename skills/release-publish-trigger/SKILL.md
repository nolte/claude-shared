...
description: Validate every release-automation pre-publish gate locally, then dispatch release-publish.yml via `gh workflow run` for the open release-drafter draft on develop, per spec/project/release-skill-layer/<canonical_language>.md §"Skill B — Release publish trigger". Verifies that exactly one open draft exists, the draft tag is reachable from the develop tip, version-bearing files (per release-automation §Version-bearing files) align under their declared transform, the `chore(release): <tag>` alignment commit is present when required, every required status check on develop is SUCCESS, and `.github/workflows/release-publish.yml` exists in the repo. Refuses to dispatch on any failed gate; routes red checks to workflow-health triage. Never calls `gh release edit --draft=false` directly. Invoke when the user asks to "publish the release", "trigger release publish", "ship the release", "dispatch the release-publish workflow", or equivalent German-language requests ("Release veröffentlichen", "Draft-Release ausliefern", "Release-Publish-Workflow auslösen"). Don't use to curate the body (that's `release-notes-curate`), to merge a pull request (that's `pull-request-merge`), or to bypass the workflow with `gh release edit --draft=false` (forbidden by spec).
tags: [release]
---
...
- `release-cd-refresh-master.yml` should fire automatically after `release-publish.yml` succeeds. When it doesn't fire (the known-platform-constraint case where `release: published` from `GITHUB_TOKEN` doesn't cascade to a fresh workflow run, per `workflow-health` §Known platform constraints), surface this and route to manual fast-forward of `main` per `branching-model` §Local release operation.
