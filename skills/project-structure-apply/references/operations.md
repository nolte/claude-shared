...
- **`Taskfile.yml`**: include `test`, `lint`, `docs` targets at a minimum; wire them to the real stack commands detected in the repo. The canonical target vocabulary, the namespacing scheme, and the shared-Taskfile conventions are owned by `spec/project/taskfile/` — read it before scaffolding and never restate its rules here. Concretely, a scaffolded `Taskfile.yml` consumes portfolio-common automation from the shared [`nolte/taskfiles`](https://github.com/nolte/taskfiles) collection wherever that collection actually covers the need, per that spec's §Shared Taskfiles:
  ...
  - **Read the include before you delegate to it.** Fetch the file and check which tasks it actually declares; never assume a target exists because the topic matches. Two live examples worth knowing: `taskfile-include-mkdocs.yaml` ships only `start` (`mkdocs serve`), so it can't back a `docs` build target, and the `pre-commit` include's tasks source a global `~/.venvs/development` interpreter, which conflicts with the project-local `.venv` convention in §Python development below. Delegate to an included target only when it exists and its assumptions hold in the target repo; otherwise write the target locally and note why.
  - The `check`, `lint`, `test`, and `docs` targets are the repository's CI gate, so they stay locally-written unless an included target demonstrably runs the same command in the same environment. Shared includes are additive convenience (`task mkdocs:start` for a local preview) on top of that gate, not a replacement for it.
  ...
  - When the repo already has a `Taskfile.yml` that re-implements behaviour the collection genuinely covers with an equivalent, runnable target, classify it as drift and offer the swap. Verify the equivalence first: a local `docs: mkdocs build --strict` is **not** drift against `mkdocs:start`, and a local `lint: pre-commit run --all-files` is **not** drift against a `pre-commit:start` that needs a global venv. Never rewrite the file silently, and leave repo-specific overrides layered on top intact.
