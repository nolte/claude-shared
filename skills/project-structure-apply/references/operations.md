...
- **`.github/workflows/`**: if empty, scaffold at minimum a `ci.yml` that invokes Taskfile targets (`task lint`, `task test`, `task docs`). Don't invent language-specific pipelines beyond what Taskfile already exposes. When the repo's `Taskfile.yml` carries an `includes:` block pointing at `nolte/taskfiles` (see the Taskfile item below), the workflow **must** set `TASK_X_REMOTE_TASKFILES: "1"` — without it, Task refuses to resolve the remote includes and every target in CI fails.
...
- **`Taskfile.yml`**: include `test`, `lint`, `docs` targets at a minimum; wire them to the real stack commands detected in the repo. The canonical target vocabulary, the namespacing scheme, and the shared-Taskfile conventions are owned by `spec/project/taskfile/` — read it before scaffolding and never restate its rules here. Concretely, a scaffolded `Taskfile.yml` consumes portfolio-common automation from the shared [`nolte/taskfiles`](https://github.com/nolte/taskfiles) collection instead of re-implementing it locally, per that spec's §Shared Taskfiles:
  ...
  - Prefer delegating to an included target over writing an equivalent local one (a `setup:` target calling `pre-commit:install`, a `docs:` target calling the shared MkDocs build). Keep hand-written targets for what's genuinely repo-specific.
  - Task's remote-Taskfile resolution is experimental, so every environment that resolves these includes needs `TASK_X_REMOTE_TASKFILES=1`. Whenever you scaffold a `Taskfile.yml` carrying an `includes:` block, also set `TASK_X_REMOTE_TASKFILES: "1"` in the scaffolded CI workflow (workflow-level `env:` is the simplest placement) and tell the user the same flag is needed for local runs.
  - When the repo already has a `Taskfile.yml` that re-implements shared behaviour inline (its own `mkdocs build` or `pre-commit run` target while the collection ships an include for it), classify it as drift and offer the swap. Never rewrite the file silently, and leave repo-specific overrides layered on top intact.
