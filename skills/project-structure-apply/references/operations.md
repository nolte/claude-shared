...
- **`Taskfile.yml`**: include `test`, `lint`, `docs` targets at a minimum; wire them to the real stack commands detected in the repo. The canonical target vocabulary, the namespacing scheme, and the shared-Taskfile conventions are owned by `spec/project/taskfile/` — read it before scaffolding and never restate its rules here. Concretely, a scaffolded `Taskfile.yml` consumes portfolio-common automation from the shared [`nolte/taskfiles`](https://github.com/nolte/taskfiles) collection wherever that collection actually covers the need, per that spec's §Shared Taskfiles:
  - Pin the collection through a single `vars:` entry — `TASK_COLLECTION_BASE: https://raw.githubusercontent.com/nolte/taskfiles/<ref>/src` — so every include resolves against one declared, reviewable source rather than scattered inline URLs. Ask the user for the ref, or fall back to `develop` and flag it, mirroring the release-tag handling of the `nolte/gh-plumbing` workflow callers above.
  - Add an `includes:` block for the shared files the repo actually needs, always referenced through that variable (`pre-commit: "{{.TASK_COLLECTION_BASE}}/taskfile-include-pre-commit.yaml"`). Wire an include only when the matching surface exists in the repo: `taskfile-include-pre-commit.yaml` when `.pre-commit-config.yaml` is present, `taskfile-include-mkdocs.yaml` when `mkdocs.yml` is present, `taskfile-include-asdf.yaml` when `.tool-versions` is present, `taskfile-include-kind.yaml` / `taskfile-include-k8s.yaml` for a repo with a Kubernetes surface, and `taskfile-include-worktree.yaml` when the repo wants the shared worktree helpers instead of a local script. Never invent an include filename; when unsure, list `src/` in `nolte/taskfiles` and offer only what's actually there.
  ...
  - Task's remote-Taskfile resolution is experimental, so every environment that resolves these includes needs two things, not one: `TASK_X_REMOTE_TASKFILES=1`, **and** an explicit trust acceptance. Whenever you scaffold a `Taskfile.yml` carrying an `includes:` block, set `TASK_X_REMOTE_TASKFILES: "1"` in the scaffolded CI workflow (workflow-level `env:` is the simplest placement) *and* invoke every target as `task --yes <target>` there. A CI runner has no TTY to answer the trust prompt, so a bare `task lint` aborts rather than prompting. Tell the user the same flag applies to local runs, where the prompt appears once interactively.
