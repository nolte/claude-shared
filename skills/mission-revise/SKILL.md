---
name: mission-revise
description: Revise an existing `project/mission.md` per spec/project/mission/<canonical_language>.md. Invoke when the user says things like "revise the mission", "update project/mission.md", "flip mvp_status to in_progress / achieved / stabilised", "the MVP is achieved, mark it", "the MVP regressed, reopen it", "the stabilisation gate is satisfied", "rephrase the mission statement", "swap the verifying feature", "add an audience to the mission", "Mission überarbeiten", "mvp_status umstellen auf …", "die Mission ist stabilisiert", or "Stabilisierung markieren". Supports three operations: (A) revise the statement, audiences, verifying-feature pointer, or time_bound; (B) flip `mvp_status` along the legal lifecycle (`defining → in_progress → achieved → stabilised`, plus the regression path `stabilised → in_progress`); (C) revise after `mvp_status: stabilised` with the mandatory rationale paragraph in `## Source`. Verifies the stabilisation-gate conditions (every `mvp: true` roadmap item is `done`, the MVP-closing sprint is `closed`, one full subsequent sprint has closed without regression, no in-flight defect-fix work) by reading `project/roadmap.md` and `project/sprints/` before allowing a flip to `stabilised`. Also handles equivalent German-language requests.
tags: [scaffolding]
---
