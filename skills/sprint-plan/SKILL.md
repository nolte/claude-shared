...
This skill only assigns `sprint` and flips the gate-clearing status; it never edits a pulled-in feature's description, acceptance criteria, or consistency check. When a feature's decomposition itself needs revising, route the operator back to `feature-decompose` rather than editing the feature here.
...
- **`verifies_sprint_value` lives on a feature, not on the sprint.** Exactly one feature in the sprint's `features` list carries the `verifies_sprint_value: acceptance-<m>` field per the feature spec; the sprint frontmatter has **no** `verifies_sprint_value` key of its own. The sprint schema is fixed at the nine fields written in step 7 (plus the optional `cancelled_reason`), and lints reject any unknown key — so the value-verifying feature is identified only through the `features` list (named in step 6), never by adding a verifier key to the sprint frontmatter.
