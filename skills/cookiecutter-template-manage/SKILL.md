---
name: cookiecutter-template-manage
...
5. **Dispatch `cookiecutter-template-author`** — pass the locked decisions as the explicit precondition payload. The agent runs in `scaffold` mode and returns a structured report (files created, anti-pattern audit, spec-conformance audit, local bake result).
...
4. **Dispatch `cookiecutter-template-author`** in `refactor` mode with the locked decisions as context.
...
3. **Dispatch `cookiecutter-template-author`** in the appropriate mode (`hook` or `tests`).
...
- Never dispatch `cookiecutter-template-author` before every one-way decision for the current operation is explicitly confirmed by the user.
