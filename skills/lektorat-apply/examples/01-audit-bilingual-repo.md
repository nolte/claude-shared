...
8. Record the run configuration in `run.json` — operation `audit`, severity floor `suggestion`, the DE pipeline name and version (whichever tool the implementation chose per `spec/project/lektorat/` §Open Questions §OQ-2), the resolved scope summary, the `ran_at` UTC timestamp.
