---
name: mission-define
description: Author a project's first `project/mission.md` per spec/project/mission/<canonical_language>.md. Invoke when the user says things like "define the mission for this project", "write project/mission.md", "set up the mission file", "anchor the roadmap with a mission statement", "draft the SMART mission", "scaffold the mission per the mission spec", "Mission für dieses Projekt definieren", "project/mission.md anlegen", "die SMART-Mission formulieren", or "Mission-Datei aufsetzen". Walks SMART one letter at a time (Specific statement, Measurable verifies_via pointer, Achievable MVP scope size, Relevant outcome IDs, Time-bound shape), gathers a per-audience MVP-deliverable paragraph for every audience, composes the eight-field frontmatter plus the four required body sections (Statement, Audiences, Verification, Source), presents the draft to the user, and writes the file with `mvp_status: defining`. Refuses to run when `project/mission.md` already exists — use `mission-revise` for edits. Refuses to run when `project/goals.md` or the audience artefact is missing — dispatches `audience-identify` or asks the user to author goals first.
tags: [scaffolding]
---
