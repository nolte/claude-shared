...
3. **Step 3 — Pre-handover self-check on the EN draft.** Walk every per-post acceptance criterion in `spec/project/post-writing-style/` §Acceptance criteria (a-1 through a-17, scoped to EN where the criterion is EN-only) and in `spec/project/post-audience-communication/` §Acceptance criteria (a-1 through a-13). For each criterion, record one of `passed`, `finding: <reason>`, or `override: <reference to §Override procedure>` in the self-check manifest (see §Outputs). Repair any unhandled finding before Step 4; the spec forbids leaving them for the lektor.
...
5. **Step 5 — Pre-handover self-check on the DE draft and the pair.** Run the DE-half of the per-language criteria, then the per-pair block: cross-language binding key identical, file-slug identical, audience fields identical, AI-disclosure flag set in both, build command ready to run. Repair findings before Step 6.
