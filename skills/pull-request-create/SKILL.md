...
2. **Branch-name collision check (first push only).** When the local branch has no upstream yet (`git rev-parse --abbrev-ref HEAD@{upstream}` fails), run **both** `git ls-remote origin "refs/heads/$OPERATING_BRANCH"` and `gh pr list --head "$OPERATING_BRANCH" --state open --json number,title,headRefOid`. If either returns a hit, treat this as a collision per `spec/project/pull-request-workflow/<canonical_language>.md` §Branch identity and collision safeguards:
   ...
   **Tooling (optional GitHub MCP) — go/no-go: GO (narrow).** Prefer `github:list_pull_requests` (head + state filter) for the open-PR collision lookup above when a GitHub MCP server is connected, falling back to the `gh pr list` shown, per `spec/claude/mcp-tool-preference/`. The `git ls-remote` ref check, `git push`, the rebase / branch-freshness plumbing, and the `gh pr create` write all stay on `gh`/git per P3 §Reads-versus-writes. `gh`/git stays authoritative; output is identical.
