...
4. Translate into each other configured language. Translation preserves structure (headings, requirement order, checklist items). RFC 2119 keywords stay in English and are glossed in-language, for example `MUSS [MUST]`, `SOLLTE [SHOULD]`, `KANN [MAY]`. The acceptance-criteria heading takes one contract form per language, `## Acceptance Criteria` in English and `## Akzeptanzkriterien` in German; a translation never coins its own variant, because tooling and cross-references look the section up by that exact heading.
