...
- When `Taskfile.yml` exists and lacks a `docs` target, propose a locally-written `docs` target invoking `mkdocs build --strict` (or the project's existing equivalent) so a local pass matches CI. This target is the CI gate and stays repo-local:
  - The shared MkDocs Taskfile in the [`nolte/taskfiles`](https://github.com/nolte/taskfiles) collection currently ships a `start` task only (`mkdocs serve` against a `~/.venvs/docs` interpreter). It builds nothing, so it can't satisfy the `docs` target and it isn't a substitute for one. Verify what the include actually exposes before proposing it; never assume a shared file carries a target just because the topic matches.
  - Offer the include additively when the repo wants the local preview convenience: an `includes:` entry `mkdocs: "{{.TASK_COLLECTION_BASE}}/taskfile-include-mkdocs.yaml"` reusing the repo's existing `TASK_COLLECTION_BASE` variable, giving the developer `task mkdocs:start`. Flag its `~/.venvs/docs` assumption, which conflicts with the project-local `.venv` convention in `spec/project/project-structure/` §Python development. When the repo has no such variable yet, propose adding it (`https://raw.githubusercontent.com/nolte/taskfiles/<ref>/src`) so the include resolves against one declared, reviewable source; ask the user for the ref, or fall back to `develop` and flag it. The conventions are owned by `spec/project/taskfile/` §Shared Taskfiles; never restate its rules here.
  - Remote-Taskfile resolution is an experimental Task feature, and adding the include has a cost the whole repo pays: every environment resolving it needs `TASK_X_REMOTE_TASKFILES=1` **and** `task --yes <target>` (a CI runner has no TTY to answer Task's trust prompt, so a bare `task docs` aborts instead of prompting). The flag alone isn't enough. Since this include is optional preview convenience, it must never be the reason CI turns red: when the repo has no remote include yet, propose the workflow patch (`env:` plus `--yes` at each `task` call site) **in the same approval step** as the `includes:` line, and drop the include entirely if the user declines that patch. Never write the include and leave the CI adjustment as a note for the user to remember.
