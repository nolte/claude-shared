...
- When `Taskfile.yml` exists and lacks a `docs` target, propose a locally-written `docs` target invoking `mkdocs build --strict` (or the project's existing equivalent) so a local pass matches CI. This target is the CI gate and stays repo-local:
  ...
  - Remote-Taskfile resolution is an experimental Task feature: whenever this wire-up introduces the repo's first remote include, surface that `TASK_X_REMOTE_TASKFILES=1` has to be set locally and in the CI workflow, **and** that every non-interactive invocation needs `task --yes` (a CI runner has no TTY to answer Task's trust prompt, so a bare `task docs` aborts instead of prompting). The flag alone isn't enough.
