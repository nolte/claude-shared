...
- **Site identity (`site_url`)**: `site_url` is declared in `mkdocs.yml` and matches the repository's published base URL. Derive the expected value from `repo_url` (or `git remote get-url origin`): the default is the GitHub-Pages project subpath `https://<owner>.github.io/<repo>/`; a repo named `<owner>.github.io` publishes at the root `https://<owner>.github.io/`; a `CNAME` file (repo root or `docs/`) means a custom domain. Classify: `missing` when `site_url` is absent, `drift` when it doesn't match the derived base. When `mkdocs-static-i18n` declares more than one language, a missing `site_url` is **critical** (it ships broken absolute language-switcher and `hreflang` links per `spec/project/mkdocs-structure/` §i18n and parity). A present-but-slashless value is at most a `suggestion` (canonical-form SHOULD), never a failure.
...
- Create `mkdocs.yml` declaring:
  - `site_url` derived from the repository's coordinates: read `repo_url` (or `git remote get-url origin`) and emit the GitHub-Pages project subpath `https://<owner>.github.io/<repo>/` with a trailing slash. Honour the three exceptions before writing — an `<owner>.github.io` repo publishes at the root `https://<owner>.github.io/`; a `CNAME` file (repo root or `docs/`) sets `site_url` to that custom domain; an inconclusive signal means propose-and-confirm rather than write silently. `site_url` is required so `mkdocs-static-i18n` builds correct absolute language-switcher and `hreflang` links (per spec §i18n and parity).
...
- When `Taskfile.yml` exists and lacks a `docs` target, propose one so a local pass matches CI. Two shapes are valid, and which one applies depends on what the collection actually exposes at the ref this repo pins — read the include at that ref before proposing either, and never assume a shared file carries a target just because the topic matches:
  - The reference wiring is the worked pair shipped with `spec/project/taskfile/` under `templates/`. Adapt `templates/Taskfile.yml` and `templates/ci.yml` rather than composing the wiring from scratch here.
  - **Delegating shape** — where the pinned ref exposes a `build` task, propose a `docs` target that delegates to `mkdocs:build`, with the module's interpreter redirected at the repo's project-local environment through an `includes:`-site variable override (`PYTHON_VENV_DIR_DOCS`). The module defaults to a machine-global `~/.venvs/docs` interpreter, which conflicts with the project-local `.venv` convention in `spec/project/project-structure/` §Python development; the override is what reconciles them. An override only takes effect where the module writes the variable in the `default` form — check that at the pinned ref, and treat a variable that isn't as an upstream defect to raise against the collection rather than a reason to fork it locally.
  - **Local shape** — where the pinned ref exposes no usable `build`, or where the module's environment assumptions don't hold, propose a locally-written `docs` target invoking `mkdocs build --strict` (or the project's existing equivalent) and record why the include was skipped. `mkdocs:start` may still be offered additively for local preview convenience under the same verification.
  - Either shape needs `TASK_COLLECTION_BASE` when an include is written. When the repo has no such variable yet, propose adding it (`https://raw.githubusercontent.com/nolte/taskfiles/<ref>/src`) so the include resolves against one declared, reviewable source; ask the user for the ref and prefer an immutable one, never defaulting to a moving branch. The conventions are owned by `spec/project/taskfile/` §Shared Taskfiles; never restate its rules here.
  - Remote-Taskfile resolution is an experimental Task feature, and adding the include has a cost the whole repo pays: every environment resolving it needs `TASK_X_REMOTE_TASKFILES=1` **and** `task --yes <target>` (a CI runner has no TTY to answer Task's trust prompt, so a bare `task docs` aborts instead of prompting). The flag alone isn't enough, and the prompt fires when the Taskfile loads rather than when a target runs, so `--yes` belongs at every call site — not only the ones that touch the include. Adding an include must never be the reason CI turns red: when the repo has no remote include yet, propose the workflow patch (`env:` plus `--yes` at each `task` call site) **in the same approval step** as the `includes:` line, and drop the include entirely if the user declines that patch. Never write the include and leave the CI adjustment as a note for the user to remember. Under the delegating shape the include *is* the docs gate, so declining the workflow patch means falling back to the local shape rather than shipping a half-wired Taskfile.
...
- **Missing or incorrect `site_url`**: when `site_url` is absent, propose adding it as a top-level key derived from the repository's coordinates (project subpath `https://<owner>.github.io/<repo>/` by default; org/user-pages root or custom-domain exceptions per the §Site identity audit area). When it's present but doesn't match the derived base URL, surface the mismatch and propose the corrected value — never overwrite a deliberate custom-domain or root value without confirming it's wrong. A present-but-slashless value is offered as an optional canonical-form tidy, not a required fix. After writing, re-run `mkdocs build --strict` and (when feasible) eyeball the built `index.html` `hreflang` tags to confirm they carry the repo subpath.
