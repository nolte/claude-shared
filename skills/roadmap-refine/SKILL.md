---
name: roadmap-refine
description: Enforce the detail-level invariant on `project/roadmap.md` per `spec/project/roadmap/` §Detail-level convention and refinement rule. Invoke when the user asks to "refine the roadmap", "check roadmap detail levels", "is the roadmap ready for the next sprint", "promote roadmap items to fine", "Roadmap verfeinern", "Roadmap-Detailstufen prüfen", "Roadmap auf den nächsten Sprint vorbereiten", or any equivalent invariant-check request. Also handles equivalent German-language requests. Resolves the current and next sprint by reading `project/sprints/`, walks every roadmap item, emits a structured violation record (item id, offending target_sprint, current detail, resolved current and next sprint numbers, one-line remediation hint) on stderr for every item with `target_sprint` equal to the current or next sprint and `detail` other than `fine`, exits non-zero when any violation is open, and walks per-item fix proposals one at a time. Do NOT use to add items, retarget sprints, or flip MVP flags — that is `roadmap-planner`. Do NOT use to scaffold the roadmap from scratch — that is `roadmap-init`.
tags: [audit]
---
