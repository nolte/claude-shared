...
- **`security_relevance: true`** — a concrete advisory marker only: a CVE / GHSA indicator surfaced by `dependency-audit` or a Renovate / Dependabot `vulnerabilityAlerts` PR, a supply-chain advisory referenced by the lockfile, or a secret-scanning leaked-credential alert. A label, repository topic, or branch name that merely contains the word `security` (for example a Renovate group label on a version-pinning group such as `security pinned versions`) is **not** a signal on its own — it counts only when backed by one of the concrete markers above (per `spec/portfolio/portfolio-inflight-management/` §Classification and prioritisation `security_relevance` detection MUST; the bare-label match was the load-bearing error in the 2026-06-07 audit). Default `false` when no concrete marker is present.
