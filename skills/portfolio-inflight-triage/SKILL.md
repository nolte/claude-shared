...
- **Mid-flow user confirmation on threshold overrides and roster-gap escalations.** Two interactive checkpoints are load-bearing: honouring a Portfolio-Member's `project/inflight.yml` threshold override over the spec default (operator sees both side-by-side), and escalating a roster-gap finding to "author a new specialist" once the recurrence threshold in `continuous-improvement` §Portfolio gap closure triggers. An agent's fire-and-forget shape would lose both.
- **Persistent on-disk artefact as deliverable.** The audit writes `.audits/portfolio-inflight/<YYYY-MM-DD>.md` in `claude-shared`. Skills own persistent write paths; agents return structured reports into the caller's context.
- **Context-window-protective data collection via agent.** Raw per-repository collection is delegated to `portfolio-inflight-collector`, which returns a pre-reduced structured summary, so raw issue / PR / branch / review-comment / discussion bodies never enter the orchestrating conversation — the §Findings-Report shape `MUST NOT` on raw-body inclusion.
- **Skill-orchestrates-agent-executes is the explicit spec rule.** §Audit operation requires `MUST be implemented as a dedicated skill … MUST NOT be implemented as a Claude Agent` and `MUST dispatch a read-only specialist agent portfolio-inflight-collector`. The spec pins the hybrid pattern; this skill is the orchestrator half.
- **Counter-dimension considered:** a tool-restricted agent could perform classification and matrix-axis derivation cleanly, since the collector already absorbs the data volume. The decisive counter is the mid-flow user-confirmation gates plus the persistent write target — both skill-side per `spec/claude/skill-vs-agent/`.
...
This skill writes its single output artefact (`.audits/portfolio-inflight/<YYYY-MM-DD>.md`) inside the `claude-shared` repository, the same repository that hosts `.audits/portfolio/` for `portfolio-audit`. Detection: presence of `.claude-plugin/plugin.json` AND a `spec/portfolio/portfolio-inflight-management/` directory.

When invoked inside any other Portfolio-Member repository (the typical adopter), stop and route the operator to `claude-shared` for the audit — the Findings-Report write path lives there, matching the §Audit operation `SHOULD` and mirroring `portfolio-audit`'s detection rule.
...
1. **Detect Portfolio-Member set** per §Portfolio scope `MUST` — reuse the `portfolio-management` resolution. Either dispatch `portfolio-manifest-collector` (Agent) when the active conversation hasn't already collected the set, or instruct `portfolio-inflight-collector` to resolve fresh via `gh api orgs/nolte/repos --paginate` (it applies the same `archived: false` / `private: false` / `portfolio: excluded` filters). Record `portfolio: excluded` and per-source `inflight: skip-<source>` markers as `Info`-grade entries so omissions stay inspectable.

2. **Dispatch the collector** — invoke `portfolio-inflight-collector` (Agent) with the resolved list, all four data sources enabled, and `release-drafter` draft collection enabled (needed for the `release_blocking` matrix axis). Wait for its structured collection report. Per §Findings-Report shape `MUST NOT`, raw issue / PR / branch / review-comment / discussion bodies never enter this conversation — the agent reduces to a structured summary and discards bodies before returning. From the same report, derive the **consumer-measured-finding count** for the repository that holds the portfolio-wide specs: the open issues whose `namesOtherPortfolioMember` or `hasSpecFindingLabel` flag is `yes` (a union, not an intersection — each half alone under-counts).
...
4. **Apply stalling thresholds** per §Stalling thresholds using the per-repository thresholds confirmed in step 3:
   - **Issue**: open longer than `issue.maxAge` (default 30 days), no priority label, no assignee, no maintainer comment in `issue.maxQuietWindow` (default 30 days). Per §Stalling thresholds `MUST NOT`, first drop any issue whose collector `author` is `renovate[bot]`, `dependabot[bot]`, or `github-actions[bot]` from the `issue/stalled-issue-no-triage` class (permanent bot dashboards, not triage signal); `branch/stalled-branch-no-pr` stays in scope regardless of authorship.
...
10. **Confirm in the user's language** — the path of the new Findings-Report, the per-severity counts, the count of roster-gap findings (if any), the consumer-measured-finding count from step 2 — flagged as a portfolio gap when it reaches the threshold `spec/project/continuous-improvement/` §Portfolio gap closure states, naming the action that section names (schedule an `issue-batch-orchestrate` run over the counted findings) while the decision to start it stays with the operator — and the next step (open the report and triage `Critical` items first via the recommended slash commands; route roster-gap escalations through `continuous-improvement-triage`).
...
Also touches `spec/claude/review-plan/` (the artefact frame) and `spec/project/continuous-improvement/` (§Specialist dispatch, §Portfolio gap closure — the matching pattern and recurrence rule in step 7, and the consumer-measured-finding threshold plus its `issue-batch-orchestrate` action reported in steps 2 and 10).

When the spec disagrees with this skill, the spec wins. Propose a skill update rather than silently diverging.

## Examples

- Read `examples/01-stalled-pr-with-red-checks.md` when a PR's only driver is a red required check (the `workflow-health-triage` exclusivity routing applies).
- Read `examples/02-release-blocker-detection.md` when a PR carries a `release-blocker` label or its head SHA appears in an open `release-drafter` draft (the `release_blocking` detection MUSTs apply).
- Read `examples/03-roster-gap-3-recurrence.md` when a roster-gap class crosses the recurrence threshold against the prior audit artefact (the "author a new specialist" gate applies).
- Read `examples/04-security-relevance-lifts-to-critical.md` when the `security_relevance` axis lifts a finding to Critical.
- Read `examples/05-cross-repo-blocking-lifts-to-critical.md` when a `project/roadmap.md` / `project/sprints/*.md` cross-reference lifts an item to Critical via `cross_repo_blocking`.
- Read `examples/06-zero-findings-run.md` when the in-flight surface is empty and the run still writes a zero-findings report.
...
- **`cross_repo_blocking` detection is exact-string only.** The `project/roadmap.md` / `project/sprints/*.md` scan looks for the literal short cross-reference (`nolte/<repo>#<number>`) or the full GitHub URL. Don't infer from titles, branch names, or partial SHAs — per §Classification and prioritisation `MUST` the scan is exact text matching, no fuzzy matching.
- **`release-drafter`-draft head-SHA membership requires the collector's draft list.** When the collector report lacks the `Open release-drafter drafts` block, only the `release-blocker` label signal applies for `release_blocking` — don't fetch drafts inline; the collector owns every `gh api` call per §Audit operation.
- **Threshold-override confirmation happens once per audit, not per-finding.** Step 3 surfaces each repository's `project/inflight.yml` override once; the confirmed thresholds apply to every finding from that repository in step 4.
- **Recurrence counts the most recent prior `.audits/portfolio-inflight/*.md` plus the current run.** The step-7 3-recurrence escalation walks only the immediately previous artefact, not older ones; with no prior artefact, recurrence is the in-run count only per §Specialist recommendation `MUST`.
- **Bot-authored dashboards are out of the stalled-issue class.** Per §Stalling thresholds `MUST NOT` and the 2026-07-24 audit's gate-2 decision (#445 / #468), step 4 drops `renovate[bot]` / `dependabot[bot]` / `github-actions[bot]` issues from `issue/stalled-issue-no-triage`; `branch/stalled-branch-no-pr` is unaffected.
- **`gh api` rate limits scale with N repos × M open PRs.** The collector prechecks headroom (its §Preconditions step 2), but a full run still eats a non-trivial slice of the hourly budget; check `gh api rate_limit` before back-to-back audits. **Tooling (optional GitHub MCP):** the collector's data-source reads are MCP-preferred with a `gh` fallback (`spec/claude/mcp-tool-preference/`); `gh api rate_limit` itself stays `gh`.
...
- Never use a severity outside the canonical four (`Critical` / `Warning` / `Suggestion` / `Info`). ALL-CAPS variants and extra levels (`HIGH`, `P0`, …) are forbidden per `spec/claude/review-plan/` §Severity scale and §Classification and prioritisation `MUST NOT`.
...
- Never scan repositories outside the resolved Portfolio-Member set, even when the operator names another `nolte/<other>` repository, per §Portfolio scope `MUST NOT`. Scope expansion happens via `portfolio-management`, never ad-hoc.
- Never include raw issue / PR / branch / review-comment / discussion bodies in the Findings-Report per §Findings-Report shape `MUST NOT`. The audit reduces every finding to a structured summary; verbatim source bodies stay in GitHub, reachable via the finding identifier.
