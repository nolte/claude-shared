...
When invoked inside any other Portfolio-Member repository (the typical adopter), stop and route the operator to `claude-shared` for the audit — the Findings-Report write path lives there, matching the §Audit operation `SHOULD` and mirroring `portfolio-audit`'s detection rule. Don't try to write the report into the active checkout.
...
7. **Attach a recommended specialist** per §Specialist recommendation to every finding:
   - **Match the catalog.** Read every `description` line across all catalog roots (`skills/*/SKILL.md`, `agents/*.md`, `plugins/*/skills/*/SKILL.md`, `plugins/*/agents/*.md`); name the matching slug verbatim (for example `dependency-audit`, `workflow-health-triage`, `feature-decompose`, `pull-request-merge`, `vocab-drift-audit`).
...
Also touches `spec/claude/review-plan/` (the artefact frame) and `spec/project/continuous-improvement/` (§Specialist dispatch, §Portfolio gap closure — the matching pattern and 3-recurrence rule in step 7).
...
- **Bot-authored dashboards are out of the stalled-issue class.** Per the 2026-07-24 in-flight audit's gate-2 decision (recorded in #445 / #468 and `.audits/portfolio-inflight/2026-07-24.md`) and §Stalling thresholds `MUST NOT`, step 4 drops `renovate[bot]` / `dependabot[bot]` / `github-actions[bot]` issues from `issue/stalled-issue-no-triage`; `branch/stalled-branch-no-pr` is unaffected — a stale branch is a one-line disposition, not a dashboard.
- **`gh api` rate limits scale with N repos × M open PRs.** The collector prechecks headroom (its §Preconditions step 2), but a full run still eats a non-trivial slice of the hourly budget; check `gh api rate_limit` before back-to-back audits. **Tooling (optional GitHub MCP):** the collector's four data-source reads are MCP-preferred with a `gh` fallback (`spec/claude/mcp-tool-preference/`); `gh api rate_limit` stays `gh` (OQ-D).
