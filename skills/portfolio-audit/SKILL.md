...
description: Audits, renders, and bootstraps the cross-repository capability portfolio across `nolte/*` per `spec/portfolio/portfolio-management/`. Audit collects per-repository `project/portfolio.yml` manifests, detects capability duplicates, surfaces gaps (broken peer references, spec-demanded gaps, copy-paste smells), and writes a Findings-Report under `.audits/portfolio/` with Critical / Warning / Suggestion / Info severities. Render regenerates the aggregated inventory under `docs/<lang>/portfolio/`. Bootstrap creates a repository's first `project/portfolio.yml`. Invoke when the user asks to "audit the portfolio," "check for portfolio duplicates," "what gaps does the portfolio have," "render the portfolio inventory," or German equivalents ("Portfolio auditieren," "Portfolio auf Duplikate prüfen," "Lücken im Portfolio finden," "Portfolio-Inventar rendern"). Don't use to consolidate duplicates (operator opens cross-repo PRs) or to author new capabilities (use each repository's roadmap / feature flow).
tags: [audit]
---
...
- **Orchestrator that dispatches a context-window-protective agent.** Audit dispatches `portfolio-manifest-collector` (a future read-only agent) to fetch and parse manifests across the portfolio without polluting the main conversation with raw YAML; the skill stays focused on synthesis. The skill-orchestrates pattern (per `spec/claude/skill-vs-agent/`) defaults the orchestrator to skill form.
...
1. **Reuse the manifest collection** from operation 1 if Audit has just been run, or trigger a fresh collection via the `portfolio-manifest-collector` agent.
...
- Never run the `portfolio-manifest-collector` agent against repositories outside the resolved Portfolio-Member set. Scope drift here is a confidentiality risk — private repositories or non-`nolte/*` forks must never enter the manifest collection.
