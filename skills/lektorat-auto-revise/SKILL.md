...
dont_use_when:
  - situation: "You want the read-only editorial audit, not remediation"
    alternative: lektorat-apply
  - situation: "You want to drive per-finding fixes interactively by hand"
    alternative: lektorat-apply
  - situation: "You want to author a new artefact from scratch"
...
4. **Compose the briefing per file** (§Author briefing composition): the file's findings verbatim, the resolved audience set (via the `lektorat` priority chain, no other rule), the bound writing-style spec(s), and the target D1–D5 dimensions. Never dispatch an author without **both** a resolved audience set and a bound writing-style spec — a missing either is a per-file stop condition.
...
6. **Re-audit convergence gate** (§Re-audit convergence gate). After each author pass, re-run the `lektorat audit` on the revised artefact with the same configuration. A file is **converged** only when it has no remaining finding at or above the severity floor **and** its post-revision finding count is ≤ its pre-revision count. On non-convergence, re-dispatch the author with the residual findings up to **2 author passes** per file, then **escalate** the residual to the operator — never loop further. Flag any **regression** (post-count higher than pre-count) and never auto-accept it as converged. Treat any semantic-preservation violation (§Semantic preservation) as a failed pass.
