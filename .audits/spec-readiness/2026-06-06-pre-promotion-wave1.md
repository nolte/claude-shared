...
- 2026-06-06 — blog-author — Critical C-1 (cross-spec scope conflict with lektorat) resolved by extending `lektorat` §Scope/§Non-Goals to admit blog posts as an opt-in consumer surface (EN+DE), per operator decision; the blog-author target-state route now references that opt-in. Critical C-2 (load-bearing OQ1) resolved: condition 1 met (lektorat now accepted), OQ1 rewritten as the standing two-route design; Warning W-2 (DE link anchor) carried over. Promoted draft→accepted. lektorat (already accepted) revised in place for the scope opt-in.
