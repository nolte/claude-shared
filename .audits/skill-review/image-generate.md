...
target: "plugins/nolte-media/skills/image-generate/SKILL.md"
target-kind: skill
specs-applied:
...
# Skill Review: image-generate

## Scope

Target: `plugins/nolte-media/skills/image-generate/` (full review; frontmatter, body, and every referenced supporting file).
...
- [x] [skill-review.references] Declared spec path ${CLAUDE_PLUGIN_ROOT}/spec/tools/image-generation/ is unresolvable in every install context (the plugin ships no spec/ tree).
      Resolution: reference now resolves in the target repository with an inlined fallback baseline for consumer installs; hub spec named for provider details (fixed in this batch).
