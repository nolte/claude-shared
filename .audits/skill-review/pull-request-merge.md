...
Target: `skills/pull-request-merge/` (full review; frontmatter, body, and every referenced supporting file).
...
- [ ] [skill-review.body-size] Reviewer estimated over-cap; authoritative validator measured 4,759 (approaching, not over) — downgraded per §Scope note.
      → deferred: move §Sequential merge or step-9 cleanup into references/ on the next substantive edit.
...
- [x] [skill-review.references] Two reference pointers used purpose phrasing instead of conditional load triggers.
      Resolution: both rephrased to Read-X-when-Y form (fixed in this batch).
