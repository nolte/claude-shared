...
target: "plugins/nolte-engineering/skills/observability-audit/SKILL.md"
target-kind: skill
specs-applied:
...
# Skill Review: observability-audit

## Scope

Target: `plugins/nolte-engineering/skills/observability-audit/` (full review; frontmatter, body, and every referenced supporting file).
...
Go/no-go: PASS — no open Critical; every Critical/actionable Warning fixed in this batch, 2 item(s) deferred with annotation.
...
- [ ] [skill-review.standing-instructions] Pinned version literals (SemConv/OTLP/Logs/INP) will stale; declared as spec-deferring defaults.
      → deferred: cite only the spec section on next edit.
