...
target: "skills/blog-author-trigger/SKILL.md"
target-kind: skill
specs-applied:
...
# Skill Review: blog-author-trigger

## Scope

Target: `skills/blog-author-trigger/` (full review; frontmatter, body, and every referenced supporting file).
...
- [ ] [skill-review.gotchas] Cross-repo pitfalls dispersed across sections instead of a Gotchas block.
      → deferred: collect SHA-resolution/silent-cd/deferral pitfalls into §Gotchas on next edit.

### Info

- [x] [skill-review.paths] Machine-specific clone path stated as the blog-consumer reference.
      Resolution: clone path now declared as CLAUDE.md-resolved with the tilde path marked illustrative (fixed in this batch).
