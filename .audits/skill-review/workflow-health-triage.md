...
target: "skills/workflow-health-triage/SKILL.md"
target-kind: skill
specs-applied:
...
# Skill Review: workflow-health-triage

## Scope

Target: `skills/workflow-health-triage/` (full review; frontmatter, body, and every referenced supporting file).
...
- [x] [skill-review.paths] Operation 3 labelled ~/.claude/agents/*.md as project-distributed (it is user-level).
      Resolution: glob corrected to .claude/agents/*.md with the user-level path labelled separately (fixed in this batch).
