...
2026-09-25 P3 dispatched to `nolte-claude-dev:claude-plugin-developer` — skill operations `reconfirm` and `migrate` (procedure in new references/reconfirm-and-migrate.md), runner-exit-codes and approval-batching updated, scanner proposes `declaration.sections` only from the cited location and never resolves ambiguity; SKILL.md body ~4480 → ~4447 tokens; 669 passed; no v1.1 schema reference left in plugins/. No v1.2 example added: tests/test_reach_audit_examples.py pins five probes with a placeholder commit anchor.
2026-09-25 P4 replay (orchestrator) — `reach_replay.py --repo kamerplanter --declaration spec/req/REQ-025_Datenschutz-Betroffenenrechte.md --rev origin/develop` (bd697827a5de), probes 2×row:AK-OS-07, 3×heading:3.1.1+3.5: 18 commits; file-anchor stale events 85, section-anchor stale events 11 — identical to the independent pre-measurement.
2026-09-25 Independent verification (`nolte-engineering:python-code-reviewer`, read-only): 1 Critical (two-commit laundering through a self-made provable intermediate anchor, section and blob), 1 Warning (earlier weakening adopted by an unlabelled re-anchor — also present for blob anchors since #673), 5 Suggestions. Fixed by `nolte-engineering:fullstack-developer`: every anchor move checks the previous recording recursively and forbids intermediate probe changes; the new anchor must itself have changed; BOM and HTML-comment handling; replay first-parent test with a merge. Red run 9 failed / 58 passed; mutation probes M1–M4 each turn tests red. Exit path verified: only delete-then-new-id in a later commit reaches clean (operator accepted, documented). 687 passed, 2 skipped.
