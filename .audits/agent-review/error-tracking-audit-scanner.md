...
- [ ] [agent-management.tool-access-sanctioned-classes] The `## Read-only Bash justification` names package-manager metadata reads (`pip show sentry-sdk`, `npm ls @sentry/react`), a command class `agent-management` §Tool access does not sanction.
...
- [ ] [agent-review.recurring-gap] This is the same spec gap the 2026-07-25 sweep recorded and deferred, now on its second and third artefacts with a third command class.
