...
Maintainers of `claude-shared` get an evidence-based list of shared agents and skills that warrant rework *beyond their descriptions* — merge, split, retire, or rewrite — produced by a fresh full `skills-agents-sweep`. This is the analysis gate for R-11's deep-rework strand: F-5's focused analysis (`.audits/shared-plugin-analysis/2026-07-19.md`) deliberately ran only a frontmatter-level pass and found no duplicate capabilities and clean family clusters, so deep rework is candidate-driven, not a blanket sweep. This feature runs the full per-artefact pass that F-5 skipped, so concrete candidates surface with evidence before any rework (the deferred P2 strand of R-11) is decomposed.
