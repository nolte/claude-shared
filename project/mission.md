...
- **2026-06-07 — audience identifiers normalised, no change in meaning** (`01575fa`, #286). The `audiences:` list moved from the display names "Downstream Claude Code users in portfolio projects" and "Plugin author dogfooding inside this repo" to the `AUDIENCES.md` identifiers `downstream-user` and `dogfooding-author` they already denoted, so the list resolves mechanically. Same two audiences, same deliverable; `revised_at` stays, because no audience was added, removed, or redefined. Recorded retroactively on 2026-09-13 (#591), after the 2026-Q4 spec-drift audit found the revision without a rationale.
- **2026-07-22 — slash-command example updated for the plugin carve-out, no change in meaning** (`87d4f35`, #422). The downstream-user paragraph dropped `/nolte-shared:skill-management` from its list of example commands, because that skill moved to the `nolte-claude-dev` plugin. The statement, audiences, verification, and time bound are unchanged, so `revised_at` stays. Recorded retroactively on 2026-09-13 (#591).
